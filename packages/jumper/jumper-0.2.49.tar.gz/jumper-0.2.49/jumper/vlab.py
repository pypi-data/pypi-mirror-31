...
class Vlab(object):
    ...
    def __init__(
        ...
        try:
            ...
            if not self._local_jemu and not (self._jemu_path and (user_os.startswith(CORE_LINUX_OS) or user_os.startswith(CORE_WINDOWS_OS1))):
                self._add_event({'event': 'os error', "labels": {'os': user_os}})
            ...
            for key in args.keys():
                if args[key]:
                    self._add_event({'event': 'jumper run', 'labels': {'parameter': str(key)}})
    ...
    def _build_components_methods(self):
        ...
        if os.path.isfile(board_json):
            ...
            components_list, self.platform = self._parse_bsp_json(board_json)
        elif os.path.isfile(bsp_json):
            if self.platform:
                print("Notice: 'bsp.json' exists so '--platform' flag will be ignored")
            components_list, self.platform = self._parse_bsp_json(bsp_json)
        elif os.path.isfile(peripherals_json):
            raise VlabException('peripherals.json format is not supported anymore', 7)
        elif self.platform:
            if self.platform == _STM32F4:
                components_list, self.platform = self._parse_bsp_json(default_stm32f4_json)
            elif self.platform == _NRF52:
                components_list, self.platform = self._parse_bsp_json(default_nrf52_json)
            else:
                raise VlabException("platform: {} does not supported".format(self.platform), 7)
        else:
            components_list, self.platform = self._parse_bsp_json(default_nrf52_json)
