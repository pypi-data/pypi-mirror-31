...
# Copyright 2018 by Rebecca Ann Heineman becky@burgerbecky.com
...
from makeprojects import AutoIntEnum, FileTypes, ProjectTypes, \
	ConfigurationTypes, IDETypes, PlatformTypes, Property
...
class MWCodeGen_X86(object):
	def __init__(self, configuration):
		if configuration == ConfigurationTypes.debug:
			disableopt = '1'
			optimizeasm = '0'
		else:
			disableopt = '0'
...
class FILE(object):
	def __init__(self, platform, configuration, filename):
		...
		if self.filename.endswith('.lib') or self.filename.endswith('.a'):
			...
			self.kind = 'Text'
			if configuration != ConfigurationTypes.release and \
...
class GROUP(object):
	...
	def write(self, fileref, level=2):
		...
		filerefs = sorted(self.filerefs, cmp=lambda x, y: cmp(x.filename.lower(), y.filename.lower()))
...
class ORDEREDTARGET(object):
	...
	def write(self, fileref, level=2):
		tabs = TAB * level
		fileref.write(tabs + '<ORDEREDTARGET><NAME>' + str(self.target.name) + '</NAME></ORDEREDTARGET>\n')
...
def generate(solution):
	...
	for configuration in solution.configurations:
		...
		target = codewarriorprojectfile.addtarget(configuration, linker)
		...
		#
		target.settinglist.append(SearchPath(solution.platform, 'bin', 'Project', 'OutputDirectory'))
		...
		if includedirectories:
			...
			for item in includedirectories:
				...
				entry.subsettings.append(SearchPathAndFlags(solution.platform, item, 'Project', False))
		...
		for item in solution.codewarrior.burgersdkspaths:
			...
			entry.subsettings.append(SearchPathAndFlags(solution.platform, item, 'BURGER_SDKS', False))

		for item in solution.codewarrior.systemsearchpaths:
			entry = systemsearchpaths.addsetting()
			entry.subsettings.append(SearchPathAndFlags(solution.platform, item, 'CodeWarrior', True))
		...
		definelist = Property.getdata(solution.properties,name="DEFINE",configuration=configuration,platform=platform)
		# C/C++ Preprocessor
		...
		if solution.platform.iswindows():
			...
			# x86 Target
			target.settinglist.append(MWProject_X86(solution.projecttype, \
				solution.projectname + idecode + 'w32' + configuration.getshortcode()))

			# x86 CodeGen
			target.settinglist.append(MWCodeGen_X86(configuration))

			# Global Optimizations
			target.settinglist.append(GlobalOptimizer_X86(configuration))
		...
		if solution.projecttype != ProjectTypes.library:
			...
			elif configuration == ConfigurationTypes.internal:
		...
		if alllists or liblist:
			...
			for item in alllists:
				...
				# Add to file group
				codewarriorprojectfile.addtogroups(solution.platform, configuration, parts)
			...
			for item in liblist:
				...
				target.linkorder.append(FILEREF(solution.platform, None, item))
				# Add to file group
				codewarriorprojectfile.addtogroups(solution.platform, configuration, ['Libraries', item])
	...
	if cwfile != None and solution.platform.iswindows():
		mcppathname = os.path.join(solution.workingDir, codewarriorprojectfile.projectnamecode + '.mcp')
