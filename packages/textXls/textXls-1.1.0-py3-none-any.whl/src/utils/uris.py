...
def to_fs_path(uri):
    ...
    Also uses the platform specific path separator.
...
def uri_with(uri, scheme=None, netloc=None, path=None,
             params=None, query=None, fragment=None):
    """Return a URI with the given part(s) replaced.

    Parts are decoded / encoded.
    """
    old_scheme, old_netloc, old_path, \
        old_params, old_query, old_fragment = urlparse(uri)
