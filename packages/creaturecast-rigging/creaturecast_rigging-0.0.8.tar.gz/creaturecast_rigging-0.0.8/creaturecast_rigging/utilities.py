...
def get_side_aim_vector(side):
    if side == -1:
        return [x * -1 for x in hdr.rig_settings['aim_vector']]
    return hdr.rig_settings['aim_vector']


def get_side_up_vector(side):
    if side == -1:
        return [x * -1 for x in hdr.rig_settings['up_vector']]
    return hdr.rig_settings['up_vector']
