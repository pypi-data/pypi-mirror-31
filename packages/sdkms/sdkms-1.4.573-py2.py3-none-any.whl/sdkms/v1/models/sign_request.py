...
class SignRequest(object):
    ...
    def __init__(self, hash_alg=None, hash=None, data=None, mode=None):
        """
    ...
    def hash(self):
        ...
        Hash of the data to be signed. One of `hash` and `data` is required. For compliance with FIPS 186-4, `data` must be used instead of `hash`. 
    ...
    def data(self):
        ...
        Data to be signed. One of `hash` and `data` is required. To reduce request size and avoid reaching the request size limit, prefer `hash`. 
