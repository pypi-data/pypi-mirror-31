...
def build(deployment_info, **kwargs):
    ...
    print('\nBuild docker image')
    res = subprocess.run('docker build --build-arg SSH_PRIVATE_KEY="$(cat {ssh_path})" --build-arg SERVICE_VERSION={service_version} -t {image_name}:{image_tag} .'.format(
        image_name=env.FOSSILCI_DOCKER_IMAGE, image_tag=deployment_info.image_tag, ssh_path=BITBUCKET_SSH_PATH, service_version=deployment_info.service_version), shell=True)
    if res.returncode != 0:
