...
class DeploymentInfo:
    ...
    def parse(self, git_branch, git_tag, git_commit, bitbucket_build_number, bitbucket_repo_slug):
        ...
        self.image_tag = self.__get_image_tag(
            git_tag, git_branch, bitbucket_build_number, bitbucket_repo_slug)

    def __is_deploy(self):
        '''
        Check if the current building has deployment permission.
        '''
        if re.compile(r"^master[A-Za-z0-9_.]*$").match(self.__git_branch):
    ...
    def __get_image_tag(self, git_tag, git_branch, bitbucket_build_number, bitbucket_repo_slug):
        '''
        ...
        if re.compile(r"^master[A-Za-z0-9_.]*$").match(self.__git_branch):
            ...
            image_tag = [
                "{r}-{b}-{n}".format(r=bitbucket_repo_slug, b=git_branch, n=bitbucket_build_number)]
        elif re.compile(r"^release/\d+\.\d+\.\d+").match(self.__git_branch):
            image_tag = [
                "{r}-{b}-{n}".format(r=bitbucket_repo_slug, b=git_branch.replace("/", "-"), n=bitbucket_build_number)]
        else:
            return False
