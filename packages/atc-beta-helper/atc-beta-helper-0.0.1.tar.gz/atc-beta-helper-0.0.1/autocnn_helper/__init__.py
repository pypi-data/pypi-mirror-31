...
def get_parameter():
    ...
    try:
        param = _load_json(parameter) if parameter else None
        _parameter = param
        return _parameter
    except (ValueError, TypeError):
...
def get_job_info():
    ...
    info = os.getenv('AUTOCNN_JOB_INFO', None)
    try:
        return json.loads(info) if info else None
    except (ValueError, TypeError):
...
def send_metrics(**metrics):
    ...
    api = get_api()
    if not all([task_name, user_token, api]):
        print('Environment information not found, '
              'please make sure this is running inside a autocnn job.')
        return

    values = task_name.split('.')
    user, algorithm, task = values[0], values[1], values[-1]
    ...
    try:
        requests.post('{}/{}/{}/tasks/{}/metrics'.format(api, user, algorithm, task),
