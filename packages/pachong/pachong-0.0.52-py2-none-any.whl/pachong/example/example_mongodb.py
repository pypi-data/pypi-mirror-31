...
len_per_file = 700
for i_file in range(min(int(math.ceil(len(ids) * 1./len_per_file)), 18)):
    with open('inputs/itemids{}.txt'.format(i_file), 'w') as f:
...
len_per_file = 2115234
import json
for i_file in range(min(int(math.ceil(len(docs) * 1./len_per_file)), 38*4)):
    with open('inputs/users{}.txt'.format(i_file), 'w') as f:
