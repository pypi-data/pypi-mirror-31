...
def parse_shop(fetcher):
    if not is_available(fetcher, '没有找到相应的店铺信息'):
        return None
...
def parse_shop_rate(fetcher):
    ...
    if dsr:
        chong['dsr'] = {}
        chong['dsr']['match'] = parse_dsr(dsr, '宝贝与描述相符')
        chong['dsr']['service'] = parse_dsr(dsr, '卖家的服务态度')
        chong['dsr']['logistics'] = parse_dsr(dsr, '物流服务的质量')
    return chong
...
def parse_comment_pages(fetcher):
    ...
    fetcher.proxy.new_har()
    fetcher.find('a', {'shortcut-label': '查看累计评论'}).click()
