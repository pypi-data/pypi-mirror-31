...
def to_caterpillar_case(camel_cased_string: str) -> str:
    ...
    return sub('(?<!^)((?=[A-Z])|\_)', '-', camel_cased_string).replace("_", "-").lower().split(".")[-1]
...
def is_camel_case(check_string: str) -> str:
    """Return True if string is camel case"""
    return bool(match(r"^_?[a-z]+(?:[A-Z]{1,2}[a-z]*)*$", check_string))

def is_snake_case(check_string: str) -> str:
    """Return True if string is snake_cased"""
    return bool(match(r"^\b_?[a-z]+(_[a-zA-Z]+)*\b$", check_string))
