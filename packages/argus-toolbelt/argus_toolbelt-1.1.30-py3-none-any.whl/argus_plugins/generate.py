...
from api_generator.parsers.openapi2 import load
from argus_cli.settings import settings
from argus_cli.plugin import register_command
from api_generator import DEFAULT_API_DIRECTORY
...
def all_endpoints(directory: str = '', as_plugins: bool = False) -> list:
    ...
    for schema in settings["api"]["definitions"]:
        path = join(getcwd(), directory or DEFAULT_API_DIRECTORY)
        if not directory or directory == '':
            generate_api("%s%s" % (settings["api"]["api_url"], schema))
        else:
            ...
            write_endpoints_to_disk(
                ...
                output=path,
                with_plugin_decorators=directory == DEFAULT_API_DIRECTORY
            )
