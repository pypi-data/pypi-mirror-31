...
class Node(object):
    ...
    def __delete__(self):
        ...
        self.unparent()
        env.name_handler.names.remove(self.data['name'])
        env.node_handler.delete.emit(self)
    ...
    def __init__(self, *args, **kwargs):
        ...
        data['name'] = env.name_handler.create_name_string(**data)
    ...
    def set_parent(self, parent):
        ...
        """
        if self == parent:
            env.error_handler.error.emit('Cannot parent %s to itself' % self.data['name'])
        elif parent in self.get_descendants():
            env.error_handler.error.emit('Cannot parent %s to one of its children' % self.data['name'])
            self.unparent()

        if parent:
            env.node_handler.start_parent.emit(self, parent)

            self.parent = parent
            self.data['parent_id'] = parent.data['uuid']
            parent.children.append(self)

            env.node_handler.end_parent.emit(self, parent)
    ...
    def unparent(self):
        ...
        """
        if self.parent:
            env.node_handler.start_unparent.emit(self)
            if self.parent:
                self.parent.children.remove(self)
            self.parent = None
            env.node_handler.end_unparent.emit(self)
    ...
    def get_index_name(self):
        return env.name_handler.get_index_name(**self.data)
    ...
    def get_short_name(self):
        return env.name_handler.get_short_name(**self.data)
    ...
    def create(self):
        ...
        env.node_handler.created.emit(self)
