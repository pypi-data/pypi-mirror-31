...
import yaml
import unittest
from configurationutil.cfg_providers import yaml_provider
...
class TestYAMLConfig(unittest.TestCase):
    ...
    def setUp(self):
        self.cfg_file = os.path.join(pop_path(__file__), u'test_yaml_config.yaml')

        self.config = yaml_provider.YAMLConfig(config_file=self.cfg_file,
                                               create=True)
    ...
    def test_instantiation(self):
        self.assertEqual(self.config.cfg.get(u'test'), 123,
                         u'YAMLConfig instantiation failed')
    ...
    def test_instantiation_invalid_config_file(self):
        with self.assertRaises(IOError):
            self.cfg_test = yaml_provider.YAMLConfig(config_file=u'does_not_exist!')

    def test_instantiation_default(self):
        self.cfg_file = os.path.join(pop_path(__file__), u'test_yaml_config_default.yaml')

        self.config = yaml_provider.YAMLConfig(config_file=self.cfg_file,
                                               create=True)

        self.assertEqual(self.config.cfg, None,
                         u'YAMLConfig default instantiation failed')

    def test_instantiation_missing_default(self):

        default_template = yaml_provider.YAMLConfig.DEFAULT_TEMPLATE
        del yaml_provider.YAMLConfig.DEFAULT_TEMPLATE

        self.cfg_file = os.path.join(pop_path(__file__), u'test_yaml_config_missing_default.yaml')

        with self.assertRaises(NotImplementedError):
            self.config = yaml_provider.YAMLConfig(config_file=self.cfg_file,
                                                   create=True)

        yaml_provider.YAMLConfig.DEFAULT_TEMPLATE = default_template
