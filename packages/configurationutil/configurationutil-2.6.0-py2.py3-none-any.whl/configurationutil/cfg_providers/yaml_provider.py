...
from fdutil.parse_yaml import write_yaml_to_file
...
TEMPLATE = u'yaml_provider_default.yaml'


class YAMLConfig(ConfigObject):
    ...
    def _load_config(self,
        ...
        if not os.path.exists(self._config_file) and create:
            ...
            with open(self.DEFAULT_TEMPLATE) as f:
                template = self.YAML.load(f)

            # Create the file
            write_yaml_to_file(content=template,
                               output_dir=pth,
                               filename=fn,
                               file_ext=ext)

        with open(self._config_file) as f:
            self._cfg = self.YAML.load(f)
    ...
    def save(self):
        ...
        write_yaml_to_file(content=self._cfg,
                           output_dir=path,
                           filename=fn,
                           file_ext=ext)
