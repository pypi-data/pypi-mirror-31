...
class Game(object):
    ...
    def __init__(self, name = "Game", speed = 25, icon = None):
        ...
        # Check if icon is not None.
        if icon != None:

            # Check if icon ends with '.ico'
            if str(icon).lower().endswith(".ico"):
                ...
                # Log unsuccessful icon set.
                log(f"Could not set {self.__class__.__name__} {hex(id(self))} icon to '{icon}' (not valid type)", "error")
    ...
    def empty(self):
        ...
        if len(self.levels) > 0:
            ...
            # Create event data.
            data = { "game": self, "levels": self.levels }
    ...
    def stop(self):
        ...
        # Create event data.
        data = { "game": self, "level": self.current }
...
class Level(object):
    ...
    def empty(self):
        ...
        if len(self.layers) > 0:
            ...
            # Create trigger data.
            data = {"level": self, "layers": self.layers}
                            ...
                            for row_i, row_v in enumerate(shape.grid):
                                ...
                                for col_i, col_v in enumerate(row_v):
                                    ...
                                    + layer.x and x < self.width):
                                        ...
                                        # Check if value is a space and shape is opaque.
                                        if shape.opaque == True and col_v == "":

                                            # Set value as double space to represent opaque.
                                            value = "  "
            ...
            for row in range(self.height):
                ...
                for col in range(self.width):
                    ...
                    for layer in range(len(self.layers) -1, -1, -1):
                        ...
                        # Check if cell is empty or whitespace.
                        if layers[layer][row][col] == "" or layers[layer][row][col] == " ":

                            # Add whitespace.
                            grids[layer][row][col] = " "

                        # Check if cell marked as opaque.
                        elif layers[layer][row][col] == "  ":

                            # Add whitespace to grid.
                            grids[layer][row][col] = " "
...
class Layer(object):
    ...
    def empty(self):
        ...
        if len(self.shapes) > 0:
            ...
            # Create trigger data.
            data = {"layer": self, "shapes": self.shapes}
                    ...
                    for row_i, row_v in enumerate(shape.grid):
                        ...
                        for col_i, col_v in enumerate(row_v):
                            ...
                            and col_i + shape.x >= 0 and col_i + shape.x < self.width):
                                ...
                                # Add character to grid if.
                                grid[row_i + shape.y][col_i + shape.x] = value
    ...
    def set_position(self, x, y):
        ...
        # Create Trigger data.
        data = { "layer": self, "from_x": orig_x, "from_y": orig_y, "to_x": self.x, "to_y": self.y }
...
class Shape(object):
    ...
    def __init__(self, width, height, character = "", x = 0, y = 0, layer = None, solid = False, opaque = False, speed_x = 0, speed_y = 0):
        ...
        # Check that character is zero or single length.
        if len(str(character)) == 0 or len(str(character)) == 1:
            ...
            # Log unsuccessful character set.
            log(f"Could not set {self.__class__.__name__} {hex(id(self))} character to '{character}' (not valid length)", "error")
        ...
        # Check if speed_x is integer.
        if isinstance(speed_x, int):
            ...
            log(f"Could not set {self.__class__.__name__} {hex(id(self))} x speed to '{speed_x}' (not valid type)", "error")

        # Check if speed_y is integer.
        if isinstance(speed_y, int):
            ...
            # Log unsuccessful direction set.
            log(f"Could not set {self.__class__.__name__} {hex(id(self))} y speed to '{speed_y}' (not valid type)", "error")
        ...
        # Set x and y speed counters for animations.
        self.x_count = 0
        self.y_count = 0
    ...
    def draw(self, character, width, height, x, y):
        ...
        # Check that character has a length of zero or one.
        if len(str(character)) == 0 or len(str(character)) == 1:
            ...
            if int(width) > 0:
                ...
                if int(height) > 0:
                    ...
                    if x + width >= 0 and x + width <= self.width and y + height >= 0 and y + height <= self.height:
                        ...
                        # Create trigger data.
                        data = {"shape": self, "character": character, "width": width, "height": height, "x": x, "y": y }
        ...
        if int(width) > 0:
            ...
            if int(height) > 0:
                ...
                if x + width >= 0 and x + width <= self.width and y + height >= 0 and y + height <= self.height:
                    ...
                    # Use draw to insert empty string.
                    ...
                    # Create trigger data.
                    data = { "shape": self, "width": width, "height": height, "x": x, "y": y }
    ...
    def fill(self, character):
        ...
        if len(str(character)) == 0 or len(str(character)) == 1:
            ...
            # Create trigger data.
            data = { "shape": self, "character": character }
    ...
    def empty(self):
        ...
        # Use fill to insert empty string.
        self.fill("")
            ...
            if int(x) >= 0 and int(x) < self.width and int(y) >= 0 and int(y) < self.height:
                ...
                if isinstance(wrap, bool):
                    ...
                    if min(max, len(text)) == len(text):
                        ...
                        # Log writing.
                        log(f"Wrote '{text}' into {self.__class__.__name__} {hex(id(self))} at ({x}, {y})")

                        # Create trigger data.
                        data = { "shape": self, "text": text, "x": x, "y": y }
                        ...
                        # Create trigger data.
                        data = { "shape": self, "text": text[:max], "x": x, "y": y }
    ...
    def set_position(self, x, y):
        ...
        # Create trigger data.
        data = { "shape": self, "from_x": orig_x, "from_y": orig_y, "to_x": self.x, "to_y": self.y }
    ...
    def move(self, x, y):
        ...
        if boundary != None:
            ...
            # Create trigger data.
            data = { "shape": self, "layer": self.layer, "edge": int(boundary) }

            # Trigger on_hit_edge if so.
            self.on_hit_edge(data)

        # Check if hit obstacle.
        if obstacles != None:

            data = { "shape": self, "obstacle": obstacles }

            # Trigger on_collide if so.
            self.on_collide(data)

            # Trigger obstacle's on_collide.
            # obstacles.on_collide({ "obstacle": self })
    ...
    def animate(self):
        ...
        # Check if x speed counter matches speed_x.
        if int(self.x_count) == abs(self.speed_x):

            # Check if speed_x is positive.
            if self.speed_x > 0:

                # Move to the right.
                self.move(1, 0)

            # Check if speed_x is negative.
            elif self.speed_x < 0:

                # Move to the left.
                self.move(-1, 0)

            # Reset x speed counter.
            self.x_count = 0

        else:

            # Increment x speed counter.
            self.x_count += 1

        # Check if y speed counter matches speed_y.
        if int(self.y_count) == abs(self.speed_y):

            # Check if speed_y is positive.
            if self.speed_y > 0:

                # Move down.
                self.move(0, 1)

            # Check if y_speed is negative.
            elif self.speed_y < 0:

                # Move up.
                self.move(0, -1)

            # Reset y speed counter.
            self.y_count = 0

        else:

            # Increment y speed counter.
            self.y_count += 1


    def freeze(self):
        """Stops the shape from animating but maintains speed values."""
        ...
        if self.animate in self.layer.level.game.on_tick.subscribers:
            ...
            # Create trigger data.
            data = {"shape": self, "speed_x": self.speed_x, "speed_y": self.speed_y}
    ...
    def stop(self):
        """Stops the shape from moving and clears speed values."""
    ...
    def reverse(self):
        ...
        # Ensure shape is moving on an axis.
        if self.speed_x != 0 or self.speed_y != 0:

            # Store current speeds.
            curr_speed_x = self.speed_x
            curr_speed_y = self.speed_y
            ...
            # Create trigger data.
            data = {"shape": self, "from_speed_x": curr_speed_x, "from_speed_y": curr_speed_y, "to_speed_x": self.speed_x, "to_speed_y": self.speed_y}

            # Trigger on_reverse.
            self.on_reverse(data)
...
def build_shape(width, height, character = ""):
    ...
    if int(width) > 0:
        ...
        if int(height) > 0:
            ...
            # Check if character is single or zero length.
            if len(str(character)) == 0 or len(str(character)) == 1:
