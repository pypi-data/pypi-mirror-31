...
from spell_cli.utils import prettify_time, tabulate_rows, HiddenOption
...
def workspaces(ctx, raw):
    ...
    workspaces = []
    ws_client = WorkspacesClient(token=token, **ctx.obj["client_args"])
    with api_client_exception_handler():
        logger.info("Retrieving workspace information from Spell")
        workspaces = ws_client.get_workspaces()
