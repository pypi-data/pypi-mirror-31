...
from trosnoth.const import (
    TURRET_DEATH_HIT, TROSBALL_DEATH_HIT, SHOXWAVE_HIT, SHOT_HIT, GRENADE_HIT,
    ...
    SHOXWAVE_RADIUS, ZONE_CAP_DISTANCE,
)
...
from trosnoth.messages import (
    ...
    RemovePlayerMsg, PlayerHitMsg, PlayerIsReadyMsg,
...
class UIOptions(object):
    ...
    def dumpState(self):
        return {
            ...
            'showGameOver': self.showGameOver,
            'winningTeamId': self.winningTeamId,
            'paused': self.showPauseMessage,
            'humanTeams': self.teamIdsHumansCanJoin,
        }
    ...
    def restoreState(self, data):
        ...
        self.showGameOver = data['showGameOver']
        self.winningTeamId = data['winningTeamId']
        self.showPauseMessage = data['paused']
        self.teamIdsHumansCanJoin = data['humanTeams']
...
class Universe(MessageConsumer):
    ...
    def killAnyTurrets(self, tagger, zone):
        pass
    ...
    def zoneTagged(self, msg):
        ...
        zone.tag(player)
        self.killAnyTurrets(player, zone)
    ...
    def processDelayedCalls(self):
        ...
        while self.delayedCalls:
            if self.delayedCalls[0][0] > thisTick:
                break
    ...
    def callAfterTicks(self, _ticks, _fn, *args, **kwargs):
        ...
        heapq.heappush(self.delayedCalls, (futureTick, result))
        return result
...
class ServerUniverse(Universe):
    ...
    def __init__(self, game, *args, **kwargs):
        ...
        botsOnly = kwargs.pop('botsOnly', False)
    ...
    def startLevel(self, level):
        ...
        # initialised yet. This is ok for setupMap(), but self.resumeStats()
        #  and level.start() need to be able to access the game.
        d = defer.Deferred()
    ...
    def stopCurrentLevel(self):
        ...
        return self.startLevel(LobbyLevel(self.game.lobbySettings))  # inlineCallbacks
        ...
        for shot in list(self.shots):
            ...
            if hitPlayers:
                ...
                self.sendServerCommand(PlayerHitMsg(
                    ...
                    shot.originatingPlayer.id, shotId=shot.id,
                ))
    ...
    @defer.inlineCallbacks
    def _startGame(self, botManager=None, level=None):
        if self.botManager is not None:
            self.botManager.stop()
    ...
    def tick(self):
        ...
        if self.stillLoadingCentralPathFinding:
            loading = any(
                p.bot and p.team is not None and p.agent.needsPathFinding
                for p in self.players)
        else:
            loading = False
        ...
        if loading or len(self.players) == 0 or self.paused:
            return
        if not self.botsOnly and all(p.bot for p in self.players):
            return
    ...
    def killAnyTurrets(self, killer, zone):
        player = zone.turretedPlayer
        if player is not None:
            self.sendKill(player, TURRET_DEATH_HIT, killer)
    ...
    def grenadeExploded(self, killer, pos, radius):
        ...
        for player in self.players:
            if (
                    player.isFriendsWith(oldKiller) or player.dead
                    or player.phaseshift or player.isInvulnerable()
                    or player.turret):
                continue
    ...
    def sendKill(self, player, hitKind, killer, maxHits=None):
        killerId = killer.id if killer else NO_PLAYER
    ...
    def gotFireShoxwaveMsg(self, msg):
        ...
        for player in self.players:
            if not (player.isFriendsWith(firingPlayer)
                    ...
                    or player.isInvulnerable()
                    or player.phaseshift
                    or player.turret):
