...
class HuntingGoal(Goal):
    ...
    def noMoreRabbits(self):
        raise NotImplementedError('{}.noMoreRabbits'.format(
...
class HuntEnemies(HuntingGoal):
    def noMoreRabbits(self):
        if self.bot.player.dead:
            self.bot.respawn()
            return
        self.bot.world.callLater(3, self.reevaluate)
        self.setSubGoal(MessAroundInZone(self.bot, self))
