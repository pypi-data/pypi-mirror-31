...
def evalSynonymsV2(fastTextModel, skill_synonyms_file_name=None):
    if skill_synonyms_file_name == None:
        raw_synonyms = IOUtil.readToArrayFromPackage('skill_synonyms.txt')  # load from file
    else:
    ...
    for synonymArray in synonymes:
        ...
        real_synonymes, negative_examples = Common.splitListAtElement("-not-", synonymArray)
...
def evalRelationsV2(fastTextModel, skill_relations_file_name=None):
    if skill_relations_file_name == None:
        ...
        raw_relations = IOUtil.readToArray(skill_relations_file_name, encoding=None)
