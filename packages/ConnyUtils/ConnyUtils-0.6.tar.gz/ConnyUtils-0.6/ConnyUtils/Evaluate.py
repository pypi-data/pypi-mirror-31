import re
from ConnyUtils import IOUtil as io
from ConnyUtils import Common as com
import numpy as np
...
def evalSynonymsV2(fastTextModel, skill_synonyms_file_name=None):
    if skill_synonyms_file_name == None:
        #io.readToArray("", "")
        raw_synonyms = io.read_to_array_from_package('skill_synonyms.txt')  # load from file
    else:
        raw_synonyms = io.readToArray(skill_synonyms_file_name, encoding=None)

    synonymes = com.LamdaList(raw_synonyms) \
    ...
    for synonymArray in synonymes:
        ...
        real_synonymes, negative_examples = com.splitListAtElement("-not-", synonymArray)
...
def evalRelationsV2(fastTextModel, skill_relations_file_name=None):
    if skill_relations_file_name == None:
        raw_relations = io.read_to_array_from_package('skill_relations.txt')  # load from file
    else:
        raw_relations =io.adToArray(skill_relations_file_name, encoding=None)
    # or for a file-like stream:
    _relations = com.LamdaList(raw_relations) \
        .map(lambda s: s.decode('utf-8')) \
        .map(lambda s: s.split(" ")).result()
