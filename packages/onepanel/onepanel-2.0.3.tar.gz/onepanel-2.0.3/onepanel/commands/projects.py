...
def create_project(ctx, account_uid, home):
    ...
    if vc.exists_remote(project_uid, remote_project):
        if vc.exists_local(home):
            ...
            vc.save(home)
            git = GitWrapper()
            git.init(onepanel_acc_uid=account_uid, gitlab_token=vc.conn.gitlab_impersonation_token,
...
def projects_clone(ctx, path, directory, include, exclude):
    ...
    if not exclude and not include:
        exclude = '*'
    git_clone_code = git.clone(account_uid,conn.gitlab_impersonation_token,home, account_uid, project_uid)
    git_attr_exists = False
    if git_clone_code == 0:
        # Check if ".gitattributes" exists for git-lfs
        git_attr_exists = os.path.isfile(home + '/.gitattributes')
    ...
    if git_attr_exists and ctx.obj['git_utility'].git_lfs_installed:
        curr_dir = os.getcwd()
        # Change dir so we can execute lfs_pull properly.
        os.chdir(home)
        if git.lfs_pull(include=include, exclude=exclude) == 0:
            vc.account_uid = account_uid
            vc.project_uid = project_uid
            vc.save(home)
            git.exclude(home, ProjectViewController.EXCLUSIONS)
        ...
        os.chdir(curr_dir)
    elif git_attr_exists and (ctx.obj['git_utility'].git_lfs_installed is False):
