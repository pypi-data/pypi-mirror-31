...
def make_pi_s(params, gamma, ciphertext, cm, k, r, clear_m, hidden_m):
	...
	Bw = [wk[i]*gamma + wm[i]*h for i in range(len(hidden_m))]
...
def make_pi_v(params, aggr_vk, sigma, hidden_m, t):
	""" prove correct of kappa=(X + m*Y) """
	(G, o, g1, hs, g2, e) = params
	(g2, alpha, beta) = aggr_vk
	(h, s) = sigma
	# create the witnesses
	wm = [o.random() for _ in hidden_m]
	wt = o.random()
	# compute the witnesses commitments
	Aw = wt*g2 + alpha + ec_sum([wm[i]*beta[i] for i in range(len(hidden_m))])
	Bw = wt*h
	# create the challenge
	c = to_challenge([g1, g2, alpha, Aw, Bw]+hs+beta)
	# create responses 
	rm = [(wm[i] - c*hidden_m[i]) % o for i in range(len(hidden_m))]
	rt = wt - c*t % o
	return (c, rm, rt)
