...
class CssValue:
    ...
    def merge(self, value):
        ...
        self.value = utils.sadd(self.value, value)
    ...
    def unmix(self, value):
        ...
        if not isinstance(self.value, basestring):
            raise Exception(MERGE_VALUE_ERROR)
        self.value = utils.sremove(self.value, value)
...
class CssStyles:
    ...
    # Directions in use in many CSS properties (padding, margin, etc), defined
    ...
    def __init__(self, elem=None, attrs=None, other=None, **kwargs):
        ...
        if attrs and attrs.has_key('style'):
        ...
        for k, v in kwargs.iteritems(): self.add(k, v)
    ...
    def addClass(self, value):
        ...
        self.classes = utils.sadd(self.classes, value)

    def deleteClass(self, value):
        '''Remove CSS class(es) in p_value from self.classes'''
        self.classes = utils.sremove(self.classes, value)
    ...
    def merge(self, other, override=True):
        ...
        for name, value in other.get().iteritems():
            if name == 'classes':
                if value:
                    self.addClass(value)
            else:
