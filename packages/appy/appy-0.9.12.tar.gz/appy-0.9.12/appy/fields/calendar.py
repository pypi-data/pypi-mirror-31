...
from appy.shared.dates import getLastDayOfMonth
...
class Calendar(Field):
    ...
    pxTotalCols = Px('''
     <table cellpadding="0" cellspacing="0" class="timeline"
    ...
    pxView = pxCell = Px('''
     <div var="defaultDate=field.getDefaultDate(zobj);
      ...
      <script>:field.getAjaxData(ajaxHookId, zobj, render=render, \
                 month=month, activeLayers=','.join(activeLayers), \
                 popup=inPopup and '1' or '0')</script>
    ...
    def getAjaxData(self, hook, zobj, **params):
        ...
        req = zobj.REQUEST
        params['className'] = req.get('className', '')
