...
# Messages ---------------------------------------------------------------------
CONVERT_NON_LOCAL = 'Cannot convert a non-local user %s.'
...
class UserWrapper(AbstractWrapper):
    ...
    def getFirstNameInitial(self, suffix='.', sep='-'):
        ...
        if '-' in r:
            a, b = r.split(sep, 1)
            return '%s%s%s%s%s' % (a[0], suffix, sep, b[0], suffix)
        else:
            return '%s%s' % (r[0], suffix)
    ...
    def showRoles(tool):
        ...
        if user.has_role('Owner', user): return ('view', 'xml')

    def showResetPassword(self):
        '''Action "reset password" is available to Managers only'''
        if self.user.has_role('Manager') and (self.source == 'zodb') and \
           not self.isSpecial(includeAdmin=False):
            return 'buttons'
    ...
    def doResetPassword(self, noSecurity=False):
        ...
        if not noSecurity and not self.user.has_role('Manager'):
    ...
    def setLogin(self, oldLogin, newLogin):
        ...
        self.log('login change: local roles updated in %d/%d object(s).' % \
                 (context['nb'], context['total']))
    ...
    def convertTo(self, source):
        ...
        if origSource != 'zodb': raise Exception(CONVERT_NON_LOCAL % self.login)
        # Transfer the Zope user from acl_users to self
        zopeUser = self.getZopeUser()
