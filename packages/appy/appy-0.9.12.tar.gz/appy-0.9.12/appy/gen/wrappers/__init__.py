...
class AbstractWrapper(object):
       ...
       <table class="payload">
        <tr valign="top">
         ...
         <td if="showPortlet"
          ...
          <div id=":collapse.id" class="portlet"
               style=":collapse.style">:tool.pxPortlet</div>
          <x>:collapse.px</x>
         </td>
         <!-- Content -->
         <td var="space=not inPopup and ' contentSpace' or ''"
              class=":'content' + space">
          <!-- Navigation strip -->
    ...
    pxViewAsResult = Px('''
     <tr var2="obj=zobj.appy(); mayView=zobj.mayView();
      ...
      <td if="mode.checkboxes" class="cbCell"
          style=":'display:%s' % mode.cbDisplay">
       <input type="checkbox" name=":mode.checkboxesId" id=":cbId"
    ...
    def compute(self, klass, sortBy='', context=None, expression=None,
        ...
        for brain in tool.executeQuery(contentType, search=search, \
            ...
            obj = tool.toObject(brain, noSecurity=noSecurity, appy=True)
            # Ignore dead brains
    ...
    def sendMailIf(self, privilege, subject, body, attachments=None,
        ...
        '''Sends a mail related to this object to any active user having
           p_privilege on it.
           # -------------------------------------------------------------------
           # If p_privilegeType is... | p_privilege is a (list or tuple of)...
           # -------------------------------------------------------------------
        ...
        if pt in ('group', 'user'):
            ...
            objs = self.search(pt.capitalize(), noSecurity=True, login=logins)
            if not objs:
            ...
            # Get all users
            users = self.search('User', noSecurity=True)
