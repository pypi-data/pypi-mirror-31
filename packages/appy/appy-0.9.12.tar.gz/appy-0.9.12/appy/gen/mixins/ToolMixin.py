...
class ToolMixin(BaseMixin):
    ...
    def getHomeObject(self, inPopup=False):
        ...
        try:
            ...
            user = self.getUser()
            if not user:
                # On heavy load, user could be None
                self.log('No user on Tool::getHomeObject', type='error')
                return
            if user.has_role('Manager'): return self.appy()
    ...
    def getPageTitle(self, obj):
        ...
        if obj and ('page' in self.REQUEST):
            ...
            if text:
                if res: res += ' :: '
                res += text
        return self.truncateValue(res, width=100)
    ...
    def switchContext(self):
        ...
        rq = self.REQUEST
        ctx = rq.get('ctx', '')
        if not authContext or not authContext._maySwitchTo(self.appy(), ctx):
            self.raiseUnauthorized()
        # Perform the switch by updating the authentication cookie
        gutils.updateCookie(rq, ctx)
        # Clean session info that may have become obsolete
        if rq.SESSION.has_key('searchCriteria'):
            del rq.SESSION['searchCriteria']
        return self.goto(self.getReferer())
    ...
    def executeQuery(self, className, searchName=None, startNumber=0,
        ...
        if search2: search2.updateSearchCriteria(params, klass)
    ...
    def _getDefaultSearchCriteria(self):
        ...
        if not search: return res
        # In "search" we have the Search instance representing default advanced
        # search criteria.
        wrapperClass = self.getAppyClass(rq.form['className'], wrapper=True)
        search.updateSearchCriteria(res, wrapperClass, advanced=True)
        return res
    ...
    def getSearch(self, className, name, ui=False):
        ...
        if name == 'customSearch':
            ...
            try:
                criteria = self.REQUEST.SESSION['searchCriteria']
            except KeyError, ke:
                # The session has been deleted, probably due to a server restart
                # or a context switching. Display a nice error message.
                self.raiseUnauthorized(self.translate('search_broken'))
    ...
    def getUser(self, authenticate=False, source='any'):
        ...
        if authenticate and (place != 'sso'):
            if (user.state == 'inactive') or \
               ...
               (cfg.authContext and \
                cfg.authContext.isMandatory(tool) and not ctx):
        ...
        req.userLogin = login
        req.authContext = ctx
        if ctx: req.authContextName = cfg.authContext.getName(tool, ctx)
    ...
    def backFromPopup(self, initiator=None):
        ...
        if initiator and initiator.backFromPopupUrl:
            backUrl = self.quote(initiator.backFromPopupUrl)
        else:
            backUrl = 'null'
        context = {'ztool': self, 'backUrl': backUrl}
        return self.appy().pxBack(context)
