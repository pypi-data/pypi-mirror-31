var wrongTextInput = '#F9EDBE none',
    loadingLink = '<img src="ui/loading.gif"/>',
...
function askTimeoutField(hookId) {
  ...
  askField(hookId, data.url, 'view', data.params);
}
...
function prepareForAjaxSave(id, objectId, objectUrl, layoutType) {
  // Prepare widget whose ID is p_id for ajax-saving its content
  var tag = getNode(id),
      // Determine the tag type
      tagType = tag.tagName;
  if (tag.type == 'checkbox') tagType = tag.type;
  tag.focus();
  // For input fields, select all text
  if (tagType == 'INPUT') tag.select();
  ...
  tag.obj = {id: objectId, url: objectUrl, done: false, name:id,
             layoutType: layoutType};
  tag.addEventListener('keydown', performInlineSave);
  // tag.addEventListener('blur', performInlineSave);
  if (tagType == 'checkbox') {
    tag.addEventListener('change', performInlineSave);
  }
}
...
function toggleVisibility(node, nodeType){
  var sNode, elements = node.getElementsByTagName(nodeType);
...
function toggleCb(checkbox) {
  var name = checkbox.getAttribute('name'),
      elems = _rsplit(name, '_', 3),
      // Get the DOM node corresponding to the Ref field    
      node = document.getElementById(elems[0] + '_' + elems[1]),
      // Get the array that stores checkbox statuses
      statuses = node['_appy_' + elems[2] + '_cbs'],
      // Get the array semantics
      semantics = node['_appy_' + elems[2] + '_sem'],
      uid = checkbox.value;
...
function initCbs(id) {
  var elems = _rsplit(id, '_', 3),
      // Get the DOM node corresponding to the Ref field
      node = getNode(elems[0] + '_' + elems[1], true),
      // Get the array that stores checkbox statuses
      statuses = node['_appy_' + elems[2] + '_cbs'],
      // Get the array semantics
      semantics = node['_appy_' + elems[2] + '_sem'],
      value = (semantics == 'unchecked')? false: true,
      // Update visible checkboxes
      checkboxes = getElementsHavingName('input', id);
...
function onDeleteEvent(objectUid, eventTime) {
  var f = document.getElementById('deleteEventForm');
...
function onLinkMany(action, id) {
  var elems = _rsplit(id, '_', 3),
      ...
      statuses = node['_appy_' + elems[2] + '_cbs'],
      uids = stringFromDict(statuses, true),
      // Get the array semantics
      semantics = node['_appy_' + elems[2] + '_sem'];
...
function generatePod(node, uid, fieldName, template, podFormat, queryData,
                     customParams, getChecked, mailing, mailText) {
  ...
  if (getChecked) {
    ...
    if (cNode && cNode.hasOwnProperty('_appy_objs_cbs')) {
      f.checkedUids.value = stringFromDict(cNode['_appy_objs_cbs'], true);
...
function backFromPopup(url) {
  var popup = closePopup('iframePopup'),
      // Get the DOM node to ajax-refresh
      nodeId = popup['back'];
  if (url) window.parent.location = url; // Ignore the back node
  else if (nodeId && getNode(':' + nodeId)) askAjax(':' + nodeId);
...
function askConfirm(actionType, action, msg, showComment, popupWidth,
                    commentLabel, comment, commentRows) {
...
function manageTab(tabId, action) {
  ...
  content.style.display = (action == 'show')? 'table-row': 'none';
  ...
  var left = document.getElementById('tab_' + tabId + '_left'),
      ...
      right = document.getElementById('tab_' + tabId + '_right'),
      suffix = (action == 'hide')? 'u': '',
      path = changeImage(left, 'tabLeft' + suffix + '.png');
...
function showTab(tabId) {
  ...
  for (var i=0; i<tds.length; i++) {
    elem = tds[i];
    if ((elem.className == 'tab') && (elem.id != ('tab_' + tabId))) {
...
function onSelectObject(checkboxId, initiatorId, objectUrl) {
  ...
  var checkbox = document.getElementById(checkboxId),
      ...
      tdDisplay = checkbox.parentNode.style.display;
