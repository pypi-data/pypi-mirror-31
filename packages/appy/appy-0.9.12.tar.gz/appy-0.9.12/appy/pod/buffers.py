...
class MemoryBuffer(Buffer):
    ...
    def hasTag(self, tag):
        '''Is ODF (opening) p_tag found in this buffer or any of its
           sub-buffers ?'''
        r = tag in self.content
        if r: return r
        # Search among the sub-buffers
        for sub in self.subBuffers.itervalues():
            if sub.hasTag(tag):
                return True
    ...
    def insertSubBuffer(self, subBuffer, after=None):
        '''Inserts p_subBuffer just after the main opening tag (or after the
           ...
           complete.'''
        # Find the first p_after opening tag in this buffer (including its
        # sub-buffers).
        if after:
            tag = '<%s' % after
            # Get the sub-buffers and their index, in order
            subs = self.subBuffers.items()
            subs.sort()
            # Get the index of the first p_after opening tag encountered in this
            # buffer.
            afterIndex = self.content.find(tag)
            afterFound = afterIndex != -1
            # Walk sub-buffers
            for subIndex, sub in subs:
                # Is the opening tag present in this sub-buffer ?
                if afterFound:
                    # Check if "tag" is found earlier than "afterIndex" in the
                    # sub-buffer.
                    if (subIndex < afterIndex) and sub.hasTag(tag):
                        sub.insertSubBuffer(subBuffer, after)
                        return
                else:
                    if sub.hasTag(tag):
                        sub.insertSubBuffer(subBuffer, after)
