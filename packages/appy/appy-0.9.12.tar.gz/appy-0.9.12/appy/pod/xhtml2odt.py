...
from appy.pod.doc_importers import px2cm, getUuid
    ...
    def addInnerParagraph(self, env):
        ...
        if self.elem == 'li':
            ...
            if css:
                ...
                p.cssStyles = self.innerCssStyles
                toDump = env.getOdfAttributes(p)
                if not toDump:
            ...
            # Check if a style must be applied on 'p' tags
            p.cssStyles = self.innerCssStyles
            style = env.findStyle(p)
            if style:
                dump(style.getOdfAttributes(p.attrs))
            elif self.innerStyle:
...
class HtmlTable:
    ...
    def setTableStyle(self, attrs, env):
        ...
        margins = tableProps.getMargins()
        # Do not define a specific table style if not necessary
        if not hasWidth and not margins and (align == 'left'):
...
class Xhtml2OdtConverter:
    ...
    def __init__(self, s, encoding, stylesManager, localStylesMapping,
                 keepWithNext, keepImagesRatio, imagesMaxWidth, renderer, html):
        self.renderer = renderer
