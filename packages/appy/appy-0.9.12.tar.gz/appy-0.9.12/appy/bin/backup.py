# ------------------------------------------------------------------------------
import sys, time, os, os.path, smtplib, socket, shutil
from optparse import OptionParser
import ZODB.FileStorage
import ZODB.serialize
from DateTime import DateTime
from StringIO import StringIO
from appy.gen.mail import MailConfig, sendMail
...
class ZodbBackuper:
    ...
    toRemoveExts = ('.doc', '.pdf', '.odt')
    rsyncOptions = ['rsync', '-Paz', '--no-o', '--no-g', '--no-l', '--delete',
      '--quiet', '--exclude', 'Data.fs*', '--exclude', 'Z2.*']
    # --no-o and --no-g options are for preventing rsync to perform chown and
    # chgrp commands that produce errors when done via an sshfs mount.
    ...
    def __init__(self, storageLocation, backupFolder, options):
        ...
        self.filesBackupFolder = options.filesBackupFolder
    ...
    def packZodb(self):
        ...
        for fileSuffix in ('', '.index'):
            ...
            executeCommand(['chown', self.zopeUser, fileName])
    ...
    def sendEmails(self):
        ...
        # Extract server, port, login and password
        login = password = None
        info = self.options.smtpServer.split(':', 3)
        if len(info) == 1: # We simply have the server
            server = info[0]
            port = 25
        elif len(info) == 2: # We have server and port
            server, port = info
            port = int(port)
        else: # We also have login and password
            server, port, login, password = info
        # Secure (TLS) or not ?
        if server.endswith('*'):
            secure = True
            server = server[:-1]
            ...
            secure = False
        # Create a MailConfig instance
        cfg = MailConfig(fromEmail=self.options.fromAddress, server=server,
                       port=port, secure=secure, login=login, password=password)
        # Send the mail
        subject = 'Appy nightlife %s > %s' % \
                  (self.storageLocation, self.backupFolder)
        sendMail(cfg, self.emails, subject, self.logMem.getvalue(),log=self.log)

    def removeTempFiles(self):
        '''LibreOffice (LO) produces temp files that a Appy/Zope process tries
           do delete once he has generated it and returned it to the browser.
           But in some cases it is not possible: Zope runs with a given user
           and LO runs with root and produces files that can't be deleted by the
           user running Zope. This method removes the temp files that could not
           have been removed by Zope.'''
        w = self.log
        w('Removing temp files in "%s"...' % self.tempFolder)
        pdfCount = docCount = odtCount = 0
        ...
        w('%d .pdf, %d .doc and %d .odt file(s) removed.' % \
          (pdfCount, docCount, odtCount))
    ...
    def run(self):
        ...
        self.executeCommand([self.zopectl, 'stop'])
        # Check if we are on the "full backup day"
        dayFull = self.options.dayFullBackup
        if (dayFull == 'All') or time.asctime().startswith(dayFull):
        ...
        if mode == 'zodb':
            w('> Performing backup...')
            self.executeCommand([self.options.python, self.repozo, '-BvzQ',
                           '-r', self.backupFolder, '-f', self.storageLocation])
            # R-sync the DB-controlled file system
            if self.filesBackupFolder:
                filesFolder = os.path.dirname(self.storageLocation)
                w('> Rsyncing DB-controlled file system @%s...' % filesFolder)
                command = self.rsyncOptions + [filesFolder + '/',
                                               self.filesBackupFolder]
        ...
        if self.options.command:
            ...
            self.executeCommand([self.zopectl, 'run', jobScript,
                                 self.options.command])
        self.removeTempFiles()
        # Start the instance again, in normal mode
        w('> Restarting Zope instance...')
        self.executeCommand([self.zopectl, 'start'])
        stopTime = time.time()
...
class ZodbBackupScript:
    ...
    def checkArgs(self, options, args):
        ...
        # Check files folder
        filesFolder = options.filesBackupFolder
        if filesFolder and not os.path.isdir(filesFolder):
            raise BackupError('"%s" is not a folder.' % filesFolder)
        ...
        # Check day of week
        dfb = options.dayFullBackup
        if (dfb != 'All') and (dfb not in self.weekDays):
            raise BackupError(
                "Day of week must be one of %s or 'All'." % str(self.weekDays))
    ...
    def run(self):
        ...
        optParser.add_option("-d", "--day-full-backup", dest="dayFullBackup",
            help="Day of the week where the full backup must be performed " \
                 "(defaults to 'Sun'). Must be one of %s or 'All' if you " \
                 "want a full backup everyday." % str(self.weekDays),
        ...
        optParser.add_option("-s", "--smtp-server", dest="smtpServer",
            help="SMTP server and port (ie: localhost:25) for sending mails. " \
                 ...
                 "myPassword. If you want a secure connection to the SMTP " \
                 "server (TLS), use a star, ie secure.smtp.server*:25.",
            default='localhost:25', metavar="SMTPSERVER", type='string')
        ...
        optParser.add_option("-F", "--filesBackupFolder",
            dest="filesBackupFolder",
            help="Folder where backups of the DB-controlle file hierarchy " \
                 "will be stored.", default='',
                 metavar="FILESBACKUPFOLDER", type='string')
