...
def humerusPose(mkrs, s='R'):
    ...
    Yhum = getVersor(GH - E)
    if s == 'R':
        Xhum = getVersor(np.cross(Yhum, EL - EM))
    else:
        Xhum = getVersor(np.cross(Yhum, EL - EM))
...
def scapulaPose(mkrs, s='R'):
    ...
    # Create versors
    Osca = AA.copy()
    Ysca = getVersor(AA - TS)
    if s == 'R':
        Xsca = getVersor(np.cross(Ysca, AA - AI))
    else:
        Xsca = getVersor(np.cross(Ysca, AA - AI))
    Zsca = getVersor(np.cross(Xsca, Ysca))
...
def R2yxy(Rvect):
    ...
    x1 = np.arccos(R[1,1])
    if R[1,1] > 1:
        print('R2yxy: element [1,1] is more than 1')
