...
def calculateShoulderSegmentsPoses(
    ...
    if scapulaAvailable and not scapulaError:
        ...
        results['scapula_pose_reconstruction_info'] = info1
    if humerusAvailable and not humerusError:
        results['humerus_pose'] = RT2
        results['humerus_pose_reconstruction_info'] = info2
...
def calculateShoulderKinematics(
    ...
    ALs = {m: np.array(landmarksLoc[m]) for m in ["AA","AI","TS"]}
    ...
    ALs = {m: np.array(landmarksLoc[m]) for m in ["GH","EL","EM"]}
    ...
    angles = kine.getJointAngles(Ra1, Ra2, R2anglesFun=kine.R2yxy, funInput='jointR', s=side)
