...
#Calculate the password to start operation
    ...
    def normal_request(self,who,where,what):
        ...
        print('message prima while',message,who)
        while message[1]!=who:
            print('while message',message)
            message = self.read_data()
    ...
    def stato_request(self,who,where):
        ...
        #e la Invio
        if not self.send_data(stato_request):
            self.disconnection()
        ...
        if check_message[1]==who:
        #verifico se ho ricevuto un NACK
            if check_message[len(check_message)- 6:] == OpenWebNet.NACK:
                _LOGGER.exception("Errore Comando non effettuato")
        #o un ACK
            else:
    ...
    def grandezza_request(self,who,where, grandezza):
        ...
        print('grandezza_request',who,where,grandezza)
        ...
        #e la Invio
        if not self.send_data(grandezza_request):
            self.disconnection()
            self.grandezza_request(who,where,grandezza)
    ...
    def stato_luce(self,where):
        ...
        if stato[1] == '1'and stato[2] == where:
            return True
        if stato[1] =='0' and stato[2] == where:
            return False
