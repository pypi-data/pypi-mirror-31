...
def picard_CollectTargetedPcrMetrics_to_df(metric_path, logger):
    select = 'AMPLICON_SET'
    df = picard_select_tsv_to_df(metric_path, select, logger)
    return df

def picard_CollectTargetedPcrMetrics_histogram_to_df(metric_path, logger):
...
def run(job_uuid, metric_path, bam, input_state, engine, logger, metric_name):
    ...
    df = picard_CollectTargetedPcrMetrics_to_df(metric_path, logger)
    df['bam'] = bam
    ...
    df = picard_CollectTargetedPcrMetrics_histogram_to_df(metric_path, logger)
    df['bam'] = bam
    df['input_state'] = input_state
    df['job_uuid'] = job_uuid
    df.to_sql(table_name, engine, if_exists='append')    
