...
class _CachedSessionWrapper(object):
    def __init__(self, cached_session):
        self.__cached_session = cached_session
    ...
    def __should_send_using_default(self, request):
    ...
    def __wrap_cached_session_send(self, cached_session):
        ...
        def wrapped_send(request, **kwargs):
            cached_session.send = prev_send
            if self.__should_send_using_default(request):
    ...
    def wrap_for_async(self):
        self.__wrap_cached_session_send(self.__cached_session)
...
class BackendMetadataDB(BackendMixin, BaseMetadataDB):
    ...
    def __create_query_session(self, query_text):
        is_stable_version = self.__is_stable_query(query_text)

        if is_stable_version:
            from requests_cache import CachedSession

            session = CachedSession(self.__cache_folder_full_path)
        else:
    ...
    def query(self, query_text, **kwargs):
        ...
        session = self.__create_query_session(query_text)

        if async:
            session_wrapper = _CachedSessionWrapper(session)
            session_wrapper.wrap_for_async()
        else:
