...
class BigQueryMetadataDB(BaseMetadataDB, BigQueryMixin):
    ...
    def add_data_using_url(self, metadata_url, dry_mode=False):
        ...
        temp_table_ref = self._get_table_ref(uuid4().hex)

        do_defer(self._delete_temp_table, temp_table_ref.project, temp_table_ref, _countdown=self.TEMP_TABLE_TTL, _tag=temp_table_ref.project)
    ...
    def _query(self, sql_vars, select_fields, where, max_results=None, start_index=None):
        ...
        system_limit = '' if user_limit else 'LIMIT 1000'

        offset = sql_vars.get('offset', 0)
        ...
        def _data_iter_with_group(data_iter):
            ...
            for result in data_iter:
                ...
                current_group_val = result.get(datapoint_key)
            ...
            # edge case: result returns a single group with total items smaller then max_results
            should_send_look_ahead = total_result < max_results
            if should_send_look_ahead:
                for group_result in look_ahead:
                    yield unflatten(group_result, separator='.')
        ...
        with bq_client.get_cursor() as bq_dataset:
            ...
            def do_query():
                ...
                full_result_query_job = self._query_async(query_sql, job_config)

                def process_size_result(results):
                    for result in results:
                        return result['size'], result['count']

                total_size, total_count = BqJob(self._query_async(size_query, job_config), process_size_result).wait()

                data_iter, total_rows = self._query_sync_by_job(
                    full_result_query_job,
                    max_results=actual_max_results, start_index=start_index, process_row=self.build_dict)

                iter_method = _data_iter_with_group if datapoint_key is not None else _data_iter

                return iter_method(data_iter), total_count, total_size
