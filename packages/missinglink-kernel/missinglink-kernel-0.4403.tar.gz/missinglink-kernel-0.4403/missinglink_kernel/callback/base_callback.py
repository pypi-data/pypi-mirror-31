...
class BaseCallback(object):
    def __init__(self, owner_id, project_token, stopped_callback=None, host=None, framework='none', resume_token=None):
    ...
    @property
    def rm_active(self):
        return self.rm_properties.get('in_context') == 'True'
    ...
    def _export_resource_manager(cls):
        rm_data = {'error': "non rm context"}
        mali_env = cls._get_rm_env()
        if len(mali_env) != 0:
            rm_data = cls._rm_map_values(mali_env)
        return rm_data
    ...
    def bind_data_generator(
            ...
            cache_directory=None, batch_size=32, use_threads=False, processes=-1, shuffle=True):
        ...
        return MLDataGenerator(volume_id, query, data_callback, cache_directory, batch_size, use_threads, processes, shuffle)
    ...
    def _sync_working_dir_if_needed(self, repo_obj):
        try:
            ...
            shadow_repo = source_tracking.GitRepoSyncer.clone_shadow_repo(shadow_url)
            commit_tag = source_tracking.GitRepoSyncer.merge_src_to_shadow(repo_obj.repo, shadow_repo, self.post_requests.experiment_token)
    ...
    def _call_new_experiment(self, throw_exceptions=None):
        ...
        res = self.post_requests.create_new_experiment(
            keep_alive_interval, throw_exceptions=throw_exceptions, resume_token=self.properties.get('resume_token'))
