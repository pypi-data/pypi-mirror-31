...
class _CachedSessionWrapper(object):
    def __init__(self, async, is_stable_version, cache_folder_full_path):
        self.__is_stable_query = is_stable_version
        self.__cache_folder_full_path = cache_folder_full_path
        self.__async = async
        self._first_request = None
        self.__session = None

    def store_response(self, result_url, total_rows, total_size, explicit_query):
        response_data = {
            'ok': True,
        ...
        if not self.__is_stable_query:
            return response_data

        with io.BytesIO() as raw_bytes:
            ...
            cache_key = self.__session.cache.create_key(self._first_request)
            self.__session.cache.save_response(cache_key, response)

            raw_string.seek_to_beginning()

            response_data = json.load(io.TextIOWrapper(raw_bytes))
    ...
    def __should_send_using_default(self, request, cached_session):
        if self._first_request is not None:
            return True

        self._first_request = request

        cache_key = cached_session.cache.create_key(self._first_request)

        response, timestamp = self.__session.cache.get_response_and_time(cache_key)

        has_cached_response = response is not None

        return not has_cached_response

    def __wrap_cached_session_send(self, cached_session):
        prev_send = cached_session.send

        async_session = requests.session()

        def wrapped_send(request, **kwargs):
            if self.__should_send_using_default(request, cached_session):
                return async_session.send(request, **kwargs)

            return prev_send(request, **kwargs)

        cached_session.send = wrapped_send

    def create(self):
        from requests_cache import CachedSession

        if self.__is_stable_query:
            cached_session = CachedSession(self.__cache_folder_full_path)

            self.__wrap_cached_session_send(cached_session)
...
class BackendMetadataDB(BackendMixin, BaseMetadataDB):
    ...
    def query(self, query_text, **kwargs):
        ...
        session_wrapper = _CachedSessionWrapper(async, is_stable_version, self.__cache_folder_full_path)

        session = session_wrapper.create()
        ...
        with session:
            ...
            result = self._handle_api(
                self._config, session.get, url, retry=default_api_retry(stop_max_attempt_number=self.max_query_retry), async=async)

            if async and isinstance(result, (tuple, list)):
                result_url, total_rows, total_size, explicit_query = result

                # process result_url into same response as when working with non async
                result = session_wrapper.store_response(result_url, total_rows, total_size, explicit_query)
            elif not result['ok']:
