...
from .scam.luqum.tree import UnknownOperation
from .scam import QueryParser, LuceneTreeTransformer, FunctionOffset, resolve_tree
...
def _download_entity(config_init_dict, volume_id, metadata):
    return DownloadEntity.download(config_init_dict, volume_id, metadata)
...
class AddOffsetFunctionTransformer(LuceneTreeTransformer):
    def __init__(self, offset):
        self.__offset_function = FunctionOffset.create_with_args(offset)
        self.__has_offset = False

    # noinspection PyUnusedLocal
    def visit_function_offset(self, node, parents, context):
        self.__has_offset = True
        return self.__offset_function

    def __call__(self, tree):
        result = self.visit(tree)
        if not self.__has_offset:
    ...
    class DownloadIterResults(collections.Iterator):
        ...
        def __get_next_results(self):
            current_results, current_total_data_points = self.__repo.metadata.query(
                self.__query, max_results=self.__batch_size, start_index=self.__query_offset)
        ...
        def __next__(self):
            ...
            self.__query_offset += len(results)
                ...
                try:
                    while True:
                        for _ in self.__download_results(multi_process_control, storage, results, handle_item):
                            pass  # no need to wait for the feature because the closing context above will wait for the entire pool
