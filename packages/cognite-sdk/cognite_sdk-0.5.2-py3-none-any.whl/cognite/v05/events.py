...
def get_events(type=None, sub_type=None, asset_id=None, **kwargs):
    ...
    headers = {'api-key': api_key, 'content-type': 'application/json', 'accept': 'application/json'}
    params = {'type': type, 'subtype': sub_type, 'assetId': asset_id, 'sort': kwargs.get('sort'), 'cursor': kwargs.get('cursor'), 'limit': (kwargs.get('limit', 25) if (not kwargs.get('autopaging')) else _constants.LIMIT_AGG), 'hasDescription': kwargs.get('has_description'), 'minStartTime': kwargs.get('min_start_time'), 'maxStartTime': kwargs.get('max_start_time')}
    ...
    while (next_cursor and kwargs.get('autopaging')):
        ...
        events.extend(res.json()['data']['items'])
        next_cursor = res.json()['data'].get('nextCursor')
    return EventListResponse({'data': {'nextCursor': next_cursor, 'previousCursor': res.json()['data'].get('previousCursor'), 'items': events}})
