...
from cognite.v05.data_objects import DatapointsResponse, DatapointsResponseIterator, LatestDatapointResponse, DatapointDTO, TimeSeriesDTO, TimeseriesResponse

def get_datapoints(timeseries, aggregates=None, granularity=None, start=None, end=None, **kwargs):
    ...
    partial_get_dps = partial(_get_datapoints_helper_wrapper, timeseries=timeseries, aggregates=aggregates, granularity=granularity, protobuf=kwargs.get('protobuf', True), api_key=api_key, project=project)
    if (steps == 1):
        dps = _get_datapoints_helper(timeseries, aggregates, granularity, start, end, protobuf=kwargs.get('protobuf', True), api_key=api_key, project=project)
        return DatapointsResponse({'data': {'items': [{'tagId': timeseries, 'datapoints': dps}]}})
    prog_ind = _utils.ProgressIndicator([timeseries])
    p = Pool(steps)
    ...
    prog_ind.terminate()
    return DatapointsResponse({'data': {'items': [{'name': timeseries, 'datapoints': concat_dps}]}})

def _get_datapoints_helper_wrapper(args, timeseries, aggregates, granularity, protobuf, api_key, project):
    return _get_datapoints_helper(timeseries, aggregates, granularity, args['start'], args['end'], protobuf=protobuf, api_key=api_key, project=project)

def _get_datapoints_helper(timeseries, aggregates=None, granularity=None, start=None, end=None, **kwargs):
    ...
    url = (config.get_base_url(api_version=0.5) + '/projects/{}/timeseries/data/{}'.format(project, timeseries))
...
def post_datapoints(timeseries, datapoints, **kwargs):
...
def get_latest(timeseries, **kwargs):
...
def get_multi_time_series_datapoints(datapoints_queries, aggregates=None, granularity=None, start=None, end=None, **kwargs):
    ...
    body = {'items': items, 'aggregates': (','.join(aggregates) if (aggregates is not None) else None), 'granularity': granularity, 'start': start, 'end': end}
    headers = {'api-key': api_key, 'content-type': 'application/json', 'accept': 'application/json'}
    datapoints_responses = []
    has_incomplete_requests = True
    while has_incomplete_requests:
        res = _utils.post_request(url=url, body=body, headers=headers, cookies=config.get_cookies()).json()['data']['items']
...
def get_datapoints_frame(timeseries, aggregates, granularity, start=None, end=None, **kwargs):
    ...
    step_starts = [(start + (i * step_size)) for i in range(steps)]
    args = [{'start': start, 'end': (start + step_size)} for start in step_starts]
    partial_get_dpsf = partial(_get_datapoints_frame_helper_wrapper, timeseries=timeseries, aggregates=aggregates, granularity=granularity, api_key=api_key, project=project)
    if (steps == 1):
        return _get_datapoints_frame_helper(timeseries, aggregates, granularity, start, end, api_key=api_key, project=project)
    prog_ind = _utils.ProgressIndicator(timeseries)
    p = Pool(steps)
...
def _get_datapoints_frame_helper_wrapper(args, timeseries, aggregates, granularity, api_key, project):
    return _get_datapoints_frame_helper(timeseries, aggregates, granularity, args['start'], args['end'], api_key=api_key, project=project)

def _get_datapoints_frame_helper(timeseries, aggregates, granularity, start=None, end=None, **kwargs):
    "Returns a pandas dataframe of datapoints for the given timeseries all on the same timestamps.\n\n    This method will automate paging for the user and return all data for the given time period.\n\n    Args:\n        timeseries (list):     The list of timeseries names to retrieve data for. Each timeseries can be either a string containing the\n                            ts name or a dictionary containing the ts name and a list of specific aggregate functions.\n\n        aggregates (list):  The list of aggregate functions you wish to apply to the data for which you have not\n                            specified an aggregate function. Valid aggregate functions are: 'average/avg, max, min,\n                            count, sum, interpolation/int, stepinterpolation/step'.\n\n        granularity (str):  The granularity of the aggregate values. Valid entries are : 'day/d, hour/h, minute/m,\n                            second/s', or a multiple of these indicated by a number as a prefix e.g. '12hour'.\n\n        start (Union[str, int, datetime]):    Get datapoints after this time. Format is N[timeunit]-ago where timeunit is w,d,h,m,s.\n                                    E.g. '2d-ago' will get everything that is up to 2 days old. Can also send time in ms since\n                                    epoch or a datetime object which will be converted to ms since epoch UTC.\n\n        end (Union[str, int, datetime]):      Get datapoints up to this time. Same format as for start.\n\n    Keyword Arguments:\n        api_key (str): Your api-key.\n\n        project (str): Project name.\n\n    Returns:\n        pandas.DataFrame: A pandas dataframe containing the datapoints for the given timeseries. The datapoints for all the\n        timeseries will all be on the same timestamps.\n\n    Note:\n        The ``timeseries`` parameter can take a list of strings and/or dicts on the following formats::\n\n            Using strings:\n                ['<timeseries1>', '<timeseries2>']\n\n            Using dicts:\n                [{'name': '<timeseries1>', 'aggregates': ['<aggfunc1>', '<aggfunc2>']},\n                {'name': '<timeseries2>', 'aggregates': []}]\n\n            Using both:\n                ['<timeseries1>', {'name': '<timeseries2>', 'aggregates': ['<aggfunc1>', '<aggfunc2>']}]\n    "
    (api_key, project) = (kwargs.get('api_key'), kwargs.get('project'))
    ...
    body = {'items': [({'name': '{}'.format(ts)} if isinstance(ts, str) else {'name': '{}'.format(ts['name']), 'aggregates': ts.get('aggregates', [])}) for ts in timeseries], 'aggregates': aggregates, 'granularity': granularity, 'start': start, 'end': end, 'limit': per_tag_limit}
    ...
    while (((not dataframes) or (dataframes[(- 1)].shape[0] == per_tag_limit)) and (body['end'] > body['start'])):
        ...
        dataframes.append(pd.read_csv(io.StringIO(res.content.decode((res.encoding if res.encoding else res.apparent_encoding)))))
...
def get_timeseries(prefix=None, description=None, include_metadata=False, asset_id=None, path=None, **kwargs):
    ...
    headers = {'api-key': api_key, 'accept': 'application/json'}
    params = {'q': prefix, 'description': description, 'includeMetadata': include_metadata, 'assetId': asset_id, 'path': path, 'limit': (kwargs.get('limit', 10000) if (not kwargs.get('autopaging')) else 10000)}
    timeseries = []
    ...
    while (next_cursor and kwargs.get('autopaging')):
        ...
        timeseries.extend(res.json()['data']['items'])
        next_cursor = res.json()['data'].get('nextCursor')
    return TimeseriesResponse({'data': {'nextCursor': next_cursor, 'previousCursor': res.json()['data'].get('previousCursor'), 'items': timeseries}})

def post_time_series(time_series, **kwargs):
    'Create a new time series.\n\n    Args:\n        timeseries (list[v05.data_objects.TimeSeriesDTO]):   List of time series data transfer objects to create.\n\n    Keyword Args:\n        api_key (str): Your api-key.\n\n        project (str): Project name.\n    Returns:\n        An empty response.\n    '
    (api_key, project) = config.get_config_variables(kwargs.get('api_key'), kwargs.get('project'))
    url = (config.get_base_url(api_version=0.5) + '/projects/{}/timeseries'.format(project))
...
def delete_time_series(timeseries, **kwargs):
    ...
    (api_key, project) = config.get_config_variables(kwargs.get('api_key'), kwargs.get('project'))
    url = (config.get_base_url(api_version=0.5) + '/projects/{}/timeseries/{}'.format(project, timeseries))
    headers = {'api-key': api_key, 'accept': 'application/json'}
