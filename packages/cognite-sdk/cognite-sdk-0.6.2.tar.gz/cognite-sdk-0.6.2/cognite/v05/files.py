...
def upload_file(file_name, file_path=None, directory=None, source=None, file_type=None, content_type=None, **kwargs):
    ...
    body = {'fileName': file_name, 'directory': directory, 'source': source, 'fileType': file_type, 'metadata': kwargs.get('metadata', None), 'assetIds': kwargs.get('asset_ids', None)}
...
def list_files(name=None, directory=None, file_type=None, source=None, **kwargs):
    ...
    headers = {'api-key': api_key, 'accept': 'application/json'}
    params = {'assetId': kwargs.get('asset_id'), 'dir': directory, 'name': name, 'type': file_type, 'source': source, 'isUploaded': kwargs.get('is_uploaded'), 'sort': kwargs.get('sort'), 'limit': (kwargs.get('limit', 100) if (not kwargs.get('autopaging')) else 10000), 'cursor': kwargs.get('cursor')}
    file_list = []
