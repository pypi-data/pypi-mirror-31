...
from cognite.v05.dto import DatapointsResponse, DatapointsResponseIterator, LatestDatapointResponse, Datapoint, TimeSeries, TimeSeriesResponse, TimeseriesWithDatapoints
...
def get_datapoints(name, aggregates=None, granularity=None, start=None, end=None, **kwargs):
    ...
    partial_get_dps = partial(_get_datapoints_helper_wrapper, name=name, aggregates=aggregates, granularity=granularity, protobuf=kwargs.get('protobuf', True), api_key=api_key, project=project)
    if (steps == 1):
        dps = _get_datapoints_helper(name, aggregates, granularity, start, end, protobuf=kwargs.get('protobuf', True), api_key=api_key, project=project)
...
def _get_datapoints_helper_wrapper(args, name, aggregates, granularity, protobuf, api_key, project):
    return _get_datapoints_helper(name, aggregates, granularity, args['start'], args['end'], protobuf=protobuf, api_key=api_key, project=project)
...
def _split_TimeseriesWithDatapoints_if_over_limit(timeseries_with_datapoints, limit):
    ...
    if (len(timeseries_with_datapoints.datapoints) > limit):
        ...
        while (i < len(timeseries_with_datapoints.datapoints)):
            timeseries_with_datapoints_list.append(TimeseriesWithDatapoints(name=timeseries_with_datapoints.name, datapoints=timeseries_with_datapoints.datapoints[i:(i + limit)]))
            i += limit
    else:
...
def post_multi_tag_datapoints(timeseries_with_datapoints, **kwargs):
    ...
    timeseries_to_upload_binned: List[List[TimeseriesWithDatapoints]] = _utils.first_fit(list_items=timeseries_with_datapoints_limited, max_size=ul_dps_limit, get_count=(lambda x: len(x.datapoints)))
    for bin in timeseries_to_upload_binned:
        body = {'items': [{'tagId': ts_with_data.name, 'datapoints': [dp.__dict__ for dp in ts_with_data.datapoints]} for ts_with_data in bin]}
...
def get_datapoints_frame(time_series, aggregates, granularity, start=None, end=None, **kwargs):
    ...
    step_starts = [(start + (i * step_size)) for i in range(steps)]
    args = [{'start': start, 'end': (start + step_size)} for start in step_starts]
    partial_get_dpsf = partial(_get_datapoints_frame_helper_wrapper, time_series=time_series, aggregates=aggregates, granularity=granularity, api_key=api_key, project=project)
    if (steps == 1):
        return _get_datapoints_frame_helper(time_series, aggregates, granularity, start, end, api_key=api_key, project=project)
...
def _get_datapoints_frame_helper_wrapper(args, time_series, aggregates, granularity, api_key, project):
    return _get_datapoints_frame_helper(time_series, aggregates, granularity, args['start'], args['end'], api_key=api_key, project=project)
...
def _get_datapoints_frame_helper(time_series, aggregates, granularity, start=None, end=None, **kwargs):
    ...
    body = {'items': [({'name': '{}'.format(ts)} if isinstance(ts, str) else {'name': '{}'.format(ts['name']), 'aggregates': ts.get('aggregates', [])}) for ts in time_series], 'aggregates': aggregates, 'granularity': granularity, 'start': start, 'end': end, 'limit': per_tag_limit}
...
def get_timeseries(prefix=None, description=None, include_metadata=False, asset_id=None, path=None, **kwargs):
    ...
    while (next_cursor and kwargs.get('autopaging')):
        ...
        time_series.extend(res.json()['data']['items'])
        next_cursor = res.json()['data'].get('nextCursor')
    return TimeSeriesResponse({'data': {'nextCursor': next_cursor, 'previousCursor': res.json()['data'].get('previousCursor'), 'items': time_series}})
