        ...
        try:
            if use_gzip:
                if headers:
                    ...
                    headers = {'Content-Encoding': 'gzip'}
                res = requests.post(url, data=gzip.compress(json.dumps(body).encode('utf-8')), headers=headers, params=params, cookies=cookies)
            else:
...
def first_fit(list_items, max_size, get_count):
