...
from cognite.v04.dto import DatapointsResponse, DatapointsResponseIterator, LatestDatapointResponse, Datapoint, TimeSeries, TimeSeriesResponse, TimeseriesWithDatapoints
...
def get_datapoints(tag_id, aggregates=None, granularity=None, start=None, end=None, **kwargs):
    ...
    partial_get_dps = partial(_get_datapoints_helper_wrapper, tag_id=tag_id, aggregates=aggregates, granularity=granularity, protobuf=kwargs.get('protobuf', True), api_key=api_key, project=project)
    if (steps == 1):
        dps = _get_datapoints_helper(tag_id, aggregates, granularity, start, end, protobuf=kwargs.get('protobuf', True), api_key=api_key, project=project)
...
def _get_datapoints_helper_wrapper(args, tag_id, aggregates, granularity, protobuf, api_key, project):
    return _get_datapoints_helper(tag_id, aggregates, granularity, args['start'], args['end'], protobuf=protobuf, api_key=api_key, project=project)
...
def _split_TimeseriesWithDatapoints_if_over_limit(timeseries_with_datapoints, limit):
    ...
    if (len(timeseries_with_datapoints.datapoints) > limit):
        ...
        while (i < len(timeseries_with_datapoints.datapoints)):
            timeseries_with_datapoints_list.append(TimeseriesWithDatapoints(tagId=timeseries_with_datapoints.tagId, datapoints=timeseries_with_datapoints.datapoints[i:(i + limit)]))
            i += limit
    else:
...
def post_multi_tag_datapoints(timeseries_with_datapoints, **kwargs):
    ...
    for bin in timeseries_to_upload_binned:
        body = {'items': [{'tagId': ts_with_data.tagId, 'datapoints': [dp.__dict__ for dp in ts_with_data.datapoints]} for ts_with_data in bin]}
        res = _utils.post_request(url, body=body, headers=headers)
    return res.json()

def post_datapoints(tag_id, datapoints, **kwargs):
...
def get_multi_tag_datapoints(datapoints_queries, aggregates=None, granularity=None, start=None, end=None, **kwargs):
    ...
    body = {'items': items, 'aggregateFunctions': (','.join(aggregates) if (aggregates is not None) else None), 'granularity': granularity, 'start': start, 'end': end}
    headers = {'api-key': api_key, 'content-type': 'application/json', 'accept': 'application/json'}
...
def get_datapoints_frame(tag_ids, aggregates, granularity, start=None, end=None, **kwargs):
    ...
    step_starts = [(start + (i * step_size)) for i in range(steps)]
    args = [{'start': start, 'end': (start + step_size)} for start in step_starts]
    partial_get_dpsf = partial(_get_datapoints_frame_helper_wrapper, tag_ids=tag_ids, aggregates=aggregates, granularity=granularity, api_key=api_key, project=project)
    if (steps == 1):
        return _get_datapoints_frame_helper(tag_ids, aggregates, granularity, start, end, api_key=api_key, project=project)
...
def _get_datapoints_frame_helper_wrapper(args, tag_ids, aggregates, granularity, api_key, project):
    return _get_datapoints_frame_helper(tag_ids, aggregates, granularity, args['start'], args['end'], api_key=api_key, project=project)
...
def _get_datapoints_frame_helper(tag_ids, aggregates, granularity, start=None, end=None, **kwargs):
    ...
    body = {'items': [({'tagId': '{}'.format(tag_id)} if isinstance(tag_id, str) else {'tagId': '{}'.format(tag_id['tagId']), 'aggregates': tag_id.get('aggregates', [])}) for tag_id in tag_ids], 'aggregates': aggregates, 'granularity': granularity, 'start': start, 'end': end, 'limit': per_tag_limit}
