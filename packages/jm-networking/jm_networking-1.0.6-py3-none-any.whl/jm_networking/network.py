...
class Network:
    ...
    def get(self, url):
        self.log(f"Attempting GET {url}")
        req = requests.get(url)
        return self.finish(req)

    def put(self, url, data):
        self.log(f"Attempting PUT {url}")
        req = requests.put(url, data)
        return self.finish(req)

    def post(self, url, data):
        self.log(f"Attempting POST {url}")
        req = requests.post(url, data)
        return self.finish(req)

    def delete(self, url):
        self.log(f"Attempting DELETE {url}")
    ...
    def finish(self, result):
        if result.status_code < 400:
            ...
            self.log(f"ERROR ({result.status_code}): {result.text}", error=True)
