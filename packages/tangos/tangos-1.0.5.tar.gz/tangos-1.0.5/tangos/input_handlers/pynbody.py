...
class PynbodyInputHandler(finding.PatternBasedFileDiscovery, HandlerBase):
    ...
    def load_region(self, ts_extension, region_specification, mode=None):
        if mode is None:
            ...
            return self._isolate_subsnap(timestep[region_specification])
        elif mode=='server':
            timestep = self.load_timestep(ts_extension)
    ...
    def load_object(self, ts_extension, halo_number, object_typetag='halo', mode=None):
        if mode=='partial':
            ...
            h = self._construct_halo_cat(ts_extension, object_typetag)
            return self._isolate_subsnap(h[halo_number])
        else:
            raise NotImplementedError("Load mode %r is not implemented"%mode)
    ...
    def load_tracked_region(self, ts_extension, track_data, mode=None):
        ...
        if mode=='partial':
            ...
            return self._isolate_subsnap(f[indices])
        else:
            raise NotImplementedError("Load mode %r is not implemented"%mode)

    def _isolate_subsnap(self, subsnap):
        """Return a new SubSnap with the same data, but with no direct access to its base/ancestors

        This is useful because it guarantees consistency of behaviour between load modes -- i.e. whether or not
        the full snapshot is stored in memory on the local machine, only the subsnap appears to be present to
        the analysis code"""
        subsnap_isolated = subsnap[:]
        subsnap_isolated.base = subsnap_isolated
        return subsnap_isolated
