...
class _cache:
    ...
    def __write(self):
        ...
        if self.kbo_list:
            ...
            self.__console.error(['No keyboard information found', 'Check QMK directory location in pref.yaml : '+self.__qmk])  # GUIS?
            exit()
...
class application:
    ...
    def __read_args(self):
        ...
        parser.add_argument('-t', '--template', metavar='LAYOUT', dest='template', default='KEYMAP', help='The layout template to reference - default is KEYMAP')
    ...
    def set_kb(self, keyboard='', rev='', keymap='', template=''):
        ...
        if not keyboard:
            self.console.error(['No keyboard name given', 'Valid Names:']) # MISSING
            print(self.keyboard_list())
            exit()
        if not keymap:
        ...
        if build_kbo:
            ...
            self.console.error(['Invalid Keyboard Name - '+keyboard, 'Valid Names:']) # MISSING
            print(self.keyboard_list())
            exit()
