...
class LoginView(UIView):
    ...
    def __init__(self, connection=None):
        ...
        self.username_label = LabelRightAligned(width/2 - 100, height/2-70, 80, 20, "Username : ", font_size=24)
        self.username_box = InputBox(width/2-20, height/2-70, 200, 26, font_size=24)
        self.password_label = LabelRightAligned(width/2 - 100, height/2-35, 80, 20, "Password : ", font_size=24)
        self.password_box = PasswordInput(width/2-20, height/2-35, 200, 26, font_size=24)
        self.create_button = Button(width/2-20, height/2, 60, 30, "Create", self.create, font_size=24)
        self.login_button = Button(width/2+60, height/2, 60, 30, "Login", self.login, font_size=24)
        self.connecting = ConditionalUI(Notification("Connecting...", hideButton=True), self.checkConnection)
        super(LoginView, self).__init__(self.make_view, connection)
