...
class IntValueBox:
    ...
    def draw(self, screen):
        ...
        screen.blit(txt_surface, (self.rect.x + Label.w_padding, self.rect.y + Label.h_padding))
...
class Dropdown(object):
    ...
    def draw(self, screen):
        ...
        self.drawOption(screen, self.rect, selected_text, COLOR_INACTIVE)

        for i in range(0, len(self.dropdownRects)):
            rect = self.dropdownRects[i]
            option = self.options[i]
            self.drawOption(screen, rect, getattr(option, self.displayValue), COLOR_ACTIVE)

    def setOption(self, option):
        self.option = option
        self.selectCallback(option)

    def drawOption(self, screen, rect, text, color):
        pg.draw.rect(screen, (50, 50, 50), rect)
        # Blit the text.
        screen.blit(self.font.render(text, True, color), (rect.x + Dropdown.w_padding, rect.y + Dropdown.h_padding))
        # Blit the rect.
...
class Notification(object):
    ...
    def __init__(self, text, callback, button_text="Ok", text_color=COLOR_INACTIVE, font_size=18):
        self.padding = 10
        self.color = COLOR_INACTIVE
        self.font = pg.font.Font(None, font_size)
        self.callback = callback
