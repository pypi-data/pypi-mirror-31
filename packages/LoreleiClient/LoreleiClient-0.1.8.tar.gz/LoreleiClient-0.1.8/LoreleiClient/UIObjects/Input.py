...
class Button(object):
    ...
    def __init__(self, x, y, w, h, text, callback, fontSize=18, doHoverVisual=True):
        self.x = x
        self.y = y
        self.hover = False
        self.color = COLOR_INACTIVE
        self.font = pg.font.Font(None, fontSize)
        self.doHoverVisual = doHoverVisual
        font_render_size = self.font.size(text)

        width = w
        height = h
        fontwidth = font_render_size[0] + Button.w_padding*2
        fontheight = font_render_size[1] + Button.h_padding*2
        ...
        self.rect = pg.Rect(x, y, width, fontheight)
    ...
    def handle_event(self, event):
        if event.type == pg.MOUSEMOTION:
            if self.doHoverVisual:
                if self.rect.collidepoint(event.pos) or self.hover_rect.collidepoint(event.pos):
                    self.hover = True
                else:
                    self.hover = False
