    ...
    try:
        while True:
            ...
            try_for_each(image_paths, lambda image_path: image_urls.append(
                urljoin(url, document.xpath(image_path, smart_strings=False)[0])))

            def get_next_page(next_page_path):
                new_url = urljoin(url, document.xpath(next_page_path, smart_strings=False)[0])
...
def get_paths_to_url(document, base_url, target_url):
    ...
    is not found. Uses base_url to resolve relative urls.
    """
    xpaths = []
    ...
    for result in document.xpath('|'.join('//@' + attribute for attribute in attributes.split())):
        if urljoin(base_url, result) == target_url:
            ...
            for key, value in element.attrib.items():
                if value == result:
                    xpaths.append(document.getroottree().getpath(element) + '/@' + key)
    if xpaths:
        return xpaths
    else:
