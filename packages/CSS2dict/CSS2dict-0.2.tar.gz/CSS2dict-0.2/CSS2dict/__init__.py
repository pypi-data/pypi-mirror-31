...
class cssp(sys.modules[__name__].__class__):
    def __call__(self,text,noObject=False,includeDefaultCSSProperties=True):  # module callable
        """
CSS2dict(text) -> dictionary containing processed CSS
Example
>> var = CSS2dict('
                ...
                if readingKey:
                    ...
                    if i == ';':
                        ...
                        try:
                            ...
                            a[selector][selection] = Style(includeDefaultCSSProperties)
                        key = rw(key)
                        value = rw(value)
                        a[selector][selection].adop(key,value)
                        key,value = ('','')
