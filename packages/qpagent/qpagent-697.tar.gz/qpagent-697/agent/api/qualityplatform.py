...
from agent.config import qualityplatform_env_api_url, agent_env_port
...
class QualityPlatform(object):
    def __init__(self, env):
        self.base_uri = qualityplatform_env_api_url[env]
        self.port = agent_env_port[env]
    ...
    def upload_file(self, case_file):
        files = {
            'file': case_file
        }
        data = {
    ...
    def upload_case_file(self, test_case_id, case_file):
        fuss_hash = self.upload_file(case_file)
    ...
    def add_stability_report(self, log, status, os_type, device, app, device_id):
        data = {
            "app_version": app['appVersion'],
            "aut": app['downloadURL'],
            "log": log,
