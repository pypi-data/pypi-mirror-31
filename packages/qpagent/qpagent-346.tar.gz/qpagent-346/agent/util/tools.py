...
class DbUtils(object):
    ...
    def get_conn(self):
        try:
            conn = MySQLdb.connect(db=self.db, host=self.host, port=self.port, user=self.user, passwd=self.passwd)
            return conn
    ...
    def update_db_by_params(self, sql, params):
        conn = self.get_conn()
        cursor = conn.cursor()
        try:
...
class RedisUtils(object):
    ...
    def connect(self):
        try:
            ...
            self.session.login(self.hostname, self.username, self.password)
    ...
    def send_common_command(self, command):
        ...
        self.session.prompt()
        return self.session.before.split('\n')[1]
