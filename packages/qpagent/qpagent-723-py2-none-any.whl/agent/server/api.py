...
def get_ios_devices():
    cmd1 = "/usr/local/bin/idevice_id -l"
...
def get_android_devices():
    ...
    cmd1 = "/Users/mobile.test/Develop/android_sdk_r24/platform-tools/adb devices"
    status, output = commands.getstatusoutput(cmd1)
...
def devices():
    ...
    try:
        r = requests.get('http://10.12.116.63:9099/devices/teddy')
        if r.ok:
    ...
    try:
        r = requests.get('http://10.12.118.147:9099/devices/dolores')
        if r.ok:
            robert.append(r.json())
    except requests.RequestException:
        pass
