...
def devices():
    ...
    try:
        r = requests.get('http://10.12.121.243:9099/devices/teddy')
        if r.ok:
    ...
    try:
        r = requests.get('http://10.12.121.243:9099/devices/dolores')
        if r.ok:
            robert.append(r.json())
    except requests.RequestException:
        pass
