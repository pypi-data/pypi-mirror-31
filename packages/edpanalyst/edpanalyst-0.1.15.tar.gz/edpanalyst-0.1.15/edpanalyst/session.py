...
class Session(object):
    ...
    def __init__(
        ...
        # Tack on the edpanalyst version number to the User-Agent
        self._session = requests.Session()
        edpanalyst_version = pkg_resources.get_distribution(
            'edpanalyst').version
        default_ua = self._session.headers.get('User-Agent')
        modded_ua = '%s edpanalyst/%s' % (default_ua, edpanalyst_version)
        self._session.headers.update({'User-Agent': modded_ua})
