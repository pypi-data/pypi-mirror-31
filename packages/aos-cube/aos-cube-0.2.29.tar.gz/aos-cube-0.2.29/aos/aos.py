...
def generate_aos_program(program_path, aos_sdk_path):
    ...
    copy_directory(os.path.join(os.path.dirname(os.path.abspath(__file__)), '.vscode'), os.path.join(program_path, '.vscode'))
...
def make_build(make_args):
    ...
    if os.path.isfile(os.path.join(os.getcwd(), Cfg.file)):
        ...
        with cd(os_path):
            ...
            _run_make(['-e', '-f build/Makefile', make_args, cube_makefile, build_dir])
    else:
        # aos source code
...
def upload():
    ...
    cmds_dir = os.path.join(os.getcwd(), 'build/cmd').replace('\\', '/')
