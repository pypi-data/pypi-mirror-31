...
def _install_toolchains(build_args):
    ...
    #check if required toolchain for target board exist
    if board in boards:
...
def generate_aos_program(program_path, aos_sdk_path, template):
    ...
    example_dir = os.path.join(aos_sdk_path, 'example')
    ...
    copy_directory(os.path.join(os.path.dirname(os.path.abspath(__file__)), '.vscode'),
                   os.path.join(program_path, '.vscode'))
...
def new(name, scm='git', component=False, template=None, protocol=None):
    ...
    if d_type == 'program':
        ...
        import requests
        try:
            github_rtt = requests.get('https://raw.githubusercontent.com/alibaba/AliOS-Things/master/README.md', timeout=2).elapsed.total_seconds()
        except:
            github_rtt = 2
        try:
            gitee_rtt = requests.get('https://gitee.com/alios-things/AliOS-Things/raw/master/README.md', timeout=2).elapsed.total_seconds()
        except:
            gitee_rtt = 2
        if github_rtt < gitee_rtt:
            aos_os_url = 'https://github.com/alibaba/AliOS-Things.git'
        else:
            aos_os_url = 'https://gitee.com/alios-things/AliOS-Things.git'
...
def upload_mk3060(target):
    return None

def upload_esp32(target):
    cmd = 'esptool.py'
    port = miniterm.ask_for_port()
    aos_path = Global().get_cfg(AOS_SDK_PATH)
    image_path = os.path.join(os.getcwd(), 'out', target, 'binary', target + '.bin')
    bootloader_path = os.path.join(aos_path, 'platform/mcu/esp32/bsp/bootloader.bin')
    partition_path = os.path.join(aos_path, 'platform/mcu/esp32/bsp/custom_partitions.bin')
    log("image_path: %s\n" % image_path)
    log("bootloader_path: %s\n" % bootloader_path)
    log("partition_path: %s\n" % partition_path)
...
def upload_stm32(target):
    ...
    upload_cmd = os.path.join(cmds_dir, upload_cmds[host_os])
    log('upload_cmd: %s\n' % upload_cmd)
    addr = '0x08000000'
...
def _run_upload(target):
    app = target.split('@')[0]
    board = target.split('@')[1]
    ret = None

    if 'esp32' in board:
        ret = upload_esp32(target)
    elif 'mk3060' in board:
        ret = upload_mk3060(target)
    else:
        ret = upload_stm32(target)
    return ret
...
def upload():
    """ FIXME we only support stlink tools for now """
    ...
    ret = _run_upload(target)

    if ret == 0:
        print('firmware upload succeed')
    else:
