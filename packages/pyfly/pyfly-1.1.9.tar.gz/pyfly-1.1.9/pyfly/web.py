...
def create_img_div():
    r = html.Div([html.Img(src="http://www.runwalkcrawl.com/wp-content/uploads/NatureBlackFlies-01-BlackFlies.jpg", 
                          alt="fly", width="214", height="209", 
...
def update_fly_num_graph(n):
    ...
    # upload fly_num_list task_num_list  tps_list
    if APP_STATUS == "START" or (APP_STATUS == "STOP" and SD.slave_node.fly_num and SD.fly_num_list):

        run_time = time.time() - task_start_time
        if (not SD.time_index_list) or ((run_time - SD.time_index_list[-1]) > (display_interval-0.1)):
            SD.fly_num_list.append(SD.slave_node.fly_num)
            SD.time_index_list.append(run_time)
            SD.tps_list.append(SD.slave_node.task_num / run_time)
            SD.task_num_list.append(SD.slave_node.task_num)
        
    figure=go.Figure(
        data=[go.Scatter(x=SD.time_index_list, y=SD.fly_num_list, name="虚拟用户")],
...
def update_total_tasks_graph(n):
    ...
    figure=go.Figure(
        data=[go.Scatter(x=SD.time_index_list, y=SD.task_num_list, name="总任务数")],
...
def update_tps_graph(n):
    ...
    figure=go.Figure(
        data=[go.Scatter(x=SD.time_index_list, y=SD.tps_list, name="每秒任务数")],
...
def update_average_response_graph(n):
    ...
    for i, j in SD.task_node.items():
        y_data = j.get("response_stream", [])
        data.append(go.Scatter(x=list(index_list(len(y_data))), y=y_data, name=i)) 
...
def update_task_bar(n):
    ...
    data.append(go.Bar(x=x, y=y, name="MAX"))
    ...
    data.append(go.Bar(x=x, y=y, name="MIN"))
