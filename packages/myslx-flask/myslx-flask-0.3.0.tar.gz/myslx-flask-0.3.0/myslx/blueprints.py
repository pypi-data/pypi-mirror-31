import jwt
import requests
from flask import Blueprint, make_response, redirect, abort, request, g, current_app, render_template, url_for
...
def before_request():
    ...
    try:
        data = jwt.decode(access_token, config.OAUTH_PUBLIC_KEY, algorithms=['RS256'])
...
def callback():
    ...
    if not code:
        abort(403)
    res = requests.post(
        f"https://{config.PORTAL_DOMAIN}/auth/oauth/token?grant_type=authorization_code&code={code}&redirect_uri={url_for('myslx.callback', _external=True)}",
    ...
    if res.status_code != 200:
        current_app.logger.warn(
            'authentication failed, auth svc responsed with %s', res.status_code
        )
        abort(403)
    res_data = res.json()
