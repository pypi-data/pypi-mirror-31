...
def _create_buffer_and_serialize(instance, ignore_user_types, ifacebitmaps, cur_serialized_objs, pending_objs,
                                 reference_counting):
    """ 
    ...
    metadata = create_metadata(cur_serialized_objs, None, 0)
...
def serialize_params_or_return(params, iface_bitmaps, params_spec,
        params_order, hint_volatiles, runtime):
        ...
        for param_name in params_order:
            ...
            if param is not None:
                ...
                if real_type.__name__ == "Future" and \
                                real_type.__module__ == "pycompss.commonruntime.binding":
...
def serialize_dcobj_with_data(dc_object, pending_objs, ignore_user_types, hint, runtime,
    ...
    try: 
        ...
        object_with_data = _create_buffer_and_serialize(dc_object, ignore_user_types, None, cur_serialized_objs, pending_objs,
                                                        reference_counting)        
...
def create_metadata(cur_ser_objs, hint_for_missing, num_refs_pointing_to_obj):
    ...
    for k, v in cur_ser_objs.iteritems():
...
def serialize_for_db(object_id, metadata, object_bytes, is_store):
    """
...
def serialize_for_db_gc(instance, ignore_user_types, ifacebitmaps):
    """
    ...
    cur_serialized_objs = dict() 
    reference_counting = ReferenceCounting()
    pending_objs = list() 
    cur_serialized_objs[instance] = 0
    obj_data = _create_buffer_and_serialize(instance, ignore_user_types, ifacebitmaps, cur_serialized_objs, pending_objs,
                                            reference_counting)
    return serialize_for_db(instance.get_object_id(), obj_data[2], obj_data[3], False)
