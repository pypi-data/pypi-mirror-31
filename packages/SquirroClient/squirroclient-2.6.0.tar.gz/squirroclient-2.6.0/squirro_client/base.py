...
from .bulk import BulkApiMixin
from .exceptions import InputDataError, AuthenticationError, \
    AuthorizationError, NotFoundError, TransportError, ConnectionError, \
    UnknownError, ClientError, ConflictError, RetryError
...
ENDPOINT = {
    ...
    'bulk': 'https://bulk-api.squirro.com',
}
...
class SquirroClient(UserApiMixin, TopicApiMixin, BulkApiMixin):
    ...
    :param bulk_api_url: Endpoint URL for the topic API service.
    ...
    def _perform_request(self, method, url, **kwargs):
        ...
        for retry_count in range(2):
            ...
            try:
                res = func(
                    url.encode('utf-8'), timeout=self.timeout_secs, **kwargs)
            except self._requests.exceptions.RetryError as ex:
                raise RetryError(None, ex[0])
    ...
    def _get_verify(self):
        ...
        if not hasattr(sys, "frozen"):
            ...
            return None
        exe_dir = os.path.dirname(unicode(sys.executable,
