...
class ItemUploader(object):
    ...
    :param processing_config: A dictionary which contains specific instructions
        which are used while processing items for the source. Overriden by
        pipeline workflow name/ID.
    ...
    :param pipeline_workflow_name: Pipeline workflow name. Either name or id
        need to be set, otherwise processing_config is used.
    ...
    def __init__(self, token=None, project_id=None, project_title=None,
                 ...
                 pipeline_workflow_name=None, pipeline_workflow_id='default',
        ...
        assert not (processing_config or steps_config) or \
                not pipeline_workflow_id, \
                'Please don\'t set processing_config or steps_config which ' \
                'are deprecated when you specify pipeline_workflow_id.'
        ...
        if not all([source_id, source_secret]):
            ...
            workflow_id = self._lookup_workflow(pipeline_workflow_name,
    ...
    def _lookup_workflow(self, name, uuid):
        if uuid and uuid != 'default':
            return uuid
        if name or uuid == 'default':
            workflows = self.client.get_pipeline_workflows(self.project_id,
                                                           omit_steps=True)
            for workflow in workflows:
                if uuid == 'default' and workflow['project_default'] or \
                        workflow['name'] == name:
                    return workflow['id']
            raise NotFoundError(u'Workflow "%s" not found!' % (name,))
    ...
    def _create_source(self, source_name, processing_config=None,
                       steps_config=None, pipeline_workflow_id=None):
        ...
        if pipeline_workflow_id is None:
            source_config['processing'] = {
