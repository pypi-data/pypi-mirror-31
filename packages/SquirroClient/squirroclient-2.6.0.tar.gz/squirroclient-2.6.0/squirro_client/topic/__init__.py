...
class TopicApiBaseMixin(object):
    ...
    def recommend_facets_explore(self, project_id, method=None, count=10,
        ...
        :param method: Method of recommendation. Possible values:
            ...
            - conprob: use conditional probability for scoring
            - composition: use sum of individual feature scores for scoring
            - machine_learning: use squirro machine learning service
        ...
        if json_response:
            aquery = json_response.get('query')
            if aquery:
                return self.query(project_id=project_id, query=aquery,
    ...
    def scan(self, project_id, query=None, scroll='5m', count=10000,
        ...
        :param child_count: Maximum number of matching entities to return with
            items. The maximum value that can be set ist 10'000.
...
class TopicApiMixin(
    TopicApiBaseMixin, ContributingRecordsMixin, DashboardsMixin, EntitiesMixin,
    EnrichmentsMixin, FacetsMixin, GlobalTempMixin, MachineLearningMixin,
    ObjectsMixin, PipelineStatusMixin, PipelineWorkflowMixin, ProjectsMixin,
