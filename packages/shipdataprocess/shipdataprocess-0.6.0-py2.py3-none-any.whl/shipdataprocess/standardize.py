...
def standardize_imo(elem, check_field=True):
    if check_field:
        if type(elem)==pd.core.series.Series:
            ...
            return np.nan
        elif (type(elem)==str)|(type(elem)==int)|(type(elem)==float)|(type(elem)==long):
...
def standardize_uvi(elem, check_field=True):
    if check_field:
        if type(elem)==pd.core.series.Series:
            return elem.apply(lambda x: str(int(x)) if (type(x)==float)|(type(x)==int)|(type(x)==long) else re.sub('\s+',' ',x).strip().upper())  
        elif type(elem)==pd.core.frame.DataFrame:
            return elem[check_field].apply(lambda x: str(int(x)) if (type(x)==float)|(type(x)==int)|(type(x)==long) else re.sub('\s+',' ',x).strip().upper())
        elif (elem!=elem)|(elem==None)|(elem=='')|(elem==0):
            return None
        elif (type(elem)==int)|(type(elem)==float)|(type(elem)==long):
            return str(int(elem))
        elif type(elem)==str:
            return re.sub('\s+',' ',elem).strip().upper()
        else:
...
def standardize_geartype(elem, check_field):
    if check_field:
        if 'ALL' in check_field:
            ...
            return elem.apply(lambda x: check_field[str(x).strip().lower()] if (x==x)&(x!=None)&(x!='') else None)
    else:
        return None
