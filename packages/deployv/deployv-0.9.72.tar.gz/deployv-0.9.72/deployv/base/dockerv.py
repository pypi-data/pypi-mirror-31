...
class DockerV(object):
    ...
    def get_container_packages(self):
        ...
        pip = dict([line.split('==') for line in pip_res.split('\n') if '==' in line])
    ...
    def restart_container(self):
        ...
        try:
            ...
            return False
        logger.info('Restarting container %s', container_info.get('name'))
        # Restart container
        try:
            container_obj = self.cli2.containers.get(container_info.get('Id'))
            container_obj.restart()
        except docker.errors.APIError:
            logger.error('Failed to restart the container, it took to long to start')
            return False
