...
ReactionDiffusion<Real_t>::rhs(Real_t t, const Real_t * const y, Real_t * const __restrict__ dydt)
{
    ...
    for (int bi=0; bi<N; ++bi){
        ...
        for (int fi=0; fi<this->fields.size(); ++fi){
            if (fields[fi][bi] == 0)
                continue; // exit early
            const Real_t gfact = (g_value_parents[fi] == -1) ?                 1.0 : LINC(bi, g_value_parents[fi]);
            for (int si=0; si<n; ++si)
                if (g_values[fi][si] != 0)
        ...
        if (N>1){
            ...
            for (int si=0; si<n; ++si){ // species index si
                ...
                for (int xi=0; xi<nstencil; ++xi){
                    ...
                    unscaled_advection += A_WEIGHT(bi, xi) *                         (LINC(biw, si)*efield[bi] + LINC(bi, si)*efield[biw]);
                }
...
template<typename Real_t>
AnyODE::Status
ReactionDiffusion<Real_t>::dense_jac_rmaj(Real_t t,
...
template<typename Real_t>
AnyODE::Status
ReactionDiffusion<Real_t>::dense_jac_cmaj(Real_t t,
                                    const Real_t * const __restrict__ y,
                                    const Real_t * const __restrict__ fy,
                                    Real_t * const __restrict__ ja, long int ldj
                                    , double * const __restrict__ /* dfdt */)
...
template<typename Real_t>
AnyODE::Status
ReactionDiffusion<Real_t>::banded_jac_cmaj(Real_t t,
                                    const Real_t * const __restrict__ y,
                                    const Real_t * const __restrict__ fy,
                                    Real_t * const __restrict__ ja, long int ldj
                                    )
...
template<typename Real_t>
AnyODE::Status
ReactionDiffusion<Real_t>::compressed_jac_cmaj(Real_t t,
                                    const Real_t * const __restrict__ y,
                                    const Real_t * const __restrict__ fy,
                                    Real_t * const __restrict__ ja, long int ldj
                                    )
