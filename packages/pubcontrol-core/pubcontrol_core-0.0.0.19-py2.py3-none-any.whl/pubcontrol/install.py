...
def write_project_path(folder_path):
    ...
    with module_path.open(mode='w') as file:
        ...
        print('[Config] {} lines written back to module'.format(len(lines)))
...
def create_all():
    ...
    print('[Database] With the model names: {}'.format(module_names))
...
def install_all():
    ...
    for install_return in plugin_manager.hook.install(project_path=path):
