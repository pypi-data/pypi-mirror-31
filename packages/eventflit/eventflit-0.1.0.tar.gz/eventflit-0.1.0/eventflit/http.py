...
from eventflit.util import doc_string
from eventflit.errors import *
from eventflit.signature import sign
from eventflit.version import VERSION
...
def process_response(status, body):
    if status == 200 or status == 202:
        ...
        raise EventflitBadRequest(body)

    elif status == 401:
        raise EventflitBadAuth(body)

    elif status == 403:
        raise EventflitForbidden(body)

    else:
        raise EventflitBadStatus("%s: %s" % (status, body))


class Request(object):
    """Represents the request to be made to the Eventflit API.

    An instance of that object is passed to the backend's send_request method
    for each request.

    :param client: an instance of eventflit.Client
    ...
    @property
    def headers(self):
        hdrs = {"X-Eventflit-Library": "eventflit-http-python " + VERSION}
        if self.method == POST:
