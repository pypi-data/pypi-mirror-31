...
def getAllPlugins():
    ...
    for i in possibleplugins:
        ...
        if not os.path.isdir(location) or not mainFile + ".py" in os.listdir(location):
            continue
        info = importlib.find_module(mainFile, [location])
        plugins.append({"name": i, "info": info})
    return plugins

def loadPlugin(pluginName):
    try:
        return importlib.load_source(pluginName, os.path.join(pluginFolder[1:], pluginName, mainFile + ".py"))
    except FileNotFoundError:
        return importlib.load_source(pluginName, os.path.join(pluginFolder, pluginName, mainFile + ".py"))
