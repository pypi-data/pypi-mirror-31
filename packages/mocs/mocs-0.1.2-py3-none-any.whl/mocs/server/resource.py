...
from coupling.misc import get_boolean_from_string
...
class MockRuleResource(BaseResource):
    ...
    def _handle(self, method: str):
        ...
        for rule in config.search_rules(jpath):
        ...
        if found:
            ...
            called = search("$.called", found)
            called["times"] += 1
            config.update_rule(found["id"], {"called": called}, merge=True)
        else:
            self.set_status(404)
...
class RuleDetailResource(BaseResource):
    ...
    def put(self, ident):
        merge = get_boolean_from_string(self.get_query_argument("merge", default="false"))
        reset = get_boolean_from_string(self.get_query_argument("reset", default="false"))
