...
# QFS-14181: httplib doesn't set TCP_NODELAY on sockets. We set this for HTTPS
# connections by overriding httplib.HTTPSConnection. When Qumulo Core supports
# http (vs. https) we need to do the same for httplib.HTTPConnection.
# Alternatively, we could bundle httplib modified, or switch to httplib2 or
# requests.
class NoDelayHTTPSConnection(httplib.HTTPSConnection):
    def connect(self):
...
class Connection(object):
    ...
    def maybe_probe_reuse_flag(self):
        ...
        if (key in Connection.conn_reuse_cache):
            ...
            log.debug("Found reuse cache entry for %s => %s" % (`key`,
        ...
        try:
            rest_request(self, None, "GET", "/v1/version")
            log.debug("Probed server capable of supporting connection resuse")
        except httplib.BadStatusLine:
            self.reuse_connection = False
            log.debug("Probed server incapable of supporting connection resuse")
    ...
    def _get_connection(self, port=None):
        ...
        return NoDelayHTTPSConnection(self.host, self.port, **kwargs)
...
class APIRequest(object):
    ...
    def get_response(self):
        try:
            ...
            log.debug("Got EOF on response... closing and throwing...")
            self.conn.close()
            raise
        ...
        if self.conninfo.must_close():
            log.debug("resetting connection (older qfsd detected)")
            self.conn.close()
...
class PagingIterator(object):
    def __init__(self, initial_url, fn, page_size=None, is_reversed=False):
    ...
    def __reversed__(self):
        return PagingIterator(
            ...
            is_reversed=not self.reversed)

    def next(self):
        if self.uri == '':
            raise StopIteration
        result = self.rest_request(self.uri)
        self.uri = result.data['paging']['prev' if self.reversed else 'next']
