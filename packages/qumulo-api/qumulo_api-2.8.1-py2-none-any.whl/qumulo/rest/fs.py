...
def ref(path, id_):
    ...
    if path is not None and not unicode(path).startswith("/"):
        raise ValueError("Path must be absolute.")
    return unicode(path) if path is not None else unicode(id_)
...
def set_acl(conninfo, credentials, path=None, id_=None, control=None,
    ...
    config = {'aces': list(aces), 'control': list(control)}
    method = "PUT"
...
class LocalUser(FSIdentity):
    def __init__(self, name):
        super(LocalUser, self).__init__('LOCAL_USER', name)

class LocalGroup(FSIdentity):
    def __init__(self, name):
        super(LocalGroup, self).__init__('LOCAL_GROUP', name)
...
def set_file_attr(conninfo, credentials, mode, owner, group, size,
        ...
        extended_attrs=None, if_match=None, path=None):
    """
