...
@app.route("/model-source-init/<string:model_name>/<string:model_tag>/<string:model_type>/<string:new_model_path>/", methods=['GET'])
def model_source_init(model_name,
                      model_tag,
                      model_type,
                      new_model_path): # this dir must be empty and relative to $(PWD) of this running process
    ...
    return_dict = {"status": "complete",
                   ...
                   "model_type": model_type,
                   "new_model_path": new_model_path,
                   "new_file_path_list": rendered_filenames,
                  }
...
@app.route("/model-source-update/<string:model_name>/<string:model_tag>/<string:model_path>/<string:file_path>/<string:file_content>/", methods=['GET'])
def model_source_update(model_name,
                        model_tag,
                        model_path, # model_path must exist - relative to $(PWD) of this running process
                        file_path, # file_path will overwrite whatever is already - relative to $(PWD)
                        file_content): # model_path must exist - relative to file_path 

    model_tag = _validate_and_prep_model_tag(model_tag)

    if not model_path:
        model_path = _os.path.normpath('%s/%s/%s-%s/' % (_model_base_path, model_type, model_name, model_tag))
    else:
    ...
    if not _os.path.exists(new_model_path):
        return_dict = {"status": "incomplete",
                       "error_message": "Model path '%s' does not exist. Please specify a different path." % model_path,
                       "model_name": model_name,
                       "model_tag": model_tag,
    ...
    file_path = _os.path.expandvars(file_path)
    file_path = _os.path.expanduser(file_path)
    file_path = _os.path.abspath(file_path)
    file_path = _os.path.normpath(file_path)

    if _is_base64_encoded(file_content):
        file_content = _decode_base64(file_content)

    updated_file_path = _os.path.normpath('%s/%s' % (model_path, file_path))
    with open(updated_file_path, 'wt') as fh:
        fh.write(file_content)
        print("Saved '%s'" % updated_file_path)
...
def predict_server_build(model_name,
    ...
    # TODO:  investigate why relpath doesn't work when abs path is passed in
...
def predict_server_tar(model_name,
                       ...
                       filemode='w',
                       compression='gz'):
    return model_tar(model_name=model_name,
                model_tag=model_tag,
                model_path=model_path,
                filemode=filemode,
...
def _model_tar_and_upload(model_name,
                             model_tag,
                             model_type,
                             model_path,
                             model_runtime=None,
    ...
    upload_file_path = model_tar(model_name=model_name,
                            model_tag=model_tag,
                            model_path=model_path,
                            tar_path=tar_path,
                            filemode=filemode,
    ...
    return model_source_upload(model_name=model_name,
                               model_tag=model_tag,
                               model_type=model_type,
                               model_runtime=model_runtime,
                               model_chip=model_chip,
...
@app.route("/model-source-upload/<string:model_name>/<string:model_tag>/<string:model_type>/<string:new_model_path>/", methods=['POST'])
def model_source_upload(model_name,
                          model_tag,
                          model_type,
                          new_model_path, # relative, usually something like '<model-type>/<model-name>-<model-tag>/'
...
@app.route("/spark-kube-scale/<int:replicas>/", methods=['GET'])
