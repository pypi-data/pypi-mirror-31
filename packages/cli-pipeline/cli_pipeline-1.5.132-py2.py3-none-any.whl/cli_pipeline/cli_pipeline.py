...
def predict_tensorflow_describe(model_name, # Currently unused
    ...
    absolute_model_path = _os.path.expandvars(absolute_model_path)
    absolute_model_path = _os.path.expanduser(asbsolute_model_path)
    absolute_model_path = _os.path.abspath(absolute_model_path)
    absolute_model_path = _os.path.normpath(absolute_model_path)
...
def model_source_init(model_name,
    ...
    # Refactored to eliminate the second FileSystemLoader call
    # which was reloading templates and including .DS_Store files
    templates_base_path = _os.path.normpath(_os.path.join((_os.path.join(_templates_path(), 'models')), model_type))
    print("templates_base_path: '%s'" % templates_base_path)
    env = _jinja2.Environment(loader=_jinja2.FileSystemLoader(templates_base_path))
    absolute_rendered_filenames = []
    i = 0
    model_files_dict = dict()

    for template in env.list_templates(filter_func=filter_func):

        try:
            print("template: '%s'" % template)
            rendered_content = env.get_template(template).render(context)
        except Exception:
            rendered_content = "unable to load template: '%s'" % template
            print(rendered_content)

        rendered_filename = _os.path.normpath(_os.path.join(absolute_model_path, template))
        rendered_path, rendered_name = _os.path.split(rendered_filename)
        # TODO: Refactor this
        ...
        absolute_rendered_filenames += [rendered_filename]
        model_files_dict[i] = {
            'file_name': template,
    ...
    return_dict = {
        ...
        "model_path": model_path,
        "absolute_model_path": absolute_model_path,
        "absolute_model_file_path_list": absolute_rendered_filenames,
        "model_files": model_files_dict
    }
...
def model_source_set(model_name,
    ...
    model_path = _os.path.normpath(model_path)
    absolute_model_path = _os.path.abspath(model_path)
    print("absolute_model_path: '%s'" % absolute_model_path)
...
def train_server_start(model_name,
    ...
    cmd = '%s run -itd -p 2222:2222 -p 6006:6006 -e PIPELINE_LOCAL_MODEL=true -e PIPELINE_STREAM_LOGGER_URL=%s -e PIPELINE_STREAM_LOGGER_TOPIC=%s -e PIPELINE_STREAM_INPUT_URL=%s -e PIPELINE_STREAM_INPUT_TOPIC=%s -e PIPELINE_STREAM_OUTPUT_URL=%s -e PIPELINE_STREAM_OUTPUT_TOPIC=%s -e TF_CONFIG=%s -e PIPELINE_TRAIN_ARGS="%s" -v %s:/opt/ml/input/samples/models/ -v %s:/opt/ml/input/ -v %s:/opt/ml/output/ --name=%s %s %s %s/%s/%s-%s:%s train' % (start_cmd, stream_logger_url, stream_logger_topic, stream_input_url, stream_input_topic, stream_output_url, stream_output_topic, tf_config_local_run, train_args, current_path, input_host_path, output_host_path, container_name, memory_limit, start_cmd_extra_args, image_registry_url, image_registry_repo, image_registry_namespace, model_name, model_tag)
    print("")
    print(cmd)
    print("")
    process = _subprocess.call(cmd, shell=True)
...
@_app.route("/train-kube-start/<string:model_name>/<string:model_tag>/<string:input_host_path>/<string:output_host_path>/", methods=['GET'])
