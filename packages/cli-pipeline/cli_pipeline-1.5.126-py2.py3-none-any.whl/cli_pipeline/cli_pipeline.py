...
def model_source_init(model_name,
    ...
    uncleaned_templates_list = _jinja2.Environment(loader=_jinja2.FileSystemLoader(templates_base_path)).list_templates()
    templates_list = [t for t in uncleaned_templates_list if t != '.DS_Store']
...
def model_source_set(model_name,
                     model_tag,
                     model_path, # absolute path, must exist
                     model_file_path, # absolute path, will overwrite whatever is there (for now)
                     model_file_contents): 
...
def model_source_get(model_name, # this isn't used right now, but it's here in case we need it for extra security/validation
                     model_tag, # same as above ^
                     model_path, # absolute path, must exist
                     model_file_path): # absolute path, must exist
...
def predict_server_untar(model_name,
    ...
    return model_untar(model_name=model_name,
                  model_tag=model_tag,
                  model_path=model_path,
    ...
    if _os.path.exists(absolute_model_path):
        ...
        if _request.method == 'POST':
            ...
            if f and _allowed_file(f.filename):
                ...
                model_untar(model_name=model_name,
                       model_tag=model_tag,
