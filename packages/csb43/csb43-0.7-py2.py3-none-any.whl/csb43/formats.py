...
def _transactionRow(t, decimal_fallback):
    ...
    name = ", ".join(dec(x.item1).rstrip(' ') for x in t.optionalItems)

    extdname = ", ".join(dec(x.item2).rstrip(' ') for x in t.optionalItems)

    if t.exchange:
        o_currency = utils.export_currency_code(t.exchange.sourceCurrency)
        o_amount = utils.export_decimal(t.exchange.amount, decimal_fallback=decimal_fallback)
    else:
        o_currency = None
        o_amount = None
...
try:
    ...
    TABLIB_FORMATS = []
    warnings.warn(_("Package 'tablib' not found. Formats provided by 'tablib'"
                    " will not be available."), FormatWarning)
...
try:
    ...
    warnings.warn(_("Package 'yaml' not found. Hierarchical YAML format"
                    " will not be available."), FormatWarning)
