    ...
    if res['ret'] == 0 and res['data']:
        ...
        for val in res['data']['scene_list']:
            if SCENE_DESC.get(val['label_id']):
                res_list.append({'label_id':SCENE_DESC[val['label_id']],'label_confd':str(math.ceil(val['label_confd']*100)) + '%'})
        return res_list
    else:
        return -1
    ...
    if res['ret'] == 0 and res['data']:
        ...
        for val in res['data']['object_list']:
            if OBJECT_DESC.get(val['label_id']) :
                res_list.append({'label_id':OBJECT_DESC[val['label_id']],'label_confd':str(math.ceil(val['label_confd']*100)) + '%'})
        return res_list
    else:
        return -1
