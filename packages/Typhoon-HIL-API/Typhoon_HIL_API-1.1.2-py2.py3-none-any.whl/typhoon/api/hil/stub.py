...
import threading
import numpy as np
from typhoon.api import get_conf_value, DEBUG, LoggingMixin, ClientStub,\
    thread_safe
...
def recv_array(socket, flags=0, copy=False, track=True):
    ...
    return B.reshape(md['shape']).astype("<f4")
...
class ClientAPIStub(LoggingMixin, ClientStub):
    ...
    def __init__(self, log_file="client.log"):
        ...
        rep_port = get_conf_value("hil", "server_rep_port")
        self._server_addr = "tcp://localhost:{}".format(rep_port)
    ...
    def _ping_resp_handler(self, response):
        # Get publisher address, and bind to it...
        paddr = response["result"]
        self._pub_addr = paddr
