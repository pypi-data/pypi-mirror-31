...
def prepare_source_arbitrary_waveform(name, file=""):
    """
    Assigns an arbitrary waveform file (isg) to a given independent voltage/current source.
    ...
    .. note::
        Only one waveform can be queued for update. Further calls to this function will overwrite the previously prepared waveform.

    Args:
        name (str): source name. List of all source names can be displayed using the ``available_sources()`` function.
        file (str): absolute or relative path to the waveform file (isg extension).
...
def prepare_source_constant_value(name, value=0):
    """
    Sets a constant value to a given independent voltage/current source.
    ...
    .. note::
        Only one value change can be queued for update. Further calls to this function will overwrite the previously prepared value.
...
def prepare_source_sine_waveform(name, rms=None, frequency=None, phase=None, harmonics=None, harmonics_pu=None):
    """
    Assigns a sinusoidal signal to a given independent voltage/current source.
    ...
    .. note::
        Only one signal change can be queued for update. Further calls to this function will overwrite the previously prepared signal change.

    Args:
        name (str): source name. List of all source names can be displayed using the ``available_sources()`` function.
        rms (float): rms value.
        frequency (float): frequency (default 50Hz).
        phase (float): phase (in degrees).
        harmonics (list): list of harmonics, which should be specified as follows: list[(harmonic_number_1,rms_1,phase_1),...(harmonic_number_n,rms_n,phase_n)].
        harmonics_pu (list): list of per-unit harmonics, which should be specified as follows: list[(harmonic_number_1,rms_p_u_1,phase_1),...(harmonic_number_n,rms_p_u_n,phase_n)] where
                            'rms_p_u_n' value is harmonic rms given in a relative unit between 0 and 1.

    .. note::
        ``harmonics`` argument was left for the compatibility purposes. Old type of harmonics will be automatically converted to the new per-unit harmonics.
    ...
    Example::
        ...
        hil.prepare_source_sine_waveform("Vb", rms = "220", frequency = 50, phase = 120,harmonics = harmonics)
    """
...
def set_source_arbitrary_waveform(name, file):
    ...
    Returns:
        ``True`` if an arbitrary waveform file is successfully assigned, otherwise returns ``False``.
    ...
    """
    return clstub().set_source_arbitrary_waveform(name=name, file=os.path.abspath(file))


def set_source_constant_value(name, value=0, executeAt=None):
    """
    Sets a constant value to a given independent voltage/current source.
...
def set_source_sine_waveform(name, rms=None, frequency=None, phase=None, harmonics=None, harmonics_pu=None, executeAt=None):
    """
    Assigns a sinusoidal signal to a given independent voltage/current source.

    Args:
        name (str): source name. List of all available source names can be displayed using the ``available_sources()`` function.
        rms (float): signal rms value.
        frequency (float): signal frequency (default 50Hz).
        phase (float): signal phase (in degrees).
        harmonics (list): list of harmonics defined in absolute units, specified as follows:
                          list[(harmonic_number_1,rms_1,phase_1),...(harmonic_number_n,rms_n,phase_n)].
        harmonics_pu (list):   list of harmonics defined in relative units, specified as follows:
                               list[(harmonic_number_1,rms_pu_1,phase_1),...(harmonic_number_n,rms_pu_n,phase_n)] where 'rms_pu_n' value is harmonic rms given in relative units between 0 and 1.
        executeAt (int, float): executes this command at a specified simulation time.
                                If ``executeAt`` is not specified (or ``executeAt`` == None)
                                the command will be executed immediately.

    .. note::
        ``harmonics`` argument is left for compatibility purposes. For all new development purposes, it is strongly recommended to use the ``harmonics_pu`` attribute instead.

    .. note::
        It is strongly recommended to use the ``harmonics_pu`` argument for specifying higher harmonic content.

    .. note::
        ``executeAt`` option is not available if the ``harmonics`` or ``harmonics_pu`` arguments are used.
...
def set_source_scaling(name, scaling, executeAt=None):
    """
    ...
    Args:
        name (str): source name. List of all source names can be displayed using the ``available_sources()`` function.
        scale (float): the new scaling factor (float value)
    ...
    """
    return clstub().set_source_scaling(name=name, scaling=scaling,
                                       executeAt=executeAt)


def set_pv_input_file(name, file, illumination=0.0, temperature=0.0):
    """
    Assigns an IV curve to a given photovoltaic panel.

    Args:
        name (str): name of the photovoltaic, the list of all photovoltaic panels can be displayed using the ``available_pvs()`` function.
    ...
    """
    return clstub().set_pv_input_file(name=name, file=os.path.abspath(file),
                                      illumination=illumination,
                                      temperature=temperature)


def set_pv_amb_params(name, illumination=None, temperature=None, executeAt=None):
    """
    ...
    Args:
        name (str): name of the photovoltaic. List of all photovoltaic panels can be displayed using the ``available_pvs()`` function.
    ...
    Example::
        ...
        # NOTE: You need to wait 10000 simulation cycles until the first time command is finished
        #       and then execute the next timed command

    """
    return clstub().set_pv_amb_params(name=name, illumination=illumination,
                                      temperature=temperature,
                                      executeAt=executeAt)
...
def set_analog_output(channel, name=None, scaling=None, offset=None, device=0):
    ...
    Args:
        channel (int): analog output channel number.
        name (str): name of the analog signal. List of all analog signal names can be displayed using the ``available_analog_signals()`` function.
...
def set_digital_output(channel, name=None, invert=None, swControl=None,
    ...
    Args:
        ...
        name (str): name of assigned digital signal. List of all digital signal names can
                    be displayed using the ``available_digital_signals()`` function.
...
def set_digital_output_signal(channel, name,device = 0):
    ...
    Args:
        channel (int): digital output channel number.
        name (str): name of the assigned digital signal. List of all digital signal names can be displayed using the ``available_digital_signals()`` function.
    ...
    """
    return clstub().set_digital_output_signal(channel=channel, name=name,
                                              device=device)
...
def set_contactor(name, swControl=None, swState=None, executeAt=None):
    ...
    Args:
        name (str): contactor name. List of all contactors can be displayed
                    using the ``available_contactors()`` function.
...
def set_contactor_control_mode(name, swControl=False, executeAt=None):
    ...
    Args:
        name (str): contactor name. List of all contactors can be displayed using the ``available_contactors()`` function.
        swControl (bool): Defines the control mode. If ``True`` (software mode) the contactor state
    ...
    """
    return clstub().set_contactor_control_mode(name=name, swControl=swControl,
                                               executeAt=executeAt)
...
def set_contactor_state(name, swState=False, executeAt=None):
    ...
    """
    return clstub().set_contactor_state(name=name, swState=swState,
                                        executeAt=executeAt)
...
def set_machine_linear_torque(name="", value=0.0, executeAt=None):
    ...
    """
    return clstub().set_machine_linear_torque(name=name, value=value,
                                              executeAt=executeAt)
...
def set_machine_square_torque(name="", value=0.0, executeAt=None):
    ...
    """
    return clstub().set_machine_square_torque(name=name, value=value,
                                              executeAt=executeAt)
...
def set_machine_speed(name="", speed=0.0, executeAt=None):
    ...
    """
    return clstub().set_machine_speed(name=name, speed=speed,
                                      executeAt=executeAt)
...
def set_machine_inc_encoder_offset(name="", offset=0.0):
    ...
    Returns:
        ``True`` if the incremental encoder offset is set successfully, otherwise returns ``False``.
    ...
    """
    return clstub().set_machine_inc_encoder_offset(name=name, offset=offset)
...
def set_machine_sin_encoder_offset(name="", offset=0.0):
    ...
    Returns:
        ``True`` if the machine's sinusoidal encoder and the resolver offset are set successfully, otherwise returns ``False``.
    ...
    """
    return clstub().set_machine_sin_encoder_offset(name=name, offset=offset)


def set_pe_switching_block_control_mode(blockName="", switchName="", swControl=True, executeAt=None):
    """
...
def set_cp_input_value(cpCategory, cpGroup, cpInputName, value):
    ...
    .. note::
        This function will be soon deprecated. Please use the ``set_scada_input_value()`` fucntion instead.
    ...
    return clstub().set_cp_input_value(cpCategory=cpCategory,
                                       cpGroup=cpGroup,
                                       cpInputName=cpInputName, value=value)
...
def start_capture(cpSettings, trSettings, chSettings, dataBuffer=[], fileName="", executeAt=None, timeout=None):
    """

        Starts the capture process that will in the background collect data, return them and, if
        ``fileName`` is specified, save them into a MATLAB .mat file.
        ...
        Args:
            ...
            fileName (str): the desired name of .mat file. If you do not specify this parameter, the captured data will not be written to a .mat file
            executeAt (int, float): executes this command at a specified simulation time.
                If ``executeAt`` is not specified (or ``executeAt`` == None) the command will be executed immediately.
            timeout (int): Starts a counter to abort the capture process after n seconds. When not defined, the timeout will be inf.

        Returns:
            * in the background, the .mat file will be saved with the desired name (only if ``fileName`` is specified)
    ...
    return clstub().start_capture(cpSettings=cpSettings,
                                  ...
                                  fileName=fileName, executeAt=executeAt,
                                  timeout=timeout)
...
def read_analog_signals(signals=()):
    ...
    Args:
        signals (list): list of the analog signals names. List of all analog signal names can be displayed using the ``available_analog_signals()`` function.
...
def read_digital_signal(name="", device=None):
    ...
    Args:
        name (str): name of the digital signal. List of all digital signal names can be displayed using the ``available_digital_signals()`` function.
...
def read_digital_signals(signals=()):
    ...
    Args:
        signals (list): list of digital signal names. List of all digital signal names can be displayed using the ``available_digital_signals()`` function.
...
def get_cp_output_value(cpCategory, cpGroup, cpOutputName):
    ...
    .. note::
        This function will be soon deprecated. Please use the ``get_scada_output_value()`` instead.
...
def get_pv_panel_settings(name):
    ...
    Returns:
        Dictionary with the PV panel settings, or ``None`` if the PV panel with the given name cannot be found.
...
def get_cp_input_settings(cpCategory, cpGroup, cpInputName):
    ...
    .. note::
        ...
        This function will soon be deprecated. Please use the ``get_scada_input_settings()`` instead.
