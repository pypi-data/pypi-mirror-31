...
class Serializer(object):
    ...
    def __init__(self, data, datetime_format='timestamp', output_type='raw', include_attr=None, exclude_attr=None,
                 foreign=False, many=False, through=True, extra_attr=None, *args, **kwargs):
        if include_attr:
    ...
    def data_inspect(self, data, extra=None):
        if isinstance(data, (QuerySet, Page, list)):
                    ...
                    if field.name not in obj_dict.keys() and field.name not in self.through_fields:
                        if field.remote_field is None:
                            ...
                            if self.check_attr(field.name) and self.foreign:
                                obj_dict[field.name] = self.data_inspect(getattr(extra, field.name))
            if self.extra_attr:
                for name in self.extra_attr:
                    obj_dict[name] = self.data_inspect(getattr(data, name))
            return obj_dict
            ...
            return data.replace(tzinfo=pytz.utc).astimezone(pytz.timezone('Asia/Shanghai')).strftime('%F %T')
            ...
            return escape(data) if isinstance(data, str) else data
        elif isinstance(data, (types.MethodType, partial)):
            if isinstance(data(), (str, bool, float, int, list, dict, tuple)):
