...
from tgasdk.TGAException import TGAException, TGANetworkException, TGAIllegalDataException
...
class TGAnalytics(object):
    ...
    def __init__(self,consumer):
        ...
        self.track_pattern = re.compile(r"^(#time)|(#ip)|(#country)|(#province)|(#city)|(#os_version)|(#manufacturer)|(#lib_version)|(#os)|(#device_id)|(#screen_height)|(#device_model)|(#app_version)|(#screen_width)|(#lib)|(#network_type)|(#carrier)|(#browser)|(#browser_version)|([a-z][a-z0-9_]{0,50})$", re.I)
        self.user_pattern = re.compile(r"^(#time)|(#ip)|([a-z][a-z0-9_]{0,50})$",re.I)
    ...
    def __add(self,distinct_id,account_id,type,event_name,properties):
        ...
        # self.__consumer.send(str(data))#python3
        self.__consumer.send(json.dumps(data))#python2
        ...
        if properties is not None:
            ...
            for key, value in properties.items():
                ...
                if not key_pattern.match(key):
                    raise TGAIllegalDataException("type[%s] property key must be a valid variable name. [key=%s]" % (action_type,str(key)))

                if not is_str(value) and not is_int(value) and not isinstance(value, (float)) and not isinstance(value,datetime.datetime) and not isinstance(value,datetime.date):
                    raise TGAIllegalDataException("property value must be a str/int/float/datetime/date. [value=%s]" % type(value))
...
class LoggingConsumer(object):
    ...
    def __init__(self, prefix):
        ...
        self.logger = logging.getLogger('tga')
        self.logger.propagate = False
        self.logger.setLevel(logging.INFO)
        self.handler = None
        if self.prefix.endswith("/"):
            self.prefix = self.prefix[0:len(self.prefix)-1]
        self.handler = self.__initLogger(self.prefix+"/log."+self.suffix)


    def __initLogger(self,filename):
    ...
    def send(self, msg):
        ...
        if self.suffix != date_suffix:
            self.handler = self.__initLogger(self.prefix+"."+date_suffix)
...
class BatchConsumer(object):
    ...
    def __init__(self, server_uri,appid,batch=10, timeout=None,interval=3):
        """
        Consumer construct
    ...
    def send(self, msg):
        if len(self.__message_channel) >= self.__batch or (time.time() - self.__last_flush >= self.__interval and len(self.__message_channel)>0):
            http_msg = self.__message_channel[:self.__batch]
            self.flush(http_msg)
            self.__message_channel = self.__message_channel[self.__batch:]
    ...
    def close(self):
        if len(self.__message_channel) > 0:
            self.flush(self.__message_channel)

class HttpConsumer(object):

    def __init__(self,server_uri,appid,timeout=None):
        self.url = server_uri
        self.appid = appid
    ...
    def send(self, data):
        ...
        headers['user-agent'] = 'tga-0.1.0'
        try:
            import requests
            requests.post(self.url, data=self._gzip_string(data.encode("utf-8")),headers=headers)
        except Exception as e:
...
class AsyncBatchConsumer(object):
    ...
    def __init__(self, server_uir,appid, interval=10, flush_size=20, max_batch_size=100, queue_size=1000, request_timeout=None):
        """
        ...
        """
        self.__consumer = HttpConsumer(server_uir,appid)
...
#     http = HttpConsumer('http://127.0.0.1:8991/logagent','quanjie')
#     data = '[{"#app_id": "quanjie","data": {"#account_id": "dojk0","#distinct_id": "F53A58ED-E5DA-4F18-B082-7E1228746E88","#type": "user_set","#time": "2017-12-18 14:37:28.527","properties": {"age": 24,},"#ip":"123.123.123.123"}},{"#app_id": "abc-ios","data": {"#account_id": "dojk0","#distinct_id": "F53A58ED-E5DA-4F18-B082-7E1228746E88","#type": "user_set","#time": "2017-12-18 14:37:28.527","properties": {"age": "0"}}}]'
#     print(http.send(data))