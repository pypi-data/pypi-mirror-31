...
class FHIRServices:
    ...
    def add_point_of_care(self, email, point_of_care):
        ...
        list = self.query_list_record(email)

        # Count existing orgs
        orgs = list['entry']['resource']['entry']
        ...
        # Don't recreate Organizations if we can find them by the exact name. No fuzzy matching.
        organization_request.ifNoneExist = str(furl.Query({'name:exact': organization.name}))

        # Create the organization entry
        organization_entry = BundleEntry({
            'resource': organization.as_json()
        })
    ...
    def research_study_update(self, email, research_study_list):
        ...
        for research_study_title in research_study_list:
            ...
            research_study_request = BundleEntryRequest({
                'url': 'ResearchStudy',
                'method': 'POST',
                'ifNoneExist': str(furl.Query({'title': research_study_title}))
            })
