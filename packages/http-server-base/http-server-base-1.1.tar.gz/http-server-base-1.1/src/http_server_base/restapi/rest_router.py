import inspect
from typing import List, Union, Dict, Type, Tuple, Any, Pattern, Callable, TypeVar

import parse
import re

from http_server_base.tools import re_type, ReDict
from http_server_base.restapi import ArgumentError, ArgumentType, MethodNotAllowedError, ArgumentListType, CanonicalArgumentListType, CanonicalArgumentType
...
class RestRouter(Router):
    ...
    def get_class_name(cls:type) -> str:
        _m = inspect.getmodule(cls)
        _class_name = cls.__qualname__.split('.<locals>', 1)[0].rsplit('.', 1)[0]
        return f'{_m.__name__}.{_class_name}'
    
    @staticmethod
    def get_function_parent_class(func) -> type:
    ...
    @staticmethod
    def get_function_parent_class_name(func) -> str:
        if (inspect.ismethod(func)):
            for cls in inspect.getmro(func.__self__.__class__):
                if (cls.__dict__.get(func.__name__) is func):
    ...
    def get_canonical_form_of_argument(cls, arg:ArgumentType) -> CanonicalArgumentType:
        ...
        Where:
            argument_name: str. Can have leading '?', marking argument as an optional (None will be returned if no default). Can have '=', making everythong behind it the argument_default.
    ...
    @staticmethod
    def get_path_regular(*, path:str, path_regular:Union[str,Pattern[str]], path_args:ArgumentListType, **kwargs) -> Tuple[re_type, ArgumentListType]:
        if (path_regular is None):
            if (not path is None):
                ...
                raise ValueError(f"Either uri or uri_regular must be presented")
    
        elif (isinstance(path_regular, str)):
            _regular = re.compile(path_regular, flags=re.IGNORECASE)
    
        elif (isinstance(path_regular, re_type)):
    ...
    def rest_method(
        ...
        **kwargs,
        ...
        logger.debug(f"Rest method registrator generator is called for the method {_method.upper()}: {_regular}")
        ...
        def rest_method__registrator(func):
            ...
            @asynchronous
            async def rest_method__registrator__action(self, url, *parsed_path_args, **parsed_path_kwargs):
                try:
