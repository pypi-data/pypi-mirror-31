...
def resolve_path_or_url(path_or_url, allow_caching=True):
    ...
    """
    try:
        return resolve_name_or_url(path_or_url, allow_caching=allow_caching)
    except ValueError:
        if os.path.isfile(path_or_url):
            return resolve_name_or_url(
...
def resolve_name_or_url(name_or_url, baseurl=None, allow_caching=True):
    """
...
class Reader(object):
    ...
    def parse_doc(name_or_url, baseurl):
        backend, name, baseurl = resolve_name_or_url(name_or_url, baseurl)
        ...
        def parse(self, json_doc, baseurl):
            if TocPartitionKeys.TOCS in json_doc:
                ...
                for tile_doc in json_doc[ImagePartitionKeys.TILES]:
                    ...
                    backend, name, _ = resolve_name_or_url(name_or_url, baseurl)
