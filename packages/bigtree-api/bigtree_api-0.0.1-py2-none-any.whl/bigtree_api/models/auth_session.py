...
class AuthSessionModel(BaseModel):
    ...
    model_url = 'auth_session'

    @parse_resp_json
    def get_one(self, session_key):
        """获取一条数据"""
        url = build_url(self.service_url, session_key)
        return requests.get(url, headers=self.token_header)

    @parse_resp_json
    def get_all(self):
        """获取所有数据"""
        return requests.get(self.service_url, headers=self.token_header)
    ...
    def modify(self, entity):
        """修改数据"""
        url = build_url(self.service_url, entity.get('session_key'))
    ...
    def get_page_query(self, limit, offset, sort=None, order=None):
        """获取分页记录"""
        url = build_url(self.service_url, 'pager')
    ...
    def exist_modify(self, entity):
        """数据存在修改，不存在新增"""
        url = build_url(self.service_url, 'exists_modify')
    ...
    def tran_delete(self, pks):
        ...
        @param pks: 主键列表
        """
        url = build_url(self.service_url, 'tran_delete')
        resp = requests.post(
    ...
    def clear(self):
        """清空数据，删除所有数据"""
        url = build_url(self.service_url, 'clear')
    ...
    def clear_expired(self):
        """清除过期数据"""
        url = build_url(self.service_url, 'clear_expired')
        resp = requests.post(url, headers=self.token_header)
        return resp
