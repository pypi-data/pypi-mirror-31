...
class ConanMultiPackager(object):
    ...
    def run_builds(self, curpage=None, total_pages=None, profile_name=None):
        ...
        for build in self.builds_in_current_page:
            ...
            profile, base_profile = self._get_profiles(build, profile_name)
            if self.use_docker:
    ...
    def _get_profiles(self, build_conf, base_profile_name):
        ...
        if base_profile_name:
            base_profile_path = os.path.join(self.conan_api.client_cache.profiles_path,
                                             base_profile_name)
            base_profile_text = tools.load(base_profile_path)
            print(base_profile_path)
            raise Exception(base_profile_text)
