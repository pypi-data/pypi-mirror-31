...
from cpt import __version__ as package_tools_version, tools
from cpt.log import logger
from cpt.printer import print_profile, print_rule, foldable_output, print_command
...
class TestPackageRunner(object):
    def __init__(self, profile_text=None, username=None, channel=None, reference=None, runner=None,
        ...
        self._profile_text = profile_text or unscape_env(os.getenv("CPT_PROFILE"))
        self._args = args or os.getenv("CPT_ARGS", "")
        self._build_policy = build_policy or unscape_env(os.getenv("CPT_BUILD_POLICY"))

        self._username = username or unscape_env(os.getenv("CONAN_USERNAME"))
        self._channel = channel or unscape_env(os.getenv("CONAN_CHANNEL"))
        self._reference = reference or unscape_env(os.getenv("CONAN_REFERENCE"))
        self._conan_pip_package = conan_pip_package or unscape_env(os.getenv("CPT_PIP_PACKAGE"))
    ...
    def _run_create(self):
        ...
        api, client_cache, user_io = Conan.factory()

        if self._reference:
            name, version, user, channel = ConanFileReference.loads(self._reference)
        else:
            name, version, user, channel = None, None, self._username, self._channel
        ...
        with foldable_output("conan_create"):
            ...
            api.create(".", name=name, version=version, user=user, channel=channel,
                       build_modes=self._build_policy, profile_name=self.abs_profile_path)
...
class DockerTestPackageRunner(TestPackageRunner):
    ...
    def run(self, pull_image=True, docker_entry_script=None):
        ...
        env_vars_text = " " + " ".join(['-e %s="%s"' % (key, value) for key, value in envs.items() if value])
    ...
    def get_env_vars(self):
        ...
        ret["CONAN_USERNAME"] = escape_env(self._username)
        ret["CONAN_CHANNEL"] = escape_env(self._channel)
        ret["CONAN_REFERENCE"] = escape_env(self._reference)
...
def unscape_env(profile_text):
    if not profile_text:
        return profile_text
    return profile_text.replace("@@", "\n").replace('||', '"')


def escape_env(profile_text):
    if not profile_text:
        return profile_text
    return profile_text.replace("\n", "@@").replace('"', '||')
