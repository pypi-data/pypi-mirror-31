...
from d3m.metadata import hyperparams, base as metadata_base, params, problem
...
__all__ = (
    ...
    'GradientCompositionalityMixin', 'LossFunctionMixin', 'singleton', 'inputs_across_samples'
)
...
class PrimitiveBase(typing.Generic[Inputs, Outputs, Params, Hyperparams], metaclass=PrimitiveBaseMeta):
    ...
    * A private argument prefixed with ``_`` which is used for debugging and testing.
    ...
    def __init__(self, *, hyperparams: Hyperparams, random_seed: int = 0,
        ...
        multiple instances of a primitive.
    ...
    def _multi_produce(self, *, produce_methods: typing.Sequence[str], timeout: float = None, iterations: int = None, **kwargs: typing.Dict[str, typing.Any]) -> MultiCallResult:
        ...
        for method_name in produce_methods:
            ...
            expected_arguments = set(self.metadata.query()['primitive_code']['instance_methods'][method_name]['arguments'])
    ...
    def can_accept(cls, *, method_name: str, arguments: typing.Dict[str, typing.Union[metadata_base.Metadata, type]]) -> typing.Optional[metadata_base.DataMetadata]:
        ...
        if method is None:
            raise exceptions.InvalidArgumentValueError("Unknown method '{method_name}'.".format(method_name=method_name))
        ...
        missing_arguments_set = required_arguments_set - arguments_keys_set
        if len(missing_arguments_set):
            raise exceptions.InvalidArgumentValueError(
                "Not all required arguments are provided for method '{method_name}': {missing_arguments_set}".format(
                    method_name=method_name, missing_arguments_set=missing_arguments_set,
                )
            )

        # All passed arguments should exist among method arguments.
        extra_arguments_set = arguments_keys_set - method_arguments_set
        if len(extra_arguments_set):
            raise exceptions.InvalidArgumentValueError(
                "Extra arguments are provided for method '{method_name}': {extra_arguments_set}".format(
                    method_name=method_name, extra_arguments_set=extra_arguments_set,
                )
            )

        for argument_name, argument_metadata in arguments.items():
            if primitive_arguments[argument_name]['kind'] == metadata_base.PrimitiveArgumentKind.PIPELINE:
                if not isinstance(argument_metadata, metadata_base.Metadata):
                    raise exceptions.InvalidArgumentTypeError("Metadata for pipeline argument '{argument_name}' is not a subclass of 'Metadata' class.".format(argument_name=argument_name))

                if isinstance(argument_metadata, metadata_base.PrimitiveMetadata):
                    argument_type = argument_metadata.query().get('structural_type', None)
                else:
                    argument_type = argument_metadata.query(()).get('structural_type', None)
            else:
                if not isinstance(argument_metadata, type):
                    raise exceptions.InvalidArgumentTypeError("Type for non-pipeline argument '{argument_name}' is not a type.".format(argument_name=argument_name))
    ...
    def __getstate__(self) -> dict:
        ...
        Moreover, extend default implementation if you have additional state you
        would like to keep when pickling. An example of such additional state is
        state of randomness of you primitive which you seeded with ``random_seed``,
        like `numpy.random.RandomState`.
        ...
        expected_constructor_arguments = self.metadata.query()['primitive_code']['instance_methods']['__init__']['arguments']
...
class LossFunctionMixin(typing.Generic[Inputs, Outputs, Params, Hyperparams], metaclass=utils.GenericMetaclass):
    ...
    @abc.abstractmethod
    def get_loss_function(self) -> typing.Optional[problem.PerformanceMetricBase]:
        """
...
def inputs_across_samples(func: typing.Callable = None, inputs: typing.Sequence[str] = None, *args: str) -> typing.Callable:
    ...
    if callable(func):
        ...
        inputs = sorted(tuple(set(inputs)))
