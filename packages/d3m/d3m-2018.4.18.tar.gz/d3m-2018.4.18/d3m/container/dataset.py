...
def get_d3m_dataset_digest(dataset_doc_path: str) -> str:
    ...
    # TODO: Not (yet) part of the dataset description schema.
    #       See: https://gitlab.datadrivendiscovery.org/MIT-LL/d3m_data_supply/issues/65
...
class Loader(metaclass=utils.AbstractMetaclass):
    ...
    def load(self, dataset_uri: str, *, dataset_id: str = None, dataset_version: str = None, dataset_name: str = None, lazy: bool = False,
             compute_digest: bool = True, strict_digest: bool = False) -> 'Dataset':
        """
...
class D3MDatasetLoader(Loader):
    ...
    def _load_data(self, resources: typing.Dict, metadata: metadata_base.DataMetadata, *, dataset_path: str, dataset_doc: typing.Dict,
                   ...
                   compute_digest: bool, strict_digest: bool) -> typing.Tuple[metadata_base.DataMetadata, typing.Optional[str]]:
        if compute_digest:
            hash = hashlib.sha256()
        else:
            hash = None
    ...
    def load(self, dataset_uri: str, *, dataset_id: str = None, dataset_version: str = None, dataset_name: str = None, lazy: bool = False,
        ...
        if dataset_doc.get('about', {}).get('datasetSchemaVersion', None) != self.CURRENT_VERSION:
            raise exceptions.NotSupportedVersionError("Only supporting dataset descriptions whose schema version is {version}.".format(version=self.CURRENT_VERSION))
        ...
        # Process D3M qualities for LUPI datasets.
    ...
    def _load_collection(self, dataset_path: str, data_resource: typing.Dict, metadata: metadata_base.DataMetadata, hash: typing.Any) -> typing.Tuple[pandas.DataFrame, metadata_base.DataMetadata]:
        ...
        data = pandas.DataFrame({'filename': filenames}, columns=['filename'], dtype=object)
        ...
        metadata = metadata.update((data_resource['resID'], metadata_base.ALL_ELEMENTS, 0), {
            'name': 'filename',
            'structural_type': str,
            'location_base_uris': [
                location_base_uri,
    ...
    def _load_resource_type_table(self, dataset_path: str, data_resource: typing.Dict, metadata: metadata_base.DataMetadata,
                                  hash: typing.Any) -> typing.Tuple[pandas.DataFrame, metadata_base.DataMetadata]:
...
class CSVLoader(Loader):
    ...
    def _load_data(self, resources: typing.Dict, metadata: metadata_base.DataMetadata, *, dataset_uri: str,
                   compute_digest: bool) -> typing.Tuple[metadata_base.DataMetadata, int, typing.Optional[str]]:
        buffer, compression = self._get_buffer_and_compression(dataset_uri)
        ...
        metadata = metadata.update(('0', metadata_base.ALL_ELEMENTS), {
            'dimension': {
                'name': 'columns',
                'semantic_types': ['https://metadata.datadrivendiscovery.org/types/TabularColumn'],
                'length': len(column_names),
        ...
        for i, column_name in enumerate(column_names):
            metadata = metadata.update(('0', metadata_base.ALL_ELEMENTS, i), {
                'name': column_name,
                'structural_type': str,
            })
    ...
    def _get_buffer_and_compression(self, dataset_uri: str) -> typing.Tuple[io.BytesIO, str]:
...
class SklearnExampleLoader(Loader):
    ...
    def _load_data(self, resources: typing.Dict, metadata: metadata_base.DataMetadata, *, dataset_path: str,
                   compute_digest: bool) -> typing.Tuple[metadata_base.DataMetadata, typing.Optional[str], typing.Optional[str]]:
        ...
        for column_index in range(bunch_data.shape[1]):
            ...
            metadata = metadata.update(('0', metadata_base.ALL_ELEMENTS, column_index), column_metadata)
    ...
    def load(self, dataset_uri: str, *, dataset_id: str = None, dataset_version: str = None, dataset_name: str = None, lazy: bool = False,
        ...
        if not hasattr(datasets, 'load_{dataset_path}'.format(dataset_path=dataset_path)):
            raise exceptions.InvalidArgumentValueError("Unknown sklearn dataset '{dataset_uri}'.".format(dataset_uri=dataset_uri))
...
class Dataset(dict):
    ...
    def __init__(self, resources: typing.Mapping, metadata: metadata_base.DataMetadata = None, *, load_lazy: typing.Callable[['Dataset'], None] = None) -> None:
        super().__init__(resources)

        if isinstance(resources, Dataset) and metadata is None:
            self.metadata = resources.metadata.set_for_value(self)
        elif metadata is not None:
            self.metadata = metadata.set_for_value(self)
        else:
            raise exceptions.InvalidArgumentValueError("'metadata' is required if 'resources' is not a dataset.")
        ...
        for loader in cls.loaders:
            if loader.can_load(dataset_uri):
                return loader.load(
                    ...
                    lazy=lazy, compute_digest=compute_digest, strict_digest=strict_digest,
                )

        raise exceptions.DatasetUriNotSupportedError("No known loader could load dataset '{dataset_uri}'.".format(dataset_uri=dataset_uri))
