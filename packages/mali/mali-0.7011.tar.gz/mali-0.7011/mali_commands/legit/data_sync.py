...
class DataSync(object):
    ...
    class DownloadIterResults(collections.Iterator):
        ...
        def __prepare_iter(self):
            results_iter, self.__total_data_points, total_size = self.__get_next_results()
            self.__current_iter = results_iter

        def __next__(self):
            if self.__current_iter is None:  # when calling next directly __iter__ will not be called
                self.__prepare_iter()
    ...
    def download_items(self, metadata_items, storage, multi_process_control):
        ...
        for metadata_item in metadata_items:
            normalized_item = normalize_item(metadata_item)
            async_result = self.__download_normalized_item(multi_process_control, storage, metadata_items)
