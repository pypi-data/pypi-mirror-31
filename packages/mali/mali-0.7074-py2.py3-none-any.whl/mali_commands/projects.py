...
def subscribe_alert(ctx, projectid, alerttype):
    ...
    result = ctx.obj.handle_api(ctx.obj, requests.post, 'projects/subscribe', data)
...
def unsubscribe_alert(ctx, projectid, alerttype):
    ...
    result = ctx.obj.handle_api(ctx.obj, requests.post, 'projects/unsubscribe', data)
...
@click.pass_context
def list_alert(ctx):
    alerts = ctx.obj.handle_api(ctx.obj, requests.get, 'projects/subscriptions')
