...
from mali_commands.legit.api import BASE_URL_PATH
...
def print_as_json(result):
    if isinstance(result, types.GeneratorType):
        ...
        if is_list:
            sys.stdout.write('[')
            json.dump(first_result, sys.stdout)

            for next_result in result:
                sys.stdout.write(',')
                json.dump(next_result, sys.stdout)

            sys.stdout.write(']')
        else:
            sys.stdout.write('{')

            key, val = first_result

            def write_key_val():
                json.dump(key, sys.stdout)
                sys.stdout.write(':')
...
def ml_auth_oauth_token(api_host, code, verifier, redirect_uri):
    params = {
        'code': code,
...
def pixy_flow(ctx):
    ...
    try:
        data = ml_auth_oauth_token(ctx.api_host, code, verifier, redirect_uri)
    except HTTPError:
