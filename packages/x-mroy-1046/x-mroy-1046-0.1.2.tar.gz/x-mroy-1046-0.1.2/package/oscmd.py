...
class Proxychains(Os):
    ...
    def Setup(self):
        if not self.check("proxychains"):
            ...
            self.Run("sed -ie 's/#quiet/quiet/g' /etc/proxychains.conf")

    def Upload(self, args):
        super().Upload(args, dest="/etc/proxychains.conf")

    def Config(self, args):
        return open("/etc/proxychains.conf").read()

    def ChangePort(self, args):
        port = int(args)
        self.Run("sed -ie 's/^socks5 127.*/socks5 127.0.0.1 {port}/g' /etc/proxychains.conf".format(port=port))
...
class Shadowsocks(Os):
    ...
    def Start(self, args, server_ip=None):
        if args == 'local':
            ...
            os.popen("ps aux | grep ssserver | awk '{print $2}' |xargs > /tmp/ssserver.pid ").read()
...
class ProxyCircle:
    ...
    def Socks5Proxy(self,medial_server, exit_server, port=10081):
        self.exit_server = exit_server
        self.medial_server = medial_server
        self.shadowsocks.Connect(medial_server)
        self.Run('proxychains ssh -fCnND {port} root@{exit_server}'.format(port=port, exit_server=self.exit_server))
        self.socks5_pid =  os.popen("ps aux | grep ssh | grep fCnND | grep -v 'grep' | awk '{print $2}' |xargs > /tmp/socks5.pid").read().strip()
    ...
    def PortMap(self, dest, sshport='22', if_tor=False):
        ...
        if if_tor:
            pre = 'proxychains'
        cmd = '{if_tor} ssh -p {sshport} -LCnfN 0.0.0.0:{lport}:{rhost}:{rport} root@{rhost}'.format(sshport=sshport, lport=lport, rhost=rhost, rport=rport, if_tor=pre)
        self.Run(cmd)
        os.popen("ps aux | grep ssh | grep LCnfN | grep -v 'grep' | awk '{print $2}' |xargs > /tmp/ssh_ford.pid ").read().strip()

    def GetStatus(self):
        socks5_pid = -1
        forward_pid = os.popen("ps aux | grep ssh | grep LCnfN | grep -v 'grep' | awk '{print $2}' | xargs ").read().strip().split()
        ...
        return {
            'socks5' : socks5_pid,
            'shadowsocks_server': shadowsocks_pid,
