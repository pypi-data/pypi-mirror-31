...
class ProxyCircle:
    ...
    def PortMap(self, dest, sshport='22', if_tor=False):
        ...
        if if_tor:
            pre = 'proxychains'
        cmd = '{if_tor} ssh -p {sshport} -L 0.0.0.0:{lport}:{rhost}:{rport} root@{rhost} -CnfN'.format(sshport=sshport, lport=lport, rhost=rhost, rport=rport, if_tor=pre)
        self.Run(cmd)
