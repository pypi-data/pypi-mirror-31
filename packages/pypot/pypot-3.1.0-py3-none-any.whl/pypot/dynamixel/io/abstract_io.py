...
class _DxlAccess(object):
    readonly, writeonly, readwrite = list(range(3))
...
class AbstractDxlIO(object):
    ...
    def scan(self, ids=list(range(254))):
    ...
    def get_model(self, ids):
        ...
        self._known_models.update(list(zip(to_get_ids, models)))
    ...
    def set_status_return_level(self, srl_for_id, **kwargs):
        ...
        convert = kwargs['convert'] if 'convert' in kwargs else self._convert
        if convert:
            srl_for_id = dict(list(zip(list(srl_for_id.keys()),
                                  [('never', 'read', 'always').index(s) for s in list(srl_for_id.values())])))
    ...
    def switch_led_on(self, ids):
        ...
        self._set_LED(dict(list(zip(ids, itertools.repeat(True)))))

    def switch_led_off(self, ids):
        """ Switches off the LED of the motors with the specified ids. """
        self._set_LED(dict(list(zip(ids, itertools.repeat(False)))))

    def enable_torque(self, ids):
        """ Enables torque of the motors with the specified ids. """
        self._set_torque_enable(dict(list(zip(ids, itertools.repeat(True)))))

    def disable_torque(self, ids):
        """ Disables torque of the motors with the specified ids. """
        self._set_torque_enable(dict(list(zip(ids, itertools.repeat(False)))))
    ...
    def set_pid_gain(self, pid_for_id, **kwargs):
        ...
        pid_for_id = dict(zip(iter(pid_for_id.keys()),
                                         [tuple(reversed(t)) for t in list(pid_for_id.values())]))
    ...
    def _set_control_value(self, control, value_for_id, **kwargs):
        ...
        if convert:
            ...
            value_for_id = dict(list(zip(list(value_for_id.keys()),
                                    list(map(control.si_to_dxl, list(value_for_id.values()), models)))))
