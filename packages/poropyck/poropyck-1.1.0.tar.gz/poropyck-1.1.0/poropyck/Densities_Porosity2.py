...
def densities_porosity2():
    ...
    np.savetxt('./'+well+'/Dry_density2.out', [Dry_den,delta_dry,DryDen_down,Dry_denMC.mean,DryDen_up], fmt='%1.2f',delimiter=',',header='Rho (dry), Std_Rho (dry), Rhod_0025, Rhod_mean, Rhod_0975')
    np.savetxt('./'+well+'/Sat_density2.out', [Wet_den,delta_wet,WetDen_down,Wet_denMC.mean,WetDen_up], fmt='%1.2f',delimiter=',',header='Rho (sat.), Std_Rho (sat.), Rhos_0025, Rhos_mean, Rhos_0975')
    np.savetxt('./'+well+'/Min_density2.out', [Min_den,delta_min,MinDen_down,Min_denMC.mean,MinDen_up], fmt='%1.2f',delimiter=',',header='Rho (min.), Std_Rho (min.), Rhom_0025, Rhom_mean, Rhom_0975')
    np.savetxt('./'+well+'/Porosity2.out', [por,delta_por,Por_down,Por_MC.mean,Por_up], fmt='%1.2f',delimiter=',',header='Por (%), Std_Por (%), Por_0025, PorMC_mean, Por_0975')
    ...
    plt.savefig('./'+well+'/'+well+'_Dry_Den2.pdf',bbox_inches='tight')
    plt.show(block=True)
    ...
    plt.legend()
    plt.xlabel('Density (g/cm3)')
    plt.savefig('./'+well+'/'+well+'_Saturated_Den2.pdf',bbox_inches='tight')
    plt.show(block=True)
    ...
    plt.legend()
    plt.xlabel('Density (g/cm3)')
    plt.savefig('./'+well+'/'+well+'_Mineral_Den2.pdf',bbox_inches='tight')
    plt.show(block=True)
    ...
    plt.legend()
    plt.xlabel('Porosity (%)')
    plt.savefig('./'+well+'/'+well+'_Porosity2.pdf',bbox_inches='tight')
    plt.show(block=True)
