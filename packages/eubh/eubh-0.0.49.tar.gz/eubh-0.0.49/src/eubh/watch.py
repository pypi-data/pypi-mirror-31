...
def job_write_log(data, verbosity, host=None, tls=True):
    WriteContainerLog(pivot=data, verbosity=verbosity, host=host, tls=tls)
...
class Watch:
    def __init__(self, key, time, verbosity, host, tls):
        self.time = time
        self.key = key
        self.host = host
        self.tls = tls
        if host:
            self.api = api.Api(base_url="%s://%s" % ('https' if tls else 'http', host))
        else:
            self.api = api.Api()
            ...
            if not utils.is_empty(key):
                if option == 'init' and self.can_init:
                    ...
                    write_log = multiprocessing.Process(target=job_write_log,
                                                        args=(pivot, self.is_verbose, self.host, self.tls))
                    write_log.start()
