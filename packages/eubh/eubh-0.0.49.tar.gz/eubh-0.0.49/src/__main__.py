...
from eubh.config import VERSION
...
@click.option('-v', '--verbosity', default=True)
@click.option('-h', '--host')
@click.option('--tls/--no-tls', default=True)
def watch(key, time, verbosity, host, tls):
    watch_package.Watch(key, time, verbosity, host, tls).watch()
