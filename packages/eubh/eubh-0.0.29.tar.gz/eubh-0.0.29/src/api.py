...
class Api:
    ...
    def get_project_container_logs_by_container_id(self, container_id):
        return requests.get(self.get_url('project/container-logs/%s/logs-by-container-id' % container_id)).json()

    def post_project_container_logs(self, data={}):
        return requests.post(self.get_url('project/container-logs/%s/set' % data.container_id), data=data)
