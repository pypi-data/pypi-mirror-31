...
import status
import utils
from config import WS_ADDRESS, ROOT_DIR, DOCKER_WHITE_LIST
...
def job_write_log(data, verbosity, host=None):
    WriteContainerLog(pivot=data, verbosity=verbosity, host=host)
...
class Watch:
    def __init__(self, key, time, verbosity, host):
        self.time = time
        self.key = key
        self.host = host
        if host:
            self.api = api.Api(base_url="https://%s" % host)
        else:
            self.api = api.Api()
        ...
        self.cache_log = []
        self.jobs = []
        self.is_verbose = (True if verbosity == 2 else False)
        ...
        if type(response) is not list:
            ...
            if not utils.is_empty(key):
                if option == 'init' and self.can_init:
                    ...
                    write_log = multiprocessing.Process(target=job_write_log, args=(pivot, self.is_verbose, self.host,))
                    write_log.start()
            ...
            if option == 'clean':
                self.clean_docker()
                if not utils.is_empty(self.key):
                    put.Put(self.key, 'result').put('%s/result' % ROOT_DIR, True)
                    self.key = ''
                    self.can_init = True
    ...
    def watch(self):
        ...
        scheduler.add_job(self.upload_machine_information, 'interval', seconds=self.time, max_instances=3)
        try:
            scheduler.start()
