...
def write_docker_log(pivot, verbosity, host=None):
    client = from_env()
    ...
    if len(container_lists) > 0:
        if host:
            ...
            ws = WebSocketClient(url="wss://%s:54188/" % host)
        try:
            ws.connect()
...
class ContainerLog:
    def __init__(self, pivot, verbosity, host=None):
        write_docker_log(pivot, verbosity, host)
