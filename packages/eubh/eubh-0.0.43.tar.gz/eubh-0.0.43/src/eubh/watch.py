...
import status
import utils
from config import WS_ADDRESS, ROOT_DIR
...
def job_write_log(data):
    WriteContainerLog(pivot=data)
        ...
        if type(response) is not list:
            ...
            if not utils.is_empty(key):
                if option == 'init' and self.can_init:
                    ...
                    write_log = multiprocessing.Process(target=job_write_log, args=(pivot,))
                    write_log.start()
