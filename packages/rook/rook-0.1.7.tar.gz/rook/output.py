...
from .interfaces.config import LoggingConfiguration, OutputConfiguration, VersionConfiguration

from .services.bdb_location_service import BdbLocationService

from .lib.logger import logger
from .lib.logger.namespace_serializer_handler import NamespaceSerializerHandler

from .lib import git

from .lib.processor.namespace_serializer import NamespaceSerializer

from .lib.processor.namespaces.container_namespace import ContainerNamespace
from .lib.processor.namespaces.python_object_namespace import PythonObjectNamespace
...
class Output(object):
    ...
    def get_rook_info(self):
        ...
        try:
            ...
            try:
                raw_tags = os.environ['ROOKOUT_ROOK_TAGS']
                message.tags.extend([NamespaceSerializer.normalize_string(tag) for tag in raw_tags.split(';') if tag])
            except KeyError:
                pass

            # Try to get the proper git tag
            try:
                user_commit = os.environ.get('ROOKOUT_COMMIT')
                if not user_commit:
            ...
            try:
                arguments = {
                    'user_info': ContainerNamespace(self._user_info),
                    'pid': PythonObjectNamespace(os.getpid()),
                }
                NamespaceSerializer().dump(ContainerNamespace(arguments), message.arguments)
            except:
                logger.exception("Failed to get additional arguments")
                message.ClearField("arguments")
        except:
    ...
    def send_log_message(self, level, time, filename, lineno, text, formatted_message, arguments):
        with self._lock:
            if len(self._messages.log_messages) >= OutputConfiguration.MAX_LOG_ITEMS:
                if not self._log_limit_reached:
                    ...
                    try:
                        ...
                        six.print_("Error when handling output queue size limit")
                        traceback.print_exc()
                        return
                else:
                    return
            ...
            if arguments and arguments.size(''):
                NamespaceSerializer().dump(arguments, message.arguments, log_errors=False)
