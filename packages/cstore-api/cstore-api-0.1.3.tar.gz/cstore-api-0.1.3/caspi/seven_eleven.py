...
def get_pb_products(kind=""):
    ...
    with HeadlessChrome() as chrome:
        ...
        more_prod_btn = wait.until(EC.presence_of_element_located((By.CSS_SELECTOR, 'li.btn_more > a')))
        more_prod_script = more_prod_btn.get_attribute('href').split(':', maxsplit=2)[1]

        while True:
            try:
                chrome.execute_script(more_prod_script)

                wait = WebDriverWait(chrome, 10)
                wait.until(EC.presence_of_element_located((By.CSS_SELECTOR, 'li.btn_more')))
    ...
    with HeadlessChrome() as chrome:
        ...
        while True:
            try:
                ...
                more_prod_btn = wait.until(EC.presence_of_element_located((By.CLASS_NAME, 'btn_more')))
                more_prod_btn.click()
