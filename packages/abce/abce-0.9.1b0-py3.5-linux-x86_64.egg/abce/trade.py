...
class Trade:
    ...
    def get_offers_all(self, descending=False, sorted=True):
        ...
        *Offers that are not accepted in the same subround (def block) are
        ...
        """
        goods = list(list(self._open_offers_sell.keys()) + list(self._open_offers_buy.keys()))
    ...
    def peak_buy_offers(self, good, sorted=True, descending=False, shuffled=True):
        """ """
        ret = []
        for offer in list(self._open_offers_buy[good].values()):
            ret.append(offer)
        if shuffled:
            random.shuffle(ret)
        if sorted:
    ...
    def peak_sell_offers(self, good, sorted=True, descending=False, shuffled=True):
        """ """
        ret = []
        for offer in list(self._open_offers_sell[good].values()):
            ret.append(offer)
        if shuffled:
            random.shuffle(ret)
        if sorted:
    ...
    def _reject_polled_but_not_accepted_offers(self):
        for offer in list(self._polled_offers.values()):
            self._reject(offer)
        self._polled_offers = {}
