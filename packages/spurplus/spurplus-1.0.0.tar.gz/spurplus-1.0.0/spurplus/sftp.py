...
class ReconnectingSFTP:
    """ wraps SshShell to automatically opens a new paramiko.SFTP on connection failure. """
...
def _exists(sftp: ReconnectingSFTP, remote_path: Union[str, pathlib.Path]) -> bool:
    """
...
def _mkdir(sftp: ReconnectingSFTP,
    ...
    if not parents:
        ...
        if oserr is not None:
            raise OSError("Failed to create the directory {}: {}".format(rmt_pth.as_posix(), oserr))
    else:
        ...
        for directory in directories:
            ...
            if oserr is not None:
                raise OSError("Failed to create the directory {}: {}".format(directory.as_posix(), oserr))
