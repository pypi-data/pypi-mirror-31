...
import time
import uuid
from typing import Optional, Union, TextIO, BinaryIO, List, Dict, Callable
        ...
        if not consistent:
            ...
            try:
                ...
                if oserr is not None:
                    raise OSError("Failed to put the local file {} to the remote path {}: {}".format(
                ...
                if oserr is not None:
                    raise OSError("Failed to rename the remote temporary file {} to the remote path {}: {}".format(
                        tmp_pth, remote_path, oserr))
