...
class AD7606_SPI:
    def __init__(self,inputRange,conversionABmode,pins,SPIFreq=1000000,returnRaw=False):
        ...
        self.adc = self.io.spi_open(0,SPIFreq,0) # initialize spi on channel 0 at 1mhz. self.adc is now the "handle"
...
class AD7606_AB:
    def __init__(self,inputRange,pins,returnRaw=False):
        ...
        self.returnRaw = returnRaw
        self.containerA = zeros(3,dtype=int16)
        self.containerB = zeros(3,dtype=int16)
    ...
    def ADCread(self):
        ...
        #while self.io.read(self.p_busy) == 1:
            #continue
        self.io.write(self.p_cs,0)

        for _c in range(3):
            _A = 0
            _B = 0
            for _i in range(16):
                self.io.write(self.p_clock,0)
                _A += self.io.read(self.p_dataA)<<_i
                _B += self.io.read(self.p_dataB)<<_i
                self.io.write(self.p_clock,1)
            self.containerA[_c] = _A
            self.containerB[_c] = _B
        self.io.write(self.p_cs,1)
