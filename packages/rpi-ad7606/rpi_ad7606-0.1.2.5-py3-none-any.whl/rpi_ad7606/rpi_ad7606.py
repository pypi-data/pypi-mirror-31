...
class AD7606_AB:
    def __init__(self,inputRange,pins,returnRaw=False):
        ...
        self.p_convsta = pins['convsta']
        self.p_convstb = pins['convstb']
        self.p_dataA = pins['DoutA'] # None for SPI
        self.p_dataB = pins['DoutB']
        self.p_reset = pins['reset']
        self.p_busy = pins['busy']
        self.p_1stData = pins['1stData']
    ...
    def ADCread(self):
        ...
        if returnRaw:
            ...
            _channel1 = self.transferFunction(ch13[0])
            _channel2 = self.transferFunction(ch13[1])
            _channel3 = self.transferFunction(ch13[2])
            _channel4 = self.transferFunction(ch46[0])
            _channel5 = self.transferFunction(ch46[1])
            _channel6 = self.transferFunction(ch46[2])
