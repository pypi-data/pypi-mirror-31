...
def userid_colour(userid):
    ...
    try:
        ...
        print(userid)
        print(users[userid])
        raise Exception('userid wrong')

def make_n_char_long(x, n, *args):
    if args:
        r=args[0]
    else:
        r=' '
...
def display_username(post, *args): # lowercase letters are getting dropped form the starts of some usernames...
    try:
        u=post['user']
    except:
...
def display_username___(post, *args): # lowercase letters are getting dropped form the starts of some usernames...
    u=post['user']
    user_id=str(u['id'])
    x=u['name']
    if args:
