...
def display_thread_headers(thread):
    ...
    return (str(thread['posts'][0]['date']), str(thread['posts'][-1]['date']), make_n_char_long(thread['title'], terminal_w-4-2-10-2-15-2-3-2-3-16, '.'), display_username_forumdisplay(thread['posts'][0]), make_n_char_long(thread['nPosts'], 3), cl.green+make_n_char_long(str(thread['posts'][0]['reps']), 3))

def print_forum(j, attr):
    sort_ascending=True
    ...
    if settings['use_pandas_for_forum_and_thread_display']:
        ...
        if attr in ['reps','op-date']:
            ...
            if attr in ['rposts']:
                j=j[::-1]
        for thread in j:
            print(colour_alternating(make_n_char_long(thread['order'], 4), display_thread_headers(thread)))

def print_forumpage(thread_meta_list, **kwargs):
    for i in range(len(thread_meta_list)):
        thread_meta_list[i]['order']=i
    terminal_w=get_terminal_size((80, 20))[0]
    print('n   ', 'date . . .', make_n_char_long(' title', terminal_w-4-2-10-2-15-2-3-2-3-16), '  author         ', 'last_post_by    ', 'nPosts')
    ...
    for i in range(len(thread_meta_list)):
        ...
        print(make_n_char_long(thread_meta_list[i]['order'], 4), colour_alternating( thread['date'], make_n_char_long(thread['title'], terminal_w-4-2-10-2-15-2-3-2-3-16, '.'), display_username_forumdisplay(thread), make_n_char_long(thread['last_post_username'], 15), thread['nPosts_claimed'] ))
