...
def get_slim_init_model_fn(checkpoint_path, checkpoint_exclude_scopes=[]):
    """
    Create an init_model function for slim models in tensorflow/models git repository.

    :param checkpoint_path: The path where the checkpoint is stored.
    :param checkpoint_exclude_scopes: Which scopes should be excluded.
    :return: The init_model(hyper_params, model, session) function for the starttf.estimators.
    """
    ...
    init_op = tf.contrib.slim.assign_from_checkpoint_fn(
        checkpoint_path,
        variables_to_restore,
