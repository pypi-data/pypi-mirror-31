...
class Command(ServerManagementBaseCommand):
    ...
    def handle(self, *args, **options):  # pylint: disable=too-complex,too-many-locals,too-many-branches,too-many-statements
        ...
        local_project_path = str(django_settings.APP_DIR)
        ...
        production_domain_names = ' '.join([
            ...
            if '.vltn.digital' not in host
        ])
        staging_domain_names = ' '.join([
            host for host in
            django_settings.ALLOWED_HOSTS
            if '.vltn.digital' in host
        ])
        ...
        nginx_tasks = [
            ...
            },
            {
                ...
                'command': 'htpasswd -c -b /etc/nginx/htpasswd vltn digital',
            },
            {
            ...
            {
                'title': 'Run certbot',
                'command': 'certbot certonly --standalone -n --agree-tos --email technical@wearevolition.co '
