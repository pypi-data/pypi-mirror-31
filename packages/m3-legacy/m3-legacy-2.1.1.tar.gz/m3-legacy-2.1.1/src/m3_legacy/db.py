...
from django.db import models, connection, router, connections
    ...
    try:
        with atomic():
            models.signals.pre_delete.send(
                sender=model.__class__, instance=model
            )

            cursor = connection.cursor()  # @UndefinedVariable
            sql = "DELETE FROM %s WHERE id = %s" % (connection.ops.quote_name(
                model._meta.db_table), model.id)  # @UndefinedVariable
            cursor.execute(sql)
...
def queryset_limiter(queryset, start=0, limit=0):
    ...
    assert (
        isinstance(queryset, QuerySet) or getattr(queryset, '__iter__')
    ), (
    ...
    if limit < 0:
        limit = 0
    total = queryset.count() if isinstance(
        queryset, QuerySet) else len(queryset)
...
class BaseObjectModel(models.Model):
    ...
    def delete_related(self, affected=None, using=None):
        ...
        По умолчанию affected содержит пустой
        ...
        Пример: Model1.objects.get(id=1).delete_related(
            affected=[Model2, Model3])
        """
        ...
        assert (
            ...
            "%s object can't be deleted because its %s "
        ...
        assert isinstance(
            affected, list), 'Affected models must be the list type'
...
class BaseObjectModelWVersion(BaseObjectModel):
    ...
    def do_lock(self):
        if self.id:
            ...
            q = self.__class__.objects.filter(
                id=self.id, version=self.version).for_update()
    ...
    def save(self, *a, **k):
        ...
        Это нужно чтобы проверять,
        была ли модифицирована запись после последнего получения
        """
        if self.id:
            self.version += 1
...
class ObjectState(BaseEnumerate):
    ...
    - если запись "Закрыта",
      значит ее нельзя использовать в новых документах,
      но можно выводить в отчетах и уже существующих данных
    - если запись "Черновик",
      значит ее нельзя использовать в логике приложения и в отчетах.
      По сути, это означает что запись введена не полностью и не утверждена
    """
...
class ObjectManager(models.Manager):
    ...
    def get_queryset(self):
        # если указывали дату, то отфильтруем на дату,
        # иначе только по состоянию
        if self.query_on_date:
            ...
            kwargs = dict(state__in=self.query_state)
        return super(ObjectManager, self).get_queryset().filter(**kwargs)
