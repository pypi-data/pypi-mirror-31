        ...
        if modality == 'eeg':
            for obj in bucket.objects.all():
                ...
                if key.endswith(extension):
                    ...
                    self.files.append(pkl)
        elif modality == 'func':
            dataset_name = os.path.basename(os.path.normpath(self.base_path))
            dataset = {}
            subjects = [os.path.basename(x) for x in glob.glob(self.base_path + "/*")]
            if "chanlocs.csv" in subjects:
               subjects.remove("chanlocs.csv")
            if "metadata.json" in subjects:
               subjects.remove("metadata.json")
            ...
            for s in subjects:
               dataset.update({s:{}})
            for s in subjects:
               modalities = [os.path.basename(x) for x in glob.glob(os.path.join(self.base_path, s) + "/*")]
               # print(modalities)
               for m in modalities:
                   dataset[s].update({m:{}})
                   files = [os.path.basename(x) for x in glob.glob(os.path.join(self.base_path, s, m) + "/*")]
                   # print(files)
                   for f in files:
                       t = "".join(f.split("_")[1:]).split(".")[0]
                       dataset[s][m].update({t:f})
            ...
            for s in self.dataset.keys():
                for t in self.dataset[s][modality].keys():
                    ...
                    if f.endswith(extension):
                        self.files.append(os.path.join(self.base_path, s, modality, f))
                        self.subjects.append(s)
                        self.tasks.append(t)
        d = {
            "resource_path": self.files,
