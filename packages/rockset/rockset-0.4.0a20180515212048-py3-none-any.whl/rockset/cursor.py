...
from rockset.query import LimitQuery
import asyncio

class Cursor(object):
    """ Fetch the results of a query executed against a collection
    """
    def __init__(self, q=None, queryf=None, queryfargs={}, **kwargs):
        self.query = q
        self.sexpr = kwargs.get('sexpr', True)
        self.sql = kwargs.get('sql', False)
        self.queryf = queryf
        self.queryfargs = queryfargs
    ...
    def _run_query(self, limit=None, skip=0):
        ...
        if self.sexpr:
            ...
            if limit is not None:
                query = LimitQuery(limit=limit, skip=skip, child=query)
            qo['s_expression']= query.sexpression()
        if self.sql:
            (sqltxt, sqlparams) = self.query.sql()
            qo['sql'] = {'query': sqltxt, 'parameters': []}
            for pn, pv in sqlparams.items():
                qo['sql']['parameters'].append({'name': pn, 'value': str(pv)})
        results = self.queryf(query=qo, **self.queryfargs)
        if not isinstance(results, dict):
            raise ServerError(
                message='invalid return message (type={} results={})'.format(
...
seamless automatic pagination
        """
        if batch <= 0:
            raise ValueError('invalid cursor iterator batchsize. "{}" is not '
