...
class RSExecute(object):
    ...
    def connect(self, api_server=None, api_key=None, workspace=None, **kwargs):
        ...
        conn = rs.connect(api_server=api_server, api_key=api_key,
    ...
    def run(self, statement, exception_formatter=None,
    ...
    def table_columns(self):
        for row in self._columns(kinds=['collection']):
            yield row

    def view_columns(self):
        for row in self._columns(kinds=['view']):
            yield row
    ...
    def datatypes(self):
        dtypes = ['INTEGER', 'FLOAT', 'BOOLEAN', 'VARCHAR', 'ARRAY',
            'DOCUMENT', 'VARIANT']
        for workspace in self.schemata():
            for dt in dtypes:
                yield (workspace, dt)
