...
def getNumberOfThreadsDownload():
    """
...
def startProcessPool():
    ...
    You might not want this when you already run your own :class:`concurrent.futures.Executor`. If you run your own, let `processPool` point to it.
    ...
    If you want to run FEV\@KEGG inside another thread or process, your will have to replace `processPool` with a `concurrent.futures.Executor` that allows running outside the main thread. This may be a dummy class.
    """
    global processPool
