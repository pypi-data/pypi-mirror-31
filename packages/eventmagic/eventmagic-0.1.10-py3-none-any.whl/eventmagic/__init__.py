...
def db_connection(host, port, username, password, database):
    ...
    try:
        ...
        for row in cur:
            logger.debug("Show tables for connected DB: {}".format(row))
    except Exception as e:
        logger.error("Failed to validate DB Connection with error: {}".format(
            e
        ))
    return cnx
...
def get_schedules_from_db():
    ...
    try:
        ...
        logger.debug("Cursor is {}".format(cursor))
