...
class InterpreterBridge(object):
    ...
    def run_command(self, command, timeout_in_seconds=None, skip_limits_check=False, verbose=False):
        ...
        try:
            ...
            raise TimeoutError("command '{}' did not finish with {} seconds"
                               .format(command, timeout_in_seconds)) from None
        self.call_count += 1
        return exit_code
    ...
    def _run_command(self, command, verbose):
        ...
        for line in iter(self.p.stdout.readline, b""):
            ...
            if log_error:
                ...
                print(">>", line)
        # this happens if stoud dies before the FINSHED marker was written, eg if
        # the interpreter startup fails before 'code' was executed
        self.logger.error("{}: interpreter startup or code execution failed".format(self.NAME))
