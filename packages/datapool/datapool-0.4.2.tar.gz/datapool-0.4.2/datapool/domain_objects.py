...
from .db_objects import (SiteDbo, PictureDbo, ParameterDbo, SignalDbo, SourceDbo, SourceTypeDbo,
                         SpecialValueDefinitionDbo, ParameterAveragingDbo)

from sqlalchemy.sql.sqltypes import Integer, String, Float, Date, DateTime, LargeBinary
from .bunch import Bunch


backmap = {Integer: int, String: str, Float: float, Date: datetime.date,
           DateTime: datetime.datetime, LargeBinary: bytes}
...
def _check_attribute_types(obj):
    ...
    for name, value in obj.dbo._sa_class_manager.mapper.c.items():
        ...
        if not isinstance(attribute_value, python_type):
            raise FormatError("{!r} is not of type {}".format(attribute_value, python_type))
...
class Signal(Bunch, DomainObject):
    ...
    def __init__(self, dd):
        ...
        if "site" not in dd or dd["site"] is None:
            if not all("coord_" + k in dd for k in "xyz"):
                raise ValueError("{} must either have key 'site' or 'x', 'y' and 'z'".format(dd))
            dd["site"] = None
        else:
    ...
    def __str__(self):
        if "site" in self:
            return ("value={self.value} parameter={self.parameter} timestamp={self.timestamp} "
