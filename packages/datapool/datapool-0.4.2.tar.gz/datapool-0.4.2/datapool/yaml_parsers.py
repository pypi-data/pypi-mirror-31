...
from .domain_objects import (DomainObject, Site, Picture, Parameter, Parameters, Source,
                             SourceType, ParameterAveraging, SpecialValueDefinition)

from .errors import ConsistencyError, DataPoolException
from .landing_zone_structure import source_type_yaml_for_source_yaml
from .logger import logger
from .utils import iter_to_list, is_number
from .yaml_stuff import (parse_datetime, is_datetime, check_yaml_fields_exist,
                         load_yaml_and_bunchify)
...
def check_result_attribute_types(function):
    @wraps(function)
    def wrapped(landing_zone_folder, rel_path):
        for result in function(landing_zone_folder, rel_path):
            if isinstance(result, DomainObject):
                try:
                    result.check_attributes()
                except DataPoolException as e:
                    yield ConsistencyError("parsing {} failed: '{}'".format(rel_path, e))
            yield result
    return wrapped
...
def parse_site(landing_zone_folder, rel_path):
    ...
    for i, picture in enumerate(site.pictures):
        ...
        for name in not_allowed:
            exceptions.append(ConsistencyError("field named '{}' not allowed for pictures"
                                               .format(name)))
...
@check_result_attribute_types
def parse_source(landing_zone_folder, rel_path):
    ...
    for i, avg in enumerate(source.get("averaging", [])):
        if not isinstance(avg, dict):
            exceptions.append(ConsistencyError("aveaging entry {} is not a mapping".format(i)))
            continue
    ...
    results = parse_source_type(landing_zone_folder, source_type_yaml_for_source_yaml(rel_path))
    source_type = None
    ...
    if "special_values" in source_type:
        ...
        for i, sv in enumerate(source_type["special_values"]):
            if not isinstance(sv, dict):
                e = ConsistencyError("entry {} of 'special_values' in {} does not consist of "
            ...
            if "description" not in sv or sv.description is None:
                sv.description = ""
            ok.append(sv)
        source_type.special_values = [SpecialValueDefinition(sv) for sv in ok]
    else:
...
def check_site_yaml(site, rel_path):
    check_attributes = [
        "name", "coordinates", "coordinates.x", "coordinates.y", "coordinates.z", "street", "city",
        "pictures"
    ]
...
def _check_averaging_settings(source):
    ...
    for i, averaging in enumerate(source.averaging):
        if not isinstance(averaging, dict):
            ...
            yield ConsistencyError("item {} listed in avaraging section has no entry 'parameter'"
                                   .format(i))
        else:
            ok.append(averaging)
    ...
    for name, count in parameter_name_counts.items():
        if count > 1:
            yield ConsistencyError("parameter '{}' is listed {} times in averaging section"
                                   .format(name, count))
    ...
    for averaging in sorted(ok, key=lambda avg: avg.parameter):
        ...
        not_nones = sum(1 for k, v in averaging.items()
                        if k.startswith("integration") and v is not None)
        if not_nones == 0:
            yield ConsistencyError("at least one averaging value must be set for '{}'"
                                   .format(averaging.parameter))
        ...
        for unit in ("length_x", "length_y", "angle", "time"):
            ...
            if value is not None:
                if not is_number(value):
                    yield ConsistencyError("averaging setting '{}' of '{}' is not a number: '{}'"
                                           .format(field_name, averaging.parameter, value))
...
def _check_special_values_section(special_values, rel_path):
    ...
    for i, entry in enumerate(special_values):
        if not isinstance(entry, dict):
            yield ConsistencyError("entry {} of special_value section in {} does not consist of "
                                   "key-value pairs".format(i, rel_path))
            continue

        if "numerical_value" not in entry:
            yield ConsistencyError("field 'numerical_value' is empty in entry {} of special_value "
                                   "section of {}".format(i, rel_path))
        else:
            if not is_number(entry.numerical_value):
                yield ConsistencyError("value '{}' specified by field 'numerical_value' in {} is "
        ...
        if "categorical_value" not in entry:
            yield ConsistencyError("field 'categorical_value' is empty in entry {} of "
                                   "special_value section of {}".format(i, rel_path))
        else:
            categorical_values.append(entry.categorical_value)

        not_allowed = entry.keys() - set(('numerical_value', 'categorical_value', 'description',
                                          '_rel_path'))
        for name in not_allowed:
            yield ConsistencyError("field '{}' not allowed in entry {} of special_value section "
                                   "in {}".format(name, i, rel_path))

    for name, count in Counter(numerical_values).items():
        if count > 1:
            yield ConsistencyError("numerical_value '{}' not unique in special_value section of {}"
                                   .format(name, rel_path))

    for value, count in Counter(categorical_values).items():
        if count > 1:
            yield ConsistencyError("categorical_value '{}' not unique in special_value section of "
