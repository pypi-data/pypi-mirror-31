...
def check_and_commit(source_type, engine):
    exceptions = check(source_type, engine)
    if exceptions:
        raise exceptions[0]
    return commit(source_type, engine)


@iter_to_list
def commit(source_type, engine):
    ...
    source_type_dbos = fetch_existing_source_type_dbos(session=session)
...
def fetch_existing_source_type_dbos(*, engine=None, session=None):
    assert session is not None or engine is not None
    if session is None:
        session = create_session(engine)
...
def _determine_and_check_differences(source_type_dbos, source_type_from_landing_zone):
    ...
    added, removed, existing = check_differences(source_type_dbos, source_type_from_landing_zone)
...
def _check_for_modifications(existing):
    ...
    for source_type_dbo, source_type in existing:
        ...
        if not (ok_1 and ok_2):
            ...
            yield ConsistencyError("source_type {} was modified".format(source_type.name))
...
def _check_for_source_type_modifications(source_type_dbo, source_type_from_landing_zone):
    return source_type_dbo.name == source_type_from_landing_zone.name


def _check_for_special_values_modifications(source_type_dbo, source_type_from_landing_zone):
    special_values_in_db = {sv.categorical_value: sv for sv in source_type_dbo.special_values}
    special_values_from_lz = {sv.categorical_value: sv
                              for sv in source_type_from_landing_zone.special_values}

    removed = special_values_in_db.keys() - special_values_from_lz.keys()

    existing_categorical_values = special_values_in_db.keys() & special_values_from_lz.keys()

    def neq(sv1, sv2):
        return sv1.categorical_value != sv2.categorical_value \
            or sv1.numerical_value != sv2.numerical_value

    modified = any(neq(special_values_in_db[cv], special_values_from_lz[cv])
