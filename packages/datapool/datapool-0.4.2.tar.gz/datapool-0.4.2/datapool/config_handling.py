...
def default_config(known_config):
    ...
    if os.getpid() == 0:
        config.server.pid_file = "/var/run/datapool.pid"
    else:
        folder = pathlib.Path("~").expanduser() / ".datapool"
...
def _check_pid_file(config):
    ...
    if os.path.exists(pid_file):
        ...
        if not os.path.exists(folder):
            yield False, "- folder {} for pid file {} does not exist".format(folder, pid_file)
        else:
            try:
...
def _check_runners(config):
    ...
    julia_ok = True
    for ok, message in _check_executable("julia", "julia", config, RRunner):
    ...
    if julia_ok and config.julia.executable:
        ...
        try:
            ...
            configured_version = tuple(map(int, config.julia.version.split(".")))

            if configured_version != actual_version:
                yield False, ("- julia interpreter is of version {}, configured is {}"
                              "".format(actual_version, configured_version))
            else:
...
def _check_executable(section, name, config, runner):
    ...
    if executable == "":
        ...
        try:
            ...
            yield False, "  - could not start {} from {}: {}".format(name, executable, e)
