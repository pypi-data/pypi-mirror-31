...
from .domain_objects import Parameters, Site, SourceType, Source
from .errors import ConsistencyError
from .landing_zone_structure import lookup_parser, lookup_checker
...
def _unique_file_name(domain_objects):
    ...
    if len(file_names) > 1:
        raise RuntimeError("different filenames for sites detected: {}".format(file_names))
    file_name = file_names.pop()
    return file_name


def check_parameters_integrity(file_name, root_folder, domain_objects, engine):
    if len(domain_objects) != 1:
        for domain_object in domain_objects:
            full_path = join(root_folder, domain_object.rel_path)
            yield ConsistencyError("found multiple {}. one is at {}".format(file_name, full_path))
        return
...
def check_sites_integrity(file_name, root_folder, site_objects, engine):
    for name in _report_duplicate_names(site_objects):
        yield ConsistencyError("duplicate names: name {} appears in {}".format(name, file_name))


def check_source_types_integrity(file_name, root_folder, source_type_objects, engine):
    for name in _report_duplicate_names(source_type_objects):
        yield ConsistencyError("duplicate names: name {} appears in {}".format(name, file_name))
    from .source_type_model import fetch_existing_source_type_dbos, check_differences
    source_type_dbos = fetch_existing_source_type_dbos(engine=engine)
    removed = set(s.name for s in source_type_dbos) - set(s.name for s in source_type_objects)
    for r in removed:
         yield ConsistencyError("source type {} is present in db, but removed in landing zone".format(r))


def check_source_integrity(file_name, root_folder, source_type_objects, engine):
...
class DomainObjectsChecker:
    ...
    def check_all(self, engine):
        ...
        yield from self.check_against_db(engine)
    ...
    def load_and_check_yamls(self):
        for rel_path in self.rel_paths:
            ...
            if parser is None:
                ...
                yield ConsistencyError("can not parse {}. maybe file is at wrong position or "
                                       "name is misspelled.".format(full_path))
                continue
            if checker is None:
                full_path = join(self.root_folder, rel_path)
                yield ConsistencyError("can not check {}. maybe file is at wrong position or "
                                       "name is misspelled.".format(full_path))
                continue
    ...
    def check_integrity(self, engine):
        for domain_object_class, objects in self.domain_objects.items():
            ...
            yield from integrity_checker(file_name, self.root_folder, objects, engine)

    def check_against_db(self, engine):
        ...
        for domain_objects in self.domain_objects.values():
            for domain_object in domain_objects:
                ...
                if checker is None:
                    ...
                    continue
                ok = True
                for check_result in checker(domain_object, engine):
                    if isinstance(check_result, Exception):
                        yield check_result
                        ok = False
                if ok:
                    committer = lookup_committer(domain_object.rel_path)
                    committer(domain_object, engine)
