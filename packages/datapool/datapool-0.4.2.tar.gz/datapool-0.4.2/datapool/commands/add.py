...
def add(lz_folder, what, settings, print_ok, print_err, input_=input):
    ...
    presets = setup_presets(what, script, settings, print_err)
...
def setup_presets(what, script, settings, print_err):
    ...
    for setting in settings:
        if setting.count("=") != 1:
            print_err("- setting '{}' invalid, required syntax is VARIABLE=VALUE".format(setting))
            ok = False
        else:
