...
from ..errors import DataPoolException
from ..landing_zone import write_lock, LandingZone
...
def update_operational(development_landing_zone, verbose, overwrite, copy_raw,
                       print_ok, print_err):
    ...
    with write_lock(operational_landing_zone) as got_lock:
        ...
        try:
            files_generator = lz.update_operational(operational_landing_zone, copy_raw, delay=0.5)
        except DataPoolException as e:
        ...
        for action, file in files_generator:
            ...
            time.sleep(.5)
            num_files += 1
        print_ok("+ copied/created {} files/folders to/in {}".format(num_files,
