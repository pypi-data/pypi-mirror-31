...
from .db_objects import SiteDbo, SourceTypeDbo, ParameterDbo
from .domain_objects import DomainObject, Site, SourceType, Parameters


@singledispatch
def domain_object_to_dbo(domain_object):
    assert isinstance(domain_object, DomainObject)
    return _domain_object_to_dbo(domain_object, domain_object.dbo)
...
def source_type_to_dbo(source_type):
    ...
    source_type.special_values = [domain_object_to_dbo(sv) for sv in source_type.special_values]
