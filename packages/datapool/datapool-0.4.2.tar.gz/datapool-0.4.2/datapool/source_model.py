...
from .db_objects import (SourceDbo, SourceTypeDbo, ParameterDbo)
...
def check_and_commit(source, engine):
    exceptions = check(source, engine)
    if exceptions:
        raise exceptions[0]
    return commit(source, engine)


@iter_to_list
def commit(source, engine):
    logger().debug("called commit for source {}".format(source.name))
    session = create_session(engine)

    existing_source_dbo = session.query(SourceDbo).filter(SourceDbo.name == source.name).first()
    if existing_source_dbo is not None:
        logger().debug("source with same name {} already exists".format(existing_source_dbo))
        to_create, __, __ = _determine_averaging_differences(existing_source_dbo, source)
        _add_new_averagings(existing_source_dbo, to_create, session)
        yield from _update(existing_source_dbo, source)

    else:

        source_type = session.query(SourceTypeDbo).filter(
            SourceTypeDbo.name == source.source_type).first()
...
def check(source, engine):
    ...
    if existing_source_dbo is not None:
        ...
        # if the source is new and has a serial number, the pair (serieal, source_type) should be
        # unique:
...
def _check_if_referenced_parameters_are_valid(session, source):
    ...
    for averaging in sorted(source.averaging, key=lambda avg: avg.parameter):
        dbo = session.query(ParameterDbo).filter(ParameterDbo.name == averaging.parameter).first()
        if dbo is None:
            yield ConsistencyError("parameter name '{}' in averaging section not in db"
                                   .format(averaging.parameter))


def _check_if_referenced_source_type_is_valid(session, source):

    dbo = session.query(SourceTypeDbo).filter(SourceTypeDbo.name == source.source_type).first()
    if dbo is None:
...
def _check_unique_serial(session, source):
    dbo = session.query(SourceDbo).filter(
        SourceDbo.serial == source.serial,
        SourceDbo.source_type.has(name=source.source_type)).first()
    if dbo is not None:
        yield ConsistencyError("combination source_type '{}' serial '{}' already exists"
                               .format(source.serial, source.source_type))
...
def _check_for_modifications(source, source_dbo):
    ...
    if source_dbo.source_type.name != source.source_type:
        ...
        yield ConsistencyError("detected invalid source modifiations, see log for details")


def _check_for_averaging_modifications(source, source_dbo):
    from .pretty_printers import pretty_log  # local import avoids circular import

    to_create, dbos_to_drop, dbos_already_existing = _determine_averaging_differences(source_dbo,
                                                                                      source)

    if dbos_to_drop:
        for dbo in sorted(dbos_to_drop, key=lambda dbo: dbo.parameter.name):
            logger().error("detected removed averaging settings of '{}' for source '{}':"
                           .format(dbo.parameter, source.name))
            pretty_log(logger().error, dbo)
        yield ConsistencyError("some averaging settings for source '{}' were removed."
                               .format(source.name))
    ...
    for dbo in sorted(dbos_already_existing, key=lambda dbo: dbo.parameter.name):
        ...
        if any((avg.integration_time != dbo.integration_time,
            ...
            logger().error("detected modification in averaging settings of '{}' for source '{}':"
                           .format(dbo.parameter, source.name))
...
def _add_new_averagings(source_dbo, to_create, session):
    ...
    for averaging in to_create:
        averaging.parameter = session.query(ParameterDbo).filter(
            ParameterDbo.name == averaging.parameter).first()
