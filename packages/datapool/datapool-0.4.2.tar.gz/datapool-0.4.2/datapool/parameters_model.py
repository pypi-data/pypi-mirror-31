...
def check_and_commit(parameters, engine):
    exceptions = check(parameters, engine)
    if exceptions:
        raise exceptions[0]
    return commit(parameters, engine)


@iter_to_list
def commit(parameters, engine):
...
def _update_parameters(existing):
    ...
    for parameter_dbo, parameter_yaml in existing:
        yield from update_and_report_change(parameter_dbo, parameter_yaml, "description")


@iter_to_list
def check(parameters, engine):
    session = create_session(engine)
...
def _determine_and_check_differences(parameter_dbos, parameters_from_landing_zone):
    ...
    added, removed, existing = _check_differences(parameter_dbos, parameters_from_landing_zone)
    if removed:
...
def _check_for_parameter_modifications(parameter_dbo, parameter_from_landing_zone):
    ...
    return all(getattr(parameter, field) == getattr(parameter_dbo, field)
