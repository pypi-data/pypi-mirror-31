...
@click.option(
    ...
    " checking")
@click.option("--copy-raw-files",
              is_flag=True, help="copy raw files also to operational landing zone")
@click.argument("development-landing-zone-folder", type=str)
@print_banner
@inject_pdb
def update_operational(development_landing_zone_folder, force_count, verbose, copy_raw_files):
    """deploys local changes to operational landing zone.
    """
    from .commands import update_operational
    overwrite = (force_count > 1)
    sys.exit(
        update_operational(development_landing_zone_folder, verbose, overwrite, copy_raw_files,
                           print_ok, print_err))
...
@click.option('--force', "force_count", count=True,
              help="use this twice to perform deletion, else only checks are performed and number "
              "of affected db entries is estimated")
@click.option('--max-rows', type=click.INT, default=10,
...
def delete_signals(force_count, max_rows, filters):
    """
    This command deletes all signals where all filter conditions are true.

    Filters are exressions like "timestamp>=2000-01-01,site==abc", the surrounding quotes
    are required if symbols '<' or '>' are occur in such expressions.
    """
    from .commands.delete import delete_signals, delete_meta
    if os.getuid() != 0:
...
@click.option('--what',
              type=click.Choice(['signals', 'source', 'soure_type', 'site', 'parameter']))
@click.option('--force', "force_count", count=True,
              help="use this twice to perform deletion, else only checks are performed and number "
              "of affected db entries is estimated")
@click.option('--max-rows', type=click.INT, default=10,
