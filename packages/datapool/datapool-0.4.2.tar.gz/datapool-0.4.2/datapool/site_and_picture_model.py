...
def _update(site_dbo, site_from_landing_zone, session):
    ...
    yield from _check_and_update_and_report(site_dbo, site_from_landing_zone,
                                            NOT_ALLOWED_TO_CHANGE_SITE + ("pictures",))

    lookup = {p.filename: p for p in site_from_landing_zone.pictures}
    for picture in site_dbo.pictures:
        yield from _check_and_update_and_report(picture, lookup[picture.filename],
                                                NOT_ALLOWED_TO_CHANGE_PICTURE)
...
def _check_for_site_modifications(site_dbo, site_from_landing_zone):
    ...
    for field_name in NOT_ALLOWED_TO_CHANGE_SITE:
        if getattr(site_dbo, field_name) != getattr(site, field_name):
            ...
            yield ConsistencyError("detected invalid site modifiation of {}, see log for details".
                                   format(field_name))
...
def _compare_pictures(picture_dbo, new_picture):
    return all(getattr(picture_dbo, field_name) == getattr(new_picture, field_name)
