...
class _EventHandler(FileSystemEventHandler):
    ...
    def call_back(self, event, full_path):
        ...
        self._call_back(event, rel_path)
    ...
    def on_created(self, event):
        if isinstance(event, FileCreatedEvent):
            ...
            if IS_MAC:
                ...
                # on linux two events are fired for a created file: first a CREATED_EVENT
                # and finally a MODIFIED_EVENT when the data is written to disk.
                # but sometimes two CREATED_EVENT are fired but no MODIFIED_EVENT.
                # to handle this we record hashes of the file contents.
                with self.hash_lock:
    ...
    def on_modified(self, event):
        if isinstance(event, FileModifiedEvent):
            self.logger.debug("EventHandler: modification detected: {}".format(event.src_path))
            if not event.src_path.endswith(".inwrite"):
                with self.hash_lock:
...
class Observer:
    ...
    def __init__(self, root_folder, call_back):
        self.root_folder = root_folder
    ...
    def start(self):
        self.observer_thread = Thread(target=self._run_observer)
    ...
    def _run_observer(self):
        ...
        try:
            self._observer = start_observer(self.root_folder, self.call_back)
        except Exception as e:
            self.e = e

    def stop(self):
        shutdown_observer(self._observer)
