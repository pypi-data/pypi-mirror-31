...
from .errors import InvalidLandingZone, InvalidOperationError, DataPoolIOError
...
def create_folder_if_not_exists(path):
    if not os.path.exists(path):
        try:
            ...
            raise DataPoolIOError("coulnd not create folder {}. Please check permissions"
                                  .format(path))
    return False
...
class LandingZone:
    ...
    def _check_start_state(self):
        ...
        missing = []
        for rel_path in self._resolve_start_state():
    ...
    def _remove_from_start_state(self, path):
        files = self._resolve_start_state()
        if path in files:
            del files[path]
    ...
    def create_from(new_landing_zone, existing_landing_zone):
        ...
        already_exists = list_folder_recursive(existing_landing_zone, also_folders=False)
    ...
    def update_operational(self, operational_landing_zone, copy_raw, delay=0.1):
        ...
        for new_file in allowed_changes_in_correct_order:
            if new_file.endswith(".raw") and not copy_raw:
                continue
            target = join(operational_landing_zone, new_file)
