...
from carpalrouting.grouping import LocationGrouping, DriverGrouping
from carpalrouting.optimize import Routing
from carpalrouting.models import Driver, Location, Coordinates, GeoArea
                                    ...
                                    if ';' in locs[filtered_indices[item]].ref:
                                        for sub_item in locs[filtered_indices[item]].ref.split(';'):
                                            ...
                                            routes.append(Location(
                                                coordinates=Coordinates(locs[filtered_indices[item]].coordinates[0], 
                                        ...
                                        for item in idx_list:
                                            if ';' in locs[item].ref:
                                                for sub_item in locs[item].ref.split(';'):
                                                    itm_info = sub_item.split(',')
                                                    routes.append(Location(coordinates=Coordinates(locs[item].coordinates[0],
