...
# region load balancer internal

# needs to be Static so it will be assigned a value immediately
lb_ip_conf_internal = FrontendIPConfiguration('lb_ip_conf',
                                              privateIPAllocationMethod='Dynamic',
                                              subnet=SubResource(id=vnet.SubnetRef('subnet1')))

lb_rule_internal = LoadBalancingRule('lbrule',
                                     protocol='Tcp',
                                     loadDistribution='SourceIP',
                                     frontendPort=22,
                                     backendPort=22,
                                     frontendIPConfiguration=FrontendIPConfigurationRef(load_balancer='my_lb',
                                                                                        ip_conf=lb_ip_conf_internal),
                                     probe=ProbeRef(load_balancer='my_lb', probe='testProbe'),
                                     backendAddressPool=BackendAddressPoolRef(load_balancer='my_lb',
                                                                              backend_pool='lb_backend'))

probe_internal = Probe('testProbe', protocol='Tcp', port=22)

lb_intertnal = LoadBalancer('my_lb_internal',
                            sku=LoadBalancerSku(name='Basic'),
                            frontendIPConfigurations=[lb_ip_conf_internal],
                            backendAddressPools=[BackendAddressPool(name='lb_backend')],
                            probes=[probe_internal],
                            loadBalancingRules=[lb_rule_internal])
lb_intertnal.with_depends_on(vnet)
...
template.add_resource([lb_pub_ip, lb, lb_intertnal, vm_scale_set, vnet])
