...
def build(branch, installPath, node=False, angularBuild=False):
    ...
    with lcd(installPath):
        ...
        if angularBuild:
            local(
                'npm run build -- --prod --env=%s && cp -r public/* dist/' % environment)
        else:
            local('npm run dist')
        ...
        if node in TRUTH_VALUES:
            ...
            local(
                'if [[ -d "src/dt-cfgs" ]]; then echo "<collecting dt-cfgs>" ; rsync -ra --stats src/dt-cfgs/ dist/dt-cfgs; fi;')
