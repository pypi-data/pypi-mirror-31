  ...
  $.extend({
    msgpackRPC: {
      ...
      batchRequest: function(requests, options) {
        ...
        if(!$.isArray(requests) || requests.length === 0) throw("Invalid requests supplied for msgpackRPC batchRequest. Must be an array object that contain at least a method attribute");
        ...
        this._doRequest(msgpack.encode(data), options);
      },
      ...
      _validateRequestParams: function(params) {
        if(!(params === null ||
             ...
             $.isArray(params))) {
          throw("Invalid params supplied for msgpackRPC request. It must be empty, an object or an array.");
        }
        return true;
      },
      ...
      _doRequest: function(data, options) {
        ...
        $.ajax({
          type: 'POST',
          accepts: { msgpack_type: "arraybuffer" },
          converters: {
            'text arraybuffer msgpack_type': function(result) {
              // Do Stuff
              console.log(result);
              var msgLength = result.length;
              var array = new Uint8Array(new ArrayBuffer(msgLength));
              for(i = 0; i < msgLength; i++) {
                array[i] = result.charCodeAt(i);
              }
              var converted = msgpack.decode(array);
              return converted;
            }
          },
          async: false !== options.async,
          dataType: 'msgpack_type',
        ...
        if (msg === undefined) {
          ...
          try {
            if(typeof(msg) === 'string') {
              ...
              msg = msgpack.decode(array);
            }

            if (($.isArray(msg) && msg.length > 0 && msg[0].msgpackrpc !== '2.0') ||
                (!$.isArray(msg) && msg.msgpackrpc !== '2.0')) {
              throw 'Version error';
            }
