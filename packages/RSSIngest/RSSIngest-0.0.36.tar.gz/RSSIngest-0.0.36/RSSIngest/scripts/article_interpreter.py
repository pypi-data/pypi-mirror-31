...
class ArticleInterpreter:
    ...
    def get_topics(self, article):
        ...
        self.attributes['title_keywords'] = Counter(list(set([noun[0].lower() for noun in pos_tag(word_tokenize(title))
                                                    if noun[1].startswith('NN')])))
        self.attributes['title_topics'] = [p_noun.lower() for p_noun in self.attributes['title_keywords']
