...
class FileFinder(object):
    ...
    def iter_modules(self, prefix=''):
        if self._path_mtime == -1:
            self._fill_cache()
        for filename in list(self._path_cache):
            if filename.endswith('.py') and not (filename.startswith('__') and filename.endswith('__')):
                if prefix:
                    yield ('.'.join((prefix, filename.rsplit('.', 1)[0])), False)
                else:
                    yield (filename.rsplit('.', 1)[0], False)
            ...
            if package:
                ...
                try:
                    ...
                    try:
                        ...
                        try:
                            importer_frame = inspect.getouterframes(inspect.currentframe(), 1)
                            module_frame = importer_frame[1][0]
                            # calling_string = importer_frame[1][4]
                            _outer_module_name = module_frame.f_locals['__spec__'].name
                            _outer_module_path = module_frame.f_locals['__spec__'].origin
                            _outer_module_dir = os.path.dirname(_outer_module_path)
                            _module_name = '.'.join((_outer_module_name, name))
                            if len(set(_module_name.split('.'))) == len(_module_name.split('.')):
                                if any([filename.startswith(name+'.py') for filename in os.listdir(os.path.dirname(_outer_module_path))]):
                                    try:
                                        sys.path.insert(0, _outer_module_dir)
                                        return _gcd_import(name)
                                    finally:
                                        sys.path.remove(_outer_module_dir)
