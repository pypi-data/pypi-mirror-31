...
class AsyncDockerContainer(object):
    ...
    async def __aexit__(self, exc_type, exc_val, exc_tb):
        cont_done = self.docker.container(self.container_id)
        if cont_done is None:
            logger.error(f'Failed to obtain container status for {self.container_id}, container not found')
            return

        logger.debug(f"{self.container_id}:  __aexit__ state {cont_done.status}")
        if cont_done.status != 'exited':
            if self.kill_on_exit:
                cont_done.kill()
                self.exit = (cont_done.status, cont_done.wait())
            else:
                logger.error(f'Leaving container id {cont_done.id} Running')
        else:
            self.exit = (cont_done.status, cont_done.wait())
