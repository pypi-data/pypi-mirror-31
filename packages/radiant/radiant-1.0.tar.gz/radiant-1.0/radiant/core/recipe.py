...
from djangoforandroid.core.logger import (logger, info, warning, error, debug, shprint, info_main)
from djangoforandroid.core.util import (urlretrieve, current_directory, ensure_dir)

# this import is necessary to keep imp.load_source from complaining :)
import djangoforandroid.core.recipes
...
class Recipe(with_metaclass(RecipeMeta)):
    ...
    def download_if_necessary(self):
        ...
        target = join(self.ctx.packages_path, self.name)

        if exists(target):
            info("Skipping download for {}".format(self.name.lower()))
            return
