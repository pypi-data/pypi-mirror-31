...
from djangoforandroid.core.recipe import Recipe, PythonRecipe, CythonRecipe, CompiledComponentsPythonRecipe, BootstrapNDKRecipe, NDKRecipe

from djangoforandroid.core.archs import ArchARM, ArchARMv7_a, Archx86
from djangoforandroid.core.logger import logger, info, warning, setup_color, Out_Style, Out_Fore, Err_Style, Err_Fore, info_notify, info_main, shprint, error
from djangoforandroid.core.util import current_directory
from djangoforandroid.core.bootstrap import Bootstrap
from djangoforandroid.core.distribution import Distribution
from djangoforandroid.core.graph import get_recipe_order_and_bootstrap
from djangoforandroid.core.build import Context, build_recipes
...
def require_prebuilt_dist(func):
    ...
    def wrapper_func(self, **args):
        ...
        if dist.needs_build:
            ...
            build_dist_from_args(ctx, dist, self.args)
        func(self, **args)
    return wrapper_func
...
class ToolchainCL(object):
    ...
    def apk(self, release=False):
        ...
        with current_directory(dist.dist_dir):
            ...
            build_args = build.parse_args(self.args)
...
#                 output = shprint(gradle, "wrapper", _tail=20,
#                                  _critical=True, _env=env)
                #os.system('gradle wrapper')
                #os.system('chmod 777 gradlew')
                gradlew = sh.Command('./gradlew')
                if self.args.build_mode == "debug":
                    gradle_task = "assembleDebug"
                elif self.args.build_mode == "release":
                ...
                try:
                    ...
                    exit(1)
                output = shprint(ant, self.args.build_mode, _tail=20,
                                 _critical=True, _env=env)
