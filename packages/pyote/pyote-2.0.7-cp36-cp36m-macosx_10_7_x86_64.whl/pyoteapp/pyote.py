...
class SimplePlot(QtGui.QMainWindow, gui.Ui_MainWindow):
    def __init__(self):
        ...
        # self.showInfo(self.markDzone.toolTip())
        ...
        self.saveTooltipTexts()

    def saveTooltipTexts(self):
        self.infoButtonTText = self.infoButton.toolTip()
        self.readDataTText = self.readData.toolTip()
        self.curveToAnalyzeSpinBoxTText = self.curveToAnalyzeSpinBox.toolTip()
        self.showSecondaryCheckBoxTText = self.showSecondaryCheckBox.toolTip()
        self.doBlockIntegrationTText = self.doBlockIntegration.toolTip()
        self.setDataLimitsTText = self.setDataLimits.toolTip()
        self.smoothSecondaryButtonTTtext = self.smoothSecondaryButton.toolTip()
        self.numSmoothPointsEditTText = self.numSmoothPointsEdit.toolTip()
        self.normalizeButtonTText = self.normalizeButton.toolTip()
        self.markDzoneTText = self.markDzone.toolTip()
        self.markRzoneTText = self.markRzone.toolTip()
        self.calcFlashEdgeTText = self.calcFlashEdge.toolTip()
        self.minEventEditTText = self.minEventEdit.toolTip()
        self.maxEventEditTText = self.maxEventEdit.toolTip()
        self.calcErrBarsTText = self.calcErrBars.toolTip()
        self.writePlotTText = self.writePlot.toolTip()
        self.writeBarPlotsTText = self.writeBarPlots.toolTip()
        self.startOverTText = self.startOver.toolTip()
        self.tableTText = self.table.toolTip()

    def restoreTooltipTexts(self):
        self.infoButton.setToolTip(self.infoButtonTText)
        self.readData.setToolTip(self.readDataTText)
        self.curveToAnalyzeSpinBox.setToolTip(self.curveToAnalyzeSpinBoxTText)
        self.showSecondaryCheckBox.setToolTip(self.showSecondaryCheckBoxTText)
        self.doBlockIntegration.setToolTip(self.doBlockIntegrationTText)
        self.setDataLimits.setToolTip(self.setDataLimitsTText)
        self.smoothSecondaryButton.setToolTip(self.smoothSecondaryButtonTTtext)
        self.numSmoothPointsEdit.setToolTip(self.numSmoothPointsEditTText)
        self.normalizeButton.setToolTip(self.normalizeButtonTText)
        self.markDzone.setToolTip(self.markDzoneTText)
        self.markRzone.setToolTip(self.markRzoneTText)
        self.calcFlashEdge.setToolTip(self.calcFlashEdgeTText)
        self.minEventEdit.setToolTip(self.minEventEditTText)
        self.maxEventEdit.setToolTip(self.maxEventEditTText)
        self.calcErrBars.setToolTip(self.calcErrBarsTText)
        self.writePlot.setToolTip(self.writePlotTText)
        self.writeBarPlots.setToolTip(self.writeBarPlotsTText)
        self.startOver.setToolTip(self.startOverTText)
        self.table.setToolTip(self.tableTText)

    def eraseTooltipTexts(self):
        self.infoButton.setToolTip('')
        self.readData.setToolTip('')
        self.curveToAnalyzeSpinBox.setToolTip('')
        self.showSecondaryCheckBox.setToolTip('')
        self.doBlockIntegration.setToolTip('')
        self.setDataLimits.setToolTip('')
        self.smoothSecondaryButton.setToolTip('')
        self.numSmoothPointsEdit.setToolTip('')
        self.normalizeButton.setToolTip('')
        self.markDzone.setToolTip('')
        self.markRzone.setToolTip('')
        self.calcFlashEdge.setToolTip('')
        self.minEventEdit.setToolTip('')
        self.maxEventEdit.setToolTip('')
        self.calcErrBars.setToolTip('')
        self.writePlot.setToolTip('')
        self.writeBarPlots.setToolTip('')
        self.startOver.setToolTip('')
        self.table.setToolTip('')

    def toggleTooltips(self):
        if self.enableTooltipsCheckBox.isChecked():
            self.restoreTooltipTexts()
        else:
            self.eraseTooltipTexts()
    ...
    def changeSecondary(self):
        ...
        refStar = None
        if refNum == 1:
            refStar = [float(item) for item in self.yValCopy]
        if refNum == 2:
    ...
    def changePrimary(self):
        ...
        if refNum == 1:
            values = [float(item) for item in self.yValCopy]
        if refNum == 2:
            values = [float(item) for item in self.secondary]
        if refNum == 3:
            values = [float(item) for item in self.ref2]
        if refNum == 4:
            values = [float(item) for item in self.ref3]
    ...
    def calculateFlashREdge(self):
        ...
        if self.solution:
            self.flashEdges.append(self.solution[1] + float(self.frame[0]))
    ...
    def readDataFromFile(self):
        ...
        if self.filename:
            ...
            try:
                self.outliers = []
                self.frame, time, value, self.secondary, self.ref2, self.ref3, \
                ...
                if self.timestampListIsEmpty(time):
                    ...
                    while errmsg != 'ok':
                        errmsg, manualTime, dataEntered = \
                            manualTimeStampEntry(self.frame, TSdialog(), self.flashEdges)
                        if errmsg != 'ok':
                ...
                self.dataLen = len(self.yValues)
                self.yFrame = self.frame[:]
                self.yFrameCopy = self.frame[:]
                ...
                if self.timeDelta == 0.0 and not self.manualTimestampCheckBox.isChecked():
                    self.showInfo("Analysis of timestamp fields resulted in "
                                  "an invalid timeDelta of 0.0\n\nSuggestion: Select manual timestamp entry (checkbox at top left)"
                                  ", then press 'Start over'"
    ...
    def restart(self):
        ...
        if self.manualTimestampCheckBox.isChecked():
            ...
            errmsg = ''
            while errmsg != 'ok':
                errmsg, manualTime, dataEntered = manualTimeStampEntry(
                    self.frame, TSdialog(), self.flashEdges)
                if errmsg != 'ok':
                    self.showInfo(errmsg)
                else:
                    self.showMsg(dataEntered, bold=True)
                    if manualTime:
                        self.manualTimestampCheckBox.setChecked(False)
                        time = manualTime[:]
                        self.yTimes = time[:]
                        self.yTimesCopy = time[:]
