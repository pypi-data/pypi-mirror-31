...
import re
import time
import DigitalSystems


class AutoUpdater(threading.Thread):

    """
    AutoUpdater class provides a collection of static methods for linking DigitalSystems connections primitives Bus and Connector.
    ...
    * run(b,a) # The main execution loop to keep the connections updated.
    ...
    def add_link(
        ...
        """
        try:
            Bus = DigitalSystems.connectors.connector.Bus
            Connector = DigitalSystems.connectors.connector.Connector
...
class DigitalSystemsIndexer(object):
    ...
    * get_index
    ...
    @property
    def indices():
        return DigitalSystemsIndexer._indices

    @property
    def rev_indices():
        return DigitalSystemsIndexer._rev_indices

    @staticmethod
    def index(element):
        """ Add an element to the indexed list and return its unique ID """
        if element.__class__ not in DigitalSystemsIndexer._indices:
            DigitalSystemsIndexer._indices[element.__class__] = {}
            DigitalSystemsIndexer._rev_indices[element.__class__] = {}
            DigitalSystemsIndexer._max_index[element.__class__] = 0

        if DigitalSystemsIndexer.get_index(element) is not None:
            return DigitalSystemsIndexer.get_index(element)

        # Unique ID numbers are not recycled.
        DigitalSystemsIndexer._max_index[element.__class__] += 1

        # Unique ID for the element
        uid = DigitalSystemsIndexer._max_index[element.__class__]
    ...
    def unindex(element=None, index=None, cls=None):
        ...
        if element is not None:
            index = DigitalSystemsIndexer.get_index(element)
            cls = element.__class__

        elif (index is None) and (cls is None):
            raise Exception("Insufficient parameters passed")

        element = DigitalSystemsIndexer._indices[cls].pop(index)
        DigitalSystemsIndexer._rev_indices[cls].pop(element)

    @staticmethod
    def get_index(element):
        """ Get the index of the element """
        if element in DigitalSystemsIndexer._rev_indices[element.__class__]:
            return DigitalSystemsIndexer._rev_indices[element.__class__][element]
    ...
    def get_element(index, cls):
        ...
        >>> g1.index
        5
        >>> DigitalSystemsIndexer.get_element(5, Connector).get_state()
        0
        ...
        the various elements of DigitalSystems by its index ( the id ).

        """
        if (cls in DigitalSystemsIndexer._indices) and (
                index in DigitalSystemsIndexer._indices[cls]):
            return DigitalSystemsIndexer._indices[cls][index]
