...
class pretreatment(object):
    ...
    def missingValue(self, percent):
        ...
        for col in self.df_feature.columns:
            ...
            if series[series.isnull()].count() / series.count() > percent:
                logger.info("特征“{}”的缺失值占比超过{:.1f},舍弃此特征".format(series.name, percent))
            else:
    ...
    def analyzeComplex(self, num=None):
        # 如果没有提供复杂度要求
        if not num:
            num = len(self.df_feature) / 10
        series_list = []
        for col in self.df_feature.columns:
            series = self.df_feature[col]
            if len(series.drop_duplicates()) < num:
                logger.info("特征'{}'的小于{},舍弃此特征".format(series.name, num))
            else:
                series_list.append(series)
        self.df_feature = pd.concat(series_list, axis=1)
        return self.df_feature
...
class featureWOE(object):
    ...
    # 对特征进行分组
    def getWOEandIV(self, groupNum=4, groupMeth=''):
        sample_size = len(self.df)
        IV_list = []
        for feature in self.df.columns[:-3]: # 最后两列,'label'和'cls'列为标签相关列
...
@pmi.execInfo()
def featureEngineering(featureFile, labelFile, colF, colL, label, border, groupNum, groupMeth='',file_ouput=''):
    # 初始化pretreatment类
