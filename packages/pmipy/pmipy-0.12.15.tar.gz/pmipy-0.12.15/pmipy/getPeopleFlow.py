...
def yichuxing_backup1(df_point, df_ycx, lng, lat, meter, ncore=6):
    def tmp_func(df):
        PF = []
...
def yichuxing_backup2(df_point, df_ycx, lng, lat, meter, ncore=2):
    def proc(df):
        PF = []
        for lng1, lat1 in tqdm(df.loc[:, [lng, lat]].values):
            peo_flow = 0 # 人流指数
...
def main(pointFile, ycxFile, lnglat='经度-纬度', meter='500', core_num='max', outputFile=''):
    # 读取店铺点数文件
    if pointFile[-4:] == '.csv':
...
if __name__ == '__main__':
    ...
    main(pointFile, ycxFile, lnglat, 500, 12, 'ts')
