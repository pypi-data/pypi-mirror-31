...
class CustomInstall(install):
    def run(self):
        ...
        directories_to_be_created = [os.path.expanduser("~/.edam/"), os.path.expanduser("~/.edam/templates/"),
                                     os.path.expanduser("~/.edam/configurations/"),
        ...
        directories_to_be_copied_from_resources = ['inputs', 'templates', 'configurations']
...
def return_list_with_additional_data():
    data_files = list()
    for folder, _, filenames in os.walk(os.path.join(here, 'edam', 'resources')):
        try:
            filenames.remove('.DS_Store')
        except:
            pass
        data_files.append((folder, filenames))
    return data_files
