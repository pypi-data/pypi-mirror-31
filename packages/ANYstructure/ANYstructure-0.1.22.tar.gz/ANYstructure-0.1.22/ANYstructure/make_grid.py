...
class Grid:
    ...
    def make_empty_grid(self):
        ...
        :return:
        '''
        return [[EMPTY for dummy_col in range(self._grid_width)] for dummy_row in range(self._grid_height)]
    ...
    def get_matrix(self):
        """
        Return the complete matrix in list form.
        """
        return self._cells
    ...
    def get_highest_number_in_grid(self):
        ...
        '''
        temp = []
        [[temp.append(item) for item in items] for items in self._cells]
        return max(temp)
    ...
    def set_string(self, row, col, string):
        """
        Set cell with index (row, col) to be full
        """
        self._cells[row][col] = string
    ...
    def is_tank(self, row, col):
        """
        Checks whether cell with index (row, col) is empty
        """
        return self._cells[row][col] == TANK
    ...
    def get_adjacent_values(self,cell):
        ...
        return tuple(set([self.get_value(neighbor[0], neighbor[1])
    ...
    def get_adjacent_values_duplicates(self,cell):
        ...
        return_tuple = tuple(list([self.get_value(neighbor[0], neighbor[1])
                                   for neighbor in self.four_neighbors(cell[0], cell[1])]))

        # for neighbor in self.four_neighbors(cell[0], cell[1]):
        #     print('NIGHBOR: ',neighbor[0], neighbor[1])
        #     print(list(self.get_value(neighbor[0], neighbor[1])))
    ...
    def get_lowest_cell(self,value):
        '''
        Get the cell closes to (height,0) with a given value.
        :param value:
        :return:
        '''
        lowest = (0,0)
    ...
    def import_grid(self,grid):
        '''
        Import a grid to replace created grid.
        :param grid:
        :return:
        '''
        self._cells = grid
