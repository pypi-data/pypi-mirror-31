...
class Table(TableInitMixin):
    ...
    def __init__(self,
        ...
        self.heading_row_justifications = []
        ...
        self.summaries = Row(headings=self.column_headings.keys(),
                             row={key: u'' for key in self.column_headings.keys()}) if show_summaries else None
        self.rows = []
    ...
    def add_heading_row(self,
        ...
        if self._row_numbers_enabled:
            # Add blank row to account for row_numbers column!
            headings.insert(0, {HEADING: BLANK_CELL})
        ...
        self.heading_row_justifications.append([heading.get(JUSTIFY, Justify.left) for heading in headings])
    ...
    def html(self,
        ...
        with _table_col_div:
            ...
            self._html_table_popover(text=text_helper,
                                     jira=jira_helper,
    ...
    def _html_table_popover(self,
        ...
        # TODO: this may have to be converted back from a popover, perhaps into an extra footer row?

        if text or jira:
            # Add popover kwargs to
            table_kwargs.update({
                u'data_toggle': u'popover',
                u'data_html': u'true',
                u'data_placement': u'bottom',
                u'data_original_title': u'Click to export to:'
            })

            popover_div_id = uuid4()

            tags.script(u"clickable_popover('{table_id}', '{popover_id}');".format(table_id=table_kwargs.get(u'id'),
                                                                                   popover_id=popover_div_id))

            _popover_div = tags.div(id=popover_div_id,
            ...
            with _popover_div:
                ...
                if text:
                    escaped_text = escape_html_text(unicode(self) if text is True else text)
            ...
            if self.show_column_headings:
                ...
                for heading_row_idx, heading_row in enumerate(self.heading_rows):
                    ...
                    header = _formatted_rows(
                            widths=widths,
                            cells=headers,
                            justifications=self.heading_row_justifications[heading_row_idx])
