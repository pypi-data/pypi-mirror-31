...
NO_FLAGS = 0 #lib.SECP256K1_CONTEXT_NONE
...
class Base(object):
    ...
    def __init__(self, ctx, flags):
        ...
        if ctx is None:
            ...
            if FLAG_COMMIT == FLAG_RANGEPROOF:
...
class Pedersen(Base):
    ...
    def pedersen_init(self):
        #lib.secp256k1_pedersen_context_initialize(self.ctx);
        pass

    def rangeproof_init(self):
        #lib.secp256k1_rangeproof_context_initialize(self.ctx);
        pass
...
#*
    def pedersen_blind_sum(self, blinds, n, npositive):
        outlen = 32
...
#*
    def pedersen_verify_tally(self, commits, neg_commits, excess):
        pcnt = len(commits)
        commit_ptrs = list(range(0, pcnt))#ffi.new("unsigned char *[%d]" % (n))
...
#*
    def rangeproof_verify(self, commit, proof):
        ...
        res = secp256k1_rangeproof_verify(
            self.ctx,
            min_value,
            max_value,
...
#*
    def rangeproff_rewind(self, commit, proof, plen, nonce):
        min_value = ffi.new("uint64_t *")
        max_value = ffi.new("uint64_t *")
        ...
        assert res == 1
        return bytes(ffi.buffer(message_out, outlen)), value_out[0]
...
#*        max_value: pointer to an unsigned int64 which will be updated with the maximum value that commit could have. (cannot be NULL)
#*
    def rangeproof_info(self, proof, plen):
        exp = ffi.new("int *")
