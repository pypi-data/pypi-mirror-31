...
# Aliases and copying
    ...
    def _del_mypage(self, user, mypage):
        mypages = self.get_mypages(user)
        pages =  [x for x in mypages['pages'] if x.get('name') != mypage]
        mypages['pages']=pages
        return mypages

    def del_mypage(self, user, mypage):
        url = self.make_url('user','mypages',user)
        mypages = self._del_mypage(user, mypage)
        res = self.request("post", url, data = mypages)
        return mypages

    def post_mypage(self, user, mypage, cluster,  title, name):
        url = self.make_url('user','mypages',user)
        mypages = self._del_mypage(user, mypage)
        mypages['pages'].append({"cluster":cluster,
                            "article": mypage,
                            "title": title,
                            "name": name
                            })  
