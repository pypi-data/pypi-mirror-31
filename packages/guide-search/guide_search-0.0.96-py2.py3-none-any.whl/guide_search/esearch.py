...
# query functions
    ...
    def get_cluster(self, index, id, size=100):
        dsl = Dsl().query_cluster(id).size(100) 
    ...
    def get_cluster_list(self, index, **kwargs):
        ...
        dsl = Dsl().Q().cluster_list(max) 
        dsl.Q_filter({"terms": {"purpose": ["landing", "article"]}})
        dsl.Q_filter({"terms": {"archive": ["true"]}})
        url = self.make_url(index, "articles", "_search")
        res = self.request("get", url, data=dsl.dsl())
        ...
        dsl = Dsl().Q().fields(["id", "title", "scope"]).size(max).sort("id","asc")
        ...
        for c in cl:
            try:
                ...
                cls[c["_source"]["id"]] = { "id": c["key"], 
                                            "count": 0, 
                                            "title": "no activ pages",
                                            "scope":"no active page for cluster " + c["key"],
                                            "sens": False}
...
# Aliases and copying
    ...
    def del_mypage(self, user, path, mypage):
        url = self.make_url('user','mypages',user)
        mypages = self._del_mypage(user, path, mypage)
        res = self.request("post", url, data = mypages)
        return mypages

    def post_mypage(self, user, mypage, path,  title, name):
