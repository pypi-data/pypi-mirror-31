...
# Aliases and copying
    ...
    def _del_mypage(self, user, path, name):
        ...
        pages =  [x for x in mypages['pages'] if x.get('path') != path and x.get('name') != name]
        mypages['pages']=pages
        return mypages
    ...
    def post_mypage(self, user, mypage, path,  title, name):
        ...
        if not name:
            name = title    
        mypages = self._del_mypage(user, mypage)
        mypages['pages'].append({"path":path,
                            "article": mypage,
                            "title": title,
                            "name": name
                            })  
        mypages = sorted(mypages,key = itemgetter('path','name')) 
        res = self.request("post", url, data = mypages)                             
        return mypages 