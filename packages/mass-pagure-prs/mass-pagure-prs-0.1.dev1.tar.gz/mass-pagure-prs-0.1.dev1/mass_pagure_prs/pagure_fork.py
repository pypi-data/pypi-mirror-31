...
class PagureFork(object):
    ...
    def create_pull_request(self, from_branch, to_branch,
        ...
        # TODO: with Pagure 4.0 deployed swith to API
        # and remove the need for fas_user and fas_password.
        # Not tested yet.
        # create_pr_api = (
        #     f'{self.fork_api}/{self.pagure_user}/rpms/'
        #     f'{self.package_name}/pull-request/new')
        ...
        #     }
        #     response = self.pagure._call_api(
        #         create_pr_api, method='POST', data=payload)
        #     self.logger.debug(f"PR created: {response}")
        # except APIError as err:
        #     raise err
        # except Exception as err:
        #     raise PagureForkException(
        #         f'Failed to create a PR for {self.package_name}. '
        #         f'Error: {err}')
