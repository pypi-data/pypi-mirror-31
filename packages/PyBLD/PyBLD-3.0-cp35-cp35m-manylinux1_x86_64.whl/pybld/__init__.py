# Copyright (C) 2000-2016 Hiroshi Watabe watabe@cyric.tohoku.ac.jp
# Define PYBld Class python implemantation of BLD originally developed by Richard E Carson
# $Id: __init__.py,v 17516caac345 2017/08/20 04:44:33 watabe $
from numpy import *
...
class PYBLD:
	...
	def mullin(self,ind_y,dep_x,wt=array([0.0]),cst=0):
		...
		self.fitresmat = array([0])
		if len(dep_x.shape)==1:
			dep_x = reshape(dep_x,(1,dep_x.shape[0]))
		...
		if len(wt)==len(ind_y):
			...
			weight = 0
		#dep_x = transpose(dep_x)
		res = pybldc.mullin(ind_y,dep_x,wt,weight,cst)
		n_var = int(res[0])
		res_p = res[1:(n_var + 1)]
		...
		self.fit_z = res[(len(res)-len(ind_y)):len(res)]
