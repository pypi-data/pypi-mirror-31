...
class CodeBlock(abctools.AbjadValueObject):
    ...
    def as_latex(
        ...
        if output_proxies:
            ...
            for i, output_proxy in enumerate(output_proxies):
                ...
                string = '\n'.join(lines)
                if isinstance(output_proxy, abjadext.book.ImageOutputProxy):
                    if i < len(output_proxies) - 1:
                        next_proxy = output_proxies[i + 1]
                        if isinstance(next_proxy, abjadext.book.ImageOutputProxy):
                            string += r'\\'
    ...
    def filter_output_proxies(self):
        ...
        if code_block_specifier.hide:
            output_proxies = [
                ...
                if not isinstance(_, abjadext.book.CodeOutputProxy)
                ]
    ...
    def from_docutils_abjad_import_block(block):
        ...
        code_block_specifier, cleaned_options = \
            abjadext.book.CodeBlockSpecifier.from_options(**cleaned_options)
        image_layout_specifier, cleaned_options = \
            abjadext.book.ImageLayoutSpecifier.from_options(**cleaned_options)
        image_render_specifier, cleaned_options = \
            abjadext.book.ImageRenderSpecifier.from_options(**cleaned_options)
        code_block = abjadext.book.CodeBlock(
    ...
    def interpret(self, console):
        ...
        if self.executed_lines:
            ...
            code_output_proxy = abjadext.book.CodeOutputProxy(
                input_file_contents,
                code_block_specifier=self.code_block_specifier,
                )
    ...
    def push_line_to_console(self, line, console, line_number):
        ...
        if console.errored:
            if allow_exceptions:
                ...
                raise abjadext.book.AbjadBookError(message)
    ...
    def graph(
        ...
        **keywords
        ...
        output_proxy = abjadext.book.GraphvizOutputProxy(
            graphviz_graph,
            layout=layout,
    ...
    def show(self, argument, return_timing=False, **keywords):
        ...
        output_proxy = abjadext.book.LilyPondOutputProxy(
            illustration,
            image_layout_specifier=self.image_layout_specifier,
            image_render_specifier=image_render_specifier,
            strict=strict,
