...
class LaTeXDocumentHandler:
    ...
    def __call__(
        ...
        console = abjadext.book.AbjadBookConsole(
    ...
    def collect_asset_output_proxies(self, input_blocks):
        ...
        for code_block in input_blocks.values():
            for output_proxy in code_block.output_proxies:
                if isinstance(output_proxy, abjadext.book.CodeOutputProxy):
                    continue
        ...
        for i, line in enumerate(input_file_contents):
            if line.strip().startswith(('<abjad', '<lilypond>')):
                ...
                if line.strip().startswith('<abjad>'):
                    ...
                    code_block = abjadext.book.CodeBlock.from_latex_abjadextract_block(
                        line,
                        starting_line_number=i,
                ...
                current_block_lines = normalize(
                    current_block_lines)
                current_block_lines = current_block_lines.split('\n')
                code_block = abjadext.book.CodeBlock.from_latex_abjad_block(
                    current_block_lines,
                    starting_line_number=i,
                    **current_block_options
                    )
                ...
                current_block_lines = normalize(
                    current_block_lines)
                current_block_lines = current_block_lines.split('\n')
                code_block = abjadext.book.LilyPondBlock.from_latex_lilypond_block(
                    current_block_lines,
                    starting_line_number=i,
                    **current_block_options
                    )
    ...
    def rebuild_source(
        ...
        for source_line_range, block in reversed(blocks):
            ...
            if block is None:
                ...
                if (
                    start < len(input_file_contents) and
