...
class SphinxDocumentHandler(abctools.AbjadObject):
    ...
    def setup_sphinx_extension(app):
        ...
        app.add_directive('abjad', abjadext.book.AbjadDirective)
        app.add_directive('docs', abjadext.book.AbjadDoctestDirective)
        app.add_directive('import', abjadext.book.ImportDirective)
        app.add_directive('reveal', abjadext.book.RevealDirective)
        app.add_directive('shell', abjadext.book.ShellDirective)
        app.add_directive('thumbnail', abjadext.book.ThumbnailDirective)
        app.add_javascript('abjad.js')
        app.add_javascript('copybutton.js')
        app.add_javascript('ga.js')
        app.add_stylesheet('abjad.css')
        app.add_node(
            abjadext.book.abjad_import_block,
            html=[SphinxDocumentHandler.visit_abjad_import_block, None],
            latex=[SphinxDocumentHandler.visit_abjad_import_block, None],
            )
        app.add_node(
            abjadext.book.abjad_input_block,
            html=[SphinxDocumentHandler.visit_abjad_input_block, None],
            latex=[SphinxDocumentHandler.visit_abjad_input_block, None],
            )
        app.add_node(
            abjadext.book.abjad_output_block,
    ...
    def collect_abjad_input_blocks(self, document):
        ...
        for block in document.traverse(is_valid_node):
            if isinstance(block, abjadext.book.abjad_import_block):
                code_block = \
                    abjadext.book.CodeBlock.from_docutils_abjad_import_block(block)
                if block.get('reveal-label', None):
                    labels[block.get('reveal-label')] = code_block
                code_blocks[block] = code_block
            elif isinstance(block, abjadext.book.abjad_input_block):
                code_block = \
                    abjadext.book.CodeBlock.from_docutils_abjad_input_block(block)
                if block.get('reveal-label', None):
                    labels[block.get('reveal-label')] = code_block
                code_blocks[block] = code_block
            else:
                ...
                if code_block is not None:
                    ...
                    if code_block_specifier is None:
                        code_block_specifier = abjadext.book.CodeBlockSpecifier()
        ...
        for block in document.traverse(is_valid_node):
            ...
            for line in lines:
                if self._topleveltools_pattern.search(line) is not None:
                    should_process = True
            code_block = \
                abjadext.book.CodeBlock.from_docutils_literal_block(block)
            code_blocks[block] = code_block
    ...
    def interpret_code_blocks(app, document):
        ...
        except abjadext.book.AbjadBookError as e:
            print()
            print(e.args[0])
        except Exception:
            print()
    ...
    def parse_rst(rst_string):
        ...
        directives.register_directive(
            'abjad', abjadext.book.AbjadDirective,
            )
        directives.register_directive(
            'import', abjadext.book.ImportDirective,
            )
        directives.register_directive(
            'reveal', abjadext.book.RevealDirective,
            )
        directives.register_directive('shell', abjadext.book.ShellDirective)
    ...
    def render_png_image(self, node):
        ...
        if image_layout_specifier is None:
            image_layout_specifier = abjadext.book.ImageLayoutSpecifier()
