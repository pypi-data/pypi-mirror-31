...
from .comet import Message, Streamer, config, file_uploader, get_cmd_args_dict, generate_guid, ENV_OPTIMIZATION_ID, \
    is_valid_guid, save_matplotlib_figure
...
from .utils import in_notebook_environment, is_list_like
from ._logging import setup, INTERNET_CONNECTION_ERROR, IPYTHON_NOTEBOOK_WARNING, METRIC_ARRAY_WARNING, EXPERIMENT_OPTIMIZER_API_KEY_MISMTACH_WARNING, PARSING_ERR_MSG
...
class Experiment(object):
    ...
    def _start(self):
        ...
        try:
            if self.log_code:
                self.set_code(Experiment._get_source_code())
        except Exception as e:
