...
import ftplib
import socket
from typing import Optional, Callable, TypeVar, Union, List  # pylint: disable=unused-import
...
class Client:
    ...
    def __wrap_reconnect(self, method: Callable[[ftplib.FTP], T]) -> T:
        ...
        for _ in range(0, self.max_reconnects):
            ...
            except (ConnectionRefusedError, socket.timeout, socket.gaierror, socket.herror, ftplib.error_temp) as err:
                self.connection.close()
                self.connection = None
                last_err = err
    ...
    def mlsd(self, path="", facts=None):
        ...
        return self.reconnecting(method=lambda conn: conn.mlsd(path, facts_lst))
