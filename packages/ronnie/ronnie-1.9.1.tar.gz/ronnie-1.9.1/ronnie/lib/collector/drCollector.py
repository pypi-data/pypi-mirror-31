...
@util.timer
def buildFromDF(df, cfg, withLabels=True, labelCol=0,yNeedOneHot=True, xNeedReshape=True, xStartCol=1,imgLen=-1, batchNum=1, batchSize=-1):
	try:
		...
		logger.info("CFG: CLASSES : {0}".format(cfg[meta.DATA_LABELY_CLASSs]))
		totalClasses = cfg[meta.DATA_LABELY_CLASSs]
		logger.info("CONVERTING : totalClasses :{0}".format(totalClasses))
		lbShape = labelYs.shape[0]
		logger.info("CONVERTING: lbShape:{0}".format(lbShape))
		
		oneHotLabels = np.zeros((lbShape, totalClasses))
		
		for i in range(len(oneHotLabels)):
			lbValue = labelYs[i][0]
			oneHotLabels[i,lbValue]=1
		...
		labelYs32 = None
		if yNeedOneHot:
			labelYs32 = np.array(oneHotLabels, dtype=np.int32)
		else:
			labelYs32 = np.array(labelYs, dtype=np.int32)
		
		for i in range(10):
			logger.info("HEAD 10: {0} : oneHotLabels:{1}, labels : {2}".format(i, labelYs32[i],labelYs[i][0]))
		
		for i in range(len(labelYs32)-10, len(labelYs32)):
			logger.info("Tail 10: {0} : oneHotLabels:{1}, labels : {2}".format(i, labelYs32[i],labelYs[i][0]))
