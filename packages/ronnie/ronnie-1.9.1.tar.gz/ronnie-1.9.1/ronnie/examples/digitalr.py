...
lk.initial(PROJPATH, MODELNAME, logLevel=logging.INFO)

logger = logging.getLogger("digitalr")

try:
	cfg.basicCfg[meta.PROJECT_PATH] = PROJPATH
except:
...
###################################################################################		
		...
		trainData 		= collector.buildFromDF(orgDF,cfg.dataCfg,batchNum=1, batchSize=30000)
...
#		collector.plotImgXsSamples(imgXs,labelYs,5, 5)
		...
		evalData 		= collector.buildFromDF(orgDF,cfg.dataCfg,batchNum=5, batchSize=10000)
...
###################################################################################		
		predictData 	=collector.buildFromDF(predictDF,cfg.dataCfg, withLabels=False,xStartCol=0)
