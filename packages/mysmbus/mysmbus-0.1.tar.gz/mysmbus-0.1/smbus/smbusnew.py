...
#coding=utf-8
# Compiled at: 2017-05-20 07:14:44
...
class SMBus(object):
    ...
    def write_byte_data(self, addr, cmd, val):
        ...
        """
        self._set_addr(addr)
        if SMBUS.i2c_smbus_write_byte_data(self._fd, ffi.cast('__u8', cmd), ffi.cast('__u8', val)) == -1:
            raise IOError(ffi.errno)
 
    @validate(servoid=int)
    def XiaoRGEEK_Read_ServoAngle(self, servo_id):
        """XiaoRGEEK_Read_ServoAngle(servo_id) -> result
        """
        self._set_addr(80)
    ...
    def XiaoRGEEK_ReadVol(self):
        """XiaoRGEEK_ReadVol() -> result
        """
        self._set_addr(80)
    ...
    def XiaoRGEEK_SpeedCounter1(self):
        """XiaoRGEEK_SpeedCounter1() -> result
        """
        self._set_addr(80)
    ...
    def XiaoRGEEK_SpeedCounter2(self):
        """XiaoRGEEK_SpeedCounter2() -> result
        """
        self._set_addr(80)
    ...
    def write_word_data(self, addr, cmd, val):
        ...
        """
        self._set_addr(addr)
        if SMBUS.i2c_smbus_write_word_data(self._fd, ffi.cast('__u8', cmd), ffi.cast('__u16', val)) == -1:
            raise IOError(ffi.errno)
    ...
    def read_block_data(self, addr, cmd):
        ...
        if SMBUS.i2c_smbus_access(self._fd, int2byte(SMBUS.I2C_SMBUS_READ), ffi.cast('__u8', cmd), SMBUS.I2C_SMBUS_BLOCK_DATA, data):
            raise IOError(ffi.errno)
    ...
    def write_block_data(self, addr, cmd, vals):
        ...
        if SMBUS.i2c_smbus_access(self._fd, int2byte(SMBUS.I2C_SMBUS_WRITE), ffi.cast('__u8', cmd), SMBUS.I2C_SMBUS_BLOCK_DATA, data):
            raise IOError(ffi.errno)
    ...
    def read_i2c_block_data(self, addr, cmd, len=32):
        ...
        if SMBUS.i2c_smbus_access(self._fd, int2byte(SMBUS.I2C_SMBUS_READ), ffi.cast('__u8', cmd), arg, data):
            raise IOError(ffi.errno)
        return smbus_data_to_list(data)
    ...
    def write_i2c_block_data(self, addr, cmd, vals):
        ...
        data = ffi.new('union i2c_smbus_data *')
        list_to_smbus_data(data, vals)
        if SMBUS.i2c_smbus_access(self._fd, int2byte(SMBUS.I2C_SMBUS_WRITE), ffi.cast('__u8', cmd), SMBUS.I2C_SMBUS_I2C_BLOCK_BROKEN, data):
            raise IOError(ffi.errno)
...
def list_to_smbus_data(data, vals):
    ...
    if len(vals) > block_max or len(vals) == 0:
        raise OverflowError('Third argument must be a list of at least one, but not more than %d integers' % block_max)
