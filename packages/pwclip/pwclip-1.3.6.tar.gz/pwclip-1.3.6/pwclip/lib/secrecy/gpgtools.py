...
gpgtool module
"""
from os import path, environ, remove, name as osname, uname
...
class GPGTool(object):
	...
	def __gendefs():
		...
		return {
            ...
            'name_email': '%s@%s'%(
                user,
                environ['COMPUTERNAME'] if osname == 'nt' else uname()[1]),
            'expire_date': 0,
            'key_type': 'RSA',
            'key_length': 4096,
            'subkey_type': 'RSA',
	...
	def encrypt(self, message, **kwargs):
		...
		return self._gpg_.encrypt(
            message, fingers, always_trust=True, throw_keyids=True, output=out)
