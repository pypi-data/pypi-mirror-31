...
class Test:
    ...
    def generate_test_interface(self, cls):
        ...
        for func in test.functions:
            ...
            if name == 'initialize' or name == 'execute':
                continue
            function.operations.append('logger->add_result(this->{0}(), " - [{1}] tested");'.format(name, name[5:]))
            function.operations.append('logger->add_result(true, "---------------------------------------------------------");'.format(name, name[5:]))
    ...
    def generate_all_tests_class(self,):
        ...
        for test in test_all.members:
            var_name = test.name
            function.operations.append('logger->add_result(true, "Test case [\" + this->{0}.get_type() + \"] started");'.format(var_name))
            function.operations.append('logger->add_result(this->{0}.execute(), "Test case [\" + this->{0}.get_type() + \"] finished\\n");'.format(var_name))
        function.operations.append('return this->logger->result;')
