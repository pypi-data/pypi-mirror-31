...
class WriterCpp(Writer):
    ...
    def write_function(self, function, flags):
        ...
        if flags & FLAG_CPP and not function.is_external and not function.is_abstract:
            if self._current_class.name == 'ITestTestModelUser':
                print 'ITestTestModelUser'
    ...
    def add_serialization(self, class_, serialization_type):
        ...
        for behavior in class_.behaviors:
            if not behavior.is_serialized:
                continue
            operation = self.get_behavior_call_format().format(behavior.name, function.name)
