    ...
    for line in lines_ns:
        ...
        if line.find('%')==-1 and line.find('MiB')!=-1:
            ...
            if pid in pid_list.keys():
                ...
                pid_list[pid]=list()
                pid_list[pid].append(gpu_id)
    #str_returned='GPU Info:\n'
    #for i in range(0,len(percent)):
    #    str_returned=str_returned+str(i)+': '+str(percent[i])+'% '+str(memory[i])+'MiB\n'
    #str_returned=str_returned+'Processes:\n'
    #for (pid,glist) in pid_list.items():
    #    str_returned=str_returned+pid+' '+str(glist)+' '+get_user(pid)+'\n'
