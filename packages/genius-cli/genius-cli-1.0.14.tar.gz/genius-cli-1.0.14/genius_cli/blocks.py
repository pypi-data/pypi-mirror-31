...
def render_tag(el, theme_name='default', replace=True):
    ...
    if m:
        ...
        if replace:
            el.replaceWith(BeautifulSoup(tag_template, "html.parser"))
...
def render_blocks(file_source):
    soup = BeautifulSoup(file_source, "html.parser")
    for block in soup({'genius:blocks'}):
        render_tag(block.find(), theme_name=block.attrs.get('theme', 'default'))
        block.unwrap()
    file_source = re.sub(r'^(\s*)', r'\1\1\1\1', soup.prettify(), flags=re.MULTILINE)
