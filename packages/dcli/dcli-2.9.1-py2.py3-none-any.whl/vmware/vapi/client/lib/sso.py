...
class SSOHTTPSConnection(http_client.HTTPSConnection):
    """
    An HTTPS class that verifies server's certificate on connect.
    """
    def __init__(self, *args, **kwargs):
        """
        Initializer.  See http_client.HTTPSConnection for other arguments
    ...
    def connect(self):
        ...
        http_client.HTTPSConnection.connect() for details.
        """
...
class SsoAuthenticator(object):
    ...
    def perform_request(self,
        ...
        webservice.putheader("SOAPAction", "http://docs.oasis-open.org/ws-sx/ws-trust/200512/RST/Issue")
    ...
    def get_bearer_saml_assertion(self,
        ...
        :type  request_duration: :class:`long`
        ...
        bearer_token = self.perform_request(soap_message,
                                            public_key,
                                            private_key)
        return etree.tostring(_extract_element(etree.fromstring(bearer_token),
                                               'Assertion',
    ...
    def _get_bearer_saml_assertion_win(self,
        ...
        while True:
            _, out_buf = sspiclient.authorize(in_buf)
    ...
    def get_hok_saml_assertion(self,
        ...
        return etree.tostring(
            _extract_element(etree.fromstring(hok_token),
                             'Assertion',
                             {'saml2': "urn:oasis:names:tc:SAML:2.0:assertion"}),
            pretty_print=False)
...
class SecurityTokenRequest(object):
    ...
    def __init__(self,
        ...
        self._username = cgi.escape(username) if username else username  # pylint: disable=W1505
        self._password = cgi.escape(password) if password else password  # pylint: disable=W1505
    ...
    def construct_hok_request(self, delegatable=False, act_as_token=None,
        ...
        """
        self._binary_security_token = base64.b64encode(_extract_certificate(self._public_key))
        ...
        self.sign_request()
        return etree.tostring(self._xml, encoding='utf-8', pretty_print=False)
    ...
    def sign_request(self):
        ...
        self._request_digest = _make_hash(request)  # pylint: disable=W0612
        self._timestamp_digest = _make_hash(timestamp)  # pylint: disable=W0612
        self._signed_info = _canonicalize(SIGNED_INFO_TEMPLATE % self.__dict__)
        self._signature_value = _sign(self._private_key, self._signed_info)
    ...
    def embed_signature(self):
        ...
        self._xml_text = etree.tostring(self._xml)
...
def add_saml_context(serialized_request, saml_token, private_key_file):
    ...
    :type       serialized_request: :class:`str`
    ...
    value_map['_request_digest'] = _make_hash(_canonicalize(etree.tostring(request_body)))
    ...
    value_map['_timestamp_digest'] = _make_hash(timestamp)

    security.append(etree.fromstring(timestamp))
    value_map['_signed_info'] = _canonicalize(SIGNED_INFO_TEMPLATE % value_map)
    value_map['_signature_value'] = _sign(private_key, value_map['_signed_info'])
        ...
        for key_type in ('PRIVATE KEY', 'RSA PRIVATE KEY'):
            try:
                return crypto.load_privatekey(crypto.FILETYPE_PEM,
                                              '-----BEGIN ' + key_type + '-----\n' +
                                              base64.encodestring(der_key) +  # pylint: disable=W1505
                                              '-----END ' + key_type + '-----\n',
                                              '')
            except crypto.Error:
                pass
