...
class AppOpticsApmDjangoMiddleware(AppOpticsApmMiddlewareBase):
    ...
    def process_request(self, request):
        try:
            ...
            appoptics_apm_logger.debug("AppOptics APM middleware error: %s" % self._singleline(e))
    ...
    def process_response(self, request, response):
        ...
        try:
            ...
            span_start = request.META.pop('APPOPTICS_APM_SPAN_START', None)
            if span_start:
...
def on_load_middleware():
    ...
    try:
        ...
        if isinstance(middleware, list):
            middleware.insert(0, 'appoptics_apm.djangoware.AppOpticsApmDjangoMiddleware')
        # settings.MIDDLEWARE_CLASSES is a tuple in 1.9 and prior versions.
        elif isinstance(middleware, tuple):
            if hasattr(settings, 'MIDDLEWARE'):
                settings.MIDDLEWARE = ('appoptics_apm.djangoware.AppOpticsApmDjangoMiddleware',) + settings.MIDDLEWARE
            else:
                settings.MIDDLEWARE_CLASSES = ('appoptics_apm.djangoware.AppOpticsApmDjangoMiddleware',) + settings.MIDDLEWARE_CLASSES
        else:
            appoptics_apm_logger.error(
                "AppOptics APM error: thought MIDDLEWARE_CLASSES would be either a tuple or a list, got {mw_type}"
