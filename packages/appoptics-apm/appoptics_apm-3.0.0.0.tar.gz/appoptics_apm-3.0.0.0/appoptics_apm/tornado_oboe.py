...
def request_handler_start(self):
    ...
    if ctx.is_sampled() and evt.is_valid():
        ...
        self.request._appoptics_apm_finish_ev = ctx.create_event('exit', 'tornado')
        # report the exit event ID in the response header
        self.set_header("X-Trace", self.request._appoptics_apm_finish_ev.id())
    else:
...
def request_handler_finish(self):
    ...
    ev = getattr(self.request, '_appoptics_apm_finish_ev', None)
    ctx = getattr(self.request, '_appoptics_apm_ctx', None)
    if ev and ctx and ctx.is_sampled():
        ...
        self.request._appoptics_apm_finish_ev = None
