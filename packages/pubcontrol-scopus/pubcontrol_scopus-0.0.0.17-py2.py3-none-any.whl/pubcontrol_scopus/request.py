...
class ScopusFetcher(DictQuery):
    ...
    def request_scopus_search(self, search_query, start, increment, initial_query, timeout_tuple):
        ...
        try:
            ...
            self.logger.error('[request] Could not retrieve citation search for {} exception: {}'.format(self.id, str(e)))
...
class ScopusPublicationFetcher(ScopusFetcher):
    ...
    def process(self):
        ...
        Changed 22.04.2018
        With publications that where not cited the response would contain somethin like
        'error: results empty' but because the key is error a ConnectionError would be risen in the Builder object
        and the publication would be declared invalid, even though it just was not cited. Added a if that checks if the
        publication has been cited by the citation search results and only passes to the builder in case there actually
        are dicts to be processed.
        ...
        self.logger.info('[request] publication {} valid'.format(self.id))

        if int(self.query('opensearch:totalResults', 0)) > 0:
    ...
    def request_citations_search(self, start=0, count=200):
        ...
        self.request_scopus_search(
            'REF({})'.format(self.publication_dict['eid']),
            start,
            count,
            'search-results',
            (15, 15)
