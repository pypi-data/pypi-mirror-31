...
from .exceptions import IncompleteUrlParametersError, ExtraUrlParametersError
...
class Call:
    ...
    def validate_url_parameters(self):
        required_parameters = set(re.findall(':\w+', self.url))
        given_parameters = set(self.url_parameters)

        missing_paramters = required_parameters - given_parameters
        if missing_paramters:
            raise IncompleteUrlParametersError(missing_paramters)

        extra_parameters = given_parameters - required_parameters
        if extra_parameters:
            raise ExtraUrlParametersError(extra_parameters)
