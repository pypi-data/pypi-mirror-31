...
from LoreleiLib.CombatHelpers import CalculateThreatGain, CalculateExperienceGain, CalculatePhysicalDamage, CalculateNextLevelExperience
...
class Equipment(object):
    ...
    def checkSlot(self, item, slot):
        ...
        if isinstance(item, Weapon):
            if item.weapon_type.value > WeaponType.TwoHanded.value:
                unequipping.append("main_hand")
                unequipping.append("off_hand")
            else:
                if (slot == 1):
                    unequipping.append("main_hand")
                elif item.weapon_type.value == WeaponType.Shield.value or slot == EquipmentSlot.Off_Hand.value: #Shield or attempting to dual wield
                    unequipping.append("off_hand")
    ...
    def getAllItems(self):
        ...
        if self.main_hand is not None:
            equipment.append(self.main_hand)
        if self.off_hand is not None:
            equipment.append(self.off_hand)
        if self.head is not None:
            equipment.append(self.head)
        if self.chest is not None:
            equipment.append(self.chest)
        if self.hands is not None:
            equipment.append(self.hands)
        if self.legs is not None:
            equipment.append(self.legs)
        if self.feet is not None:
            equipment.append(self.feet)
        if self.ring1 is not None:
            equipment.append(self.ring1)
        if self.ring2 is not None:
            equipment.append(self.ring2)
        if self.necklace is not None:
            equipment.append(self.necklace)
        if self.bracelets is not None:
            equipment.append(self.bracelets)
        return equipment
