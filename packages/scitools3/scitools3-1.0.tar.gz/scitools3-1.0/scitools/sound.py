...
import math, numpy, wave, subprocess, sys, os
...
def read(filename):
    ...
    sounddata = data.reshape((old_div(len(data),channels),channels))
...
def play(data, sample_rate=44100, player=None):
    ...
    if player:
        ...
        if sys.platform[:3] == 'win':
            ...
            status, output = subprocess.getstatusoutput('%s %s' %(player, tmpfile))
            msg += '\nError message:\n%s' %output
        if status != 0:
            raise OSError(msg)
        return
    ...
    if sys.platform[:5] == 'linux':
        ...
        for cmd in open_commands:
            status, output = subprocess.getstatusoutput('%s %s' %(cmd, tmpfile))
            if status == 0:
                break
        if status != 0:
        ...
        try:
            subprocess.getstatusoutput('afplay %s' %tmpfile)
        except:
            subprocess.getstatusoutput('open %s' % tmpfile)
    else:
        # assume windows
...
def play_square(T, length):
    sample_rate = 44100
    samplesperperiod = int(sample_rate*T)
    oneperiod = numpy.hstack([numpy.ones((1, old_div(samplesperperiod,2))), -numpy.ones((1, old_div(samplesperperiod,2)))])
    oneperiod = max_amplitude*numpy.asarray(oneperiod)
    data = numpy.tile(oneperiod,(1,int(old_div(length,T))))
    data = data.astype(numpy.int16)
    m, n = numpy.shape(data)
...
def play_square_fourier(T, length, N):
    ...
    for n in range(1,N+1,2):
        data = data + old_div(numpy.sin(2*numpy.pi*n*t/T),n)
...
def play_triangle(T, length):
    sample_rate = 44100
    samplesperperiod = int(sample_rate*T)
    oneperiod = numpy.hstack([numpy.linspace(-1, 1, old_div(samplesperperiod,2)), numpy.linspace(1, -1, old_div(samplesperperiod,2))])
    oneperiod = max_amplitude*numpy.asarray(oneperiod)
    data = numpy.tile(oneperiod, (1, int(old_div(length,T))))
    data = data.astype(numpy.int16)
    m, n = numpy.shape(data)
...
def play_triangle_fourier(T, length, N):
    ...
    data = numpy.zeros(t.size)
    for n in range(1,N+1,2):
        data = data - old_div(numpy.cos(2*numpy.pi*n*t/T),n**2)
...
def Ja_vi_elsker():
    ...
    notes2freq = {notes[i]: base_freq*2**(old_div(i,12.0)) for i in range(len(notes))}
...
if __name__ == '__main__':
    ...
    print("Playing")
    play(song, 44100)
    print("Playing with echo")
    play_with_echo(song,44100, 0.6, 0.5)
    print("Playing in reverse")
    play_reverse(song, 44100)
    print("Playing with noise")
    play_with_noise(song, 44100, 0.7)
    print("Playing square wave")
    play_square(old_div(1,440.), 3.)
    print("Playing square wave (9th fourier approximation)")
    play_square_fourier(old_div(1,440.), 3., 9)
    print("Playing triangle wave")
    play_triangle(old_div(1,440.), 3.)
    print("Playing triangle wave (9th fourier approximation)")
    play_triangle_fourier(old_div(1,440.), 3., 3)
