...
class MockPublisherPool(TransientIfPool):
    ...
    def __init__(self, topics=None):
        ...
        super(MockPublisherPool, self).__init__(topics)
    ...
    def TransientMaker(self, topic_name, topic_type, *args, **kwargs):  # the topic class implementation
        return MockPublisher(topic_name, topic_type, *args, **kwargs)

    def TransientCleaner(self, topic):  # the topic class implementation
        return topic.cleanup()

    def update(self):
        for t in topics_available_remote.keys():  # explicit keys in case we work through a dictproxy
