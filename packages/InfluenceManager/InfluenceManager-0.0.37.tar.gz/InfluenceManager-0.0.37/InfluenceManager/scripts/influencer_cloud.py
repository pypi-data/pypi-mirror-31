...
class InfluencerCloud:
    ...
    def aggregate_words(self, words_dict, score):
        ...
        if not check_query:
            ...
            self.word_dict = self.words_db.find({"_id": self.username})[0]["words"]
    ...
    def sent_color_func(self, word, font_size, position, orientation, random_state=None, **kwargs):
        ...
        self.sentiment_ratio = [pos_count, neg_count]
        return hsl
    ...
    def generate_pie_chart(self):
        ...
        sizes = self.sentiment_ratio
        colors = ["#859900", "#DC312E"]

        fig, ax = plt.subplots()
        ax.pie(sizes, labels=labels, colors=colors, textprops={"color": "white"})
        ax.axis("equal")
        plt.title("@{0} Consensus Sentiment Pie Chart".format(self.username))
