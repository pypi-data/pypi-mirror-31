...
class InfluencerCloud:
    ...
    def create_cloud(self):
        ...
        wc = WordCloud(width=800, height=400, background_color="#002b36", max_words=50, stopwords=stopwords, random_state=1)
        wc.generate(tweets)
        wc.recolor(color_func=grey_color_func)

        plt.figure(figsize=(9, 5))
        plt.title("{0} Word Cloud".format(name.title()))
        plt.imshow(wc, interpolation="bilinear")
        plt.tight_layout(pad=0)
        plt.axis("off")
        # plt.show()
        plt.savefig("{0}/{1}_wc.png".format(self.output_dir, self.topic),
