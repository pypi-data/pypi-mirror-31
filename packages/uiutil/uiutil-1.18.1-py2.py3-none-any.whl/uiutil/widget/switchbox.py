...
class SwitchBox(WidgetMixIn):
    ...
    def __init__(self,
        ...
        for switch_label in iter(switches.keys()):
            ...
            switch_link = Persist(persistent_store=self.switch_states,
                                  key=switch_label,
                                  initial_value=self.switch_states.get(switch_label, default_switch_state))
    ...
    def all_switched_on(self):
        return [switch for switch in self.switch_widgets if self.switched_on(switch)]

    @property
    def all_switched_off(self):
        return [switch for switch in self.switch_widgets if self.switched_off(switch)]
