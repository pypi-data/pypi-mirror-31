...
class OptimizableParameter(Parametrizable):
    ...
    def __init__(self, name, transformation, default_value=1.0,
        ...
        if restart_function is None:
            self.restart_function = lambda optimized_value: optimized_value + \
                np.array(
                     np.random.normal(
                    loc=0.0,
    ...
    def set_params_at_random(self, trans=False):
        ...
        if self.optimized_value is not None and np.random.uniform() < 0.9:
            ...
            while not (current_value is not None and
                ...
                current_value = self.restart_function(optimized_value)
        else:
