        ...
        for item in records:
            if naming_map['bn'] in item:                                                    # ref to a pack element, either this or a child pack.
                if item[naming_map['bn']] != self.name:
                    pack_el = next( (x for x in self._data if x.name == item[naming_map['bn']]), None )
                else:
                    pack_el = self
                if pack_el:
                    cur_pack_el = pack_el
                ...
                if rec_el:
                    rec_el.do_actuate(item, naming_map)
                elif new_pack:
                    self.do_actuate(item, naming_map, cur_pack_el)
                else:
                    cur_pack_el.do_actuate(item, naming_map)
            else:
                rec_el = next((x for x in self._data if x.name == item[naming_map['n']]), None)
                if rec_el:
                    rec_el.do_actuate(item, naming_map)
                elif new_pack:
                    self.do_actuate(item, naming_map, cur_pack_el)
