...
from SweetPy.setting import sweet_settings
import platform
...
def get_message_by_httpstatus_code(code):
    ...
    for _v in APIResponseHTTPCode:
        if _v.name == code.name:
            ...
            break
    message = ''
    _locale = None if sweet_settings is None else sweet_settings.get('sweetpy.locale',None)
...
def create_response(status_code,data=None,faild_message=None):
    code, message = get_message_by_httpstatus_code(status_code)
    _data = {}
    _data['code'] = code
    _data['message'] = message
