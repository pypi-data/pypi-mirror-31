...
class RatingWidget(TextWidget):
    ...
    # translations and key
    ratingLabel = interfaces.RATING_LABEL
    cancelRatingLabel = interfaces.CANCEL_RATING_LABEL
    cancelRatingKey = interfaces.CANCEL_RATING_KEY
    ...
    @property
    def cancel(self):
        # NOTE: the label is by default hidden and replaced with an icon
        if self.showCancel:
            label = zope.i18n.translate(self.cancelRatingLabel,
                context=self.request)
            return u'<div class="cancel"><a href="#%s">%s</a></div>' % (
                self.cancelRatingKey, label)
    ...
    def getStarLink(self, scoreKey):
        ...
        url = u'#%s' % scoreKey
        label = zope.i18n.translate(self.ratingLabel, context=self.request)
