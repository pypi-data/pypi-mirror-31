...
class HistoryApi(object):
    ...
    def _init_addr(self):
        ...
        if not self.addr:
            self.addr = addr
            channel = grpc.insecure_channel(self.addr, options=[(str('grpc.max_send_message_length'), MAX_MESSAGE_LENGTH), (str('grpc.max_receive_message_length'), MAX_MESSAGE_LENGTH)])
    ...
    def get_history_bars(self, symbols, frequency, start_time, end_time,
        ...
        resp = self.stub.GetHistoryBars(req, metadata=[
            (str('authorization'), context.token), (str('grpc-internal-encoding-request'), str('gzip')),
        ...
        datas = [protobuf_to_dict(bar, is_utc_time=True, including_default_value_fields=False) for bar in resp.data]
    ...
    def get_history_ticks(self, symbols, start_time, end_time, fields=None,
        ...
        datas = [protobuf_to_dict(tick, is_utc_time=True, including_default_value_fields=False) for tick in resp.data]
        datas = datas if not df else pd.DataFrame(datas)
        return datas
