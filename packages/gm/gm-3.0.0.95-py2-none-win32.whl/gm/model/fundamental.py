...
from gm.utils import str_lowerstrip, protobuf_timestamp2datetime, load_to_list
...
def get_sec_type_str(sec_types):
    ...
    for sec_type in sec_types:
        result.append(d.get(sec_type, sec_type))
...
class FundamentalApi(object):
    ...
    def _init_addr(self):
        ...
        if not self.addr:
            self.addr = new_addr
            channel = grpc.insecure_channel(self.addr, options=[('grpc.max_send_message_length', MAX_MESSAGE_LENGTH),
    ...
    def get_fundamentals(self, table, symbols, start_date, end_date, fields=None, filter=None, order_by=None, limit=1000):
        ...
        resp = self.stub.GetFundamentals(req, metadata=[(str('authorization'), context.token)])
    ...
    def get_instruments(self, symbols=None, exchanges=None, sec_types=None, names=None, skip_suspended=True,
        ...
        resp = self.stub.GetInstruments(req, metadata=[(str('authorization'), context.token)])
        result = []
        ...
        for ins in resp.data:  # type: Instrument
            ...
            if 'created_at' in row:
                row['trade_date'] = utils.date2datetime(row['created_at']).date()
                row.pop('created_at')
            utils.protomessage2dict(ins.info, row, *info_copy_field)
            if 'listed_date' in row:
                row['listed_date'] = utils.date2datetime(row['listed_date']).date()
            if 'delisted_date' in row:
                row['delisted_date'] = utils.date2datetime(row['delisted_date']).date()
            result.append(row)
        return result
    ...
    def get_history_instruments(self, symbols, fields=None, start_date=None, end_date=None):
        ...
        resp = self.stub.GetHistoryInstruments(req, metadata=[(str('authorization'), context.token)])
        result = [protobuf_to_dict(res_order, including_default_value_fields=True) for res_order in resp.data]
        for info in result:
            if info.get('created_at'):
                info['trade_date'] = info.get('created_at').date()
                info.pop('created_at')
        return result
    ...
    def get_instrumentinfos(self, symbols=None, exchanges=None, sec_types=None, names=None, fields=None):
        ...
        req = GetInstrumentInfosReq(symbols=','.join(symbols), exchanges=','.join(exchanges),
                                    ...
                                    fields=','.join(filter_fields))
        resp = self.stub.GetInstrumentInfos(req, metadata=[(str('authorization'), context.token)])
        result = []
    ...
    def get_history_constituents(self, index, start_date=None, end_date=None):
        ...
        resp = self.stub.GetConstituents(req, metadata=[(str('authorization'), context.token)])
    ...
    def get_industry(self, code):
        ...
        resp = self.stub.GetIndustry(req, metadata=[(str('authorization'), context.token)])
    ...
    def get_concept(self, code):
        ...
        resp = self.stub.GetConcept(req, metadata=[(str('authorization'), context.token)])
    ...
    def get_trading_dates(self, exchange, start_date, end_date):
        ...
        resp = self.stub.GetTradingDates(req, metadata=[(str('authorization'), context.token)])
        if len(resp.dates) == 0:
            return []
        ds = []
    ...
    def get_previous_trading_date(self, exchange, date):
        ...
        resp = self.stub.GetPreviousTradingDate(req, metadata=[(str('authorization'), context.token)])
    ...
    def get_next_trading_date(self, exchange, date):
        ...
        resp = self.stub.GetNextTradingDate(req, metadata=[(str('authorization'), context.token)])
        rdate = resp.date  # type: Timestamp
        if not rdate.ListFields():  # 这个说明查询结果没有
            return None
        return rdate.ToDatetime().strftime('%Y-%m-%d')
    ...
    def get_dividend(self, symbol, start_date, end_date=None):
        ...
        resp = self.stub.GetDividends(req, metadata=[(str('authorization'), context.token)])
        result = []
        fields = ['symbol', 'cash_div', 'share_div_ratio', 'share_trans_ratio', 'allotment_ratio', 'allotment_price', 'created_at']
    ...
    def get_continuous_contracts(self, csymbol, start_date=None, end_date=None):
        ...
        resp = self.stub.GetContinuousContracts(req, metadata=[(str('authorization'), context.token)])
    ...
    def get_fundamentals_n(self, table, symbols, end_date, fields=None, filter=None, order_by=None, count=1):
        ...
        resp = self.stub.GetFundamentalsN(req, metadata=[(str('authorization'), context.token)])
        result = []
