...
class PdfParserData(object):
	...
	def find_tables_floats_locs(self, parsed_pdf):
		...
		for parsed_page in parsed_pdf["pages"]:
			...
			for parsed_text in parsed_page["texts"]:
				...
				# if( (self.config["TABLES_LOCS_FLOAT_MIN_TOP"] < parsed_text["top"]) and (parsed_text["bottom"] < self.config["TABLES_LOCS_FLOAT_MAX_BOTTOM"]) ):
	...
	def parse_to_model(self, filepath_pdf_in, filepath_json_out, filepath_html_out):
		...
		# parsed_pdf["pages"] = [parsed_pdf["pages"][-1]]	
						...
						if( (table_loc["left"] <= parsed_text["left"]) and (parsed_text["right"] <= table_loc["right"]) and  (table_loc["top"] <= parsed_text["top"]) and (parsed_text["bottom"] <= table_loc["bottom"]) ):
							...
							if( all(abs(parsed_text["top"] - row_top) > threshold_top_same for row_top in table_loc["rows_top"]) or (len(table_loc["rows_top"]) == 0) ):		
								...
								for col_name in table_loc["cols_name"]:
									...
									# print("\n\n\n col_name, length")
									# print(PdfParser.content_to_unicode(col_name), len(model["tables"][page_left_top][col_name]))
							...
							# print("\n\n")
							# for col in model["tables"][page_left_top]:
								# print(PdfParser.content_to_unicode(col))
								# print(PdfParser.content_to_unicode(model["tables"][page_left_top][col]))	
							# print("\n\n")
							...
							for j, col_left in enumerate(table_loc["cols_left"]):
								...
								if(parsed_text["right"] < col_left):
									...
									# print(page_left_top, col_name, last_row_index, parsed_text)
									# print("\n")
									# print(table_loc)
									# print("\n")
									...
									if(model["tables"][page_left_top][col_name][last_row_index][0] == parsed_text_empty):
										...
										# print("\n\n\n\n")
										# print("not empty")
										# print(model["tables"][page_left_top][col_name][last_row_index][0])
...
def main(argv):						 
	...
	i = i + 1
	if(len(argv) > i):
		filepath_pdf_in = argv[i]
	
	i = i + 1		
	if(len(argv) > i):
		filepath_config_in = argv[i]
		
	i = i + 1
	if(len(argv) > i):
		filepath_html_out = argv[i]
		
	i = i + 1
	if(len(argv) > i):
		filepath_json_out = argv[i]
		
	i = i + 1
	if(len(argv) > i):
		filepath_model_json_out = argv[i]
	...
	# pdf_parser_data.print_model(model)
