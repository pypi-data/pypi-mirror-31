import numpy as np
from macrofi.event_selection import fibacktest
from macrofi.event_selection import performance
...
def sp_bool_optimize(event_name, config, data, base, direction=POSITIVE, shift=2, **args):
    ...
    for sample in sample_list:
        ...
        if direction == NEGATIVE:
            pos = event_list.apply(lambda x: 0 if x > 0 else 1)
        else:  # POSTIVE
            pos = event_list.apply(lambda x: 1 if x > 0 else 0)
        # 计算回测夏普比率
        base = base[(base.index >= event_list.index[0]) & (base.index <= event_list.index[-1])]  # 基准时间和信号重叠
        # 将仓位信号转为日度
        pos = pd.Series(pos, index=base.index).ffill()
        # 和当前最大夏普比率比较，如果更好，则替换
        bt = Backtest(pos, base)
        pct = bt.pct
        base_pct = bt.base_pct
        sp = sharpe(pct)
        base_sp = sharpe(base_pct)
        excess_sp = sp - base_sp
        if excess_sp > excess_sp_max:
            excess_sp_max = excess_sp
            best_fit = sample
