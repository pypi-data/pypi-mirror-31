...
public:
  ...
  * </pre>
  *
  * where ds1, ..., dsn are the ``displayStatements`` with which the function
  * is called.
  * <p>
  ...
  */
  Optional<int> getIntOption(fmt::CStringRef name) const {
    bool exists;
    int result = static_cast<int>(internal::AMPL_Impl_getDblOption(ampl_, name.c_str(), &exists,
                                                                   internal::ErrorInfo()));
  ...
  */
  ...
  /**
   ...
   ### throws	runtime_error	In case the file does not exist.
   */
  ...
  /**
   Writes on the current OutputHandler the outcome of the %AMPL statement
    ...
    where {@code e1...en} are the strings passed to the procedure.
  ...
  where {@code e1...en} are the objects passed to the procedure.
