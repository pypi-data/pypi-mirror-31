...
def prism(step, NSIDE, H, mesh='cube', ORIENTATION=1):
    """
...
def hexagon(step, NSIDE, H, rotate=0, mesh='cube', ORIENTATION=1):
    """
...
def double_hexagon(step, NSIDE1, NSIDE2, DX, DY, H, rotate=0, mesh='cube', ORIENTATION=1):
    """
    Generate regular hexagon on X/Y plane
...
def diabolo(step, nside_upper, nside_lower, length, width, H, dir_prisms='in', 
    ...
    def conditionPrism(xi,yi,zi):
        ...
        if dir_prisms.lower() == 'in':
            in_up_Prism = ((abs(xi) <= (Habs+(yi - L/2 + 0.5))/np.sqrt(3)) 
                           and ( abs(yi - L/2) <= Habs) 
                           and (0 <= zi < H - 1/np.sqrt(2)))
            
            in_low_Prism = ((abs(-xi) <= (Habs+(-yi - L/2 + 0.5))/np.sqrt(3)) 
                           and ( abs(-yi - L/2) <= Habs) 
                           and (0 <= zi < H - 1/np.sqrt(2)))
        else:
            in_up_Prism = ((abs(xi) <= (Habs-(yi - L/2 + 0.5))/np.sqrt(3)) 
...
def split_ring(step, R, W, H, G=-1, alphaG=0, mesh='cube', ORIENTATION=1):
    """
...
def rect_split_ring(step, L1,L2,H,W, G=False, mesh='cube', ORIENTATION=1):
    ...
    """
    def conditionSR(xi,yi,zi):
        A1 = (0<=xi<L1 and 0<=yi<W and 0<=zi<H)
        A2 = (0<=xi<W and 0<=yi<L2 and 0<=zi<H)
        A3 = (L1-W<=xi<L1 and 0<=yi<L2 and 0<=zi<H)
        
        if G != False:
            A4 = ((0<=xi<(L1-G)/2 or (L1+G)/2<=xi<L1) and L2-W<=yi<L2 and 0<=zi<H) 
        else:
            A4 = False
    ...
    if mesh == 'cube':
        sp = _meshCubic( (-1, int(L1)+1), (-1, int(L2+W)+1), (-1, int(H)+1), conditionSR)
    elif mesh == 'hex':
        sp = _meshHexagonalCompact( (-1, 2*int(L1)+1), (-1, 2*int(W)+1), (-1, 2*int(H)+1), 
                                    conditionSR, ORIENTATION)
...
def _meshCubic(XRANGE, YRANGE, ZRANGE, condition):
    ...
    """
    sp=[]
    for xi in range(XRANGE[0], XRANGE[1]):
        for yi in range(YRANGE[0], YRANGE[1]):
...
def image_to_struct(img_name, nm_per_pixel, stepsize, H, threshold=100, 
                    useDarkPixel=True, returnImage=False):
    ...
    struct = np.nonzero(data)
    struct = np.concatenate([[struct[0] * stepsize], 
                             [struct[1] * stepsize], 
                             [np.zeros(len(struct[1]))]]).T 
    struct = center_struct(struct)
    
    
    ## --- build up structure height
    _stru = copy.deepcopy(struct)
    for h in range(1, H):
        _stru.T[2]= h * stepsize
        struct = np.concatenate([struct, _stru])
...
def struct_to_image(STRUCT, RGB=True, minimum_size=-1, projection='XY'):
    ...
    """
    from tools import get_step_from_geometry, get_geometry_2d_projection
    
    step = get_step_from_geometry(STRUCT)
    _X,_Y,_Z = get_geometry_2d_projection(STRUCT, projection).T
...
def rotate_XY(STRUCT, ALPHA):
    ...
    """
    A = ALPHA * np.pi / 180.
    xNew = np.cos(A)*STRUCT.T[0] - np.sin(A)*STRUCT.T[1]
    yNew = np.sin(A)*STRUCT.T[0] + np.cos(A)*STRUCT.T[1]
    STRUCT.T[0:2] = xNew, yNew
