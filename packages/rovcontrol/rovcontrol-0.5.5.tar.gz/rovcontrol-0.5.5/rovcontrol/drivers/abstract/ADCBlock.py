import rovcontrol.drivers.pwm.ADCTemp as ADCTemp
...
class ADCBlock:
    ...
    def set_mv(self, logical_chan, mv):
        self.settings[logical_chan]['controller'].set_mv(
            self.settings[logical_chan]['real_bus'], settings[logical_chan]['real_chan'], mv)
    def get_mv(self, logical_chan):
        return self.settings[logical_chan]['controller'].get_mv(
            self.settings[logical_chan]['real_bus'], settings[logical_chan]['real_chan'])
