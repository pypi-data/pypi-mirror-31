...
# -----------------------------------------------------------------------------
def keep_leaf(graph, leaf, exclude_items):
    try:
        return graph(leaf, "Asset") not in exclude_items
...
class ResidualPortfolio(UfoBase):
    ...
    is left of such portfolio once all leaves whose asset is present in a
    reference category object (RefCategory) have been removed.
    ...
    def Leaves(self, graph):
        ...
        return Structure({leaf: qty for leaf, qty in leaves.items()
                          if keep_leaf(graph, leaf, exclude_items)})
