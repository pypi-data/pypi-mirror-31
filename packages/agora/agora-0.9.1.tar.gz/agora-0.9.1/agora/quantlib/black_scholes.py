...
def opt_BAW(opt_type, spot, strike, vol, term, rd, rf):
    ...
    Inputs:
        ...
        term     - time to expiration (units must be consistent with what is
                   used for vol, rd, rf)
        rd       - domestic risk-free interest rate (cannot be negative)
        rf       - foreign risk-free interest rate (or dividend yield)
    Returns:
        A float.
    """
    if rd < 0.0:
        raise ValueError("Barone-Adesi Whaley approximation "
                         "doesn't work for negative interest rates")
    ...
    if opt_type == "C":
        if rd > 0.0:
            ...
            g = 0.5*(b + m_sqrt(b*b + 8.0 / (term*volsq)))
...
def opt_JZ(opt_type, spot, strike, vol, term, rd, rf):
    ...
    """
    if rd <= 0.0:
        raise ValueError("Ju-Zhong approximation doesn't "
                         "work for zero or negative interest rates")
