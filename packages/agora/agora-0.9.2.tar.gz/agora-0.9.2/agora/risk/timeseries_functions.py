...
from onyx.core import (RDate, Curve, DateOffset,
                       CurveIntersect, CurveUnion, ObjNotFound, GetVal)
...
def get_curve_usd(name, start, end):
    ...
    Returns:
        A Curve.
    """
    # --- we invoke GetVal with start and end set to None and crop the returned
    #     curve so that curve caching remains effective when start and end
    #     dates are changed (at the cost of a larger memory footprint).
...
def prices_for_risk(name, start, end, strict=True):
    ...
    if strict and len(prcs) and prcs.back.date != end:
        raise RuntimeError("Missing prices at the end of the curve "
