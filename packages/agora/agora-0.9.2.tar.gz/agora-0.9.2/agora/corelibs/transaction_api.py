...
from onyx.core import (unique_id, Date, ObjDbTransaction,
                       AddObj, GetObj, UpdateObj, PurgeObj,
                       ObjDbQuery, ObjNotFound, CreateInMemory,
                       GetVal, SetVal)
...
def amend_transaction(head, info):
    ...
    for attr, value in info.items():
        ...
        if not same:
            break
    if same:
        raise TransactionWarning("found nothing to amend "
    ...
    if transaction.TimeCreated >= Date.today():
        ...
        with ObjDbTransaction("Delete", "SERIALIZABLE"):
            ...
            for pos in GetVal(dummy, "PositionEffects"):
                ObjDbQuery(QUERY_INSERT_POSEFF,
                           parms=(pos.TradeDate, dummy.TimeCreated,
                                  pos.Book, pos.Qty, pos.Unit, pos.UnitType,
                                  pos.Name, pos.Event, pos.Trade))
