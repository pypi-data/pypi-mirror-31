...
def download_volumes(volume_ids, output_dir, username=None, password=None,
    ...
    if not epr:
        port = htrc.config.get_dataapi_epr()
...
def download(args):
    ...
    return download_volumes(volumeIDs, args.output, 
        ...
        token=args.token, concat=args.concat, host=args.datahost, port=args.dataport, cert=args.datacert, key=args.datakey, epr=args.epr)
