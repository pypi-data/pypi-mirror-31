...
def get_volumes(token, volume_ids, host, port, cert, key, concat=False):
    """
...
def download_volumes(volume_ids, output_dir, username=None, password=None,
                     config_path=None, token=None, concat=False, host=None, port=None, cert=None, key=None):
    ...
    if any((token, host, port)) is not None:
        ...
        try:
            data = get_volumes(token, volume_ids, host, port, cert, key, concat)
...
def download(args):
    ...
    return download_volumes(volumeIDs, args.output, 
        ...
        token=args.token, concat=args.concat, host=args.datahost, port=args.dataport, cert=args.datacert, key=args.datakey)
