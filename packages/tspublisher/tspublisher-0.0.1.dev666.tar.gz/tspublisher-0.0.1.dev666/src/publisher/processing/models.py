                ...
                for step in objective["steps"]:
                    ...
                    if "testFirstImage" in step:
                        image_asset = Asset("image", "TEST_" + step["stepId"],
                                            "test_image%04d" % int(step["testFirstImage"]))
                    else:
                        image_asset = Asset("image", "TEST_" + step["stepId"], "image%04d" % int(step["firstImage"]))

                    assets = [image_asset.dump()]

                    video_name = ""
                    if step["firstImage"] and step["lastImage"]:
                        video_name = "video-image%04d-image%04d" % (int(step["firstImage"]), int(step["lastImage"]))
                    elif "testFirstImage" in step and "testLastImage" in step:
                        video_name = "video-test_image%04d-test_image%04d" % (int(step["testFirstImage"]),
