...
from publisher.exceptions import SingleProceduresNotFoundError, MissingYamlFileError, ValidationError, \
    CombinedValidationError, MissingAssetDirectoryError, InvalidYamlFileError
...
class AssetDirectoryValidator(Validator):
    ...
    def validate(self):
        if not os.path.exists(self.asset_directory):
            raise MissingAssetDirectoryError(self.asset_directory)
...
def validate_save():
    ...
    phase_directories = map(lambda d: os.path.join(procedure_directory, d),
                            get_content_directories(procedure_directory))

    validators += map(PhaseYamlFileValidator, phase_directories)
    validators += map(AssetDirectoryValidator, phase_directories)
