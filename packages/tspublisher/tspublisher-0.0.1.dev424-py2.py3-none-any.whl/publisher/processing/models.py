    ...
    def dump(self):
        return {
            "tap": CommentedMap([
                ...
                ("duration", self.tapData["duration"]),
            ])
        }
...
class LabelWidget:
    def __init__(self, step_id, label_num, label_data):
        self.stepId = step_id
        ...
        if not self.labelData["moreInfoId"] == "":
            info_data, source_dir = get_more_info_data(self.labelData["moreInfoId"])
...
class InfoPointWidget:
    def __init__(self, step_id, info_point_num, info_point_data):
        self.stepId = step_id
        ...
        if not self.infoPointData["moreInfoId"] == "":
            info_data, source_dir = get_more_info_data(self.infoPointData["moreInfoId"])
    ...
    def format_learn_data(self):
        for i, objective in enumerate(self.learnObjectives):
            ...
            for step in objective["steps"]:
                ...
                if "hold" in step:
                    hold_widget = TapWidget(step["stepId"], step["hold"])
                ...
                if "pip" in step:
                    if step["pip"]["sequence"]:
                        pip_image = "%s%04d" % (step["pip"]["view"], int(step["firstImage"]))
                        pip_video = "video-%s%04d-%s%04d" % (step["pip"]["view"], int(step["firstImage"]),
                ...
                if "labels" in step:
                    for label in step["labels"]:
                        label_widget = LabelWidget(step["stepId"], label, step["labels"][label])
                ...
                if "moreInfoId" in step:
                    info_data, source_dir = get_more_info_data(step["moreInfoId"])
                ...
                image_asset = Asset("image", step["stepId"], "image%04d" % int(step["firstImage"]))
                assets = [image_asset.dump()]
                if step["firstImage"] and step["lastImage"] and not step["firstImage"] == step["lastImage"]:
                    name = "video-image%04d-image%04d" % (int(step["firstImage"]), int(step["lastImage"]))
        ...
        for i, objective in enumerate(self.testObjectives):
            ...
            if objective["steps"]:
                for step in objective["steps"]:
                    ...
                    elif "xyPoints" in step and step["testDragger"]:
                        if step["testText"]:
                            text_widget = TextWidget("TEST_" + step["stepId"], step["testText"])
                            widgets.append(text_widget.dump())
                        dragger_widget = DraggerWidget("TEST_" + step["stepId"], step["xyPoints"])
                        widgets.append(dragger_widget.dump())
                    elif step["type"] == "Auto":
                    ...
                    # Format image and video assets
                    image_asset = Asset("image", "TEST_" + step["stepId"], "image%04d" % int(step["testImage"]))
                    assets = [image_asset.dump()]
                    if step["firstImage"] and step["lastImage"]:
                        name = "video-image%04d-image%04d" % (int(step["firstImage"]), int(step["lastImage"]))
                        video_asset = Asset("video", "TEST_" + step["stepId"], name)
                        assets.append(video_asset.dump())
