...
def get_step_data(phase_code):
    ...
    for row in rows:
        ...
        # add dragger data to dictionary if appropriate
        if row["type"] == "Dragger":
            learn_data[-1]["steps"][-1]["dragger"] = {
                "start": [row["p1x"], row["p1y"]],
                "target": [row["p2x"], row["p2y"]],
            }
        ...
        if "gallery_sim2dData" in row and row["gallery_sim2dData"] and not row["gallery_sim2dData"] == "{}":
            ...
            for i, gallery_item in enumerate(gallery_data):
                ...
                main_file = os.path.splitext(os.path.basename(gallery_data[str(i)]["mainGraphic"]))
                thumb_file = os.path.splitext(os.path.basename(gallery_data[str(i)]["thumbnail"]))
                ...
                if main_file[1] in video_ext:
                    ...
                    raise TypeError("Unrecognised file type: %s" % (main_file[0] + main_file[1]))
        ...
        if "flipbook_sim2dData" in row and row["flipbook_sim2dData"] and not row["flipbook_sim2dData"] == "{}":
            ...
            video_file = os.path.splitext(os.path.basename(flip_book_data["videoName"]))
            image_file = os.path.splitext(os.path.basename(flip_book_data["imageName"]))
        ...
        if not row["testable"].lower() == "false":
            mcq_exists = False
            test_image = row["firstImageId"]
            if "testImage" in row and row["testImage"]:
                test_image = row["testImage"]
            ...
            if row["testText"]:
                ...
                if row["choiceCorrect"]:
                    ...
                    test_data[-1]["steps"][-1]["testDragger"] = False

            if row["type"] == "Dragger" and row["testSkipDrag"].upper() != "TRUE":
                if mcq_exists:
                ...
                test_data[-1]["steps"][-1]["testDragger"] = True
                test_data[-1]["steps"][-1]["xyPoints"] = {
                    "start": [row["p1x"], row["p1y"]],
                    "target": [row["p2x"], row["p2y"]],
                }
            else:
...
def get_csv_info(phase_code):
    csv_file = get_csv_steps_file(phase_code)

    if csv_file:
        headers, rows = read_csv_file(csv_file, delimiter='|')
    else:
