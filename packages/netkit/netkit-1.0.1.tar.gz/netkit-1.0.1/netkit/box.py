...
class Box(object):
    ...
    def __init__(self, init_data=None):
        self.header_fmt = self.header_fmt_type + ''.join(value[0] for value in list(self.header_attrs.values()))
        self.header_len = struct.calcsize(self.header_fmt)

        self.unpack_done = False

        for k, v in list(self.header_attrs.items()):
            setattr(self, k, v[1])
        self.body = b''
    ...
    def set_json(self, value):
        ...
        if not value:
            self.body = b''
            return
        # python3 json dumps 结果为str类型，所以必须转换为bytes
        self.body = str_to_bytes(json.dumps(value))
