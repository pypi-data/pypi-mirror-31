...
class AbstractMicroservice:
    ...
    def __init__(self, config, config_class_root_node_name, mqtt_client=None):
        self._config = config[config_class_root_node_name]
        self._logger = mylogger.create_logger(config["logger"], __name__)
        self._logger.info("{}.__init__ - initializing".format(self.__name__))
        self._logger.debug("{}.__init__ - config: {}".format(self.__name__, self._config))
        ...
        self._mqtt_client = mqtt_client or MQTTClient(config["mqtt"], self._logger)
        self._logger.info("{}.__init__ - AbstractMicroservice done".format(self.__name__))

    def _start(self):
        self._logger.warning("{}._start - NotImplementedError".format(self.__name__))
        raise NotImplementedError

    def start(self):
        print("{} - starting".format(self.__name__))
        self._logger.info("{} - starting".format(self.__name__))
        self._is_stopped.clear()
        self._mqtt_client.connect()
        self._mqtt_client.is_connected.wait()
        self._start()
        self._stop_service.clear()
        self._logger.info("{} - started".format(self.__name__))

    def _stop(self):
        self._logger.warning("{}._stop - NotImplementedError".format(self.__name__))
        raise NotImplementedError

    def stop(self):
        self._logger.info("{} - stopping".format(self.__name__))
        self._stop_service.set()
        self._stop()
        self._is_stopped.set()
        print("{} - stopped".format(self.__class__.__name__))
        self._logger.info("{} - stopped".format(self.__name__))
    ...
    def standalone(cls, args=None):
        """Public method to start this driver directly. Instantiates an MQTT client and creates an object for the
                given driver."""
        config, verbose = cls._args_to_config(args)
        config = mypyyaml.dict_deepcopy_lowercase(config)
        instance = cls(config)
        instance.start()
        try:
            while not instance._is_stopped.wait(0.1):  # timeout is necessary for CTRL+C
                pass
        except KeyboardInterrupt:
            instance._logger.info("KeyboardInterrupt")
            pass
        instance.stop()
