...
class Mqtt2Mqtt:
    ...
    def _on_mqtt_connect(self, client, userdata, flags, rc):
        ...
        if rc == paho.mqtt.client.CONNACK_ACCEPTED:
            for point in self._points:
                logging.info('subscribe %s', point['bc_topic'])
                client.subscribe(point['bc_topic'])
    ...
    def _on_mqtt_message(self, client, userdata, message):
        ...
        for point in self._points:
            if topic_matches_sub(point['bc_topic'], message.topic):
                if not msg:
                ...
                logging.debug('%s write %s', point['user_topic'], record)
                
                self._mqtt.publish(point['user_topic'], json.dumps(record), qos=0)
