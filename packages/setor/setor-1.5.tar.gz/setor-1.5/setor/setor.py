...
def main():
    ...
    host_input = raw_input(
        "{}[+]{} Host TOR (Enter to set default: 127.0.0.1): ".format(GREEN, END_COLOR)
    )
    port_input = raw_input(
        "{}[+]{} Port TOR (Enter to set default: 9050): ".format(GREEN, END_COLOR)
    )
    ...
    if choice.lower() == 'a':
        ...
        log_choice = raw_input(
            "{}[+]{} Are you need to save as log? [y/N]: ".format(GREEN, END_COLOR)
        )
        ...
        print "{}[!] Wrong input!{}".format(RED, END_COLOR)
    logging.shutdown()
