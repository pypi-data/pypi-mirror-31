...
class Users(Model, _HashUtil):
    """ ======================================================================
        ...
        - <:prop:last_ip> (:class:ipaddress.ip_address) the latest IP
            address to access the account
        - <:prop:joined_on> (:class:arrow.Arrow) the date which the account
            was created on as a timestamp
        - <:prop:last_activity> (:class:arrow.Arrow) the last time a user
    ...
    ORDINAL = ['uid', 'username', 'email', 'password', 'key',
               'oauth_providers', 'join_ip', 'last_ip', 'joined_on',
               'last_activity', 'activated']
    PRIVATE = ['password', 'key', 'join_ip', 'last_ip']
    uid = UID()
    ...
    last_ip = IP(not_null=True)
    joined_on = Timestamp(default=Timestamp.now(), not_null=True)
    last_activity = Timestamp(default=Timestamp.now(), not_null=True)
    ...
    def fill_defaults(self, request=None, response=None):
        ...
        if request is not None:
            ...
            self.last_ip.from_request(request)
    ...
    def after_login(self, request=None, response=None):
        ...
        last_activity = self.last_activity.eq(self.last_activity.now())
        self.last_ip.from_request(request)
        self.update(self.last_ip, last_activity, self.password)
...
class LoginAttempts(Model):
    ORDINAL = 'ip', 'tally', 'updated_on'
    ip = IP(primary=True, not_null=True)
    tally = SmallInt(default=0, not_null=True)
    updated_on = Timestamp(default=Timestamp.now(), index=True, not_null=True)
    ...
    def incr(self, count=1, ip=None):
        ...
        try:
            result = self.one().update(self.tally.incr(count),
                                       self.updated_on.eq(Timestamp.now()))
            assert result
            return result
        except:
...
class IPJail(Model):
    ORDINAL = 'ip', 'jailed_on',
    ip = IP(primary=True, not_null=True)
    jailed_on = Timestamp(default=Timestamp.now(), index=True,
                          not_null=True)
