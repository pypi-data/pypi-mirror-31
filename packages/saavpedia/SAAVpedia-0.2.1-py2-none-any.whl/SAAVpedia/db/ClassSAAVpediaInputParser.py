...
class SAAVpediaInputParser(object):
    ...
    def __stringToQuery(self, theString):
        if self.__isSAAVpediaID(theString):
            return '({0} LIKE \"{1}\")'.format(self.__itsColOfSAAVpediaID, theString.upper())
            ...
            return '({0} LIKE \"{1}\")'.format(self.__itsColOfUniprot, theString[len("NX_"):])
        elif self.__isChr(theString):
            return '({0} LIKE \"{1}\")'.format(self.__itsColOfChr, theString.upper().split('CHR:')[1])
        elif self.__isNTPos(theString):
            return '({0} LIKE \"{1}\")'.format(self.__itsColOfNTPos, theString.upper().split('NTPOS:')[1])
        elif self.__isAAPos(theString):
            return '({0} LIKE \"{1}\")'.format(self.__itsColOfAAPos, theString.upper().split('AAPOS:')[1])
    ...
    def setupToSNVRetrieval(self):
        ...
        theString = ""
        for ithDict in theQueryDictList:
            theString = theString + self.__queryDictToString(ithDict,
                                                             ['rs', 'cosmic', 'ensg', 'enst', 'ensp', 'ntpos', 'aapos',
                                                              'chr', 'uniprot', 'nextprot'])
            pass
        self.set(theString)
        pass
