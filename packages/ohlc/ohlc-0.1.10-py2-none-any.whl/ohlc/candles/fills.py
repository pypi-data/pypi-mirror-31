...
def _fill_params(ohlc, scale, offset):
...
class Filler(object):
    def __init__(self, **_3to2kwargs):
        if 'pab' in _3to2kwargs: pab = _3to2kwargs['pab']; del _3to2kwargs['pab']
        else: pab = False
        if 'heikin' in _3to2kwargs: heikin = _3to2kwargs['heikin']; del _3to2kwargs['heikin']
        else: heikin = False
        if 'fill_mode' in _3to2kwargs: fill_mode = _3to2kwargs['fill_mode']; del _3to2kwargs['fill_mode']
        else: fill_mode = THIN
        if 'shapes' in _3to2kwargs: shapes = _3to2kwargs['shapes']; del _3to2kwargs['shapes']
        else: shapes = SevenShapes
    ...
    def fill(self, ohlc, height, scale=1.0, offset=0.0):
        if height <= 0: return []
    ...
    def colorize(self, chars, params):
        if self.barcolor:
    ...
    def _simple_fill(self, params, height):
    ...
    def _thin_fill(self, params, height):
        o,h,l,c,top,bot,oc,lh,_ = params
    ...
    def _complex_fill(self, params, height):
        o,h,l,c,top,bot,oc,lh,_ = params
