...
class LineStore(object):
    ...
    def __init__(self, **_3to2kwargs):
        if 'color_mode' in _3to2kwargs: color_mode = _3to2kwargs['color_mode']; del _3to2kwargs['color_mode']
        else: color_mode = modes.SHELL
...
class CandleCanvas(LineStore):
    ...
    visible_ohlc = None
    cache_size = 10000
    def __init__(self, **fill_args):
        if 'color_mode' in fill_args: color_mode = fill_args['color_mode']; del fill_args['color_mode']
        else: color_mode = modes.SHELL
        if 'w' in fill_args: w = fill_args['w']; del fill_args['w']
        else: w = 0
        if 'h' in fill_args: h = fill_args['h']; del fill_args['h']
        else: h = 0
        self.height = h
        self.width = w
        self.candles = []
...
class TextBox(LineStore):
    def __init__(self, text, **ls_args):
        if 'h' in ls_args: h = ls_args['h']; del ls_args['h']
        else: h = 0
        if 'w' in ls_args: w = ls_args['w']; del ls_args['w']
        else: w = 0
        self.text = text
        self.height = h
        self.width = w
...
class CandleChart(LineStore):
    ...
    def __init__(self, **ls_args):
        if 'pab' in ls_args: pab = ls_args['pab']; del ls_args['pab']
        else: pab = False
        if 'heikin' in ls_args: heikin = ls_args['heikin']; del ls_args['heikin']
        else: heikin = False
        if 'fill_mode' in ls_args: fill_mode = ls_args['fill_mode']; del ls_args['fill_mode']
        else: fill_mode = fills.THIN
        if 'color_mode' in ls_args: color_mode = ls_args['color_mode']; del ls_args['color_mode']
        else: color_mode = modes.SHELL
        if 'border' in ls_args: border = ls_args['border']; del ls_args['border']
        else: border = BOX
        if 'w' in ls_args: w = ls_args['w']; del ls_args['w']
        else: w = None
        if 'h' in ls_args: h = ls_args['h']; del ls_args['h']
    ...
    def resize(self, **_3to2kwargs):
        if 'w' in _3to2kwargs: w = _3to2kwargs['w']; del _3to2kwargs['w']
        else: w = None
        if 'h' in _3to2kwargs: h = _3to2kwargs['h']; del _3to2kwargs['h']
        else: h = None
