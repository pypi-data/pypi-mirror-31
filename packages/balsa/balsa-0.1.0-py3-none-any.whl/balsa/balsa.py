...
class Balsa(object):
    ...
    def init_logger_from_args(self, args):
        ...
        """
        if hasattr(args, self.log_dir_arg_string) and args.logdir is not None:
            self.log_directory = args.logdir
        if hasattr(args, self.verbose_arg_string) and args.verbose is True:
            self.verbose = True
        if hasattr(args, self.delete_existing_arg_string) and args.dellog is True:
