...
class AbstractTableWriter(TableWriterInterface):
    ...
    def from_ndarray(self, ndarray):
        """
        Set tabular attributes to the writer from :py:class:`numpy.ndarray`.
        Following attributes are set by the method:

        - :py:attr:`~.value_matrix`.
        - :py:attr:`~.type_hint_list`.

        :param numpy.ndarray ndarray: Input array.

        :Example:
            :ref:`example-from-pandas-dataframe`
        """

        self.value_matrix = ndarray
        import numpy as np
        for column in ndarray.T:
            print(np.array(*column).dtype, column, type(column))
        self.type_hint_list = [
            self.__get_typehint_from_dtype(column.dtype) for column in ndarray.T
        ]
