...
from rllab2.envs.base import Env, Step
from rllab2.envs.box2d.box2d_viewer import Box2DViewer

from rllab2.envs.box2d.parser.xml_box2d import world_from_xml, find_body, \
    find_joint
...
class Box2DEnv(Env):
    ...
    @autoargs.arg(u"frame_skip", type=int,
                  help=u"Number of frames to skip")
    @autoargs.arg(u'position_only', type=bool,
                  help=u'Whether to only provide (generalized) position as the '
                       u'observation (i.e. no velocities etc.)')
    @autoargs.arg(u'obs_noise', type=float,
                  help=u'Noise added to the observations (note: this makes the '
                       u'problem non-Markovian!)')
    @autoargs.arg(u'action_noise', type=float,
                  help=u'Noise added to the controls, which will be '
                       u'proportional to the action bounds')
    def __init__(
    ...
    def model_path(self, file_name):
        return osp.abspath(osp.join(osp.dirname(__file__),
                                    u'models/%s' % file_name))

    def _set_state(self, state):
        splitted = np.array(state).reshape((-1, 6))
        for body, body_state in izip(self.world.bodies, splitted):
    ...
    def forward_dynamics(self, action):
        if len(action) != self.action_dim:
            raise ValueError(u'incorrect action dimension: expected %d but got '
                             u'%d' % (self.action_dim, len(action)))
        lb, ub = self.action_bounds
        action = np.clip(action, lb, ub)
        for ctrl, act in izip(self.extra_data.controls, action):
            if ctrl.typ == u"force":
    ...
    def _get_position_ids(self):
        if self._position_ids is None:
            ...
            for idx, state in enumerate(self.extra_data.states):
                if state.typ in [u"xpos", u"ypos", u"apos", u"dist", u"angle"]:
    ...
    def get_raw_obs(self):
        ...
        for state in self.extra_data.states:
            ...
            if state.body:
                ...
                if state.typ == u"xpos":
                    new_obs = position[0]
                elif state.typ == u"ypos":
                    new_obs = position[1]
                elif state.typ == u"xvel":
                    new_obs = linearVel[0]
                elif state.typ == u"yvel":
                    new_obs = linearVel[1]
                elif state.typ == u"apos":
                    new_obs = body.angle
                elif state.typ == u"avel":
                    new_obs = body.angularVelocity
                elif state.typ == u"dist":
                    new_obs = np.linalg.norm(position - to.position)
                elif state.typ == u"angle":
            ...
            if state.transform is not None:
                if state.transform == u"id":
                    pass
                elif state.transform == u"sin":
                    new_obs = np.sin(new_obs)
                elif state.transform == u"cos":
