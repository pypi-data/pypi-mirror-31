...
class XmlChild(AttrDecl):
    ...
    def from_xml(self, xml):
        xml_elems = [child for child in xml if child.tag == self._tag]
        if len(xml_elems) > 1:
            raise ValueError(u'Multiple candidate found for tag %s' % self._tag)
        if len(xml_elems) == 0:
            if self._required:
                raise ValueError(u'Missing xml element with tag %s' % self._tag)
            else:
                return None
        elem = xml_elems[0]
...
class XmlAttr(AttrDecl):
    ...
    def from_xml(self, xml):
        if self._name in xml.attrib:
            ...
            raise ValueError(u"Missing required attribute %s" % self._name)
        else:
            return None
...
class XmlElem(object):
    ...
    def from_xml(cls, xml):
        if cls.tag != xml.tag:
            raise ValueError(
                u"Tag mismatch: expected %s but got %s" % (cls.tag, xml.tag))
        attrs = cls.get_attrs()
        inst = cls()
        used_attrs = []
        ...
        for attr in list(xml.attrib.keys()):
            if attr not in used_attrs:
                raise ValueError(u"Unrecognized attribute: %s" % attr)
        return inst

    @classmethod
    def get_attrs(cls):
        if not hasattr(cls, u'_attrs'):
    ...
    def __str__(self):
        ...
        for name, _ in self.__class__.get_attrs():
            attrs.append(u"%s=%s" % (name, unicode(getattr(self, name))))
        return self.__class__.__name__ + u"(" + u", ".join(attrs) + u")"
