...
import numpy as np
import pygame
from rllab2.envs.box2d.box2d_env import Box2DEnv
from rllab2.envs.box2d.parser import find_body

from rllab2.core.serializable import Serializable
from rllab2.envs.box2d.parser.xml_box2d import _get_name
...
class CarParkingEnv(Box2DEnv, Serializable):
    ...
    @autoargs.arg(u"random_start", type=bool,
                  help=u"Randomized starting position by uniforming sampling starting car angle"
                       u"and position from a circle of radius 5")
    @autoargs.arg(u"random_start_range", type=float,
                  help=u"Defaulted to 1. which means possible angles are 1. * 2*pi")
    def __init__(self, *args, **kwargs):
        Serializable.__init__(self, *args, **kwargs)
        self.random_start = kwargs.pop(u"random_start", True)
        self.random_start_range = kwargs.pop(u"random_start_range", 1.)
        super(CarParkingEnv, self).__init__(
            self.model_path(u"car_parking.xml"),
            *args, **kwargs
        )
        self.goal = find_body(self.world, u"goal")
        self.car = find_body(self.world, u"car")
        self.wheels = [
            body for body in self.world.bodies if u"wheel" in _get_name(body)]
        self.front_wheels = [
            body for body in self.wheels if u"front" in _get_name(body)]
        self.max_deg = 30.
        self.goal_radius = 1.
        self.vel_thres = 1e-1
        self.start_radius = 5.
