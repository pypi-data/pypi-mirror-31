...
class OcclusionEnv(ProxyEnv, Serializable):
    u''' Occludes part of the observation.'''
    ...
    def _set_sensor_mask(self, env, sensor_idx):
        ...
        if len(sensor_idx) > obsdim:
            raise ValueError(u"Length of sensor mask ({0}) cannot be greater than observation dim ({1})".format(len(sensor_idx), obsdim))
        if len(sensor_idx) == obsdim and not np.any(np.array(sensor_idx) > 1):
            ...
            raise ValueError(u"Double entries or boolean mask with dim ({0}) < observation dim ({1})".format(len(sensor_idx), obsdim))
        else:
