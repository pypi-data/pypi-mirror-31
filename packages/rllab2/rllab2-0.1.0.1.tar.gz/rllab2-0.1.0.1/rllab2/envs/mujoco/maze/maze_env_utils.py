...
def construct_maze(maze_id=0, length=1):
    ...
    if maze_id == 0:
        if length != 1:
            raise NotImplementedError(u"Maze_id 0 only has length 1!")
        structure = [
            [1, 1, 1, 1, 1],
            [1, u'r', 0, 0, 1],
            [1, 1, 1, 0, 1],
        ...
        structure = [
            [1] * (2 * length + 5),
            [1, u'g'] + [0] * length + [u'r'] + [0] * length + [u'g', 1],
            [1] * (2 * length + 5),
            ]
    ...
    if structure:
        ...
        raise NotImplementedError(u"The provided MazeId is not recognized")


def plot_ray(self, reading, ray_idx, color=u'r'):
    ...
    fig, ax = plt.subplots()
    im = ax.pcolor(-np.array(structure_plot), cmap=u'gray', edgecolor=u'black', linestyle=u':', lw=1)
    x_labels = range(len(structure[0]))
    y_labels = range(len(structure))
    ...
    robot_xy = np.array(self.wrapped_env.get_body_com(u"torso")[:2])  # the coordinates of this are wrt the init!!
    ...
    ax.set_title(u'sensors debug')
    print u'plotting now, close the window'
    # plt.show(fig)
    # plt.close()


def plot_state(self, name=u'sensors', state=None):
    if state:
    ...
    for ray_idx in xrange(self._n_bins):
        ...
        end_xy = (robot_xy + length_wall * np.array([math.cos(ray_ori), math.sin(ray_ori)]))
        end_xy_plot = (o_ij + end_xy / size_scaling) * 2
        plt.plot([robot_xy_plot[0], end_xy_plot[0]], [robot_xy_plot[1], end_xy_plot[1]], u'r')
        ...
        end_xy_plot = (o_ij + end_xy / size_scaling) * 2
        plt.plot([robot_xy_plot[0], end_xy_plot[0]], [robot_xy_plot[1], end_xy_plot[1]], u'g')

    log_dir = logger.get_snapshot_dir()
    ax.set_title(u'sensors: ' + name)

    plt.savefig(osp.join(log_dir, name + u'_sesors.png'))  # this saves the current figure, here f
    plt.close()
