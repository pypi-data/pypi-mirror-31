...
from rllab2.sampler import parallel_sampler
from rllab2.plotter import plotter
...
class SimpleReplayPool(object):
    def __init__(
        ...
        self._terminals = np.zeros(max_pool_size, dtype=u'uint8')
        self._bottom = 0
        self._top = 0
        self._size = 0
    ...
    def random_batch(self, batch_size):
        ...
        indices = np.zeros(batch_size, dtype=u'uint64')
        transition_indices = np.zeros(batch_size, dtype=u'uint64')
        count = 0
...
class DDPG(RLAlgorithm):
    ...
    def __init__(
            ...
            max_path_length=250,
            qf_weight_decay=0.,
            qf_update_method=u'adam',
            qf_learning_rate=1e-3,
            policy_weight_decay=0,
            policy_update_method=u'adam',
    ...
    def train(self):
        ...
        for epoch in xrange(self.n_epochs):
            logger.push_prefix(u'epoch #%d | ' % epoch)
            logger.log(u"Training started")
            ...
            logger.log(u"Training finished")
            if pool.size >= self.min_pool_size:
            ...
            if self.plot:
                ...
                if self.pause_for_plot:
                    raw_input(u"Plotting evaluation run: Press Enter to "
                              u"continue...")
        self.env.terminate()
    ...
    def do_training(self, itr, batch):
        ...
        obs, actions, rewards, next_obs, terminals = ext.extract(
            batch,
            u"observations", u"actions", u"rewards", u"next_observations",
            u"terminals"
        )

        # compute the on-policy y values
        target_qf = self.opt_info[u"target_qf"]
        target_policy = self.opt_info[u"target_policy"]
        ...
        f_train_qf = self.opt_info[u"f_train_qf"]
        f_train_policy = self.opt_info[u"f_train_policy"]
    ...
    def evaluate(self, epoch, pool):
        logger.log(u"Collecting samples for evaluation")
        ...
        average_discounted_return = np.mean(
            [special.discount_return(path[u"rewards"], self.discount) for path in paths]
        )

        returns = [sum(path[u"rewards"]) for path in paths]
        ...
        average_action = np.mean(np.square(np.concatenate(
            [path[u"actions"] for path in paths]
        )))
        ...
        logger.record_tabular(u'Epoch', epoch)
        ...
        if len(self.es_path_returns) > 0:
            logger.record_tabular(u'AverageEsReturn',
                                  np.mean(self.es_path_returns))
            logger.record_tabular(u'StdEsReturn',
                                  np.std(self.es_path_returns))
            logger.record_tabular(u'MaxEsReturn',
                                  np.max(self.es_path_returns))
            logger.record_tabular(u'MinEsReturn',
                                  np.min(self.es_path_returns))
        logger.record_tabular(u'AverageDiscountedReturn',
                              average_discounted_return)
        logger.record_tabular(u'AverageQLoss', average_q_loss)
        logger.record_tabular(u'AveragePolicySurr', average_policy_surr)
        logger.record_tabular(u'AverageQ', np.mean(all_qs))
        logger.record_tabular(u'AverageAbsQ', np.mean(np.abs(all_qs)))
        logger.record_tabular(u'AverageY', np.mean(all_ys))
        logger.record_tabular(u'AverageAbsY', np.mean(np.abs(all_ys)))
        logger.record_tabular(u'AverageAbsQYDiff',
                              np.mean(np.abs(all_qs - all_ys)))
        logger.record_tabular(u'AverageAction', average_action)

        logger.record_tabular(u'PolicyRegParamNorm',
                              policy_reg_param_norm)
        logger.record_tabular(u'QFunRegParamNorm',
                              qfun_reg_param_norm)
