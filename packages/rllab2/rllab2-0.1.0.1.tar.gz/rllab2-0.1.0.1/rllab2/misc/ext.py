...
import random
from rllab2.misc.console import colorize, Message
...
def extract_dict(x, *keys):
    return dict((k, x[k]) for k in keys if k in x)
...
def cached_function(inputs, outputs):
    import theano
    with Message(u"Hashing theano fn"):
        if hasattr(outputs, u'__len__'):
            hash_content = tuple(imap(theano.pp, outputs))
        else:
            hash_content = theano.pp(outputs)
    cache_key = hex(hash(hash_content) & (2 ** 64 - 1))[:-1]
    cache_dir = Path(u'~/.hierctrl_cache')
    cache_dir = cache_dir.expanduser()
    cache_dir.mkdir_p()
    cache_file = cache_dir / (u'%s.pkl' % cache_key)
    if cache_file.exists():
        with Message(u"unpickling"):
            with open(cache_file, u"rb") as f:
                try:
                    return pickle.load(f)
                except Exception:
                    pass
    with Message(u"compiling"):
        fun = compile_function(inputs, outputs)
    with Message(u"picking"):
        with open(cache_file, u"wb") as f:
...
def compile_function(inputs=None, outputs=None, updates=None, givens=None, log_name=None, **kwargs):
    import theano
    if log_name:
        msg = Message(u"Compiling function %s" % log_name)
        msg.__enter__()
    ret = theano.function(
        inputs=inputs,
        outputs=outputs,
...
def is_iterable(obj):
    return isinstance(obj, unicode) or getattr(obj, u'__iter__', False)
...
def flatten_hessian(cost, wrt, consider_constant=None,
                    disconnected_inputs=u'raise', block_diagonal=True):
    u"""
    ...
    assert isinstance(cost, Variable), \
        u"tensor.hessian expects a Variable as `cost`"
    assert cost.ndim == 0, \
        u"tensor.hessian expects a 0 dimensional variable as `cost`"
    ...
    for input in wrt:
        assert isinstance(input, Variable), \
            u"tensor.hessian expects a (list of) Variable as `wrt`"
        # assert input.ndim == 1, \
        ...
        hess, updates = theano.scan(lambda i, y, x: grad(
            ...
            disconnected_inputs=u'ignore').flatten(),
                                    sequences=arange(expr.shape[0]),
                                    non_sequences=[expr, input])
        assert not updates, \
            (u"Scan has returned a list of updates. This should not "
             u"happen! Report this to theano-users (also include the "
             u"script that generated the error)")
...
def flatten_tensor_variables(ts):
    import theano.tensor as TT
    return TT.concatenate(list(imap(TT.flatten, ts)))
...
def print_lasagne_layer(layer, prefix=u""):
    params = u""
    if layer.name:
        params += u", name=" + layer.name
    if getattr(layer, u'nonlinearity', None):
        params += u", nonlinearity=" + layer.nonlinearity.__name__
    params = params[2:]
    print prefix + layer.__class__.__name__ + u"[" + params + u"]"
    if hasattr(layer, u'input_layers') and layer.input_layers is not None:
        [print_lasagne_layer(x, prefix + u"  ") for x in layer.input_layers]
    elif hasattr(layer, u'input_layer') and layer.input_layer is not None:
        print_lasagne_layer(layer.input_layer, prefix + u"  ")
...
def unflatten_tensor_variables(flatarr, shapes, symb_arrs):
    ...
    arrs = []
    n = 0
    for (shape, symb_arr) in izip(shapes, symb_arrs):
...
def sliced_fun(f, n_slices):
    def sliced_f(sliced_inputs, non_sliced_inputs=None):
        ...
        ret_vals = None
        for start in xrange(0, n_paths, slice_size):
            ...
            if ret_vals is None:
                ...
                ret_vals = [x + y for x, y in izip(ret_vals, scaled_ret_vals)]
