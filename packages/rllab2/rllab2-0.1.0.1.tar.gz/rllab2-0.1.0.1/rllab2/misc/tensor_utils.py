...
def unflatten_tensors(flattened, tensor_shapes):
    tensor_sizes = list(imap(np.prod, tensor_shapes))
    indices = np.cumsum(tensor_sizes)[:-1]
    return [np.reshape(pair[0], pair[1]) for pair in izip(np.split(flattened, indices), tensor_shapes)]


def pad_tensor(x, max_len, mode=u'zero'):
...
def pad_tensor_dict(tensor_dict, max_len, mode=u'zero'):
...
def high_res_normalize(probs):
    return [x / sum(imap(float, probs)) for x in list(imap(float, probs))]
...
def split_tensor_dict_list(tensor_dict):
    ...
    for k in keys:
        ...
        if ret is None:
            ...
            for v, cur_dict in izip(vals, ret):
                cur_dict[k] = v
    return ret
