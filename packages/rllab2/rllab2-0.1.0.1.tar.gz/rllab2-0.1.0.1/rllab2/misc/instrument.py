...
from rllab2 import config
from rllab2.misc.console import mkdir_p
...
from rllab2.viskit.core import flatten
import collections
...
class StubBase(object):
    def __getitem__(self, item):
        return StubMethodCall(self, u"__getitem__", args=[item], kwargs=dict())
    ...
    def __pow__(self, power, modulo=None):
        return StubMethodCall(self, u"__pow__", [power, modulo], dict())

    def __call__(self, *args, **kwargs):
        return StubMethodCall(self.obj, self.attr_name, args, kwargs)

    def __add__(self, other):
        return StubMethodCall(self, u"__add__", [other], dict())

    def __rmul__(self, other):
        return StubMethodCall(self, u"__rmul__", [other], dict())

    def __div__(self, other):
        return StubMethodCall(self, u"__div__", [other], dict())

    def __rdiv__(self, other):
        return StubMethodCall(BinaryOp(), u"rdiv", [self, other], dict())  # self, "__rdiv__", [other], dict())

    def __rpow__(self, power, modulo=None):
        return StubMethodCall(self, u"__rpow__", [power, modulo], dict())
...
class StubAttr(StubBase):
    def __init__(self, obj, attr_name):
        self.__dict__[u"_obj"] = obj
        self.__dict__[u"_attr_name"] = attr_name

    @property
    def obj(self):
        return self.__dict__[u"_obj"]

    @property
    def attr_name(self):
        return self.__dict__[u"_attr_name"]

    def __str__(self):
        return u"StubAttr(%s, %s)" % (unicode(self.obj), unicode(self.attr_name))
...
class StubMethodCall(StubBase, Serializable):
    ...
    def __str__(self):
        return u"StubMethodCall(%s, %s, %s, %s)" % (
            unicode(self.obj), unicode(self.method_name), unicode(self.args), unicode(self.kwargs))
...
class StubClass(StubBase):
    ...
    def __call__(self, *args, **kwargs):
        if len(args) > 0:
            ...
            kwargs = dict(list(izip(spec.args[1:], args)), **kwargs)
            args = tuple()
    ...
    def __setstate__(self, dict):
        self.proxy_class = dict[u"proxy_class"]
    ...
    def __str__(self):
        return u"StubClass(%s)" % self.proxy_class
...
class StubObject(StubBase):
    ...
    def __setstate__(self, dict):
        self.args = dict[u"args"]
        self.kwargs = dict[u"kwargs"]
        self.proxy_class = dict[u"proxy_class"]
    ...
    def __getattr__(self, item):
        ...
        if hasattr(self.proxy_class, item):
            return StubAttr(self, item)
        raise AttributeError(u'Cannot get attribute %s from %s' % (item, self.proxy_class))

    def __str__(self):
        return u"StubObject(%s, *%s, **%s)" % (unicode(self.proxy_class), unicode(self.args), unicode(self.kwargs))
...
class VariantDict(AttrDict):
    ...
    def dump(self):
        return dict((k, v) for k, v in self.items() if k not in self._hidden_keys)
...
class VariantGenerator(object):
    ...
    def _populate_variants(self):
        ...
        methods = [x[1].__get__(self, self.__class__)
                   for x in methods if getattr(x[1], u'__is_variant', False)]
        for m in methods:
            self.add(m.__name__, m, **getattr(m, u"__variant_config", dict()))

    def variants(self, randomized=False):
        ret = list(self.ivariants())
        if randomized:
            np.random.shuffle(ret)
        return list(imap(self.variant_dict, ret))
    ...
    def to_name_suffix(self, variant):
        suffix = []
        for k, vs, cfg in self._variants:
            if not cfg.get(u"hide", False):
                suffix.append(k + u"_" + unicode(variant[k]))
        return u"_".join(suffix)

    def ivariants(self):
        dependencies = list()
        for key, vals, _ in self._variants:
            if hasattr(vals, u"__call__"):
                args = inspect.getargspec(vals).args
                if hasattr(vals, u'im_self') or hasattr(vals, u"__self__"):
        ...
        while len(sorted_keys) < len(self._variants):
            ...
            if len(free_nodes) == 0:
                error_msg = u"Invalid parameter dependency: \n"
                for k, v in dependencies:
                    if len(v) > 0:
                        error_msg += k + u" depends on " + u" & ".join(v) + u"\n"
    ...
    def _ivariants_sorted(self, sorted_keys):
        if len(sorted_keys) == 0:
            ...
            if hasattr(last_vals, u"__call__"):
                last_val_keys = inspect.getargspec(last_vals).args
                if hasattr(last_vals, u'im_self') or hasattr(last_vals, u'__self__'):
            ...
            for variant in first_variants:
                if hasattr(last_vals, u"__call__"):
                    last_variants = last_vals(
                        **dict((k, variant[k]) for k in last_val_keys))
...
timestamp = now.strftime(u'%Y_%m_%d_%H_%M_%S')
...
def run_experiment_lite(
        ...
        batch_tasks=None,
        exp_prefix=u"experiment",
        exp_name=None,
        log_dir=None,
        script=u"scripts/run_experiment_lite.py",
        python_command=u"python",
        mode=u"local",
        dry=False,
        docker_image=None,
        aws_config=None,
    ...
    """
    assert stub_method_call is not None or batch_tasks is not None, u"Must provide at least either stub_method_call or batch_tasks"
    ...
    if use_cloudpickle is None:
        for maybe_stub in (batch_tasks or [stub_method_call]):
            ...
            if isinstance(maybe_stub, StubBase):
                ...
                assert hasattr(maybe_stub, u'__call__')
    ...
    for task in batch_tasks:
        call = task.pop(u"stub_method_call")
        if use_cloudpickle:
            import cloudpickle
            data = base64.b64encode(cloudpickle.dumps(call)).decode(u"utf-8")
        else:
            data = base64.b64encode(pickle.dumps(call)).decode(u"utf-8")
        task[u"args_data"] = data
        exp_count += 1
        params = dict(kwargs)
        if task.get(u"exp_name", None) is None:
            task[u"exp_name"] = u"%s_%s_%04d" % (
                exp_prefix, timestamp, exp_count)
        if task.get(u"log_dir", None) is None:
            task[u"log_dir"] = config.LOG_DIR + u"/local/" + \
                              exp_prefix.replace(u"_", u"-") + u"/" + task[u"exp_name"]
        if task.get(u"variant", None) is not None:
            variant = task.pop(u"variant")
            if u"exp_name" not in variant:
                variant[u"exp_name"] = task[u"exp_name"]
            task[u"variant_data"] = base64.b64encode(pickle.dumps(variant)).decode(u"utf-8")
        elif u"variant" in task:
            del task[u"variant"]
        task[u"remote_log_dir"] = osp.join(
            config.AWS_S3_PATH, exp_prefix.replace(u"_", u"-"), task[u"exp_name"])
        task[u"env"] = task.get(u"env", dict()) or dict()
        task[u"env"][u"rllab2_USE_GPU"] = unicode(use_gpu)

    if mode not in [u"local", u"local_docker"] and not remote_confirmed and not dry and confirm_remote:
        remote_confirmed = query_yes_no(
            u"Running in (non-dry) mode %s. Confirm?" % mode)
        if not remote_confirmed:
            sys.exit(1)

    if hasattr(mode, u"__call__"):
        ...
        for task in batch_tasks:
            del task[u"remote_log_dir"]
            env = task.pop(u"env", None)
        ...
        for task in batch_tasks:
            ...
            task[u"resources"] = params.pop(
                u"resources", config.KUBE_DEFAULT_RESOURCES)
            task[u"node_selector"] = params.pop(
                u"node_selector", config.KUBE_DEFAULT_NODE_SELECTOR)
            task[u"exp_prefix"] = exp_prefix
            ...
            if dry:
                print pod_str
            dir = u"{pod_dir}/{exp_prefix}".format(
                pod_dir=config.POD_DIR, exp_prefix=exp_prefix)
            ensure_dir(dir)
            fname = u"{dir}/{exp_name}.json".format(
                dir=dir,
                exp_name=task[u"exp_name"]
            )
            with open(fname, u"w") as fh:
                fh.write(pod_str)
            kubecmd = u"kubectl create -f %s" % fname
            print kubecmd
            if dry:
                return
            retry_count = 0
...
_find_unsafe = re.compile(ur'[a-zA-Z0-9_^@%+=:,./-]').search
...
def _shellquote(s):
    u"""Return a shell-escaped version of the string *s*."""
    if not s:
        return u"''"
...
def _to_param_val(v):
    if v is None:
        ...
        return u" ".join(imap(_shellquote, list(imap(unicode, v))))
    else:
        return _shellquote(unicode(v))


def to_local_command(params, python_command=u"python", script=osp.join(config.PROJECT_PATH,
                                                                      u'scripts/run_experiment.py'),
                     use_gpu=False):
    command = python_command + u" " + script
    if use_gpu and not config.USE_TF:
        command = u"THEANO_FLAGS='device=gpu,dnn.enabled=auto,floatX=float32' " + command
    for k, v in config.ENV.items():
        command = (u"%s=%s " % (k, v)) + command
    pre_commands = params.pop(u"pre_commands", None)
    post_commands = params.pop(u"post_commands", None)
    if pre_commands is not None or post_commands is not None:
        print u"Not executing the pre_commands: ", pre_commands, u", nor post_commands: ", post_commands

    for k, v in params.items():
        if isinstance(v, dict):
            for nk, nv in v.items():
                if unicode(nk) == u"_name":
                    command += u"  --%s %s" % (k, _to_param_val(nv))
                else:
                    command += \
                        u"  --%s_%s %s" % (k, nk, _to_param_val(nv))
        else:
            command += u"  --%s %s" % (k, _to_param_val(v))
    return command


def to_docker_command(params, docker_image, python_command=u"python", script=u'scripts/run_experiment_lite.py',
    ...
    :return:
    """
    log_dir = params.get(u"log_dir")
    docker_args = params.pop(u"docker_args", u"")
    ...
    # script = 'rllab2/' + script
    # if not dry:

    # create volume for logging directory
    if use_gpu:
        command_prefix = u"nvidia-docker run"
    else:
        command_prefix = u"docker run"
    ...
    if env is not None:
        for k, v in env.items():
            command_prefix += u" -e \"{k}={v}\"".format(k=k, v=v)
    command_prefix += u" -v {local_mujoco_key_dir}:{docker_mujoco_key_dir}".format(
        local_mujoco_key_dir=mujoco_path, docker_mujoco_key_dir=u'/root/.mujoco')
    command_prefix += u" -v {local_log_dir}:{docker_log_dir}".format(
    ...
    command_prefix += u" -v {local_code_dir}:{docker_code_dir}".format(
        local_code_dir=local_code_dir,
        docker_code_dir=config.DOCKER_CODE_DIR
    )
    params = dict(params, log_dir=docker_log_dir)
    if use_tty:
        command_prefix += u" -ti " + docker_image + u" /bin/bash -c "
    else:
        command_prefix += u" -i " + docker_image + u" /bin/bash -c "
    command_list = list()
    if pre_commands is not None:
        command_list.extend(pre_commands)
    command_list.append(u"echo \"Running in docker\"")
    ...
    if post_commands is None:
        post_commands = [u'sleep 120']
    command_list.extend(post_commands)
    return command_prefix + u"'" + u"; ".join(command_list) + u"'"


def dedent(s):
    lines = [l.strip() for l in s.split(u'\n')]
    return u'\n'.join(lines)


def launch_ec2(params_list, exp_prefix, docker_image, code_full_path,
               python_command=u"python",
               script=u'scripts/run_experiment.py',
    ...
    """.format(exp_name=params_list[0].get(u"exp_name"), aws_region=config.AWS_REGION_NAME))
    if config.LABEL:
        sio.write(u"""
    ...
    if config.FAST_CODE_SYNC:
        # sio.write("""
        #     aws s3 cp {code_full_path} /tmp/rllab2_code.tar.gz --region {aws_region}
        # """.format(code_full_path=code_full_path, local_code_path=config.DOCKER_CODE_DIR,
        #            aws_region=config.AWS_REGION_NAME))
        sio.write(u"""
            aws s3 cp {code_full_path} /tmp/rllab2_code.tar.gz
        """.format(code_full_path=code_full_path, local_code_path=config.DOCKER_CODE_DIR))
        sio.write(u"""
        ...
        """.format(code_full_path=code_full_path, local_code_path=config.DOCKER_CODE_DIR,
                   aws_region=config.AWS_REGION_NAME))
        sio.write(u"""
            tar -zxvf /tmp/rllab2_code.tar.gz -C {local_code_path}
        """.format(code_full_path=code_full_path, local_code_path=config.DOCKER_CODE_DIR,
                   aws_region=config.AWS_REGION_NAME))
    else:
        # sio.write("""
    ...
    s3_mujoco_key_path = config.AWS_CODE_SYNC_S3_PATH + u'/.mujoco/'
    # sio.write("""
    ...
    for params in params_list:
        log_dir = params.get(u"log_dir")
        remote_log_dir = params.pop(u"remote_log_dir")
        env = params.pop(u"env", None)

        sio.write(u"""
            aws ec2 create-tags --resources $EC2_INSTANCE_ID --tags Key=Name,Value={exp_name} --region {aws_region}
        """.format(exp_name=params.get(u"exp_name"), aws_region=config.AWS_REGION_NAME))
        sio.write(u"""
            mkdir -p {log_dir}
        """.format(log_dir=log_dir))
        if periodic_sync:
            include_png = u" --include '*.png' " if sync_s3_png else u" "
            include_pkl = u" --include '*.pkl' " if sync_s3_pkl else u" "
            include_log = u" --include '*.log' " if sync_s3_log else u" "
            # sio.write("""
            #     while /bin/true; do
    ...
    if terminate_machine:
        ...
        """.format(aws_region=config.AWS_REGION_NAME))
    sio.write(u"} >> /home/ubuntu/user_data.log 2>&1\n")

    full_script = dedent(sio.getvalue())

    import boto3
    import botocore
    if aws_config[u"spot"]:
        ec2 = boto3.client(
            u"ec2",
    ...
    if len(full_script) > 10000 or len(base64.b64encode(full_script.encode()).decode(u"utf-8")) > 10000:
    ...
    print full_script
    with open(u"/tmp/full_script", u"w") as f:
        f.write(full_script)

    instance_args = dict(
        ImageId=aws_config[u"image_id"],
        KeyName=aws_config[u"key_name"],
        UserData=user_data,
        InstanceType=aws_config[u"instance_type"],
        EbsOptimized=config.EBS_OPTIMIZED,
        SecurityGroups=aws_config[u"security_groups"],
        SecurityGroupIds=aws_config[u"security_group_ids"],
        NetworkInterfaces=aws_config[u"network_interfaces"],
        IamInstanceProfile=dict(
            Name=aws_config[u"iam_instance_profile_name"],
        ),
        **config.AWS_EXTRA_CONFIGS,
    )

    if len(instance_args[u"NetworkInterfaces"]) > 0:
        ...
        if disable_security_group:
            instance_args.pop(u"SecurityGroups")
            instance_args.pop(u"SecurityGroupIds")

    if aws_config.get(u"placement", None) is not None:
        instance_args[u"Placement"] = aws_config[u"placement"]
    if not aws_config[u"spot"]:
        instance_args[u"MinCount"] = 1
        instance_args[u"MaxCount"] = 1
    print u"************************************************************"
    print instance_args[u"UserData"]
    print u"************************************************************"
    if aws_config[u"spot"]:
        instance_args[u"UserData"] = base64.b64encode(instance_args[u"UserData"].encode()).decode(u"utf-8")
        spot_args = dict(
            DryRun=dry,
            InstanceCount=1,
            LaunchSpecification=instance_args,
            SpotPrice=aws_config[u"spot_price"],
        ...
        if not dry:
            ...
            print response
            spot_request_id = response[u'SpotInstanceRequests'][
                0][u'SpotInstanceRequestId']
            for _ in xrange(10):
                try:
                    ec2.create_tags(
                        Resources=[spot_request_id],
                        Tags=[
                            {u'Key': u'Name', u'Value': params_list[0][u"exp_name"]}
                        ],
                    )
                    break
    ...
    if config.FAST_CODE_SYNC:
        try:
            current_commit = subprocess.check_output(
                [u"git", u"rev-parse", u"HEAD"]).strip().decode(u"utf-8")
        except subprocess.CalledProcessError, _:
            print u"Warning: failed to execute git commands"
            current_commit = None

        file_name = unicode(timestamp) + u"_" + hashlib.sha224(
            subprocess.check_output([u"pwd"]) + unicode(current_commit).encode() + unicode(timestamp).encode()
        ).hexdigest() + u".tar.gz"

        file_path = u"/tmp/" + file_name

        tar_cmd = [u"tar", u"-zcvf", file_path, u"-C", config.PROJECT_PATH]

        for pattern in config.FAST_CODE_SYNC_IGNORES:
            tar_cmd += [u"--exclude", pattern]
        tar_cmd += [u"-h", u"."]
        ...
        remote_path = u"%s/%s" % (base, file_name)

        upload_cmd = [u"aws", u"s3", u"cp", file_path, remote_path]

        mujoco_key_cmd = [
            u"aws", u"s3", u"sync", config.MUJOCO_KEY_PATH, u"{}/.mujoco/".format(base)]

        print u" ".join(tar_cmd)
        print u" ".join(upload_cmd)
        print u" ".join(mujoco_key_cmd)
        ...
        try:
            current_commit = subprocess.check_output(
                [u"git", u"rev-parse", u"HEAD"]).strip().decode(u"utf-8")
            clean_state = len(
                subprocess.check_output([u"git", u"status", u"--porcelain"])) == 0
        except subprocess.CalledProcessError, _:
            print u"Warning: failed to execute git commands"
            has_git = False
        dir_hash = base64.b64encode(subprocess.check_output([u"pwd"])).decode(u"utf-8")
        code_path = u"%s_%s" % (
            dir_hash,
            (current_commit if clean_state else u"%s_dirty_%s" % (current_commit, timestamp)) if
            has_git else timestamp
        )
        full_path = u"%s/%s" % (base, code_path)
        cache_path = u"%s/%s" % (base, dir_hash)
        cache_cmds = [u"aws", u"s3", u"cp", u"--recursive"] + \
                     flatten([u"--exclude", u"%s" % pattern] for pattern in config.CODE_SYNC_IGNORES) + \
                     [cache_path, full_path]
        cmds = [u"aws", u"s3", u"cp", u"--recursive"] + \
               flatten([u"--exclude", u"%s" % pattern] for pattern in config.CODE_SYNC_IGNORES) + \
               [u".", full_path]
        caching_cmds = [u"aws", u"s3", u"cp", u"--recursive"] + \
                       flatten([u"--exclude", u"%s" % pattern] for pattern in config.CODE_SYNC_IGNORES) + \
        ...
        if not dry:
            ...
            try:
                ...
                print u'Unable to sync mujoco keys!'
...
def upload_file_to_s3(script_content):
    ...
    remote_path = os.path.join(
        config.AWS_CODE_SYNC_S3_PATH, u"oversize_bash_scripts", unicode(uuid.uuid4()))
    subprocess.check_call([u"aws", u"s3", u"cp", f.name, remote_path])
    os.unlink(f.name)
    return remote_path
...
def to_lab_kube_pod(
    ...
    log_dir = params.get(u"log_dir")
    remote_log_dir = params.pop(u"remote_log_dir")
    resources = params.pop(u"resources")
    node_selector = params.pop(u"node_selector")
    exp_prefix = params.pop(u"exp_prefix")

    kube_env = [
        {u"name": k, u"value": v}
        for k, v in (params.pop(u"env", None) or dict()).items()
        ]
    mkdir_p(log_dir)
    pre_commands = list()
    pre_commands.append(u'mkdir -p ~/.aws')
    pre_commands.append(u'mkdir ~/.mujoco')
    # fetch credentials from the kubernetes secret file
    pre_commands.append(u'echo "[default]" >> ~/.aws/credentials')
    pre_commands.append(
        u"echo \"aws_access_key_id = %s\" >> ~/.aws/credentials" % config.AWS_ACCESS_KEY)
    pre_commands.append(
        u"echo \"aws_secret_access_key = %s\" >> ~/.aws/credentials" % config.AWS_ACCESS_SECRET)
    s3_mujoco_key_path = config.AWS_CODE_SYNC_S3_PATH + u'/.mujoco/'
    pre_commands.append(
        u'aws s3 cp --recursive {} {}'.format(s3_mujoco_key_path, u'~/.mujoco'))

    if config.FAST_CODE_SYNC:
        pre_commands.append(u'aws s3 cp %s /tmp/rllab2_code.tar.gz' % code_full_path)
        pre_commands.append(u'mkdir -p %s' % config.DOCKER_CODE_DIR)
        pre_commands.append(u'tar -zxvf /tmp/rllab2_code.tar.gz -C %s' % config.DOCKER_CODE_DIR)
    else:
        pre_commands.append(u'aws s3 cp --recursive %s %s' %
                            (code_full_path, config.DOCKER_CODE_DIR))
    pre_commands.append(u'cd %s' % config.DOCKER_CODE_DIR)
    pre_commands.append(u'mkdir -p %s' %
                        (log_dir))

    if sync_all_data_node_to_s3:
        print u'Syncing all data from node to s3.'
        if periodic_sync:
            if sync_s3_pkl:
                pre_commands.append(u"""
                            while /bin/true; do
    ...
    post_commands = list()
    post_commands.append(u'aws s3 cp --recursive %s %s' %
                         (log_dir,
                          remote_log_dir))
    if not terminate_machine:
        post_commands.append(u'sleep infinity')
    command_list = list()
    if pre_commands is not None:
        command_list.extend(pre_commands)
    command_list.append(u"echo \"Running in docker\"")
    command_list.append(
        u"%s 2>&1 | tee -a %s" % (
            ...
            u"%s/stdouterr.log" % log_dir
        )
    )
    if post_commands is not None:
        command_list.extend(post_commands)
    command = u"; ".join(command_list)
    pod_name = config.KUBE_PREFIX + params[u"exp_name"]
    # underscore is not allowed in pod names
    pod_name = pod_name.replace(u"_", u"-")
    print u"Is gpu: ", is_gpu
    if not is_gpu:
        return {
            u"apiVersion": u"v1",
            u"kind": u"Pod",
            u"metadata": {
                u"name": pod_name,
                u"labels": {
                    u"owner": config.LABEL,
                    u"expt": pod_name,
                    u"exp_time": timestamp,
                    u"exp_prefix": exp_prefix,
                },
            },
            u"spec": {
                u"containers": [
                    {
                        u"name": u"foo",
                        u"image": docker_image,
                        u"command": [
                            u"/bin/bash",
                            u"-c",
                            u"-li",  # to load conda env file
                            command,
                        ],
                ...
                u"nodeSelector": node_selector,
                u"dnsPolicy": u"Default",
            }
        }
    return {
        u"apiVersion": u"v1",
            ...
            u"containers": [
                {
                    ...
                    u"volumeMounts": [
                        {
                            u"name": u"nvidia",
                            u"mountPath": u"/usr/local/nvidia",
                            u"readOnly": True,
                        }
                    ],
                    u"securityContext": {
            ...
            u"volumes": [
                {
                    u"name": u"nvidia",
                    u"hostPath": {
                        u"path": u"/var/lib/docker/volumes/nvidia_driver_352.63/_data",
                    }
                }
            ],
            u"restartPolicy": u"Never",
...
def concretize(maybe_stub):
    if isinstance(maybe_stub, StubMethodCall):
        ...
        if not hasattr(maybe_stub, u"__stub_cache"):
            args = concretize(maybe_stub.args)
            kwargs = concretize(maybe_stub.kwargs)
            try:
                ...
                print (u"Error while instantiating %s" % maybe_stub.proxy_class)
                import traceback
                traceback.print_exc()
        ...
        return maybe_stub.__class__(list(imap(concretize, maybe_stub)))
    else:
        return maybe_stub
