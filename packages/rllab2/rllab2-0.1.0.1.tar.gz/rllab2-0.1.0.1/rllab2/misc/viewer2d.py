...
class Viewer2D(object):
    ...
    def polygon(self, color, points):
        if len(color) > 3:
            ...
            s.fill((0, 0, 0, 0))
            pygame.draw.polygon(s, color, list(imap(self.scale_point, points)))
            self.screen.blit(s, (0, 0))
        else:
            pygame.draw.polygon(self.screen, color, list(imap(self.scale_point, points)))
    ...
    def checker(self, colors=[Colors.white, Colors.black], granularity=4, offset=(0, 0)):
        ...
        for row in xrange(-1, int(np.ceil(screen_height * 1.0 / checker_size))+1):
            for col in xrange(-1, int(np.ceil(screen_width * 1.0 / checker_size))+1):
    ...
    def pause(self):
        print u"press any key on the screen to continue..."
        while True:
