...
from rllab2.spaces.base import Space
import numpy as np
...
class Product(Space):
    ...
    def __init__(self, *components):
        ...
        dtypes = [c.new_tensor_variable(u"tmp", extra_dims=0).dtype for c in components]
        if len(dtypes) > 0 and hasattr(dtypes[0], u"as_numpy_dtype"):
    ...
    def contains(self, x):
        return isinstance(x, tuple) and all(c.contains(xi) for c, xi in izip(self._components, x))
    ...
    def flatten(self, x):
        return np.concatenate([c.flatten(xi) for c, xi in izip(self._components, x)])

    def flatten_n(self, xs):
        xs_regrouped = [[x[i] for x in xs] for i in xrange(len(xs[0]))]
        flat_regrouped = [c.flatten_n(xi) for c, xi in izip(self.components, xs_regrouped)]
    ...
    def unflatten(self, x):
        ...
        return tuple(c.unflatten(xi) for c, xi in izip(self._components, flat_xs))

    def unflatten_n(self, xs):
        dims = [c.flat_dim for c in self._components]
        flat_xs = np.split(xs, np.cumsum(dims)[:-1], axis=-1)
        unflat_xs = [c.unflatten_n(xi) for c, xi in izip(self.components, flat_xs)]
        unflat_xs_grouped = list(izip(*unflat_xs))
