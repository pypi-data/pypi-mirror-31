...
import numpy as np
from rllab2.distributions.base import Distribution
...
class DiagonalGaussian(Distribution):
    ...
    def kl_sym(self, old_dist_info_vars, new_dist_info_vars):
        old_means = old_dist_info_vars[u"mean"]
        old_log_stds = old_dist_info_vars[u"log_std"]
        new_means = new_dist_info_vars[u"mean"]
        new_log_stds = new_dist_info_vars[u"log_std"]
        u"""
    ...
    def kl(self, old_dist_info, new_dist_info):
        old_means = old_dist_info[u"mean"]
        old_log_stds = old_dist_info[u"log_std"]
        new_means = new_dist_info[u"mean"]
        new_log_stds = new_dist_info[u"log_std"]
        u"""
        Compute the KL divergence of two multivariate Gaussian distribution with
        diagonal covariance matrices
        """
    ...
    def log_likelihood_sym(self, x_var, dist_info_vars):
        means = dist_info_vars[u"mean"]
        log_stds = dist_info_vars[u"log_std"]
    ...
    def sample(self, dist_info):
        means = dist_info[u"mean"]
        log_stds = dist_info[u"log_std"]
