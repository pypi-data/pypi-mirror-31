...
def from_onehot(x_var):
    ret = np.zeros((len(x_var),), u'int32')
...
class Categorical(Distribution):
    ...
    def kl_sym(self, old_dist_info_vars, new_dist_info_vars):
        ...
        """
        old_prob_var = old_dist_info_vars[u"prob"]
        new_prob_var = new_dist_info_vars[u"prob"]
    ...
    def kl(self, old_dist_info, new_dist_info):
        ...
        """
        old_prob = old_dist_info[u"prob"]
        new_prob = new_dist_info[u"prob"]
        return np.sum(
    ...
    def entropy_sym(self, dist_info_vars):
        prob_var = dist_info_vars[u"prob"]
        return -TT.sum(prob_var * TT.log(prob_var + TINY), axis=1)

    def log_likelihood_sym(self, x_var, dist_info_vars):
        probs = dist_info_vars[u"prob"]
        # Assume layout is N * A
        return TT.log(TT.sum(probs * TT.cast(x_var, u'float32'), axis=-1) + TINY)

    def log_likelihood(self, xs, dist_info):
        probs = dist_info[u"prob"]
        # Assume layout is N * A
        N = probs.shape[0]
    ...
    def sample_sym(self, dist_info):
        probs = dist_info[u"prob"]
        return self._srng.multinomial(pvals=probs, dtype=u'uint8')
