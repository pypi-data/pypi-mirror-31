...
import pickle
import os
from rllab2.misc.ext import compile_function
import collections
...
def gauss_newton_product(cost, p, v, s):  # this computes the product Gv = J'HJv (G is the Gauss-Newton matrix)
    ...
    for si in s:
        Jv = T.Rop(si, p, v)
        HJv = T.grad(T.sum(T.grad(cost, si, disconnected_inputs=u'ignore') * Jv), si, consider_constant=[Jv], disconnected_inputs=u'ignore')
        Gv = T.grad(T.sum(HJv * si), p, consider_constant=[HJv, Jv], disconnected_inputs=u'ignore')
        Gv = list(imap(T.as_tensor_variable, Gv))  # for CudaNdarray
        if sum_Gv is None:
            sum_Gv = Gv
        else:
            sum_Gv = [a+b for a, b in izip(Gv, sum_Gv)]
    return sum_Gv


class hf_optimizer(object):
    u'''Black-box Theano-based Hessian-free optimizer.
    ...
    def __init__(self, _p, inputs, s, costs, h=None, ha=None):
        u'''Constructs and compiles the necessary Theano functions.
        ...
        self.sizes = list(imap(numpy.prod, self.shapes))
        self.positions = numpy.cumsum([0] + self.sizes)[:-1]

        g = T.grad(costs[0], _p)
        g = list(imap(T.as_tensor_variable, g))  # for CudaNdarray
        ...
        if h is not None:  # structural damping with cross-entropy
            ...
            Gv = [a + b for a, b in izip(Gv, Gv_damping)]
            givens = {h_constant: h}
        else:
            givens = {}
    ...
    def cg(self, b, verbose=False):
        ...
        x = self.cg_last_x if hasattr(self, u'cg_last_x') else numpy.zeros_like(b)  # sharing information between CG runs
        r = b - self.batch_Gv(x)
        d = M * r
        ...
        for i in xrange(1, 1 + self.max_cg_iterations):
            ...
            if verbose:
                progress = u' [CG iter %i, phi=%+.5f, cost=%.5f]' % (i, phi_i, backtracking[-1][0])
                sys.stdout.write(u'\b' * backspaces + progress)
                sys.stdout.flush()
        ...
        if self.global_backtracking:
            ...
            while j > 0 and backtracking[j - 1][0] < backtracking[j][0]:
                j -= 1
        if verbose:
            print u' backtracked %i/%i' % (backtracking[j][2], i),
            sys.stdout.flush()
    ...
    def flat_to_list(self, vector):
        return [vector[position:position + size].reshape(shape) for shape, size, position in
                izip(self.shapes, self.sizes, self.positions)]
    ...
    def train(self, gradient_dataset, cg_dataset, initial_lambda=0.1, mu=0.03, global_backtracking=False,
        ...
        if isinstance(save_progress, unicode) and os.path.isfile(save_progress):
            save = pickle.load(file(save_progress))
            self.cg_last_x, best, self.lambda_, first_iteration, init_p = save
            first_iteration += 1

            if verbose: print u'* recovered saved model'

        try:
            for u in xrange(first_iteration, 1 + num_updates):
                if verbose: print u'update %i/%i,' % (u, num_updates),
                sys.stdout.flush()
...
#                     import ipdb; ipdb.set_trace()
                ...
                if verbose: print u'cost=', numpy.mean(costs, axis=0),
                if verbose: print u'lambda=%.5f,' % self.lambda_,
                sys.stdout.flush()
                ...
                for i, delta in izip(self.p, self.flat_to_list(flat_delta)):
            ...
            if verbose: print u'Interrupted by user.'
...
class SequenceDataset(object):
    u'''Slices, shuffles and manages a small dataset for the HF optimizer.'''

    def __init__(self, data, batch_size, number_batches, minimum_size=10):
        u'''SequenceDataset __init__
