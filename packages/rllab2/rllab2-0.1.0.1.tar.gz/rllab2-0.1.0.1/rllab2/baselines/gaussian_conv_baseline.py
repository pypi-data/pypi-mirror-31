...
from rllab2.regressors.gaussian_conv_regressor import GaussianConvRegressor
...
class GaussianConvBaseline(Baseline, Parameterized):
    ...
    @overrides
    def fit(self, paths):
        observations = np.concatenate([p[u"observations"] for p in paths])
        returns = np.concatenate([p[u"returns"] for p in paths])
        self._regressor.fit(observations, returns.reshape((-1, 1)))

    @overrides
    def predict(self, path):
        return self._regressor.predict(path[u"observations"]).flatten()
