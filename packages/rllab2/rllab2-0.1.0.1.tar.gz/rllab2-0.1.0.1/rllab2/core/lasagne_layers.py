...
class BatchNormLayer(L.Layer):
    ...
    **kwargs
    ...
    """
    def __init__(self, incoming, axes=u'auto', epsilon=1e-4, alpha=0.1,
                 mode=u'low_mem', beta=lasagne.init.Constant(0), gamma=lasagne.init.Constant(1),
                 mean=lasagne.init.Constant(0), std=lasagne.init.Constant(1), **kwargs):
        super(BatchNormLayer, self).__init__(incoming, **kwargs)

        if axes == u'auto':
            # default: normalize over all but the second axis
            axes = (0,) + tuple(xrange(2, len(self.input_shape)))
        ...
        if any(size is None for size in shape):
            raise ValueError(u"BatchNormLayer needs specified input sizes for "
                             u"all axes not normalized over.")
        if beta is None:
            self.beta = None
        else:
            self.beta = self.add_param(beta, shape, u'beta',
                                       trainable=True, regularizable=False)
        if gamma is None:
            self.gamma = None
        else:
            self.gamma = self.add_param(gamma, shape, u'gamma',
                                        trainable=True, regularizable=False)
        self.mean = self.add_param(mean, shape, u'mean',
                                   trainable=False, regularizable=False)
        self.std = self.add_param(std, shape, u'std',
                                      trainable=False, regularizable=False)
    ...
    def get_output_for(self, input, deterministic=False, **kwargs):
        ...
        use_averages = kwargs.get(u'batch_norm_use_averages',
                                  deterministic)
        if use_averages:
            mean = self.mean
            std = self.std
        ...
        update_averages = kwargs.get(u'batch_norm_update_averages',
                                     not deterministic)
        if update_averages:
        ...
        pattern = [u'x' if input_axis in self.axes
                   else param_axes.next()
                   for input_axis in xrange(input.ndim)]
...
def batch_norm(layer, **kwargs):
    ...
    **kwargs
    ...
    """
    nonlinearity = getattr(layer, u'nonlinearity', None)
    if nonlinearity is not None:
        layer.nonlinearity = lasagne.nonlinearities.identity
    if hasattr(layer, u'b') and layer.b is not None:
