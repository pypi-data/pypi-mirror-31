    ...
    for layer in all_layers:
        if layer not in all_outputs:
            try:
                ...
                raise ValueError(u"get_output() was called without giving an "
                                 u"input expression for the free-floating "
                                 u"layer %r. Please call it with a dictionary "
                                 u"mapping this layer to an input expression."
                                 % layer)
            if hasattr(layer, u"get_full_output_for"):
