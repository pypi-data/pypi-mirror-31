...
from rllab2.sampler.stateful_pool import singleton_pool, SharedGlobal
...
def _worker_init(G, id):
    if singleton_pool.n_parallel > 1:
        import os
        os.environ[u'THEANO_FLAGS'] = u'device=cpu'
        os.environ[u'CUDA_VISIBLE_DEVICES'] = u""
    G.worker_id = id
...
def _worker_terminate_task(G, scope=None):
    G = _get_scoped_G(G, scope)
    if getattr(G, u"env", None):
        G.env.terminate()
        G.env = None
    if getattr(G, u"policy", None):
        G.policy.terminate()
        G.policy = None


def populate_task(env, policy, scope=None):
    logger.log(u"Populating workers...")
    if singleton_pool.n_parallel > 1:
...
def _worker_set_seed(_, seed):
    logger.log(u"Setting seed to %d" % seed)
    ext.set_seed(seed)
...
def _worker_collect_one_path(G, max_path_length, scope=None):
    ...
    return path, len(path[u"rewards"])
...
def truncate_paths(paths, max_samples):
    ...
    # make a copy
    paths = list(paths)
    total_n_samples = sum(len(path[u"rewards"]) for path in paths)
    while len(paths) > 0 and total_n_samples - len(paths[-1][u"rewards"]) >= max_samples:
        total_n_samples -= len(paths.pop(-1)[u"rewards"])
    if len(paths) > 0:
        last_path = paths.pop(-1)
        truncated_last_path = dict()
        truncated_len = len(last_path[u"rewards"]) - (total_n_samples - max_samples)
        for k, v in last_path.items():
            if k in [u"observations", u"actions", u"rewards"]:
                truncated_last_path[k] = tensor_utils.truncate_tensor_list(v, truncated_len)
            elif k in [u"env_infos", u"agent_infos"]:
