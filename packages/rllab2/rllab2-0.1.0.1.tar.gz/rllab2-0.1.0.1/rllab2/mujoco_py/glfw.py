...
__author__ = u'Florian Rhiem (florian.rhiem@gmail.com)'
__copyright__ = u'Copyright (c) 2013 Florian Rhiem'
__license__ = u'MIT'
__version__ = u'1.0.1'
...
if sys.version_info.major > 2:
    _to_char_p = lambda s: s.encode(u'utf-8')
else:
...
def _find_library_candidates(library_names,
    ...
    for library_name in library_names:
        for search_path in library_search_paths:
            glob_query = os.path.join(search_path, u'*'+library_name+u'*')
            for filename in glob.iglob(glob_query):
                filename = os.path.realpath(filename)
                if filename in candidates:
                    continue
                basename = os.path.basename(filename)
                if basename.startswith(u'lib'+library_name):
                    basename_end = basename[len(u'lib'+library_name):]
                ...
                for file_extension in library_file_extensions:
                    if basename_end.startswith(file_extension):
                        if basename_end[len(file_extension):][:1] in (u'', u'.'):
                            candidates.add(filename)
                    if basename_end.endswith(file_extension):
                        basename_middle = basename_end[:-len(file_extension)]
                        if all(c in u'0123456789.' for c in basename_middle):
                            candidates.add(filename)
    return candidates
...
def _load_library():
    ...
    if sys.platform.startswith(u"darwin"):
        libfile = osp.abspath(osp.join(osp.dirname(__file__),u"../../vendor/mujoco/libglfw.3.dylib"))
    elif sys.platform.startswith(u"linux"):
        libfile = osp.abspath(osp.join(osp.dirname(__file__),u"../../vendor/mujoco/libglfw.so.3"))
    elif sys.platform.startswith(u"win"):
        libfile = osp.abspath(osp.join(osp.dirname(__file__),u"../../vendor/mujoco/glfw3.dll"))
    else:
        raise RuntimeError(u"unrecognized platform %s"%sys.platform)
...
def _glfw_get_version(filename):
    ...
    args = [sys.executable, u'-c', textwrap.dedent(version_checker_source)]
...
_glfw = _load_library()
if _glfw is None:
    raise ImportError(u"Failed to load GLFW3 shared library.")
...
class _GLFWvidmode(ctypes.Structure):
    ...
    '''
    _fields_ = [(u"width", ctypes.c_int),
                (u"height", ctypes.c_int),
                (u"red_bits", ctypes.c_int),
                (u"green_bits", ctypes.c_int),
                (u"blue_bits", ctypes.c_int),
                (u"refresh_rate", ctypes.c_uint)]
...
class _GLFWgammaramp(ctypes.Structure):
    ...
    '''
    _fields_ = [(u"red", ctypes.POINTER(ctypes.c_ushort)),
                (u"green", ctypes.POINTER(ctypes.c_ushort)),
                (u"blue", ctypes.POINTER(ctypes.c_ushort)),
                (u"size", ctypes.c_uint)]
    ...
    def unwrap(self):
        ...
        Returns a nested python sequence.
        '''
        red = [self.red[i]/65535.0 for i in xrange(self.size)]
        green = [self.green[i]/65535.0 for i in xrange(self.size)]
        blue = [self.blue[i]/65535.0 for i in xrange(self.size)]
...
def get_monitors():
    ...
    monitors = [result[i] for i in xrange(count_value.value)]
    return monitors
...
def get_video_modes(monitor):
    ...
    videomodes = [result[i].unwrap() for i in xrange(count_value.value)]
    return videomodes
