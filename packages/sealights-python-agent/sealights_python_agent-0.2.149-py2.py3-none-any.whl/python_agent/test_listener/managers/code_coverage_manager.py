...
class CodeCoverageManager(object):
    def __init__(self, config_data):
        if constants.IN_TEST:
            ...
            config_data.include.extend(["*%s*" % config_data.workspacepath, "*%s*" % python_agent.__name__])
            self.coverage = Coverage(source=None, include=config_data.include, omit=None, data_suffix=True)
        else:
