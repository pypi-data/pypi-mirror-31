...
class XPSFunctions:
    ...
    @staticmethod
    def find_peak(energy, counts, range, background_model, show_fit=False, gaussian=False, orbital=None):
        x,y, background = XPSFunctions._prepare_data_for_fit(energy,counts,range=range,background_model=background_model)

        if orbital is 'C1s':
            popt,pcov = XPSFunctions._fit_C1s_peaks(x,y,background,show_fit=show_fit)
        elif orbital is 'Pd3d':
            popt,pcov = XPSFunctions._fit_Pd3d_peaks(x,y,background,show_fit=show_fit)
            return [popt[1],popt[4]], [np.sqrt(np.diag(pcov)[1]),np.sqrt(np.diag(pcov)[4])]
        elif orbital is 'Pt4f':
            popt, pcov = XPSFunctions._fit_Pt4f_peaks(x, y - background)
        else:
    ...
    @staticmethod
    def _fit_C1s_peaks(x,y,background,show_fit=False):
        ...
        if show_fit:
            ...
            #plt.plot(x, XPSFunctions._voigt(x, *popt1) + background, 'k')
    ...
    @staticmethod
    def _fit_Pd3d_peaks(x, y, background,show_fit=False):
        mean = x[np.argmax(y)]  # note this correction
        sigma = 1  # note this correction
        max = np.max(y - background)
        ...
        popt1, pcov1 = curve_fit(XPSFunctions._six_voigts, x, y - background, initial_parameters,
                                 bounds=(lower_boundaries, upper_boundaries), maxfev=200000, xtol=1e-7)

        #
        # lower_boundaries = (0, popt1[1] - 3, popt1[2] - 2, popt1[3] - 2,
        #                     popt1[0] * 0.1, popt1[1] - 1, popt1[2] - 2, popt1[3] - 2,
        #                     popt1[0] * 0.1, popt1[1] - 1, popt1[2] - 2, popt1[3] - 2,
        #
        #                     0, popt1[5] - 3, popt1[6] - 2, popt1[7] - 2,
        #                     popt1[4] * 0.1, popt1[5], popt1[6] - 2, popt1[7] - 2,
        #                     popt1[4] * 0.1, popt1[5], popt1[6] - 2, popt1[7] - 2
        #                     )
        #
        # upper_boundaries = (np.inf, popt1[1] + 3, popt1[2] + 0.3, popt1[3] + 0.3,
        #                     np.inf, popt1[1] + 5.3, popt1[2] + 0.3, popt1[3] + 0.3,
        #                     np.inf, popt1[1] + 6, popt1[2] + 0.3, popt1[3] + 0.3,
        #
        #                     np.inf, popt1[5] + 3, popt1[6] + 0.3, popt1[7] + 0.3,
        #                     np.inf, popt1[5] + 4, popt1[6] + 0.3, popt1[7] + 0.3,
        #                     np.inf, popt1[5] + 6, popt1[6] + 0.3, popt1[7] + 0.3
        #                     )
        #
        # initial_parameters = [popt1[0], popt1[1], popt1[2], popt1[3],
        #                       popt1[0] * 0.3, popt1[1] + 1.7, popt1[2], popt1[3],
        #                       popt1[0] * 0.3, popt1[1] + 3, popt1[2], popt1[3],
        #
        #                       popt1[4], popt1[5], popt1[6], popt1[7],
        #                       popt1[4] * 0.3, popt1[5] + 1.7, popt1[6], popt1[7],
        #                       popt1[4] * 0.3, popt1[5] + 3, popt1[6], popt1[7]
        #                       ]
        # print(initial_parameters)
        # popt1, pcov1 = curve_fit(XPSFunctions._six_voigts, x, y - background, initial_parameters,
        #                          bounds=(lower_boundaries, upper_boundaries), maxfev=200000, xtol=1e-3)
        if show_fit:
            print('plotting!')
            plt.plot(x, y, 'm')
            plt.plot(x, background, 'g')
