...
GITHUB_RELEASE_API_URL  = "https://api.github.com/repos/OpenROV/trident-releases/releases/latest"
...
# Fetch latest release hash
@app.route('/latestRelease', methods=['GET'])
def latestRelease():
    try:
        # Make request to github API
        res = requests.get( GITHUB_RELEASE_API_URL, timeout=REQUEST_TIMEOUT_SECS )
        res.raise_for_status()
        release = res.json()

        # Trim the 'v' off the front
        commit = res.json()[ "tag_name" ][1:]
        resp = { "commit" : commit }
