...
from .views import checkin, event, item, order, organizer, voucher,common, waitinglist, sms, team,wholesale,pos,opening_closing_balance
...
pos_open= routers.DefaultRouter()
pos_open.register(r'Openclosebalance', opening_closing_balance.OpeenclosebalanceViewSet)
...
urlpatterns = [
    ...
    url(r'^organizers/(?P<organizer>[^/]+)/events/(?P<event>[^/]+)/', include(pos_open.urls)),
    ...
    url(r'^organizers/(?P<organizer>[^/]+)/events/(?P<event>[^/]+)/wholesale/openingbalance/',wholesale.opening_balance,name='opening_balance'),
    #url(r'^login/', pos.login, name='login'),
    url(r'^login/', pos.CustomAuthToken.as_view()),
    url(r'^organizers/(?P<organizer>[^/]+)/events/(?P<event>[^/]+)/orders/(?P<pk>[^/]*)/print_status_update/',common.order_print_status_update,name='print_status'),
    url(r'^organizers/(?P<organizer>[^/]+)/events/(?P<event>[^/]+)/couponcocouponcodesdes/',common.couponcodes,name='couponcodes'),
     url(r'^organizers/(?P<organizer>[^/]+)/events/(?P<event>[^/]+)/userwise_order_list/',common.userwise_order_list,name='userwise_order_list'),
