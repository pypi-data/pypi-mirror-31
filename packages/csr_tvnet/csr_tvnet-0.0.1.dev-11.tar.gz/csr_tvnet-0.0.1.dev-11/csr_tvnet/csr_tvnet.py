#!/usr/bin/env python
from csr_cloud import csr_cloud as cloud
import time
import random
import string
import logging
...
class csr_transit():
    ...
    def configure_tunnel(self, tunn_addr):
        ...
        if 'hub' in role:
            ...
            for key, value in self.section_dict.items():
                if 'hub-' in key:
                    nbmaconfig += self.configure_tunnel_nhsnbma(value['nbma'], value['pip'])
    ...
    def configure_tunnel_nhsnbma(self, DMVPNHubTunnelIp, DMVPNHubIp):
        ...
        self.cmd_execute(
            ...
            spoke will connect to Hub at {} IP ".format(DMVPNHubIp))
        return cmd
    ...
    def configure_routing_nbma(DMVPNHubTunnelIp):
    ...
    def get_tunnel_addr(self):
        ...
        if 'hub' in role:
            ...
            tunn_addr = tunnel_network.network_address + str(self.section_dict['hub_number'])
    ...
    def configure_transit_vnet(self):
        ...
        self.cmd_execute(
            "send log [INFO] [CSRTransitVNET] Success. Configured all the required IOS configs.".format(self.section_dict['role'].lower()))
    ...
    def write_all_files(self):
        ...
        for file_content in file_list:
            try:
                ...
                self.cloud.tvnet.write_file_contents(
    ...
    def get_all_files(self):
        if 'spoke' in self.section_dict['role']:
            ...
            folder_contents = self.cloud.tvnet.list_directory_contents(
                self.section_dict['file_share'],
                self.section_dict['folder'])
