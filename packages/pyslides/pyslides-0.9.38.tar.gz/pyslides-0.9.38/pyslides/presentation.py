...
parser.add_argument('-o', help='output file',default='slides.pdf')
...
class Presentation(object):
 ...
 def get_next_m(self):
  ...
  m = 0
  while exist:
   if not os.path.isfile('.cache/slide_' + str(m) + '.pdf'): 
    return m
   m +=1  
 ...
 def render(self):
  ...
  #load database--------------
  if os.path.isfile('.cache/slides.p'): 
   with open('.cache/slides.p') as f:
   ...
   for slide in self.sections[section]:
     ...
     for m,slide_db in slides_db.items():
      if slide_db == slide:
       ...
       n += 1
       print('Include slide ' + str(n))
       list_files.append('.cache/slide_' + str(m) + '.pdf')
       exist = True
       break
     ...
     if not exist:
      ...
      savefig('.cache/slide_' + str(m) + '.pdf',dpi=500)  
      #savefig('tmp/slide_' + str(m) + '.pdf',dpi=100)  
      list_files.append('.cache/slide_' + str(m) + '.pdf')

  with open('.cache/slides.p', 'wb+') as f:
 ...
 def reorder_database(self):
  ...
  #update slides-------------------------
  with open('.cache/slides.p','rb') as f:
   slides_db = pickle.load(f)
  ...
  for i,m in enumerate(index):
   ...
   os.system('mv .cache/slide_' + str(old_index) + '.pdf' + ' .cache/slide_tmp_' + str(i) + '.pdf' )
  ...
  for f in files:
   if len(f.split('_')) > 2:
    ...
    os.system('mv .cache/slide_tmp_' + str(n) + '.pdf' + ' .cache/slide_' + str(n)  + '.pdf')
