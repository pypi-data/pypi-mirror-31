...
class AuthController():
    ...
    def ppsschangepassword(self):
        ...
        retdict = {'logintpl': Conf.logintemplateinherit,'msg':"",'res':True}
    ...
    def addUser(self):
        userid = int(self.request.matchdict.get("userid","-1"))
        l.info("change password")
        if userid<0:
            user = PPSsuser()
        else:
        ...
        retdict = {'logintpl': Conf.logintemplateinherit,'msg':"",'res':True
            ,'userid':userid , "user" : user, 'allgroups':allgroups }
        if self.request.POST:
            if userid<0:
            ...
            return HTTPFound(self.request.route_url('ppssaddcreateuser',userid = user.id))
            #return retdict
