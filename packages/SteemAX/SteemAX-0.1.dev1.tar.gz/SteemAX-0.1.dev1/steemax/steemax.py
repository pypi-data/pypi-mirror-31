...
def enter_account_name(a):
    ...
    while True:
        ...
        if not re.match( r'^[a-z0-9\-]+$', acct) or len(acct) == 0 or len(acct) > 32:
            ...
            if axverify.get_vote_value(acct, 100, 0, ""):
                break
...
def enter_memo_id(acct):
    ...
    while True:
        ...
        if not re.match( r'^[0-9]+$', memoid) or len(memoid) == 0 or len(memoid) > 32:
            ...
            if axdb.x_verify_memoid(acct, memoid, ""):
                break
...
def enter_ratio(acct1, acct2, per, flag):
    ...
    while True:
        ...
        if not re.match( r'^[0-9\.]+$', ratio) or len(ratio) == 0 or len(ratio) > 5 or float(ratio) < 0.01 or float(ratio) > 99:
            ...
            if axverify.x_eligible_votes(acct1, acct2, per, ratio, "", flag):
                break
...
def enter_key(acct):
    ...
    while True:
        ...
        if not re.match( r'^[A-Za-z0-9]+$', key) or len(key) == 0 or len(key) > 64:
            ...
            if axverify.x_verify_key(acct, key, ""):
                break
...
class MyPrompt(Cmd):
    ...
    def do_invite(self, args):
        ...
        memoid = axdb.x_add_invite (acct1, acct2, key, per, ratio, dur, "")
    ...
    def do_accept(self, args):
        ...
        if not axdb.x_verify_invitee(acct, memoid, ""):
            return
        key = enter_key(acct)
        if axdb.x_accept_invite(acct, memoid, key, ""):
    ...
    def do_barter(self, args):
        ...
        acct = enter_account_name(1)
        memoid = enter_memo_id(acct)
        asin = axdb.x_verify_account(acct, memoid, "")
        if not asin:
            return
        ...
        if not axdb.x_verify_memoid(acct, memoid, ""):
            print ("Memo ID does not match the account provided.")
        else:
            if axdb.x_update_invite(per, ratio, dur, memoid, ""):
    ...
    def do_cancel(self, args):
        ...
        acct = enter_account_name(1)
        memoid = enter_memo_id(acct)
        if not axdb.x_verify_account(acct, memoid, ""):
            return
        if axdb.x_cancel(acct, memoid, ""):
    ...
    def do_account(self, args):
        ...
        acct = enter_account_name(1)
        axdb.x_verify_account(acct, "", "")


    def do_pool(self, args):

        axverify.x_get_steemit_balances("verbose")
