...
class AkismetFilterStrategy(Component):
    ...
    noheaders = ['HTTP_COOKIE', 'HTTP_HOST', 'HTTP_REFERER',
    ...
    api_url = Option('spam-filter', 'akismet_api_url',
        'rest.akismet.com/1.1/', """URL of the Akismet service.""",
        doc_domain='tracspamfilter')
        ...
        try:
            ...
            if resp.strip().lower() != 'false':
                ...
                return -abs(self.karma_points), \
                       N_("Akismet says content is spam")
    ...
    def train(self, req, author, content, ip, spam=True):
        ...
        try:
            ...
            url = 'http://%s.%ssubmit-%s' \
                  % (self.api_key, self.api_url, which)
