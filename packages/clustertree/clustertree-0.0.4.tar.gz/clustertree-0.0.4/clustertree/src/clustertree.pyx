...
cdef class ClusterTree:
    ...
    cdef c_insert(self, int s, int e, id):
        ''' Insert an interval with start, end, id as parameters'''
        self.tree.root = clusternode_insert(self.tree, self.tree.root, s, e, id)
    ...
    cdef c_getregions_only(self):
        ...
        while (itr):
            regions.append((itr.node.start, itr.node.end))
            itr = itr.next
...
def find_clusters(mincols, minregions, long [::1] starts, long [::1] ends):

    cdef int start
    cdef int end

    ct = ClusterTree(mincols, minregions)
    ...
    while i < len(starts):
        ...
        ct.c_insert(starts[i], ends[i], 0)
        i += 1

    starts_arr = np.zeros(len(starts), dtype=np.long)
    ends_arr = np.zeros(len(starts), dtype=np.long)

    cdef long[::1] out_starts
    cdef long[::1] out_ends

    out_starts = starts_arr
    out_ends = ends_arr

    for start, end in ct.c_getregions_only():
    ...
    starts_arr.resize(j, refcheck=False)
    ends_arr.resize(j, refcheck=False)
