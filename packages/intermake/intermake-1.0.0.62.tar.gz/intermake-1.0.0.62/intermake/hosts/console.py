...
from intermake.hosts.base import ERunMode, PluginHost, ERunStatus
...
class _ConsoleHostSettings:
    ...
    :attr console_width_override : Specifies the width of console.
                                   If this is zero (the default) the system is queried.
                                   This setting is used for word wrap and is not always respected.
    :attr clear_screen           : When set, the console display is cleared between command executions.
    :attr force_echo             : When set, commands are repeated back to the user.
                                   This is applied even if the user did not invoke the command themselves.
    :attr remove_ansi            : When set, ANSI colour codes are stripped from the output.
                                   This only applies to output relayed through Intermake, application specific
                                   output may not respect this setting.
    :attr hide_streams           : When set, streams are no longer shown to the left of the messages displayed in the
                                   console.
    :attr remove_utf             : When set, non-ASCII sequences are substituted in the output.
                                   This only applies to output relayed through Intermake, application specific
                                   output may not respect this setting.
    :attr show_external_streams  : Show output from running external applications.
    """
...
class ConsoleHostConfiguration:
    ...
    :attr clear_screen         : Clear screen between commands
    :attr force_echo           : Echo commands
    :attr report               : Reporting mode
    :attr use_dot              : Use `.` instead of `_`
    :attr keep_results         : Number of results to keep in history
    :attr cluster_index        : Index for compute clusters
    :attr cluster_count        : Count for compute clusters
    """
    ...
    def __init__( self, run_mode: ERunMode ):
        ...
        self.debug_error_traceback = False
...
class ConsoleHost( PluginHost, IProgressReceiver ):  # this host is always single-threaded so it acts as its own progress receiver
    ...
    def on_status_changed( self, status: ERunStatus ):
        if self.run_mode == ERunMode.JUP and status == ERunStatus.STOP:
    ...
    def on_run_host( self, args: RunHost ):
        ...
        if run_mode == ERunMode.ARG:
            ...
            code.interact( banner = cli_helper.format_banner( "Python Interactive", "help()", "cmdlist()" ), local = MENV.plugins.to_dictionary() )
        elif run_mode == ERunMode.JUP:
            builtins.__dict__.update( MENV.plugins.to_dictionary() )  # TODO: This isn't great, how do we get out of it?
    ...
    def console_settings( self ) -> _ConsoleHostSettings:
        if self.__settings is None:
            ...
            if not LocalData.is_ready():
                return _ConsoleHostSettings()
            
            self.__settings = MENV.local_data.store.bind( "console", _ConsoleHostSettings() )
    ...
    def call_virtual_run( self, plugin: Plugin, args: ArgsKwargs ) -> Optional[object]:
        ...
        * Raises exceptions in case of error, since everything is verbatim
        ...
        try:
            ...
            async_result = Result( plugin.name, result, None, None, mandate.get_message_records(), plugin )
        except BaseException as ex:
            async_result = Result( plugin.name, None, ex, exception_helper.get_traceback(), mandate.get_message_records(), plugin )
