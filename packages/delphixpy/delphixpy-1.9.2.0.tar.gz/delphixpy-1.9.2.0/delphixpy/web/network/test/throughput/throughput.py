...
def create(engine, network_throughput_test_parameters):
    ...
    :type network_throughput_test_parameters:
        :py:class:`delphixpy.web.vo.NetworkThroughputTestParameters`
    :rtype: ``basestring``
    """
...
def get(engine, ref):
    ...
    :param engine: The Delphix Engine
    :type engine: :py:class:`delphixpy.delphix_engine.DelphixEngine`
    :param ref: Reference to a :py:class:`delphixpy.web.objects.NetworkThroughp
        utTest.NetworkThroughputTest` object
    :type ref: ``str``
    :rtype: :py:class:`delphixpy.web.vo.NetworkThroughputTest`
    """
    assert API_VERSION == engine.API_VERSION, "Wrong API version (%s) for parameter 'engine' (%s)" % (API_VERSION, engine.API_VERSION)
    ...
    result = response_validator.validate(response, engine)
    raw_result = getattr(engine, 'raw_result', False)
    return response_validator.parse_result(result, undef_enabled=True, return_types=[u'NetworkThroughputTest'], returns_list=False, raw_result=raw_result)
...
def get_all(engine):
    ...
    :param engine: The Delphix Engine
    :type engine: :py:class:`delphixpy.delphix_engine.DelphixEngine`
    :rtype: ``list`` of :py:class:`delphixpy.web.vo.NetworkThroughputTest`
    """
    assert API_VERSION == engine.API_VERSION, "Wrong API version (%s) for parameter 'engine' (%s)" % (API_VERSION, engine.API_VERSION)
    url = "/resources/json/delphix/network/test/throughput"
    response = engine.get(url)
    result = response_validator.validate(response, engine)
    raw_result = getattr(engine, 'raw_result', False)
    return response_validator.parse_result(result, undef_enabled=True, return_types=[u'NetworkThroughputTest'], returns_list=True, raw_result=raw_result)
