...
def create(engine, network_dsp_test_parameters):
    ...
    :type network_dsp_test_parameters:
        :py:class:`delphixpy.web.vo.NetworkDSPTestParameters`
    :rtype: ``basestring``
    """
...
def get(engine, ref):
    ...
    :param ref: Reference to a
        :py:class:`delphixpy.web.objects.NetworkDSPTest.NetworkDSPTest` object
    :type ref: ``str``
    :rtype: :py:class:`delphixpy.web.vo.NetworkDSPTest`
    """
    assert API_VERSION == engine.API_VERSION, "Wrong API version (%s) for parameter 'engine' (%s)" % (API_VERSION, engine.API_VERSION)
    ...
    result = response_validator.validate(response, engine)
    raw_result = getattr(engine, 'raw_result', False)
    return response_validator.parse_result(result, undef_enabled=True, return_types=[u'NetworkDSPTest'], returns_list=False, raw_result=raw_result)
...
def get_all(engine):
    ...
    :param engine: The Delphix Engine
    :type engine: :py:class:`delphixpy.delphix_engine.DelphixEngine`
    :rtype: ``list`` of :py:class:`delphixpy.web.vo.NetworkDSPTest`
    """
    assert API_VERSION == engine.API_VERSION, "Wrong API version (%s) for parameter 'engine' (%s)" % (API_VERSION, engine.API_VERSION)
    url = "/resources/json/delphix/network/test/dsp"
    response = engine.get(url)
    result = response_validator.validate(response, engine)
    raw_result = getattr(engine, 'raw_result', False)
    return response_validator.parse_result(result, undef_enabled=True, return_types=[u'NetworkDSPTest'], returns_list=True, raw_result=raw_result)
