...
def get(engine, ref):
    ...
    :param ref: Reference to a :py:class:`delphixpy.web.objects.Job.Job` object
    :type ref: ``str``
    ...
    return response_validator.parse_result(result, undef_enabled=True, return_types=[u'Job'], returns_list=False, raw_result=raw_result)

def get_all(engine, to_date=None, page_offset=None, search_text=None, job_state=None, target=None, page_size=None, add_events=None, from_date=None, max_total=None, job_type=None):
    """
    ...
    :type job_type: ``basestring``
    :rtype: ``list`` of :py:class:`delphixpy.web.vo.Job`
    """
    assert API_VERSION == engine.API_VERSION, "Wrong API version (%s) for parameter 'engine' (%s)" % (API_VERSION, engine.API_VERSION)
    url = "/resources/json/delphix/job"
    query_params = {"toDate": to_date, "pageOffset": page_offset, "searchText": search_text, "jobState": job_state, "target": target, "pageSize": page_size, "addEvents": add_events, "fromDate": from_date, "maxTotal": max_total, "jobType": job_type}
    ...
    result = response_validator.validate(response, engine)
    raw_result = getattr(engine, 'raw_result', False)
    return response_validator.parse_result(result, undef_enabled=True, return_types=[u'Job'], returns_list=True, raw_result=raw_result)
...
def update(engine, ref, job=None):
    ...
    :type job: :py:class:`delphixpy.web.vo.Job`
    """
    assert API_VERSION == engine.API_VERSION, "Wrong API version (%s) for parameter 'engine' (%s)" % (API_VERSION, engine.API_VERSION)
    url = "/resources/json/delphix/job/%s" % ref
