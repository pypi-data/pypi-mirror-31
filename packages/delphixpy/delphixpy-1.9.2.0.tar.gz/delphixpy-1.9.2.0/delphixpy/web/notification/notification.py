...
def get_all(engine, max=None, channel=None, timeout=None):
    """
    ...
    :param timeout: Timeout, in milliseconds, to wait for one or more
        responses.
    :type timeout: ``basestring``
    :rtype: ``list`` of :py:class:`delphixpy.web.vo.Notification`
    """
    assert API_VERSION == engine.API_VERSION, "Wrong API version (%s) for parameter 'engine' (%s)" % (API_VERSION, engine.API_VERSION)
    url = "/resources/json/delphix/notification"
    query_params = {"max": max, "timeout": timeout, "channel": channel}
    ...
    return response_validator.parse_result(result, undef_enabled=True, return_types=[u'Notification'], returns_list=True, raw_result=raw_result)
