...
def create(engine, source_config=None):
    ...
    :type source_config: :py:class:`delphixpy.web.vo.SourceConfig`
    :rtype: ``basestring``
    """
...
def get(engine, ref):
    ...
    :param ref: Reference to a
        :py:class:`delphixpy.web.objects.SourceConfig.SourceConfig` object
    :type ref: ``str``
    ...
    result = response_validator.validate(response, engine)
    raw_result = getattr(engine, 'raw_result', False)
    return response_validator.parse_result(result, undef_enabled=True, return_types=[u'SourceConfig'], returns_list=False, raw_result=raw_result)

def get_all(engine, environment=None, repository=None, cdb_config=None, pdb_config_only=None):
    """
    ...
    :param cdb_config: Restrict PDB configs to those belonging to the specified
        CDB source config.
    :type cdb_config: ``basestring``
    :param pdb_config_only: Restrict source configs to be Oracle PDB configs
        only.
    :type pdb_config_only: ``bool``
    :rtype: ``list`` of :py:class:`delphixpy.web.vo.SourceConfig`
    """
    assert API_VERSION == engine.API_VERSION, "Wrong API version (%s) for parameter 'engine' (%s)" % (API_VERSION, engine.API_VERSION)
    url = "/resources/json/delphix/sourceconfig"
    query_params = {"environment": environment, "cdbConfig": cdb_config, "repository": repository, "pdbConfigOnly": pdb_config_only}
    ...
    result = response_validator.validate(response, engine)
    raw_result = getattr(engine, 'raw_result', False)
    return response_validator.parse_result(result, undef_enabled=True, return_types=[u'SourceConfig'], returns_list=True, raw_result=raw_result)
...
def validate_credentials(engine, ref, source_config_connectivity):
    ...
    :type source_config_connectivity:
        :py:class:`delphixpy.web.vo.SourceConfigConnectivity`
    """
    assert API_VERSION == engine.API_VERSION, "Wrong API version (%s) for parameter 'engine' (%s)" % (API_VERSION, engine.API_VERSION)
