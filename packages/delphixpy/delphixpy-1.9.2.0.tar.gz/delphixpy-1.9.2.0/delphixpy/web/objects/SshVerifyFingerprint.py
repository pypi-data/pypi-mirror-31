...
class SshVerifyFingerprint(SshVerifyBase):
    """
    *(extends* :py:class:`delphixpy.web.vo.SshVerifyBase` *)* SSH verification
    ...
    def from_dict(cls, data, dirty=False, undef_enabled=True):
        ...
        if obj._fingerprint_type[0] is not None and obj._fingerprint_type[0] is not obj.__undef__:
            assert isinstance(obj._fingerprint_type[0], basestring), ("Expected one of [u'string'], but got %s" % type(obj._fingerprint_type[0]))
            assert obj._fingerprint_type[0] in [u'SHA256', u'SHA512'], "Expected enum [u'SHA256', u'SHA512'] but got %s" % obj._fingerprint_type[0]
        ...
        if obj._fingerprint[0] is not None and obj._fingerprint[0] is not obj.__undef__:
            assert isinstance(obj._fingerprint[0], basestring), ("Expected one of [u'string'], but got %s" % type(obj._fingerprint[0]))
            common.validate_format(obj._fingerprint[0], "hostFingerprint", None, None)
        return obj

    def to_dict(self, dirty=False):
        dct = super(SshVerifyFingerprint, self).to_dict(dirty)
        ...
        def dictify(obj):
            if isinstance(obj, list):
                ...
                return obj
        if "fingerprint_type" == "type" or (self.fingerprint_type is not self.__undef__ and not (dirty and not self._fingerprint_type[1])):
            dct["fingerprintType"] = dictify(self.fingerprint_type)
        if "fingerprint" == "type" or (self.fingerprint is not self.__undef__ and not (dirty and not self._fingerprint[1])):
