#
# Copyright 2018 by Delphix
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#
#    http://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.
#

#
# This class has been automatically generated from:
#     /delphix-mssql-existing-specific-backup-sync-parameters.json
#
# Do not edit this file manually!
#

from delphixpy.web.objects.MSSqlExistingBackupSyncParameters import MSSqlExistingBackupSyncParameters
from delphixpy import common

class __Undef(object):
    def __repr__(self):
        return "undef"

_UNDEFINED = __Undef()

class MSSqlExistingSpecificBackupSyncParameters(MSSqlExistingBackupSyncParameters):
    """
    *(extends* :py:class:`delphixpy.web.vo.MSSqlExistingBackupSyncParameters`
    *)* The parameters to use as input to sync MSSQL databases using an
    existing specific full or differential backup.
    """
    def __init__(self, undef_enabled=True):
        super(MSSqlExistingSpecificBackupSyncParameters, self).__init__()
        self._type = ("MSSqlExistingSpecificBackupSyncParameters", True)
        self._backup_uuid = (self.__undef__, True)

    API_VERSION = "1.9.2"

    @classmethod
    def from_dict(cls, data, dirty=False, undef_enabled=True):
        obj = super(MSSqlExistingSpecificBackupSyncParameters, cls).from_dict(data, dirty, undef_enabled)
        if "backupUUID" not in data:
            raise ValueError("Missing required property \"backupUUID\".")
        obj._backup_uuid = (data.get("backupUUID", obj.__undef__), dirty)
        if obj._backup_uuid[0] is not None and obj._backup_uuid[0] is not obj.__undef__:
            assert isinstance(obj._backup_uuid[0], basestring), ("Expected one of [u'string'], but got %s" % type(obj._backup_uuid[0]))
            common.validate_format(obj._backup_uuid[0], "None", None, None)
        return obj

    def to_dict(self, dirty=False):
        dct = super(MSSqlExistingSpecificBackupSyncParameters, self).to_dict(dirty)

        def dictify(obj):
            if isinstance(obj, list):
                return [dictify(o) for o in obj]
            elif hasattr(obj, "to_dict"):
                return obj.to_dict()
            else:
                return obj
        if "backup_uuid" == "type" or (self.backup_uuid is not self.__undef__ and not (dirty and not self._backup_uuid[1])):
            dct["backupUUID"] = dictify(self.backup_uuid)
        return dct

    def dirty(self):
        return self.from_dict(self.to_dict(dirty=False), dirty=True)

    def force_dirty(self):
        self._backup_uuid = (self._backup_uuid[0], True)

    def is_dirty(self):
        return any([self._backup_uuid[1]])

    def __eq__(self, other):
        if other is None:
            return False
        if not isinstance(other, MSSqlExistingSpecificBackupSyncParameters):
            return False
        return super(MSSqlExistingSpecificBackupSyncParameters, self).__eq__(other) and \
               self.backup_uuid == other.backup_uuid

    def __ne__(self, other):
        return not self.__eq__(other)

    def __repr__(self):
        return common.generate_repr_string(self)

    @property
    def backup_uuid(self):
        """
        The UUID of the full or differential backup of the source database to
        restore from. The backup should be present in the shared backup
        location for the source database.

        :rtype: ``basestring``
        """
        return self._backup_uuid[0]

    @backup_uuid.setter
    def backup_uuid(self, value):
        self._backup_uuid = (value, True)

