...
class HttpConnectorConfig(TypedObject):
    """
    *(extends* :py:class:`delphixpy.web.vo.TypedObject` *)* Configuration for
    ...
    def from_dict(cls, data, dirty=False, undef_enabled=True):
        ...
        if obj._http_mode[0] is not None and obj._http_mode[0] is not obj.__undef__:
            assert isinstance(obj._http_mode[0], basestring), ("Expected one of [u'string'], but got %s" % type(obj._http_mode[0]))
            assert obj._http_mode[0] in [u'HTTP_ONLY', u'HTTPS_ONLY', u'HTTP_REDIRECT', u'BOTH'], "Expected enum [u'HTTP_ONLY', u'HTTPS_ONLY', u'HTTP_REDIRECT', u'BOTH'] but got %s" % obj._http_mode[0]
        ...
        for item in data.get("tlsVersions") or []:
            ...
            assert item in [u'SSLv3', u'TLSv1', u'TLSv1_1', u'TLSv1_2'], "Expected enum [u'SSLv3', u'TLSv1', u'TLSv1_1', u'TLSv1_2'] but got %s" % item
    ...
    def to_dict(self, dirty=False):
        dct = super(HttpConnectorConfig, self).to_dict(dirty)
        ...
        def dictify(obj):
            if isinstance(obj, list):
                ...
                return obj
        if "http_mode" == "type" or (self.http_mode is not self.__undef__ and not (dirty and not self._http_mode[1])):
            dct["httpMode"] = dictify(self.http_mode)
        if "tls_versions" == "type" or (self.tls_versions is not self.__undef__ and not (dirty and not self._tls_versions[1])):
            dct["tlsVersions"] = dictify(self.tls_versions)
        return dct
