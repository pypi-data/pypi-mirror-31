...
class SshVerifyBase(SshVerificationStrategy):
    """
    *(extends* :py:class:`delphixpy.web.vo.SshVerificationStrategy` *)* Base
    for ssh verification strategies that use the host's key.
    """
    ...
    def from_dict(cls, data, dirty=False, undef_enabled=True):
        ...
        if obj._key_type[0] is not None and obj._key_type[0] is not obj.__undef__:
            assert isinstance(obj._key_type[0], basestring), ("Expected one of [u'string'], but got %s" % type(obj._key_type[0]))
            assert obj._key_type[0] in [u'RSA', u'DSA', u'ECDSA', u'ED25519'], "Expected enum [u'RSA', u'DSA', u'ECDSA', u'ED25519'] but got %s" % obj._key_type[0]
            common.validate_format(obj._key_type[0], "None", None, None)
        return obj

    def to_dict(self, dirty=False):
        dct = super(SshVerifyBase, self).to_dict(dirty)
        ...
        def dictify(obj):
            if isinstance(obj, list):
                ...
                return obj
        if "key_type" == "type" or (self.key_type is not self.__undef__ and not (dirty and not self._key_type[1])):
