...
#     /delphix-auth-get-by-properties-parameters.json
#
...
class AuthGetByPropertiesParameters(TypedObject):
    """
    *(extends* :py:class:`delphixpy.web.vo.TypedObject` *)* The parameters to
    use as input to get an authorization with given user, target, and role.
    """
    def __init__(self, undef_enabled=True):
        super(AuthGetByPropertiesParameters, self).__init__()
        self._type = ("AuthGetByPropertiesParameters", True)
    ...
    def from_dict(cls, data, dirty=False, undef_enabled=True):
        obj = super(AuthGetByPropertiesParameters, cls).from_dict(data, dirty, undef_enabled)
        if "role" not in data:
            raise ValueError("Missing required property \"role\".")
    ...
    def to_dict(self, dirty=False):
        dct = super(AuthGetByPropertiesParameters, self).to_dict(dirty)
    ...
    def is_dirty(self):
        return any([self._role[1], self._user[1], self._target[1]])

    def __eq__(self, other):
        if other is None:
            return False
        if not isinstance(other, AuthGetByPropertiesParameters):
            return False
        return super(AuthGetByPropertiesParameters, self).__eq__(other) and \
