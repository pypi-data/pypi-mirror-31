...
class UnixHost(Host):
    """
    *(extends* :py:class:`delphixpy.web.vo.Host` *)* The representation of a
    Unix host object.
    """
    def __init__(self, undef_enabled=True):
        ...
        self._user_truststore_password = (self.__undef__, True)
    ...
    def from_dict(cls, data, dirty=False, undef_enabled=True):
        ...
        obj._certificate_chain = []
        for item in data.get("certificateChain") or []:
            assert isinstance(item, basestring), ("Expected one of [u'string'], but got %s" % type(item))
            common.validate_format(item, "None", None, None)
            obj._certificate_chain.append(item)
        obj._certificate_chain = (obj._certificate_chain, dirty)
        obj._user_truststore_password = (data.get("userTruststorePassword", obj.__undef__), dirty)
        if obj._user_truststore_password[0] is not None and obj._user_truststore_password[0] is not obj.__undef__:
            assert isinstance(obj._user_truststore_password[0], basestring), ("Expected one of [u'string'], but got %s" % type(obj._user_truststore_password[0]))
            common.validate_format(obj._user_truststore_password[0], "password", 1, None)
        return obj
    ...
    def to_dict(self, dirty=False):
        ...
        if "certificate_chain" == "type" or (self.certificate_chain is not self.__undef__ and not (dirty and not self._certificate_chain[1])):
            dct["certificateChain"] = dictify(self.certificate_chain)
        if dirty and "certificateChain" in dct:
            del dct["certificateChain"]
        if "user_truststore_password" == "type" or (self.user_truststore_password is not self.__undef__ and not (dirty and not self._user_truststore_password[1])):
            dct["userTruststorePassword"] = dictify(self.user_truststore_password)
        return dct
    ...
    def force_dirty(self):
        ...
        self._user_truststore_password = (self._user_truststore_password[0], True)

    def is_dirty(self):
        return any([self._toolkit_path[1], self._ssh_verification_strategy[1], self._certificate_chain[1], self._user_truststore_password[1]])
    ...
    def __eq__(self, other):
        ...
        return super(UnixHost, self).__eq__(other) and \
               ...
               self.ssh_verification_strategy == other.ssh_verification_strategy and \
               self.certificate_chain == other.certificate_chain and \
               self.user_truststore_password == other.user_truststore_password
    ...
    def ssh_verification_strategy(self):
        ...
        :rtype: :py:class:`delphixpy.web.vo.SshVerificationStrategy`
        """
    ...
    @property
    def user_truststore_password(self):
        """
        The password for the user managed truststore.

        :rtype: ``basestring``
        """
        return self._user_truststore_password[0]

    @user_truststore_password.setter
    def user_truststore_password(self, value):
        self._user_truststore_password = (value, True)
