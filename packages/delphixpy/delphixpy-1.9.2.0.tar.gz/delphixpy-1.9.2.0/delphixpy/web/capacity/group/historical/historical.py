...
def get_all(engine, start_date=None, group=None, resolution=None, end_date=None):
    """
    ...
    :rtype: ``list`` of
        :py:class:`delphixpy.web.vo.HistoricalGroupCapacityData`
    """
    assert API_VERSION == engine.API_VERSION, "Wrong API version (%s) for parameter 'engine' (%s)" % (API_VERSION, engine.API_VERSION)
    url = "/resources/json/delphix/capacity/group/historical"
    query_params = {"startDate": start_date, "group": group, "resolution": resolution, "endDate": end_date}
    ...
    return response_validator.parse_result(result, undef_enabled=True, return_types=[u'HistoricalGroupCapacityData'], returns_list=True, raw_result=raw_result)
