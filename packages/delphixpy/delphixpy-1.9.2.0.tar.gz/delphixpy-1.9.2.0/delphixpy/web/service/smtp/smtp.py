...
def get(engine):
    ...
    :rtype: :py:class:`delphixpy.web.vo.SMTPConfig`
    """
    ...
    return response_validator.parse_result(result, undef_enabled=True, return_types=[u'SMTPConfig'], returns_list=False, raw_result=raw_result)
...
def set(engine, smtp_config=None):
    ...
    :type smtp_config: :py:class:`delphixpy.web.vo.SMTPConfig`
    """
    assert API_VERSION == engine.API_VERSION, "Wrong API version (%s) for parameter 'engine' (%s)" % (API_VERSION, engine.API_VERSION)
    url = "/resources/json/delphix/service/smtp"
...
def validate(engine, validate_smtp_parameters):
    ...
    :type validate_smtp_parameters:
        :py:class:`delphixpy.web.vo.ValidateSMTPParameters`
    """
    assert API_VERSION == engine.API_VERSION, "Wrong API version (%s) for parameter 'engine' (%s)" % (API_VERSION, engine.API_VERSION)
