...
def get(engine):
    ...
    :rtype: :py:class:`delphixpy.web.vo.HttpConnectorConfig`
    """
    ...
    return response_validator.parse_result(result, undef_enabled=True, return_types=[u'HttpConnectorConfig'], returns_list=False, raw_result=raw_result)
...
def set(engine, http_connector_config=None):
    ...
    :type http_connector_config:
        :py:class:`delphixpy.web.vo.HttpConnectorConfig`
    """
    assert API_VERSION == engine.API_VERSION, "Wrong API version (%s) for parameter 'engine' (%s)" % (API_VERSION, engine.API_VERSION)
    url = "/resources/json/delphix/service/httpConnector"
