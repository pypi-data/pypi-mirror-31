...
def get(engine):
    ...
    :rtype: :py:class:`delphixpy.web.vo.UserInterfaceConfig`
    """
    ...
    return response_validator.parse_result(result, undef_enabled=True, return_types=[u'UserInterfaceConfig'], returns_list=False, raw_result=raw_result)
...
def set(engine, user_interface_config=None):
    ...
    :type user_interface_config:
        :py:class:`delphixpy.web.vo.UserInterfaceConfig`
    """
    assert API_VERSION == engine.API_VERSION, "Wrong API version (%s) for parameter 'engine' (%s)" % (API_VERSION, engine.API_VERSION)
    url = "/resources/json/delphix/service/userInterface"
