...
def get(engine):
    ...
    :rtype: :py:class:`delphixpy.web.vo.DNSConfig`
    """
    ...
    return response_validator.parse_result(result, undef_enabled=True, return_types=[u'DNSConfig'], returns_list=False, raw_result=raw_result)
...
def set(engine, dns_config=None):
    ...
    :type dns_config: :py:class:`delphixpy.web.vo.DNSConfig`
    """
    assert API_VERSION == engine.API_VERSION, "Wrong API version (%s) for parameter 'engine' (%s)" % (API_VERSION, engine.API_VERSION)
    url = "/resources/json/delphix/service/dns"
