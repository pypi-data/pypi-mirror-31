...
def get(engine):
    ...
    :rtype: :py:class:`delphixpy.web.vo.ProxyService`
    """
    ...
    return response_validator.parse_result(result, undef_enabled=True, return_types=[u'ProxyService'], returns_list=False, raw_result=raw_result)
...
def set(engine, proxy_service=None):
    ...
    :type proxy_service: :py:class:`delphixpy.web.vo.ProxyService`
    """
    assert API_VERSION == engine.API_VERSION, "Wrong API version (%s) for parameter 'engine' (%s)" % (API_VERSION, engine.API_VERSION)
    url = "/resources/json/delphix/service/proxy"
