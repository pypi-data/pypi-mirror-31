...
def get(engine):
    ...
    :rtype: :py:class:`delphixpy.web.vo.GlobalLinkingSettings`
    """
    ...
    return response_validator.parse_result(result, undef_enabled=True, return_types=[u'GlobalLinkingSettings'], returns_list=False, raw_result=raw_result)
...
def set(engine, global_linking_settings=None):
    ...
    :type global_linking_settings:
        :py:class:`delphixpy.web.vo.GlobalLinkingSettings`
    """
    assert API_VERSION == engine.API_VERSION, "Wrong API version (%s) for parameter 'engine' (%s)" % (API_VERSION, engine.API_VERSION)
    url = "/resources/json/delphix/service/linkingsettings"
