...
def get(engine):
    ...
    :rtype: :py:class:`delphixpy.web.vo.SyslogConfig`
    """
    ...
    return response_validator.parse_result(result, undef_enabled=True, return_types=[u'SyslogConfig'], returns_list=False, raw_result=raw_result)
...
def set(engine, syslog_config=None):
    ...
    :type syslog_config: :py:class:`delphixpy.web.vo.SyslogConfig`
    """
    assert API_VERSION == engine.API_VERSION, "Wrong API version (%s) for parameter 'engine' (%s)" % (API_VERSION, engine.API_VERSION)
    url = "/resources/json/delphix/service/syslog"
