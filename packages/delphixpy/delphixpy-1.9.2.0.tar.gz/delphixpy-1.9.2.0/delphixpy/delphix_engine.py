#
# Copyright 2014, 2018 by Delphix
#
...
class HttpSession(object):
    ...
    def _handle_response(self, (status, headers, data), request_method,
                         request_url, json_data=""):
        """
        Handle a response from a HTTP request

        :param (status, headers, data): HTTP status as an integer e.g. 404,
                                        HTTP headers as a dictionary,
                                        HTTP data as raw data.
...
class DelphixEngine(object):
    ...
    def __init__(self, address, user, password, namespace):
        ...
        self._http_session = HttpSession(address)
