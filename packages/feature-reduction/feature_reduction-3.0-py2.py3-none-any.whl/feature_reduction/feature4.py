...
# In[41]:
def LayerFourSelection(fourInputData,resultIndex):
    ...
    if nFeatures > 1:
        ...
        if pcaResultIndex.rmse <= pcaRmse:
            ...
            '''
            db.outputparameter.insert({"n_components":n_components,
...
def feature_selection(n_samples, n_features, data, target, feature_names, username):
    ...
    for i in oneInputDatadict1:
        if "expert_remain3" in i.keys():
            expert_remain3 = i["expert_remain3"]
            print(expert_remain3)
            continue
    ...
    for train_index, test_index in kf:
        ...
        if no_target == False:
            oneOutputData = LayerFourSelection(oneInputData,orgResultIndex)
            print(oneOutputData)
