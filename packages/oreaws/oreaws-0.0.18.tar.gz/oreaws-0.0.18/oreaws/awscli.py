...
def describe_instances(ctx, instance_id):
    oreaws.ec2.describe_instances(ctx, instance_id)
...
def start_instances(ctx, instance_id):
    oreaws.ec2.start_instances(ctx, instance_id)
...
def stop_instances(ctx, instance_id):
    oreaws.ec2.stop_instances(ctx, instance_id)

## AMI List ##
@ec2.command(help='Amazon Linux Image List API')
@click.pass_context
def describe_ami(ctx):
    oreaws.ec2.describe_ami(ctx)
...
def create_ami(ctx, instance_id, aminame):
    oreaws.ec2.create_ami(ctx, instance_id, aminame)
...
def delete_ami(ctx, imageid):
    oreaws.ec2.delete_ami(ctx, imageid)
...
def describe_asg(ctx):
    oreaws.autoscaling.describe_asg(ctx)
...
def update_max(ctx, asgname, max):
    oreaws.autoscaling.update_max(ctx, asgname, max)
...
def update_min(ctx, asgname, min):
    oreaws.autoscaling.update_min(ctx, asgname, min)
...
def update_desire(ctx, asgname, desire):
    oreaws.autoscaling.update_desire(ctx, asgname, desire)
...
def describe_zones(ctx):
    oreaws.route53.describe_zones(ctx)
...
def describe_records(ctx, zone_id):
    oreaws.route53.describe_records(ctx, zone_id)
