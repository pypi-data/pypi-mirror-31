...
def acc_plot(obj,title='',xTitle='Date',yTitle='',mode='lines+markers',tablewidth=350,asFigure=False):
    ...
    try:
        ...
        #separate logic if input is dataframe
            ...
            normalized_df=obj.T.apply(lambda x: x/x.max(), axis=0)
            fig = normalized_df.iplot(kind='scatter',mode=mode,asFigure=True)
            ...
            for error_measure in fig['data']:
                ...
                error_measure['text'] = np.around(obj.T[error_measure['name']].values,decimals=4)
