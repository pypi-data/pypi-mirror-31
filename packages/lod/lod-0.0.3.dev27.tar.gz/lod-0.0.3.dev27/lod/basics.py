...
class Identifier:
    ...
    Some objects can actually be uniquely identified by their name. In those cases, if both an ID and a name are given, the ID takes precedence. Until an Identifier has been processed by the server, it can not be properly compared to another Identifier: One of them could have been specified by the name and one by the ID. The comparison will only work once the server has processed the Identifier and filled in the missing fields.
    """
...
class Rule:
    ...
    """
    def __init__(self, identifier, creator_id, description, dependencies, threshold, trigger_rule, actions, existing_variables, rating_numerator, rating_count):
        self.identifier = identifier
        self.creator_id = creator_id
        self.description = description
...
def parse_rule(dictionary):
    ...
    rating_numerator = dictionary['rating_numerator']
    rating_count = dictionary['rating_count']
    res = Rule(identifier, creator_id, description, dependencies, threshold, trigger_rule, actions, existing_variables, rating_numerator, rating_count)
    return res
