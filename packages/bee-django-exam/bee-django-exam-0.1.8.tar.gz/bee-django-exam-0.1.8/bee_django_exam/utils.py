...
CERT_FIELD_LIST = [{"name": "考级名称", "field": "bee_django_exam_grade_name"},
...
def get_user_name(user_id):
    try:
        ...
        return getattr(user, settings.EXAM_USER_NAME_FIELD)
    except:
        return None
...
class Cert(object):
    ...
    def create_text_img(self, field):
        ...
        if not text:
            return None
        img_size = (field.text_width + self.text_padding, field.font_size + self.text_padding)
        ...
        dr.text(self.get_text_pos_tuple(field, text), text, font=font, fill=font_color)
        return text_img
    ...
    # 获取文字在文字图片中的坐标
    def get_text_pos_tuple(self, field, text):
...
def check_user_record(user_id):
    user_records = UserExamRecord.objects.filter(user__id=user_id).filter(Q(status__isnull=True) | Q(status=0))
    if user_records.count() >= 1:
        return "您当前已有一个未完成考级"
    else:
        return None
