...
from .app import UrlVoidApiService
...
if len(sys.argv) != 2:
    print("Usage: dxlurlvoidservice <configuration files directory>")
    sys.exit(1)
...
logging_config_path = os.path.join(config_dir, UrlVoidApiService.LOGGING_CONFIG_FILE)
...
with UrlVoidApiService(sys.argv[1]) as app:
    try:
        # Run the application
        app.run()
        running = True
