...
class Spark(object):
    ...
    def filter(self, filters):
        """
        Filter dataframe
        :param filters: filters in format of dictionary (column: [filter, operation]). Only filters by equal, by now [Dictionary]
        :return:
        """
        try:
            for k, v in filters.items():
                condition = "{column} {operator} '{value}'".format(column=k, operator=v[1], value=v[0])
