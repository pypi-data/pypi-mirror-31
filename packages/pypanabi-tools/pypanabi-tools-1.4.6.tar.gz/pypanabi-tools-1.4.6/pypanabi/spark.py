...
class Spark(object):
    ...
    def concatenate(self, columns, concatenation_column_name='concat', remove_concatenated_columns=False):
        ...
        try:
            ...
            if remove_concatenated_columns:
                self._dataframe = self.remove_columns(columns)
    ...
    def order(self, columns_in_order):
        """
        Define specific order for the columns
        :param columns_in_order: list of columns in the desired order [List<String>]
        :return: date with columns in the desired order [Dataframe]
        """
        try:
            self._dataframe = self._dataframe.select(columns_in_order)
            return self._dataframe
        except Exception as ex:
            self._logger.error(repr(ex))
            raise
    ...
    def add(self, column_name, value):
        ...
        """
        try:
            self._dataframe = self._dataframe.withColumn(column_name, value)
            return self._dataframe
        except Exception as ex:
            self._logger.error(repr(ex))
            raise
    ...
    def filter(self, filters):
        """
        Filter dataframe
        :param filters: filters in format of dictionary (column: filter). Only filters by equal, by now [Dictionary]
        :return:
        """
