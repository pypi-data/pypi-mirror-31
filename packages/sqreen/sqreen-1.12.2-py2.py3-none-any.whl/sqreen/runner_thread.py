...
from ._vendors.urllib3.exceptions import MaxRetryError
...
BLACKLISTED_COMMANDS = BLACKLISTED_PARENT_COMMANDS + (
    'newrelic-admin',
)
...
def start():
    ...
    for blacklisted_command in BLACKLISTED_COMMANDS:
        if blacklisted_command in command:
            LOGGER.debug('Sqreen agent is disabled when running %s.',
                         blacklisted_command)
            return
...
class RunnerThread(Thread):
            ...
            try:
                ...
                while True:
                    try:
                        ...
                        LOGGER.exception("An HTTP error occured, restarting runner")
                        try:
                ...
                if session is not None and session.is_connected():
                    try:
                        ...
                        LOGGER.exception("An HTTP error occured while logout")
                    except Exception:
    ...
    @classmethod
    def _random_sleep(cls):
        """Sleep for a random duration before trying again."""
        delay = randint(0, 10)
        LOGGER.info("Sleeping %s seconds before retry", delay)
        time.sleep(delay)
