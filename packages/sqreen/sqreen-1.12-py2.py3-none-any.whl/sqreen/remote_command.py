...
def _load_rules(runner, params=None, check_signature=True):
...
def actions_reload(runner, _):
    ...
    LOGGER.debug('Reloaded actions %s' % actions)

    unsupported = ACTION_STORE.reload_from_dicts(actions)
    if unsupported:
        return {
            'status': False,
            'output': {
