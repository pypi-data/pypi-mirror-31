...
class Commands(object):
    ...
    def module_get_scm_info(self, scm_url=None, branch=None, check_repo=True):
        ...
        :kwarg check_repo: a boolean that determines if check_repo should be
        run when an scm_url is not provided.
    ...
    def module_local_build(self, file_path, stream, local_builds_nsvs=None, verbose=False,
        ...
        :kwarg mbs_config: a string, path to alternative MBS config file to use.
        :kwarg mbs_config_section: a string, name of alternative config section to use.
        :kwargs default_streams: a list, contains strings with default name:stream pairs
        which are passed to mbs-manager using the '-s' command line argument.
        :return: None
        """
        command = ['mbs-manager']
        if self.quiet:
    ...
    def module_submit_build(self, api_url, scm_url, branch, auth_method,
        ...
        if not resp.ok:
            try:
                ...
                raise rpkgError('The build failed with an unexpected error')
            if 'message' in data:
    ...
    def module_watch_build(self, api_url, build_ids):
        ...
        if len(build_ids) > 1:
            warning = (
                'WARNING: Module builds {0} and {1} were generated using '
                'stream expansion. Only module build {2} is being watched.'
                .format(', '.join([str(b_id) for b_id in build_ids[0:-1]]),
                        str(build_ids[-1]), str(build_id)))
        # Load the Koji session anonymously so we get access to the Koji web
        # URL
        self.load_kojisession(anon=True)
        done = False
