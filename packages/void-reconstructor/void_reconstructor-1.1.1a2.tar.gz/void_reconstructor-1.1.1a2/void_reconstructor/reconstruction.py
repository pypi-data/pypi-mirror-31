...
class Reconstructor:
    ...
    def __init__(self, X,ID,mass,particle_mass = None, V = None):
    ...
    def get_grid(start=[], stop=[], Resolution=8):
        ...
        y_init = np.arange(start[1], stop[1], step[0]) + 0.5 * step[1]
        z_init = np.arange(start[2], stop[2], step[0]) + 0.5 * step[2]
    ...
    def get_final_position(self, start=[0, 0, 0], stop=[100, 100, 100], Resolution=8, mirror_length=0,seed=1,box_size=100.0):
        ...
        X = X[iran, :]
        ID = ID[iran]
        X_mirror = self.mirror_positions(X,mirror_length=mirror_length,box_size=box_size)
    ...
    def full_reconstruction_nested(self,division_number,alpha,Resolution_full, Resolution_hom,beta,starts_all = None,rank=0,box_sizes_return = False,box_size=100.0):
        ...
        for i, start in enumerate(starts_all[:,0:3]):
            ...
            X, ID, X_mirror = self.get_final_position(start_final, stop_final, Resolution=Resolution,
                                                      mirror_length=length_final,box_size=box_size)
            ...
            print("Rank %d: Finished task %d of %d" % (rank, i, N_tasks))

        box_sizes = np.asarray([box_size] * len(ID_tot)) #to keep track of the volume size in which a particle is
        if box_sizes_return == True:
            return X_r_tot,ID_tot, box_sizes
        else:
            return X_r_tot, ID_tot
