...
class GrainedGeneticAlgorithmBase(GeneticAlgorithmBase):
    ...
    def _start_MPI(self, channels):
        ...
        msg = Messenger(server_user=self._server_user, server_password=self._server_password,
                        ...
                        queue=str(uuid.uuid4()), subscription_routing_keys=self._queues_to_consume,
    ...
    @log_method()
    @timeout(60)
    def _collect_data(self, current_generation, write):
        """
        ...
        while True:
            text_file = open(write, "w")

            text_file.write(str(time.strftime("%Y-%m-%d %H:%M:%S", time.gmtime())) + " " + str(
                current_generation) + " z " + str(
                self._number_of_generations)
                            + " pocet prijatych je " + str(len(neighbours.objects))
                            + " neighbours " + str(neighbours.objects))
            text_file.close()
...
#
           ...
           # text_file.close()
            ...
            received_data= self._Individuals()
            received_data.append_object(self._Individual(float(8), self._gen_individual()),
                                     source='3')
            #self._collect_data(generation, write)
        ...
        #self._stop_MPI()
        return [self._Individual(9, self._gen_individual())]
