...
class CoarseGrainedBase(geneticGrainedBase.GrainedGeneticAlgorithmBase):
    ...
    def _send_individuals_reproduce(self):
        ...
        while len(chromosomes_reproducing) >= 2:
            fatherr = chromosomes_reproducing.pop(random.randrange(len(
                 chromosomes_reproducing))).chromosome
            motherr = chromosomes_reproducing.pop(random.randrange(len(
                 chromosomes_reproducing))).chromosome

            father = [0,0,1,1]
            mother = [1,1,0,0]
            #logger.info("father " + str(father) + " mother " + str(mother))
            father, mother = self._crossover(father, mother)
            # mutate
            father = self._mutation(father)
            mother = self._mutation(mother)
            tmp.append(father)
            tmp.append(mother)
