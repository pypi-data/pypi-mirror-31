...
class CoarseGrainedBase(geneticGrainedBase.GrainedGeneticAlgorithmBase):
    ...
    def _send_individuals_reproduce(self):
        ...
        if best_individual is not None:
            logger.info("Ultimete best individual was found.")
        ...
        logger.warning("Based on fitness, no individuals were pseudorandomly chosen." +
                       "Maybe it is the fitness function what is wrong.")
    ...
    def _check_collected_data(self, neighbours):
        if len(neighbours.objects) >= self._neighbourhood_size:
    ...
    def _finish_processing(self, neighbouring_individuals):
        ...
        """
        logger.info("Number of neighbouring individuals chosen is " + str(len(
            neighbouring_individuals.individuals)))
