...
class GeneticAlgorithmBase(object):
    ...
    @log_method()
    def _choose_individuals_based_on_fitness(self, evaluation_data, individuals_required=1):
        ultimate_min = 0
        ultimate_prob = 1
        ...
        while len(chromosomes_reproducing.individuals) < individuals_required:
            logger.info("KURVAAAAAAAAAAAAAAAAA " + str(individuals_to_choose_from))
            self.pseudo_randomly_choose_individuals(chromosomes_reproducing, individuals_to_choose_from,
                                                fitness_min, ultimate_min, ultimate_prob)
        return chromosomes_reproducing

    def pseudo_randomly_choose_individuals(self, chromosomes_reproducing, evaluation_data,
                                           fitness_min, ultimate_min, ultimate_prob):
        for chromosome_data in evaluation_data:
            ...
            logger.info("PROB is " + str(prob) + " chrom is " + str(chromosome_data.chromosome)
                        + " fit is " + str(chromosome_data.fit))
