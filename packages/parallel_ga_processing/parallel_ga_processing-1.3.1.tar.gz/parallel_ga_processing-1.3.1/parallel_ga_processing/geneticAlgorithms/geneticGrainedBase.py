...
class GrainedGeneticAlgorithmBase(GeneticAlgorithmBase):
    ...
    @log_method()
    def _store_specific_initial_data(self, initial_data):
        raise NotImplementedError

    @log_method()
    def _store_initial_data(self, initial_data, messenger):
        self._store_specific_initial_data(initial_data)
        self._producing_queue = messenger.produced_messages
        self._data_consuming_queue = messenger.consumed_messages
        self._messenger = messenger
        self._queue_to_produce = messenger.production_routing_keys

    def __call__(self, initial_data, messanger):
        to_return = []
        self._store_initial_data(initial_data, messanger)
