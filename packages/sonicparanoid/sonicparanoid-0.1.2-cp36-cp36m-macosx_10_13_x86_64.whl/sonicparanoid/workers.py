...
def check_storage_for_mmseqs_dbs(outDir, reqSp=3, gbPerSpecies=1.2, debug=False):
    ...
    tmpOut = subprocess.run(['df', '-m', outDir], stdout=subprocess.PIPE)
    # get the line with the required information
    dfLn = tmpOut.stdout.decode('utf-8').split('\n')[1]
    # List to contain only the non-empty parts of the output string
    flds = []
    for subStr in dfLn.split(' '):
        if len(subStr) > 0:
            flds.append(subStr)
    diskName = flds[0]
    availSpaceGb = round(float(flds[3]) / 1000., 2)
    ...
    if requiredSpaceGb >= availSpaceGb:
        ...
        #colout.colored_log(outLn=infoLn, lnType='i', debug=debug)
        ...
        #colout.colored_log(outLn=infoLn, lnType='w', debug=debug)
        print('Please consider freeing some disk space to take advantage of MMseqs2 index files.')
    if debug:
        print('Disk name: {:s}'.format(diskName))
        print('Available storage (Gigabytes): {:0.2f}'.format(availSpaceGb))
...
def consume_mmseqs_search_1cpu(jobs_queue, results_queue, inDir, outDir, dbDir, keepAlign, sensitivity, cutoff):
    while True:
        ...
        # it MUST use 1 CPU
        parsedOutput, search_time, convert_time, parse_time, tot_time = mmseqs_1pass(inSeq, dbSeq, dbDir=dbDir, outDir=outDir, tmpDirName=tmpMMseqsDirName, keepAlign=keepAlign, sensitivity=sensitivity, evalue=1000, cutoff=cutoff, threads=1, debug=False)
...
def mmseqs_1pass(inSeq, dbSeq, dbDir=os.getcwd(), outDir=os.getcwd(), tmpDirName=None, keepAlign=False, sensitivity=4.0, evalue=1000, cutoff=40, threads=4, debug=False):
    ...
    #parse_time = round(time.process_time() - start_time, 2)
...
def mmseqs_createindex(dbPath, debug=False):
    ...
    outDir = '{:s}/tmp_{:s}/'.format(os.path.dirname(dbPath), os.path.basename(dbPath.split('.', 1)[0]))
    systools.makedir(outDir)
    makeIdxCmd = '{:s} createindex {:s} {:s} --threads 2 -v 0'.format(get_mmseqs_path(), dbPath, outDir)
    #'''
    # command to be executed
...
def mmseqs_search(inSeq, dbSeq, dbDir=os.getcwd(), outDir=os.getcwd(), tmpDirName=None, sensitivity=4.0, evalue=1000, threads=4, cleanUp=False, debug=False):
    ...
    #searchCmd = '%s search %s %s %s %s -s %s --threads %d -v 0 --min-ungapped-score 30'%(get_mmseqs_path(), queryDBpath, targetDBpath, rawOutPath, tmpDir, str(sensitivity), threads)
    searchCmd = '{:s} search {:s} {:s} {:s} {:s} -s {:s} --threads {:d} -v 0 --min-ungapped-score {:d}'.format(get_mmseqs_path(), queryDBpath, targetDBpath, rawOutPath, tmpDir, str(sensitivity), threads, minUngappedScore)
    if debug:
...
def perform_mmseqs_multiproc_alignments(required_align, inDir, outDir, dbDir, create_idx=True, sensitivity=6, cutoff=0.5, threads=4, keepAlign=False, debug=False):
    ...
    if create_idx:
        create_idx = check_storage_for_mmseqs_dbs(outDir, reqSp=len(reqSpDict), gbPerSpecies=1.2, debug=debug)

    # create databases
    sys.stdout.write('\nCreating {:d} MMseqs2 databases...'.format(len(reqSpDict)))
    ...
    # call the method inside workers
    runningJobs = [mp.Process(target=consume_mmseqs_search_1cpu, args=(align_queue, results_queue, inDir, outDir, dbDir, keepAlign, sensitivity, cutoff)) for i_ in range(threads)]
