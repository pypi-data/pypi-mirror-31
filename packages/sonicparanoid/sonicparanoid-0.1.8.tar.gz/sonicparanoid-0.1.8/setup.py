...
minor_pyver = int(sys.version_info.minor)
major_pyver = int(sys.version_info.major)
curr_pyver = round(float('{:d}.{:d}'.format(major_pyver, minor_pyver)), 1)

if curr_pyver < min_python_ver:
    sys.stderr.write('\nERROR: SonicParanoid requires a version of Python equal or higher than {:s}, while you are using version {:s}:\nPlease update your Python3 installation.\n'.format(str(min_python_ver), curr_pyver))
    sys.exit(-5)
