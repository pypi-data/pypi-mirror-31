...
def get_params():
    ...
    parser.add_argument('-m', '--mode', required=False, help='SonicParanoid execution mode. The default mode is suitable for most studies. Use sensitive if the input proteomes are not closely related.', choices=['fast', 'default', 'sensitive'], default='default')
...
def check_input_files(inputDir, debug=False):
    ...
    for f in flist:
        ...
        if '.' in f:
            ...
            sys.stderr.write('Your input file names must not contain any \'.\' in its name, please remane your proteome files accordingly.\n')
            sys.exit(-5)
...
def check_mmseqs_installation(root):
    ...
    if not os.path.isfile(mmseqsPath):
        sys.stderr.write('\nERROR: the MMseqs2 version supported by SonicParanoid was not found.\n')
        ...
        if currentVer is not None:
            if currentVer != supportedVer:
                sys.stderr.write('\nERROR: the provided MMseqs2 version is different from the version supported by SonicParanoid ({:s}).\n'.format(supportedVer))
                sys.stderr.write('Please go to\nhttp://iwasakilab.bs.s.u-tokyo.ac.jp/sonicparanoid\nfor instructions on the installation.')
                sys.exit(-5)
            else: # everything went fine
                return True
        else:
            sys.stderr.write('\nERROR: the MMseqs2 version could not be extracted. Were the binaries installed?\n')
            sys.stderr.write('Please go to\nhttp://iwasakilab.bs.s.u-tokyo.ac.jp/sonicparanoid\nfor instructions on the installation.')
            sys.exit(-5)
...
def load_config_file(debug=False):
    """Reads the config file and set MMseqs2 path."""
    ...
    if not os.path.isfile(cfgPath):
        sys.stderr.write('\nERROR: the configuration file config.json was not found in the main directory.\n')
        sys.stderr.write('Please run the script setup_sonicparanoid.py before continuing.\n')
        sys.exit(-5)
    else:
        import json
        configDict = json.loads(open(cfgPath).read())
        return configDict['mmseqs']
...
def main():
    ...
    #sys.exit('DEBUG :: sonic_paranoid.py :: MMseqs2 check')
    ...
    if args.sensitivity:
        if 1. <= args.sensitivity <= 7.5:
            ...
            sys.stderr.write('\nERROR: the sensitivity parameter must have a value between 1.0 and 8.5\n')
    ...
    print('\nSonicParanoid will start with following parameters:')
    ...
    if updateId is None:
        ...
        # this is not required by every user
        # extract ortholog relations
        # CORE
        ##orthodetect.extract_ortholog_pairs(rootDir=outDir, outDir=relDict, outName=None, pairsFile=pairsFile, coreOnly=True, splitMode=False, debug=debug)
        # ALL
        orthodetect.extract_ortholog_pairs(rootDir=outDir, outDir=relDict, outName='ortholog_pairs.tsv', pairsFile=pairsFile, coreOnly=False, splitMode=True, debug=debug)
        ...
        relDict = '%sortholog_relations/'%outDir
        orthodetect.extract_ortholog_pairs(rootDir=outDir, outDir=relDict, outName='{:s}_relations.tsv'.format(updateId), pairsFile=pairsFile, coreOnly=False, splitMode=True, debug=debug)
