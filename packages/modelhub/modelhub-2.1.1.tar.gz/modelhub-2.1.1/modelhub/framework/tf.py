...
class TFApiModel(ApiModel):
    ...
    def _prepare_remote(self, timeout_seconds, hostport):
        ...
        if isinstance(hostport, str):
            ...
            def _stub_fn():
                hostport_str = hostport(self.model_name, version=self.server_model_version)
    ...
    def _build_request(self, inputs, schema):
        ...
        if self.server_model_version:
            request.model_spec.version = self.server_model_version
        if schema:
