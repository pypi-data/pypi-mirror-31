...
from metaDSUtils import mdb # TODO: reduce to this ie/ remove MongoClient below
from pprint import pprint
    
"""
Imports Meta to Mongo
...
TODO: right now, one DB per VISTA (station #) - move to unified once National is QA'ed 
ie/ will have a set in National and then collections combining locals. National will have
latest (super)set
...
"""
    ...
    4.6G	/data/xx2011/metaDS/primary
    233M	/data/xx2011/metaDS/secondary
    4.8G	/data/xx2011/metaDS
    
    or Zip
    
    224M
    
    ie/ storage is a lot bigger.
"""
...
"""
- id to _id
- leaving pointer as is (may revisit
...
"""
def prepareRecordForMongo(record):
    ...
    def toMongoDate(dtStr):
        ...
        if not re.search(r'T', dtStr):
            ...
            # Seems to work: 6:14:01 marked mountain is stored as 12:14:01 ie/ 6 hour difference
            return datetime.strptime(dtStr, "%Y-%m-%dT%H:%M:%S").replace(tzinfo=TZ_MOUNTAIN)
