    ...
    def wrapper(func: Callable):
        def _f_wrapper_(*args, **kwargs):
            ...
            if os.path.exists(cache_file_path) and \
                    (time.time() - os.path.getmtime(cache_file_path)) / 60 < expired_time:
...
def mongo_inject(config_name: str, collection: str = None, inject_name: str = 'mongo'):
    """
    mongo collection 注入
    :param config_name: 配置名
    :param collection: collection名
    :param inject_name: 注入参数名
    :return:
    """
    ...
    def wrapper(func):
        def __inner_wrapper__(*args, **kwargs):
            ...
            with __get_mongo_connection__(config_name) as mongo_client:
                ...
                kwargs.update({inject_name: coll})
                data = func(*args, **kwargs)
            return data
...
def mysql_inject(config_name: str, use_dict: bool = False, inject_name: str = 'mysql'):
    """
    mysql cursor注入
    :param config_name: 配置名
    :param use_dict: 使用dictCursor
    :param inject_name: 注入参数名
    :return:
    """
        ...
        def __inner_wrapper__(*args, **kwargs):
            ...
            with conn:
                if use_dict:
                    ...
                    cursor = conn.cursor()
                with cursor:
                    kwargs.update({inject_name: cursor})
                    data = func(*args, **kwargs)
            return data
