...
class Entropy():
    ...
    def EntropyCalculate(self,dataSet):
        ...
        P_New=[]  
        T_New=[]  
        Normal_TB_Entropy=self.Tukey_biweight_Normal(dataSet) 
        T=Normal_TB_Entropy[-1]
        ...
        for i in range(len(dataSet)):
            ...
            H=H+P_New[i]*(math.log(P_New[i],len(dataSet)));
        H=-H;  
        if H>1.000:
            H=1.000
        MaxReal=max(dataSet);  
        ...
        if T>1:
            T=1.000
        Normal_TB_Entropy.append(T)  
        return Normal_TB_Entropy;  
    ...
    def Tukey_biweight_Normal(self,T_New):
        ...
        Median=self.getMedian(T_New);
        for i in range(len(T_New)):    
            MAD.append(abs(T_New[i]-Median))
          
        S=self.getMedian(MAD); 
    ...
    def getMedian(self,dataSet):
        ...
        newData.sort()
        if(len(dataSet)%2!=0):
            return newData[len(dataSet)/2];
        else:
            return (newData[len(dataSet)/2]+newData[len(dataSet)/2-1])/2.0;
