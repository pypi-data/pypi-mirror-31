...
class MammothConnector():
    def __init__(self, email, password, account_id=None):
        ...
        """
        response = requests.post(get_url('/login'), json=dict(email=email, password=password, is_long_term=True))
    ...
    @handleError
    def add_singularity_identity(self, integration_key, identity_config):
    ...
    @handleError
    def get_singularity_identity_key_by_name(self, integration_key, name):
        url = "/integrations/{0}/identities".format(integration_key)
        r = self._make_signed_request('get', url)
        data = r.json()
    ...
    @handleError
    def validate_ds_config(self, integration_key,  identity_key, ds_config):
    ...
    @handleError
    def create_singularity_dataset(self, ds_param, wait_till_ready=False):
        url = '/datasets'
