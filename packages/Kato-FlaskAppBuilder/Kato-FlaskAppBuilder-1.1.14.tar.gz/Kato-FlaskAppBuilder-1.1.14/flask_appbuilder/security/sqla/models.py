...
assoc_roles_visibility = Table('ab_roles_assoc' , Model.metadata,
                                Column('id', Integer, primary_key=True),
                                Column('role_id', Integer, ForeignKey('ab_role.id')),
                                Column('role_metadata_id', Integer, ForeignKey('ab_role_metadata.id'))
                                );
...
class Role(Model):
    ...
    roles_visible = relationship('Role_Metadata', secondary = assoc_roles_visibility,cascade="save-update, merge, delete");

    def __repr__(self):
        return self.name;

    @renders('available_roles')
    def available_roles(self):
        fetch_values = self.roles_visible;
        return " " if len(fetch_values) <= 0 else fetch_values;


class Role_Metadata(Model):
    __tablename__ = 'ab_role_metadata'
...
class User(Model):
    ...
    user_parent = Column(String(64), nullable=False);
