...
class BaseSecurityManager(AbstractSecurityManager):
    ...
    def auth_user_remote_user(self, token):
        """
            REMOTE_USER user Authentication

            :type self: User model
        """
        user = self.find_user(token=token)
        if user is None or (not user.is_active()):
            log.info(LOGMSG_WAR_SEC_LOGIN_FAILED.format(token))
            return None
        else:
            self.update_user_auth_stat(user)
            return user
    ...
    def add_token_based_user(self,data,user):
        ...
        """
                   Generic function to create user token
        """
