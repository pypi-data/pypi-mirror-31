...
def isQisRunning(host='127.0.0.1', port=9722):
    ...
    try:
        ...
        myQis.sendAndReceiveCmd(cmd = "$help").split('\n', 1)[0][8:17]
        enablePrint()
        return True
    except:
        enablePrint()
...
class QisInterface:
    ...
    def startStream(self, module, fileName='streamData.txt', fileMaxMB=2000, streamName='Stream With No Name', streamAverage = None):
        ...
        #Create the thread
        t1 = threading.Thread(target=self.startStreamThread, name=module, args=(module, fileName, fileMaxMB, streamName, streamAverage))                
        #Start the thread
        t1.start()
    ...
    def startStreamThread(self, module, fileName='streamData.txt', fileMaxMB=2000, streamName='Stream With No Name', streamAverage = None):
        ...
        try:
            ...
            if streamAverage != None:
                stripesPerAverage = streamAverage / (float(averaging) * 4e-6)
            isRun = True
            while isRun:
                try:
    ...
    def sendCmd(self, device='', cmd='$help', sock=None, readUntilCursor=True, betweenCommandDelay=0.1):
        if sock==None:
            sock = self.sock
        if not (device == ''):
    ...
    def sendAndReceiveCmd(self, sock=None, cmd='$help', device='', readUntilCursor=True, betweenCommandDelay=0.1):
        if sock==None:
            sock = self.sock
        if not (device == ''):
            self.deviceDictSetup(device)
    ...
    # The objects connection needs to be opened (connect()) before this is used
    def streamGetStripesText(self, sock, device, numStripes=4096):
        try:
            bufferStatus = False
            if self.pythonVersion == '3':
                streamStatus = self.sendAndReceiveText(sock, 'stream?', device).decode()
    ...
    def streamModeResample(self, moduleObj, streamCom):
        if streamCom.lower() == "off" or streamCom[0:-2].isdigit():
            return self.sendAndReceiveCmd(cmd = "stream mode resample " + streamCom.lower(), device = moduleObj.ConString)
        else:
