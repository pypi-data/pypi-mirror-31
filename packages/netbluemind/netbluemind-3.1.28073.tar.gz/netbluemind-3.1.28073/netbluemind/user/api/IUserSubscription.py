...
IUserSubscription_VERSION = "3.1.0.qualifier"
...
class IUserSubscription(BaseEndpoint):
	...
	def subscribe (self, subject , subscriptions ):
		...
		response = requests.post( self.base + postUri, params = queryParams, verify=False, headers = {'X-BM-ApiKey' : self.apiKey, 'Accept' : 'application/json', 'X-BM-ClientVersion' : IUserSubscription_VERSION}, data = json.dumps(__data__));
	...
	def listSubscriptions (self, subject , type ):
		...
		response = requests.get( self.base + postUri, params = queryParams, verify=False, headers = {'X-BM-ApiKey' : self.apiKey, 'Accept' : 'application/json', 'X-BM-ClientVersion' : IUserSubscription_VERSION}, data = json.dumps(__data__));
