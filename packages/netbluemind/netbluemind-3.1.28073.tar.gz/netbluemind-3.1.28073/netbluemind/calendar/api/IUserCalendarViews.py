...
IUserCalendarViews_VERSION = "3.1.0.qualifier"
...
class IUserCalendarViews(BaseEndpoint):
	...
	def multipleGet (self, uids ):
		...
		response = requests.post( self.base + postUri, params = queryParams, verify=False, headers = {'X-BM-ApiKey' : self.apiKey, 'Accept' : 'application/json', 'X-BM-ClientVersion' : IUserCalendarViews_VERSION}, data = json.dumps(__data__));
	...
	def updates (self, changes ):
		...
		response = requests.put( self.base + postUri, params = queryParams, verify=False, headers = {'X-BM-ApiKey' : self.apiKey, 'Accept' : 'application/json', 'X-BM-ClientVersion' : IUserCalendarViews_VERSION}, data = json.dumps(__data__));
		return self.handleResult__(None, response)
	def delete (self, uid ):
		postUri = "/{uid}";
		__data__ = None
		...
		response = requests.delete( self.base + postUri, params = queryParams, verify=False, headers = {'X-BM-ApiKey' : self.apiKey, 'Accept' : 'application/json', 'X-BM-ClientVersion' : IUserCalendarViews_VERSION}, data = json.dumps(__data__));
		return self.handleResult__(None, response)
