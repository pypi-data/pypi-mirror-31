...
class GridViz(Element):
    ...
    def __init__(self, response, **options):
        ...
        if isinstance(response, Series):
            self._dataFrame = response.to_frame()
        if isinstance(response, DataFrame):
            if response.index.is_monotonic: #this would typically indicate presence of the auto index added by pandas, so reindex to the first actual data column
                response = response.set_index(response.columns[0])
            self._dataFrame = response
        if isinstance(response, ApiResponse):
            results = response.results
            df = DataFrame(results)
            df = df.set_index(df.columns[0])
            df.index.name = response.get_result_title()
            self._dataFrame = df
        if isinstance(response, string_types):
            csvFile = StringIO(response)
