...
class TestWMLClientWithCaffe2CoreML(unittest.TestCase):
    deployment_uid = None
    model_uid = None
    ...
    @classmethod
    def setUpClass(self):
        TestWMLClientWithCaffe2CoreML.logger.info("Service Instance: setting up credentials")
        self.wml_credentials = get_wml_credentials()
        self.client = get_client()
        self.model_path = os.path.join(os.getcwd(), 'artifacts', 'core_ml', 'caffe', 'caffe_model.tar.gz')

    def test_01_service_instance_details(self):
        TestWMLClientWithCaffe2CoreML.logger.info("Check client ...")
        ...
        TestWMLClientWithCaffe2CoreML.logger.debug(details)
    ...
    def test_02_publish_model(self):
        ...
        TestWMLClientWithCaffe2CoreML.model_uid = self.client.repository.get_model_uid(published_model_details)
        TestWMLClientWithCaffe2CoreML.model_url = self.client.repository.get_model_url(published_model_details)
        self.logger.info("Published model ID:" + str(TestWMLClientWithCaffe2CoreML.model_uid))
        self.assertIsNotNone(TestWMLClientWithCaffe2CoreML.model_uid)

    def test_04_publish_model_details(self):
        details_models = self.client.repository.get_model_details()
        TestWMLClientWithCaffe2CoreML.logger.debug("All models details: " + str(details_models))
    ...
    def test_05_convert_2_coreml(self):
        ...
        TestWMLClientWithCaffe2CoreML.converted_model_path = self.client.deployments._create_virtual(TestWMLClientWithCaffe2CoreML.model_uid)
        self.logger.info("Converted file location: " + str(TestWMLClientWithCaffe2CoreML.converted_model_path))
        self.assertTrue('.mlmodel' in str(TestWMLClientWithCaffe2CoreML.converted_model_path))

    def test_06_load_CoreML_model(self):
        loaded_model = coremltools.models.MLModel(TestWMLClientWithCaffe2CoreML.converted_model_path)
        ...
        if not platform in ['linux']:
            ...
            delete_directory(TestWMLClientWithCaffe2CoreML.model_uid)

    def test_10_delete_model(self):
    #    delete_directory(TestWMLClientWithCaffe2CoreML.model_uid)
        self.client.repository.delete(TestWMLClientWithCaffe2CoreML.model_uid)
