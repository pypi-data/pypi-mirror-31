...
class Training(WMLResource):
    ...
    def get_details(self, run_uid=None):
        ...
        if run_uid is None:
            response_get = requests.get(self._href_definitions.get_repo_models_href(), headers=self._client._get_headers())
            ...
            model_training_details = requests.get(get_details_endpoint, headers=self._client._get_headers())
