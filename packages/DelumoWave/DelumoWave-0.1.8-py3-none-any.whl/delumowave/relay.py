...
class DelumoWaveRelay(DelumoWaveController):
    ...
    def get_state_and_brightness_level(self):
        _LOGGER.debug('Get gstate and brightness level')
        ...
        try:
            state_and_brightness_level = self.read(node_addr=self._node_addr,
                                                   eeprom_addr=eeprom_addr,
                                                   data_size=0x02)
        except (RequestLengthError, ResponseError, RadioControllerBlocked) as ex:
            _LOGGER.error(f'Radio communications failure! {ex}')
    ...
    def set_state_and_brightness_level(self, state: bool, brightness_level: int):
        _LOGGER.debug(f'Set state {state} and brightness_level {brightness_level}')
