 ...
 *
 *		$Revision: 20963 $
 *		$Date: 2016-04-19 09:56:04 +0200 (Tue, 19 Apr 2016) $
 *		$Author: lillo $
 *
...
MGA::ConvertString(PyObject *object, string *string)
{
	if (PyString_Check(object)) {
		*string = PyString_AS_STRING(object);
		return 1;
	}
...
Entry_FromCLU(CLU_Entry *entry)
{
	...
	case CLU_BLOB:
		object = PyBuffer_New(entry->fBlob->GetSize());
		PyObject_AsWriteBuffer(object, &data, &size);
		CL_MEMCPY(data, entry->fBlob->GetData(), size);
		break;
...
Entry_FromPy(PyObject *object)
{
	...
	else if ((PyString_Check(object)) && (!PyString_AsStringAndSize(object, &text, &size))) {
	...
	else if (PyObject_AsReadBuffer(object, &buffer, &size) == 0) {
		entry->fType = CLU_BLOB;
		entry->fBlob = CL_New(CL_Blob((const void *)buffer, (uint32)size));
	}
	else {
		PyErr_Clear();
