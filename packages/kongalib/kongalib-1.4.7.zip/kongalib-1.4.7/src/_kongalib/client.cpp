 ...
 *
 *		$Revision: 25933 $
 *		$Date: 2017-08-29 12:00:48 +0200 (Tue, 29 Aug 2017) $
 *		$Author: lillo $
 *
...
_ErrorCB(MGA_Status error_no, const string& error, MGA::DeferredObject *request)
{
	...
	if ((request->fError) && (request->fError != Py_None)) {
		...
		if (!error_obj) {
			...
			error_obj = PyString_FromString(error_str.c_str());
		}
...
MGA_Client_connect(MGA::ClientObject *self, PyObject *args, PyObject *kwds)
{
	...
	char *host = NULL;
	string tenant_key;
	int32 port = 0, timeout = MGA_DEFAULT_CONNECT_TIMEOUT, flags = CLIENT_FLAG_SCRIPT, parentClientID = 0;
	PyObject *success = NULL, *error = NULL, *progress = NULL, *userdata = Py_None;
	MGA_Status result;
	...
	if (optionsObj != Py_None) {
		if (PyDict_Check(optionsObj)) {
			object = PyDict_GetItemString(optionsObj, "flags");
			if (object) {
				flags = PyLong_AsLong(object);
				if (PyErr_Occurred())
					return NULL;
			}
			object = PyDict_GetItemString(optionsObj, "sid");
			if (object) {
				if (!MGA::ConvertString(object, &spec.fSID))
					return NULL;
			}
			object = PyDict_GetItemString(optionsObj, "remote_address");
			if (object) {
				if (!MGA::ConvertString(object, &spec.fRemoteAddress))
					return NULL;
			}
			object = PyDict_GetItemString(optionsObj, "parent_client_id");
			if (object) {
				parentClientID = PyLong_AsLong(object);
				if (PyErr_Occurred())
					return NULL;
			}
			object = PyDict_GetItemString(optionsObj, "tenant_key");
			if (object) {
				if ((object != Py_None) && (!MGA::ConvertString(object, &tenant_key)))
					return NULL;
			}
		}
	}
	options.Set("flags", flags);
	options.Set("parent_client_id", parentClientID);
	options.Set("tenant_key", tenant_key);
...
MGA_Client_create_database(MGA::ClientObject *self, PyObject *args, PyObject *kwds)
{
	...
	if ((success) && (success != Py_None)) {
		...
		return PyString_FromString(uuids.c_str());
	}
}
...
{
	MGA_Status result;
	static char *kwlist[] = { "username", "password", "success", "error", "progress", "userdata", "timeout", NULL };
	...
	if (!PyArg_ParseTupleAndKeywords(args, kwds, "O&O&|OOOOi:authenticate", kwlist, MGA::ConvertString, &userName, MGA::ConvertString, &password, &success, &error, &progress, &userdata, &timeout))
		return NULL;
