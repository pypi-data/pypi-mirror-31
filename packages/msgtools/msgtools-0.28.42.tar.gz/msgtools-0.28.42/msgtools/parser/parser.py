...
def ApplyConditional(line, tag, condition):
    ret = line
    if "<IF"+tag+">" in line:
        if condition:
            line = line.replace("<IF"+tag+">", "")
        else:
            return ""
    elif "<!IF"+tag+">" in line:
        if not condition:
...
def conditionals(line, msg, field):
    ret = line
    ret = ApplyConditional(ret, "BITFIELD", "Bitfields" in field)
    ret = ApplyConditional(ret, "SCALED", "Offset" in field or "Scale" in field)
    ret = ApplyConditional(ret, "ENUM", "Enum" in field)
    ret = ApplyConditional(ret, "ARRAY", MsgParser.fieldCount(field) != 1)
    
    return ret

def DoFieldReplacements(lines, msg, replacements, field, firstTime):
    for line in lines:
        line = conditionals(line, msg, field)
    ...
    if "Messages" in inputData:
        for msg in Messages(inputData):
            ...
            try:
                ...
                forEachFieldSection = False
                forEachEnumSection = False
                forEachFieldLines = ""
                forEachEnumLines = ""
                for line in template:
                    if "<FOREACHFIELD>" in line:
                        forEachFieldSection = True
                    elif "</FOREACHFIELD" in line:
                        forEachFieldSection = False
                        for field in msg["Fields"]:
                            lines = DoFieldReplacements(forEachFieldLines, msg, replacements, field, firstTime)
                            outFile.write(lines)
                        forEachFieldLines = ""
                    elif "<FOREACHFIELD(" in line:
                        #get thing from inside parens, do just that for each field, with amount of whitespace from
                        #before the <FOREACHFIELD
                        pass
                    elif "<FOREACHENUM>" in line:
                        forEachEnumSection = True
                    elif "</FOREACHENUM" in line:
                        forEachEnumSection = False
                        for e in UsedEnums(msg, enums):
                            lines = DoEnumReplacements(forEachEnumLines, msg, replacements, e, firstTime)
                            outFile.write(lines)
                        forEachEnumLines = ""
                    elif "<FOREACHENUM(" in line:
                        #get thing from inside parens, do just that for each enum, with amount of whitespace from
                        #before the <FOREACHENUM
                        pass
                    else:
                        if forEachFieldSection:
                            forEachFieldLines += line
                        elif forEachEnumSection:
                            forEachEnumLines += line
                        else:
