...
class Conf(_Section):
    ...
    def read(self, fname):
        ...
        parsed = parser.read(fname)
        if not len(parsed):
            raise FileExistsError("Could not read the configuration file '{}'".format(fname))
        req_sections = set([_Download.SECTION, _Process.SECTION])
        if set(parser.sections()) != req_sections:
            raise Conf.ConfError("Required section: {}. Found: {}"
                                 .format(req_sections, parser.sections()))
