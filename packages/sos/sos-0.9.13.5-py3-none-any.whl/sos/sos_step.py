...
def analyze_section(section, default_input=None):
    ...
    input_statement_idx = [idx for idx,x in enumerate(section.statements) if x[0] == ':' and x[1] == 'step_input']
    ...
    if input_statement_idx is not None:
        ...
        try:
            ...
            env.sos_dict.set('step_input', Undetermined() if isinstance(step_input, Undetermined) else sos_targets(step_input))
...
class Base_Step_Executor:
    ...
    def handle_extract_pattern(pattern, ifiles, _groups, _vars):
        ...
        for pattern in patterns:
            ...
            for k, v in res.items():
                if k in ('step_input', 'output', 'depends') or k.startswith('_'):
    ...
    def prepare_task(self):
        ...
        # NOTE: we do not explicitly include 'step_input', 'output', 'depends' and 'CONFIG'
    ...
    def run(self):
        ...
        # look for input statement.
        input_statement_idx = [idx for idx,x in enumerate(self.step.statements) if x[0] == ':' and x[1] == 'step_input']
        if not input_statement_idx:
            input_statement_idx = None
        elif len(input_statement_idx) == 1:
            input_statement_idx = input_statement_idx[0]
                                    ...
                                    if env.config['sig_mode'] == 'default':
                                        ...
                                        if 'sos_run' in env.sos_dict['__signature_vars__']:
                                            ...
                                            if isinstance(matched, dict):
                                                ...
                                                # from a signature
                                                env.sos_dict.set('_input', sos_targets(matched['step_input']))
