...
class MasterTaskParams(TaskParams):
    def __init__(self, num_workers=0):
        ...
        self.sos_dict = {'_runtime': {}, '_input': sos_targets(), '_output': sos_targets(), '_depends': sos_targets(),
                'step_input': sos_targets(), 'output':sos_targets(), 'depends': sos_targets(), 'step_name': '',
                '_index': 0}
...
def collect_task_result(task_id, sos_dict):
    ...
    return {'ret_code': 0, 'task': task_id, 'step_input': input, 'output': output, 'depends': depends,
...
''' + global_def)
    ...
    for key in ['step_input', '_input',  'output', '_output', 'depends', '_depends']:
    ...
    try:
        ...
        return {'ret_code': 0, 'task': task_id, 'step_input': [],
    ...
    if has_res():
        try:
            ...
            if ('ret_code' in res and res['ret_code'] == 0) or ('succ' in res and res['succ'] == 0):
                for var in ('step_input', 'output', 'depends'):
