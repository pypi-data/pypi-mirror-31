...
class Reload(GenericReload):
    ...
    def call_service(self,
                     ...
                     *args, **kwargs):
        con = self.connection
        timeout = timeout or con.settings.RELOAD_TIMEOUT
...
class HANxosReloadService(GenericHAReload):
    """ Service to reload the device.
        ...
        :arg : reload_command : reload command to be issued on device.
               default reload_command is "reload"
    ...
    def call_service(self, reload_command='reload',
        ...
        while config_retry < 15:
            try:
                con.configure(config_commands,
                    prompt_recovery=self.prompt_recovery)
            except Exception as err:
                if re.search("Config mode cannot be entered during Standby \
                             initialization", str(err)):
                    sleep(2)
                    config_retry += 1
            else:
                config_retry = 16
...
class AttachModuleConsole(BaseService):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.start_state = "enable"
        self.end_state = "enable"
        self.service_name = "attach console module"
...
class BashService(BaseService):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.start_state = "enable"
        self.end_state = "enable"
        self.service_name = "bash console"
    ...
    class ContextMgr(object):
        ...
        def __enter__(self):
            ...
            if self.enable_bash:
                # enable bash feature
                self.conn.configure('feature bash')

            # go into console
            self.conn.sendline('run bash sudo su')
            self.conn.expect([self.prompt], timeout = self.timeout)
            return self
        ...
        def execute(self, command, timeout = None):
            ...
            # expect output until prompt again
            # wait for timeout provided by user
            out = self.conn.expect([r'(.+)%s' % self.prompt],
                                   timeout = timeout)
        ...
        def __exit__(self, exc_type, exc_value, exc_tb):
            ...
            # cheat and use core exit - the matching prompt will work :)
