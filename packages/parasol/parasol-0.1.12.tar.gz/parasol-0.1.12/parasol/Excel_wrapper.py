...
class ExcelWrap:
    ...
    def __init__(self,xlsFile="", Visible=1):
        ...
        ExcelWrap.numWkBooks = ExcelWrap.numWkBooks + 1
    ...
    def getSheetNumberFromName(self, shtName='Sheet1'):
        ...
        for i in range(1, self.xlApp.Sheets.Count+1):
            if self.xlApp.Sheets(i).Type == constants.xlWorksheet:
                if shtName==str(self.xlApp.Sheets(i).Name): ns = isheet
    ...
    def addNewSeries(self, NChart=1, NSheet=1, xColumn=1, yColumn=2):
    ...
    def close(self):
        ...
        ExcelWrap.numWkBooks = ExcelWrap.numWkBooks - 1
        if ExcelWrap.numWkBooks==0: self.xlApp.Quit()
        self.xlApp = None
    ...
    def setRangeOnCurrentSheet(self, rs, upperLeft='$A$1'):
        '''add to Excel Data Sheet from rs
        rs is a tuple of tuples, or a list of lists
        Top row of titles, then rows of data'''
