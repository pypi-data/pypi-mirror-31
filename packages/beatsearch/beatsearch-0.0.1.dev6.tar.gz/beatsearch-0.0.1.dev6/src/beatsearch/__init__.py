...
# append commit hash to __display_version__
if __version__.endswith("+"):
    import subprocess
    __version__ = __version__[:-1]  # remove "+" to comply with PEP 440 standard

    # noinspection PyBroadException
    try:
        commit_hash = subprocess.check_output(["git", "describe", "--always"]).strip()
        __display_version__ = "{}-{}".format(__version__, commit_hash.decode())  # removing the '+'
    except Exception:
        pass
