...
def get_all_load_balancers():
    return elbc.describe_load_balancers()['LoadBalancerDescriptions']


def request_all(client, funcname, topkey, MaxRecords=100):
    '''
    ...
    page_iterator = paginator.paginate(MaxRecords=MaxRecords)
...
def get_all_auto_scaling_groups():
    return request_all(asg, 'describe_auto_scaling_groups', 'AutoScalingGroups')
...
def names(list_objs):
    ...
    for key in ('GroupName', 'RoleName', 'LoadBalancerName', 'AutoScalingGroupName', 'CacheClusterId'):
        try:
    ...
    try:
        return [obj['Instances'][0]['InstanceId'] for obj in list_objs]
    except KeyError:
        assert False, 'Cannot get name of {}'.format(list_objs[0])
