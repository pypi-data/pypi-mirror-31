...
class ParseBAM:
	...
	def stat (self,q_cut=30):
		...
		print >>sys.stderr,"\n#=================================================="
		print >>sys.stderr,"#All numbers are READ count"
		print >>sys.stderr,"#==================================================\n"
		print >>sys.stderr, "%-40s%d" % ("Total records:",R_total)
		print >>sys.stderr, "\n",
		print >>sys.stderr, "%-40s%d" % ("QC failed:",R_qc_fail)
		print >>sys.stderr, "%-40s%d" % ("Optical/PCR duplicate:", R_duplicate)
		print >>sys.stderr, "%-40s%d" % ("Non primary hits", R_nonprimary)
		print >>sys.stderr, "%-40s%d" % ("Unmapped reads:",R_unmap)
		print >>sys.stderr, "%-40s%d" % ("mapq < mapq_cut (non-unique):",R_multipleHit)
		print >>sys.stderr, "\n",
		print >>sys.stderr, "%-40s%d" % ("mapq >= mapq_cut (unique):",R_uniqHit)
		print >>sys.stderr, "%-40s%d" % ("Read-1:",R_read1)
		print >>sys.stderr, "%-40s%d" % ("Read-2:",R_read2)
		print >>sys.stderr, "%-40s%d" % ("Reads map to '+':",R_forward)
		print >>sys.stderr, "%-40s%d" % ("Reads map to '-':",R_reverse)
		print >>sys.stderr, "%-40s%d" % ("Non-splice reads:",R_nonSplice)
		print >>sys.stderr, "%-40s%d" % ("Splice reads:",R_splice)	
		print >>sys.stderr, "%-40s%d" % ("Reads mapped in proper pairs:",R_properPair)
		print >>sys.stderr, "%-40s%d" % ("Proper-paired reads map to different chrom:",R_pair_diff_chrom)
	...
	def mismatchProfile(self,read_length,read_num, outfile, q_cut=30):
		...
		DOUT = open(outfile + '.mismatch_profile.txt','w')
