#!/usr/bin/python
'''-------------------------------------------------------------------------------------------------
Calculate the RNA-seq coverage over gene body. It's useful to check the 5'end or 3'end coverage bias
-------------------------------------------------------------------------------------------------'''
...
def genebody_percentile(refbed, mRNA_len_cut = 1000):
	'''
	...
	for line in open(refbed,'r'):
		...
		if len(gene_all_base) < mRNA_len_cut:
			continue
		gene_all_base = sorted(gene_all_base)
		if strand == '-':
			tmp = gene_all_base[0:mRNA_len_cut]
		elif strand == '+':
			tmp = gene_all_base[-mRNA_len_cut:]
		
		g_percentiles[geneID] = (chrom, strand, tmp)	#get 100 points from each gene's coordinates
...
def Rcode_write(dataset,file_prefix, format='pdf', colNum=100):
	...
	print >>ROUT, "x=%d:-1" % (-colNum) 
	print >>ROUT, 'icolor = colorRampPalette(c("#7fc97f","#beaed4","#fdc086","#ffff99","#386cb0","#f0027f"))(%d)' % (len(names))
	if len(names) == 1:
		print >>ROUT, "plot(x,%s,type='l',xlab=\"Distance to TES\", ylab=\"Coverage\",lwd=0.8,col=icolor[1])" % (names[0])
...
def main():
	...
	# test
	#for k,v in gene_percentiles.items():
	#	print k + '\t' + str(len(v[2]))
	...
	for bamfile in bamfiles:
		...
		if len(cvg) == 0:
			print >>sys.stderr, "Cannot get coverage signal from " + basename(bamfile) + ' ! Skip'
			continue
	...
	Rcode_write(dataset, options.output_prefix + '.geneBodyCoverage', format = options.output_format,colNum=options.min_mRNA_length)
