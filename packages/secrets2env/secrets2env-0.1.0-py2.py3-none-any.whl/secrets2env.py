...
class SecretsManagerClient(object):
    """
    Facilitates retrieving and decoding AWS Secrets, currently only
...
class EnvironmentGenerator(object):
    ...
    """
    _ok_starting_chars = string.ascii_letters + '_'
    ...
    def __init__(self):
        ...
        for definition in self.definition:
            assert definition.get('name'), 'name is missing or empty in {}'.format(definition)
            assert definition['name'][0] in self._ok_starting_chars, (
                'name starts with illegal character in {}'.format(definition),
            )
            assert '-' not in definition['name'], 'name contains hyphen in {}'.format(definition)
