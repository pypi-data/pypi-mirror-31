...
class MemoUser:
    ...
    def get_memos(self):
        ...
        self.memos_reply=[memo for memo in self.memos_send if memo.prefix == PRIFIX_BY_ACTION_NAME['Reply to memo']]
    ...
    def list_replies(self):
        for memo in self.memos_reply:
            print(memo.content_reply())
    ...
    def like_memo(self, liked_memo, tip_amount=None, sender_of_liked_memo=None):
        """
    ...
    def reply_to_memo(self,target_memo,message):
        """

        :param target_memo:
        :type target_memo: ``Memo`` or ``str``
        :param message: no more than 41 bytes
        :type message: ``str``
        :return
        """
        memo=Memo.reply_to_memo(target_memo,message,self._address)
        return self.__send_new_memo(memo)
