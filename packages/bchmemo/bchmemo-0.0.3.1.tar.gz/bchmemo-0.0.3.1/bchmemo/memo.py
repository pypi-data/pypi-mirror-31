...
SUPPORTED_PREFIX=['6d01','6d02','6d04','6d06','6d07','6d03']
ACTION_NAME=['Set name','Post memo','Like / tip memo','Follow user','Unfollow user','Reply to memo']
...
class Memo:
    ...
    def __init__(self):
        ...
        self._values= '' # hex string in OP_RETURN data except prefix


        self._name=''
        self._message=''
        self._txhash_of_target_memo= ''  # txid of the memo that is be liked or replied
        ...
        try:
            ...
            for vout in transaction['vout']:
                if vout['scriptPubKey']['hex'][0:2] == '6a' \
                    ...
                    memo.values= vout['scriptPubKey']['hex'][8:]
                    break
            else:
            ...
            memo.is_parsed_correctly=False
            memo.parsed_error=sys.exc_info()[1]
    ...
    def values(self,value):
        ...
        if self._prefix==PRIFIX_BY_ACTION_NAME['Set name']:
            self.name=value_bytes[1:].decode()
        elif self._prefix==PRIFIX_BY_ACTION_NAME['Post memo']:
            self.message=value_bytes[1:].decode()
        elif self._prefix==PRIFIX_BY_ACTION_NAME['Like / tip memo']:
            self.txhash_of_target_memo=bytes(reversed(value_bytes[1:1+32])).hex()
        elif self._prefix==PRIFIX_BY_ACTION_NAME['Follow user']:
            self.address=value_bytes[1:].hex()
        elif self._prefix==PRIFIX_BY_ACTION_NAME['Unfollow user']:
            self.address=value_bytes[1:].hex()
        elif self._prefix==PRIFIX_BY_ACTION_NAME['Reply to memo']:
            self.txhash_of_target_memo= bytes(reversed(value_bytes[1:1 + 32])).hex()
            self.message=value_bytes[34:].decode()
        else:
            raise ValueError('"{}" is not a supported memo prefix!',self._prefix)
    ...
    def name(self,name):
        ...
        if len(name)>76:
            raise ValueError('"{}" is too long.Max length of memo name is 76 bytes',name)
        else:
            self._name=name
    ...
    def message(self, message):
        ...
        if self._prefix==PRIFIX_BY_ACTION_NAME['Post memo'] and len(message)>76:
            self._message = message
            raise ValueError('"{}" is too long.Max length of memo message is 76 bytes',message)
        elif self._prefix==PRIFIX_BY_ACTION_NAME['Reply to memo'] and len(message)>43:
            self._message = message
            raise ValueError('"{}" is too long.Max length of reply memo message is 43 bytes'.format(message))
        else:
            self._message=message

    @property
    def txhash_of_target_memo(self):
        return self._txhash_of_target_memo

    @txhash_of_target_memo.setter
    def txhash_of_target_memo(self, txhash):
        if not isinstance(txhash,str):
            raise TypeError('txhash should be a string!')
        if len(txhash)!=64:
            raise ValueError('txhash("{}") should be 64 hex chars long'.format(txhash))
        else:
            self._txhash_of_target_memo=txhash
    ...
    def __get_like_memo(self):
        ...
        """
        try:
            tx_like=NetworkAPI.get_tx(self._txhash_of_target_memo)
        except:
            return
    ...
    def address(self, address):
        ...
        if len(address)!=40:
            self._address=''
            raise ValueError('"{}" is not a public key hash'.format(address))
        else:
            self._address=Address(payload=list(bytes.fromhex(address[0:40])),version= 'P2PKH').cash_address()
    ...
    def content(self):
        ...
        if self._prefix==PRIFIX_BY_ACTION_NAME['Set name']:
            ...
            line_message = self.__action_name + ': ' + self.txhash_of_target_memo
        elif self._prefix==PRIFIX_BY_ACTION_NAME['Follow user']:
    ...
    def content_like(self):
        ...
        line='Liked '\
             +' ('+self.txhash_of_target_memo + ')'\
             +' at '+datetime.fromtimestamp(int(self.transaction_time)).strftime('%Y-%m-%d %H:%M:%S')

        return line

    def content_reply(self):
        if self.prefix!=PRIFIX_BY_ACTION_NAME['Reply to memo']:
            raise TypeError('This is not a Reply to memo!')

        line_sender = get_name_from_address(self.sender) + '@...' + self.sender[-6:] \
                      + ' replied to '\
                      + 'memo({})'.format(self.txhash_of_target_memo)\
                      + ' at ' \
                      + datetime.fromtimestamp(int(self.transaction_time)).strftime('%Y-%m-%d %H:%M:%S')
        line_message = self.message
    ...
    def __create_values(self):
        ...
        if self._prefix==PRIFIX_BY_ACTION_NAME['Set name']:
            ...
            len_name_bytes = len(name_bytes).to_bytes(1, byteorder='little')
            self._values=(len_name_bytes + name_bytes).hex()
        elif self._prefix==PRIFIX_BY_ACTION_NAME['Post memo']:
            message_bytes=self.message.encode()
            len_message_bytes = len(message_bytes).to_bytes(1, byteorder='little')
            self._values=(len_message_bytes + message_bytes).hex()
        elif self._prefix==PRIFIX_BY_ACTION_NAME['Like / tip memo']:
            tx_hash_bytes=bytes.fromhex(self.txhash_of_target_memo)
            len_tx_hash_bytes=len(tx_hash_bytes).to_bytes(1,byteorder='little')
            self._values=(len_tx_hash_bytes+bytes(reversed(tx_hash_bytes))).hex()
        elif self._prefix==PRIFIX_BY_ACTION_NAME['Follow user']:
            address_bytes=address_to_public_key_hash(self.address)
            len_address_bytes=len(address_bytes).to_bytes(1,byteorder='little')
            self._values=(len_address_bytes+address_bytes).hex()
        elif self._prefix==PRIFIX_BY_ACTION_NAME['Unfollow user']:
            address_bytes=address_to_public_key_hash(self.address)
            len_address_bytes=len(address_bytes).to_bytes(1,byteorder='little')
            self._values=(len_address_bytes+address_bytes).hex()
            ...
            message_bytes=self.message.encode()
            len_message_bytes = len(message_bytes).to_bytes(1, byteorder='little')
            self._values=(len_tx_hash_bytes + bytes(reversed(tx_hash_bytes))
                         +len_message_bytes + message_bytes).hex()
    ...
    def like_memo(cls, liked_memo, sender, tip_amount=None, sender_of_liked_memo=None):
        ...
        if isinstance(liked_memo,Memo):
            ...
            memo.txhash_of_target_memo=memo.liked_memo.transaction_hash
            memo._sender_of_liked_memo=memo.liked_memo.sender
        elif isinstance(liked_memo,str):
            memo.txhash_of_target_memo=liked_memo
    ...
    @classmethod
    def reply_to_memo(cls,target_memo,message,sender):
        memo=Memo()
        memo.sender=sender
        memo.prefix=PRIFIX_BY_ACTION_NAME['Reply to memo']
        if isinstance(target_memo,Memo):
            memo.txhash_of_target_memo=target_memo.transaction_hash
        elif isinstance(target_memo,str):
            memo.txhash_of_target_memo=target_memo
        else:
            raise TypeError('Wrong type of target_memo')
        memo.message=message
        memo.__create_values()
        return memo
    ...
    def create_signed_transaction(self,private_key,leftover=None):
        ...
        data_bytes=len_prefix_bytes+prefix_bytes+values_bytes
