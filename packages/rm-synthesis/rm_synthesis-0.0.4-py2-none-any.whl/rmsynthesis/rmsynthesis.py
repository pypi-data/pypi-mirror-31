...
def main():
    ...
    numpy.savetxt(args.prefix  + '-RMSF.txt', numpy.vstack((phi_sample, phase.real, phase.imag) ) )
    pyfits.writeto(args.prefix + '-RM.FITS', RM_peak, rmhdr, clobber=True)
    pyfits.writeto(args.prefix + '-FDAMP.FITS', FDpeak, rmhdr.update('POL', 'FPEAK'), clobber=True)
    pyfits.writeto(args.prefix + '-FDANG.FITS', FDangle, rmhdr.update('POL', 'FANG'), clobber=True)
    pyfits.writeto(args.prefix + '-QRM.FITS', Faraday_Dispersion.real, fhdr, clobber=True)
    pyfits.writeto(args.prefix + '-URM.FITS', Faraday_Dispersion.imag, fhdr.update('POL', 'U'), clobber=True)
    print('>>> Done!')
