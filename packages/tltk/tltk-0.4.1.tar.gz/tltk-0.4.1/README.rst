Thai Language Toolkit Project  version 0.41
==========================================
...
List of basic tools:
>> tltk.nlp.chunk(Text) :  segment edu, word, + pos tag + (ner)

>> tltk.nlp.ner([(w,pos),....])  =>  [(w,pos,ne_mk),....]  to be added
...
>> tltk.nlp.pos_tag_wordlist(WordLst) : Input is a word list

>> tltk.nlp.pos_load() : load Thai tagger model before using pos_tag(). This has to be called only once at the beginning.
POS results are based on Universal POS tags.. http://universaldependencies.org/u/pos/index.html
