...
from .utils import parse_version, VERSION_LATEST
...
def common_parser():
    ...
    misc.add_argument("--help", "-h", action="store_true", default=False,
                      help="show this help message and exit")
    misc.add_argument("--version", action="store_true", default=False,
...
def main():
    ...
    # Grab the URI if needed
    proxy = None
    if options.sub_command != "identities" or options.sub_sub_command in ["version"]:
        if options.sub_command == "identities":
            options.identity = options.id
            config = load_config(options.id)
