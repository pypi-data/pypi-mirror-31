...
.. code-block:: shell
    ...
    lavacli identities version check <id>
    lavacli identities version upgrade <id>

In order to provide the right command line, lavacli need to know the version of the
LAVA instance that you want to access.
If the information is not provided in the configuration file (see
:ref:`configuration`), lavacli will assume that this LAVA instance is using the
lastest available version.

To automatically set the version use:

.. code-block:: shell

    lavacli identities version upgrade <id>

This will query the server and write the version into the configuration.

Then when using lavacli for this identity, the command line will only show
options that are available for this instance.
