...
def configure_parser(parser, version):
    ...
    if version >= VERSION_LATEST:
        ...
        jobs_config.add_argument("--dest", default=".",
                                 help="save files into this directory")
    ...
    jobs_resubmit.add_argument("--url", action="store_true",
                               dest="print_url", default=False,
    ...
    jobs_submit.add_argument("definition", nargs="+", type=argparse.FileType('r'),
                             help="job definition")
    jobs_submit.add_argument("--url", action="store_true",
                             dest="print_url", default=False,
                             help="Print the full url")

    if version >= VERSION_2018_1:
        # "wait"
...
def handle_config(proxy, options, _):
    ...
    # Save the files
    with open(dest / "definition.yaml", "w") as f_out:
        f_out.write(definition)
    if device is not None:
        with open(dest / "device.yaml", "w") as f_out:
            f_out.write(device)
    if dispatcher is not None:
        with open(dest / "dispatcher.yaml", "w") as f_out:
            f_out.write(dispatcher)
    if env is not None:
        with open(dest / "env.yaml", "w") as f_out:
            f_out.write(env)
    if env_dut is not None:
        with open(dest / "env.dut.yaml", "w") as f_out:
            f_out.write(env_dut)
    return 0
...
def handle_list(proxy, options, config):
    if config["version"] >= VERSION_LATEST:
        jobs = proxy.scheduler.jobs.list(options.state, options.health,
    ...
    if options.output_format == "json":
        ...
        print("Jobs (from %s to %s):" % (1 + options.start, options.start + options.limit))
        for job in jobs:
            if config["version"] >= VERSION_2018_1:
                print("* %s: %s,%s [%s] (%s) - %s" % (job["id"], job["state"],
                                                      job["health"], job["submitter"],
                                                      job["description"], job["device_type"]))
            else:
                print("* %s: %s [%s] (%s) - %s" % (job["id"], job["status"],
                                                   job["submitter"], job["description"],
                                                   job["device_type"]))
    return 0
...
def handle_logs(proxy, options, _):
    # Loop
    lines = 0
    while True:
