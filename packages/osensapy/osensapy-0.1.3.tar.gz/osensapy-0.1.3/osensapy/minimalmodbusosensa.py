...
class Instrument():
    ...
    ######################################
    def security_unlock(self, password, subfunc=1):
        """Sends password to slave to unlock for use.
        This is done using a custom password function that is in the format:
            [ID, FUNC, SUBFUNC, PWD_H, PWD_L, CRC_H, CRC_L]
        where   FUNC = CMD_CUSTOM
                SUBFUNC = 1 (customer unlock), 2 (factory unlock), 3 (change customer security code)

        Args:
            * password (int): value of password (use decimal numbers)
            * subfunc (int): sub-function to determine level of security access

        Returns:
            The bit value 0 or 1 to indicate if unlock was successful

        Raises:
            ValueError, TypeError, IOError, SecurityError

        """
        functioncode = CMD_CUSTOM
        _checkInt(subfunc, minvalue=1, maxvalue=3, description='Invalid unlock subfunction received ({})'.format(subfunc))

        self._genericCommand(functioncode, subfunc, password, payloadformat='register')
    ...
    def _genericCommand(self, functioncode, registeraddress, value=None, \
        ...
        ALL_ALLOWED_FUNCTIONCODES = list(range(1, 7)) + [15, 16, CMD_CUSTOM]  # To comply with both Python2 and Python3
        ...
        # Payload format
        if functioncode in [3, 4, 6, 16, CMD_CUSTOM] and payloadformat is None:
            payloadformat = PAYLOADFORMAT_REGISTER

        if functioncode in [3, 4, 6, 16, CMD_CUSTOM]:
            if payloadformat not in ALL_PAYLOADFORMATS:
...
def generatePacket(self, functioncode, payloadToSlave):
    return _embedPayload(self.address, self.mode, functioncode, payloadToSlave)
...
def _extractPayload(response, slaveaddress, mode, functioncode):
    ...
    * ASCII Mode: header (:) + slaveaddress byte + functioncode byte + payloaddata + LRC (which is two characters) + footer (CRLF)
    ...
    if receivedFunctioncode == _setBitOn(functioncode, BITNUMBER_FUNCTIONCODE_ERRORINDICATION):
        raise SecurityError('The slave is does not have permission to perform the action {}. The response is {!r}.'.format(functioncode, response))
...
def _predictResponseSize(mode, functioncode, payloadToSlave):
    ...
    NUMBER_OF_PAYLOAD_BYTES_IN_PASSWORD_RESPONSE = 2
    ...
    elif functioncode in [CMD_CUSTOM]:
        response_payload_size = NUMBER_OF_PAYLOAD_BYTES_IN_PASSWORD_RESPONSE
