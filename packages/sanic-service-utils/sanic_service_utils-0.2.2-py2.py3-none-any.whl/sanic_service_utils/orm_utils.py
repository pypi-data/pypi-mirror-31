...
from anji_orm import (
    ...
    IntField, FloatField, SelectionField
)
...
class OpenAPIDescriptionMixin(Model):
    ...
    @staticmethod
    def map_field(field):  # pylint: disable=too-many-return-statements
        ...
        if isinstance(field, EnumField):
            return doc.String(
                field.description,
                choices=[x.value for x in field.variants]
            )
        if isinstance(field, DatetimeField):
            return doc.DateTime(field.description)
        if isinstance(field, BooleanField):
            return doc.Boolean(field.description)
        if isinstance(field, IntField):
            return doc.Integer(field.description)
        if isinstance(field, FloatField):
            return doc.Float(field.description)
        if isinstance(field, SelectionField):
            return doc.String(
                field.description,
                choices=field.variants
            )
        return doc.String(field.description)
